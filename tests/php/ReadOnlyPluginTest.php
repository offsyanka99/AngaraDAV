...
use Baikal\Core\ReadOnlyCalendarBackend;
...
SQL);
$pdo->exec("INSERT INTO calendars (id, synctoken, components) VALUES (10, 1, 'VEVENT')");
$pdo->exec("INSERT INTO calendars (id, synctoken, components) VALUES (20, 1, 'VEVENT,VTODO')");
...
$backend = new ReadOnlyCalendarBackend($pdo, $meta);

// --- discovery advertises native SabreDAV read-only ACLs ---
$aliceCalendars = $backend->getCalendarsForUser('principals/alice');
$aliceByUri = [];
foreach ($aliceCalendars as $calendarInfo) {
    $aliceByUri[$calendarInfo['uri']] = $calendarInfo;
}
assert_true(
    !empty($aliceByUri['holidays']['{http://sabredav.org/ns}read-only']),
    'owner read-only calendar is advertised through SabreDAV metadata'
);
assert_true(
    empty($aliceByUri['default']['{http://sabredav.org/ns}read-only']),
    'writable calendar is not advertised read-only'
);
$holidayNode = new SharedCalendar($backend, $aliceByUri['holidays']);
$holidayPrivileges = array_column($holidayNode->getACL(), 'privilege');
assert_true(!in_array('{DAV:}write', $holidayPrivileges, true), 'owner read-only ACL omits DAV write');
$defaultNode = new SharedCalendar($backend, $aliceByUri['default']);
$defaultPrivileges = array_column($defaultNode->getACL(), 'privilege');
assert_true(in_array('{DAV:}write', $defaultPrivileges, true), 'writable calendar ACL keeps DAV write');

$bobCalendars = $backend->getCalendarsForUser('principals/bob');
assert_true(
    !empty($bobCalendars[0]['{http://sabredav.org/ns}read-only']),
    'sharee inherits owner read-only discovery metadata'
);
