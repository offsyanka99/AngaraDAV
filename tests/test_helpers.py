...
def assert_installed(browser: mechanicalsoup.StatefulBrowser):
    # Confirmation page
    page = browser.get_current_page()
    assert "angaradav is now installed, and its database properly configured" in page.text.lower()

    # Landing page
    browser.open(BASE_URL)
    page = browser.get_current_page()
    assert "angaradav is running" in page.text.lower()
...
def assert_upgrade(browser: mechanicalsoup.StatefulBrowser):
    browser.open(BASE_URL)
    page = browser.get_current_page()
    assert "angaradav upgrade wizard" in page.text.lower()
    browser.follow_link(text="Start upgrade")
    page = browser.get_current_page()
    assert "angaradav has been successfully upgraded" in page.text.lower()
    browser.follow_link(text="Access the AngaraDAV admin")
