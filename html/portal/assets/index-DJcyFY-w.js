...
`)}function N(e){const t=e.titleId||(e.id?`${e.id}-title`:`modal-title-${Math.random().toString(36).slice(2,9)}`),a=e.id?` id="${o(e.id)}"`:"",s=e.className?` ${e.className}`:"",n=e.rootAttrs?` ${e.rootAttrs}`:"",r=`${Xa(e.size)}${e.cardClassName?` ${e.cardClassName}`:""}`,i=e.closeAction,l=e.lockBackdrop?"":` data-action="${o(i)}"`,d=e.hideClose?"":`<button type="button" class="modal-close info-modal-close" data-action="${o(i)}" aria-label="Close">×</button>`;let c="";e.footer!==void 0&&(c=typeof e.footer=="string"?e.footer:Et(e.footer));const f=c?`<footer class="cal-modal-footer">${c}</footer>`:"",m=`<div class="cal-modal-body">${e.body}</div>`;let u;return e.form?u=`<form class="stack"${e.formAttrs?` ${e.formAttrs}`:""}>
          ...
          <div class="bulk-bar-actions">
            ...
            <button type="button" class="btn btn-small btn-danger" data-action="files-bulk-delete" ${e.state.busy?"disabled":""}>Delete</button>
              ...
              <td class="files-col-actions">
               ...
               </ul>`:`<p style="margin:0">Delete <strong>${o((D==null?void 0:D.name)??v[0])}</strong>?${(D==null?void 0:D.type)==="dir"?" This removes the folder and everything inside it.":""}</p>`;return N({id:"files-delete-modal",title:A,titleId:"files-delete-title",closeAction:"files-delete-close",size:"sm",body:F,footer:[{label:"Cancel",action:"files-delete-close",variant:"ghost"},{label:"Delete",action:"files-delete-confirm",variant:"danger",disabled:e.state.busy}]})})():"",y=e.state.filesTransfer!==null&&e.state.filesTransfer.paths.length>0?(()=>{const v=e.state.filesTransfer.op,g=e.state.filesTransfer.paths,D=g.length>1,A=e.state.filesEntries.find(I=>I.path===g[0]),F=(A==null?void 0:A.name)??Ue(g[0]),O=D?`${v==="copy"?"Copy":"Move"} ${g.length} items`:`${v==="copy"?"Copy":"Move"} ${(A==null?void 0:A.type)==="dir"?"folder":"file"}`,C=e.state.filesTransferDest===""?"Home":e.state.filesTransferDest,P=tt(e,e.state.filesTransferDest,g);return N({id:"files-transfer-modal",title:O,titleId:"files-transfer-title",closeAction:"files-transfer-close",size:"md",form:!0,formAttrs:'data-form="files-transfer"',body:`
                  ${D?`<p class="muted small" style="margin:0 0 0.75rem">${g.length} items will be ${v==="copy"?"copied":"moved"} into the destination folder (original names kept).</p>`:`<p class="muted small" style="margin:0 0 0.75rem"><span class="mono">${o(F)}</span></p>`}
              ...
              </label>`,footer:[{label:"Cancel",action:"files-mkdir-close",variant:"ghost"},{label:"Create",type:"submit",variant:"primary",disabled:e.state.busy}]}):"",$=e.state.filesUploadConflict?(()=>{const v=e.state.filesUploadConflict,g=v.conflictCount,D=Math.max(0,v.totalFiles-g),A=g===1?"1 file already exists in the destination.":`${g} of ${v.totalFiles} files already exist in the destination.`,F=D>0?D===1?" Skip existing keeps the other 1 new file.":` Skip existing keeps the other ${D} new files.`:" Skip existing cancels the upload (nothing new to send).",O=v.names.slice(0,12).map(P=>`<li><span class="mono">${o(P)}</span></li>`).join(""),C=v.names.length>12?`<li class="muted">…and ${v.names.length-12} more</li>`:"";return N({id:"files-upload-conflict-modal",title:g===1?"File already exists":"Files already exist",titleId:"files-upload-conflict-title",closeAction:"files-upload-conflict-cancel",size:"sm",body:`
              <p style="margin:0 0 0.75rem">${o(A)}${o(F)}</p>
  ...
  ${i}${l}${d}`}function Bn(e){const t=ae(e,"users");if(t&&t.available===!1)return nt(e,"users");const a=xn(e),s=e.state.adminUsersLoading&&e.state.adminUsers.length===0?'<tr><td colspan="4" class="muted admin-table-empty">Loading users…</td></tr>':a.length===0?`<tr><td colspan="4" class="muted admin-table-empty">${e.state.adminUsersError?o(e.state.adminUsersError):e.state.adminUsersQuery.trim()?"No users match this filter.":"No users found."}</td></tr>`:a.map(n=>`<tr class="contact-table-row${e.state.adminSelectedUsername&&e.state.adminSelectedUsername.toLowerCase()===n.username.toLowerCase()?" is-selected":""}" data-action="admin-user-view" data-username="${o(n.username)}" tabindex="0" role="button">
    ...
    <section class="stack items-edit-panel">
      ${n}
              ...
              <td class="col-task-title"><span class="task-title-inner">${A}<span class="contact-name-primary">${o(p.summary||p.uri)}</span></span>
    ...
    <form class="stack" data-form="initialize">
      ...
      <label>Email invite sender
        <input type="text" name="invite_from" value="${o((e==null?void 0:e.invite_from)||"")}" ${U?"disabled":""} />
      </label>
      <label>Session idle timeout (minutes)
        <input type="number" name="session_max_age_minutes" min="1" max="10080" value="${o(String((e==null?void 0:e.session_max_age_minutes)??15))}" ${U?"disabled":""} />
      </label>
    ...
    <form class="stack" data-form="database">
      ...
      <div data-backend-panel="sqlite" style="${X==="sqlite"?"":"display:none"}">
        <label>SQLite file path
          <input type="text" name="sqlite_file" value="${o((e==null?void 0:e.sqlite_file)||"")}" class="mono" ${U?"disabled":""} />
      ...
      <div data-backend-panel="pgsql" style="${X==="pgsql"?"":"display:none"}">
        <label>PostgreSQL host
          <input type="text" name="pgsql_host" value="${o((e==null?void 0:e.pgsql_host)||"")}" placeholder="localhost:5432" ${U?"disabled":""} />
        </label>
        <label>Database name
          <input type="text" name="pgsql_dbname" value="${o((e==null?void 0:e.pgsql_dbname)||"")}" ${U?"disabled":""} />
        </label>
        <label>Username
          <input type="text" name="pgsql_username" value="${o((e==null?void 0:e.pgsql_username)||"")}" autocomplete="off" ${U?"disabled":""} />
        </label>
        <label>Password
  ...
  `,Ur()}function Ur(){var t,a,s,n,r,i;const e=document.getElementById("app");e&&((t=e.querySelector('[data-action="reload"]'))==null||t.addEventListener("click",()=>{Fr()}),(a=e.querySelector('[data-action="backend-change"]'))==null||a.addEventListener("change",l=>{X=l.target.value==="pgsql"?"pgsql":"sqlite",V()}),(s=e.querySelector('[data-action="upgrade-toggle"]'))==null||s.addEventListener("change",l=>{Ze=!!l.target.checked,V()}),(n=e.querySelector('[data-action="upgrade-run"]'))==null||n.addEventListener("click",()=>{Nr()}),(r=e.querySelector('[data-form="initialize"]'))==null||r.addEventListener("submit",l=>{l.preventDefault(),Ir(l.target)}),(i=e.querySelector('[data-form="database"]'))==null||i.addEventListener("submit",l=>{l.preventDefault(),Or(l.target)}))}async function Fr(){U=!0,B=null,V();try{await qt(),J=null}catch(e){B=e instanceof Error?e.message:"Failed to load installer status"}finally{U=!1,V()}}async function Ir(e){const t=new FormData(e),a=n=>{var r;return!!((r=e.querySelector(`input[name="${n}"]`))!=null&&r.checked)},s={timezone:String(t.get("timezone")??"").trim(),cal_enabled:a("cal_enabled"),card_enabled:a("card_enabled"),tasks_enabled:a("tasks_enabled"),notes_enabled:a("notes_enabled"),files_enabled:a("files_enabled"),dav_auth_type:String(t.get("dav_auth_type")??"Digest"),invite_from:String(t.get("invite_from")??"").trim(),session_max_age_minutes:Number(t.get("session_max_age_minutes")??15),admin_password:String(t.get("admin_password")??""),admin_password_confirm:String(t.get("admin_password_confirm")??"")};U=!0,B=null,J=null,V();try{T=await rt("/initialize",{method:"POST",body:JSON.stringify(s)}),ne=T.csrfToken||ne,J="Server settings saved. Configure the database next.",w.event("install.initialize")}catch(n){B=n instanceof Error?n.message:"Initialize failed"}finally{U=!1,V()}}async function Or(e){const t=new FormData(e),a=String(t.get("backend")??X),s={backend:a,admin_password:String(t.get("admin_password")??""),admin_password_confirm:String(t.get("admin_password_confirm")??"")};a==="sqlite"?s.sqlite_file=String(t.get("sqlite_file")??"").trim():(s.pgsql_host=String(t.get("pgsql_host")??"").trim(),s.pgsql_dbname=String(t.get("pgsql_dbname")??"").trim(),s.pgsql_username=String(t.get("pgsql_username")??"").trim(),s.pgsql_password=String(t.get("pgsql_password")??"")),U=!0,B=null,J=null,V();try{T=await rt("/database",{method:"POST",body:JSON.stringify(s)}),ne=T.csrfToken||ne,J="Database configured. Installer is locked.",w.event("install.database"),T.completed||T.step}catch(n){B=n instanceof Error?n.message:"Database setup failed"}finally{U=!1,V()}}async function Nr(){if(Ze){U=!0,B=null,J=null,V();try{const e=await rt("/upgrade",{method:"POST",body:JSON.stringify({confirm:!0})});J="Upgrade completed."+(e.messages&&e.messages.length?" "+e.messages.slice(0,3).join(" · "):""),w.event("install.upgrade"),await qt()}catch(e){B=e instanceof Error?e.message:"Upgrade failed"}finally{U=!1,V()}}}async function Mr(e){document.title="AngaraDAV · Setup",document.body.classList.add("layout-install"),e.innerHTML='<section class="card"><p class="muted">Loading installer…</p></section>';try{await qt()}catch(t){B=t instanceof Error?t.message:"Failed to load installer"}V()}const Dt=document.getElementById("app");if(!Dt)throw new Error("#app missing");const ra=window.location.pathname.replace(/\/+$/,"")||"/";ra==="/portal/install"||ra.endsWith("/portal/install")?Mr(Dt):Sr(Dt);
