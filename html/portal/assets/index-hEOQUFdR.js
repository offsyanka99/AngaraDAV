...
`)}function ke(s){const c=s.titleId||(s.id?`${s.id}-title`:`modal-title-${Math.random().toString(36).slice(2,9)}`),g=s.id?` id="${o(s.id)}"`:"",h=s.className?` ${s.className}`:"",w=s.rootAttrs?` ${s.rootAttrs}`:"",q=`${ll(s.size)}${s.cardClassName?` ${s.cardClassName}`:""}`,U=s.closeAction,M=s.lockBackdrop?"":` data-action="${o(U)}"`,W=s.hideClose?"":`<button type="button" class="modal-close info-modal-close" data-action="${o(U)}" aria-label="Close">×</button>`;let ee="";s.footer!==void 0&&(ee=typeof s.footer=="string"?s.footer:wn(s.footer));const oe=ee?`<footer class="cal-modal-footer">${ee}</footer>`:"",se=`<div class="cal-modal-body">${s.body}</div>`;let te;return s.form?te=`<form class="stack"${s.formAttrs?` ${s.formAttrs}`:""}>
    ...
    <div class="info-modal" id="info-modal" hidden role="dialog" aria-modal="true" aria-labelledby="info-modal-title">
      ...
      <div class="month-grid-wrap" role="grid" aria-label="Month calendar">
        <div class="month-dow-row" role="row">
          ${v.map(T=>`<div class="month-dow">${o(T)}</div>`).join("")}
        </div>
        ...
        <div class="cal-modal-body">
          ...
          <form class="stack" data-form="edit-event">
            ...
            <div class="form-grid form-grid-2 dt-fields-row">
              ...
              ${oa({field:"end",name:"end",label:"End",value:l,dateOnly:e.allDay,disabled:m||v,allowClear:!v})}
            </div>
            <fieldset class="event-repeat" ${m?"disabled":""}>
              ...
              ${a?`<div class="form-grid form-grid-2" style="margin-top:0.5rem">
                      ...
                      ${y==="until"?oa({field:"until",name:"repeatUntil",label:"Until",value:$,dateOnly:!0,disabled:m,allowClear:!0}):y==="count"?`<label>Occurrences
      ...
      <span class="brand-text">Angara<span class="brand-dav">DAV</span><span class="brand-sep" aria-hidden="true"> · </span><span class="${a?"brand-portal brand-portal-admin":"brand-portal brand-portal-user"}">${o(a?"Administration Portal":"User Portal")}</span></span>`,n=c?o(c.displayname||c.username):"",l=qt()?`<button type="button" class="user-menu-item${h==="admin"?" is-active":""}" role="menuitem" data-action="tab" data-tab="admin">
          ...
          <a class="brand" href="/portal/">${m}</a>
        </nav>`,$=!(ge||Me||Re!==null||Ye!==null||vt||Se||lt)?Bt():"",x=t.tabs&&t.tabs.trim()!==""?`<div class="tabs-bar" role="presentation">
      ...
      ${tr()}`}function Bt(){return g?ma(g.type,g.message,{dismissible:!0}):""}function tn(e){return!Number.isFinite(e)||e<0?"":e<1024?`${e} B`:e<1024*1024?`${(e/1024).toFixed(1)} KB`:`${(e/(1024*1024)).toFixed(1)} MB`}function Ca(e){const t=Math.max(0,Math.floor(e)),a=Math.floor(t/60),r=t%60;return a>0?`${a}m ${r}s`:`${r}s`}function $t(){Ka!==null&&(clearInterval(Ka),Ka=null)}function an(){$t(),Ka=setInterval(()=>{if(!z||z.phase==="done"||z.phase==="error"){$t();return}z={...z,elapsedSec:Math.floor((Date.now()-z.startedAt)/1e3)},z.phase==="processing"&&rn(z)},1e3)}function zt(e,t={}){z&&(z={...z,phase:e,elapsedSec:Math.floor((Date.now()-z.startedAt)/1e3),...t},p())}function sn(){$t(),z=null,p()}function nn(e){!z||z.phase==="done"||z.phase==="error"||(z={...z,phase:"processing",processPercent:e.percent,processCurrent:e.current,processTotal:e.total,processImported:e.imported,processUpdated:e.updated,processSkipped:e.skipped,elapsedSec:Math.floor((Date.now()-z.startedAt)/1e3)},rn(z))}function rn(e){const t=s.querySelector("[data-import-status-line]"),a=s.querySelector(".import-progress-bar"),r=s.querySelector(".import-progress-track"),i=s.querySelector("[data-import-counts]"),m=e.kind==="calendar"?"items":"contacts";let n;if(e.phase==="processing"&&e.processTotal>0)n=`Importing ${e.processCurrent.toLocaleString()} / ${e.processTotal.toLocaleString()} ${m} (${e.processPercent??0}%) · ${Ca(e.elapsedSec)}`;else if(e.phase==="processing")n=`Importing on server… ${Ca(e.elapsedSec)}`;else return;t&&(t.textContent=n),i&&(i.textContent=`${e.processImported} new · ${e.processUpdated} updated${e.processSkipped?` · ${e.processSkipped} skipped`:""}`),a&&e.processPercent!==null&&(a.classList.remove("is-indeterminate"),a.style.width=`${Math.min(100,Math.max(0,e.processPercent))}%`),r&&e.processPercent!==null&&(r.setAttribute("aria-valuenow",String(e.processPercent)),r.removeAttribute("aria-valuetext"))}function tr(){if(!z)return"";const e=z,t=e.phase!=="done"&&e.phase!=="error",a=e.kind==="calendar"?"calendar (.ics)":"contacts (.vcf)",r=e.phase==="done"?"Import finished":e.phase==="error"?"Import failed":"Importing…",i=(()=>{const l=[{id:"reading",label:"Reading file"},{id:"uploading",label:"Uploading to server"},{id:"processing",label:"Importing on server"}],b={reading:0,uploading:1,processing:2,done:3,error:2}[e.phase]??0;return l.map((y,v)=>{let $="pending";return e.phase==="done"||v<b?$="done":v===b&&($=(e.phase==="error","active")),`<li class="import-step import-step-${$}"><span class="import-step-icon" aria-hidden="true">${$==="done"?"✓":$==="active"?"●":"○"}</span> ${o(y.label)}</li>`}).join("")})();let m="";if(t){let l=null;e.phase==="reading"&&e.readPercent!==null?l=Math.min(100,Math.max(0,e.readPercent)):e.phase==="processing"&&e.processPercent!==null&&(l=Math.min(100,Math.max(0,e.processPercent)));const u=l===null?"import-progress-bar is-indeterminate":"import-progress-bar",b=l!==null?` style="width:${l}%"`:"",y=e.kind==="calendar"?"items":"contacts";let v;e.phase==="reading"?v=e.readPercent!==null?`Reading file… ${e.readPercent}%`:"Reading file…":e.phase==="uploading"?v="Uploading to server…":e.processTotal>0?v=`Importing ${e.processCurrent.toLocaleString()} / ${e.processTotal.toLocaleString()} ${y} (${e.processPercent??0}%) · ${Ca(e.elapsedSec)}`:v=`Importing on server… ${Ca(e.elapsedSec)}`;const $=e.phase==="processing"&&e.processTotal>0?`<p class="muted small" data-import-counts style="margin:0 0 0.5rem">${e.processImported} new · ${e.processUpdated} updated${e.processSkipped?` · ${e.processSkipped} skipped`:""}</p>`:'<p class="muted small" data-import-counts style="margin:0 0 0.5rem;display:none"></p>';m=`
        ...
        <p class="import-status-line" data-import-status-line>${o(v)}</p>
        ${$}
        ...
        <div class="card auth-card">
          ...
          <span class="cal-row-actions">
            <button type="button" class="btn btn-small" data-action="export-cal" data-id="${S.id}" ${d?"disabled":""} title="Export as .ics">Export</button>
                ...
                </section>`,footer:[{label:"Close",action:"close-cal-modal",variant:"ghost"}]}):"",y=Re!==null?ne.find(S=>S.id===Re&&S.canShare)??null:null,v=y?ke({id:"cal-delete-modal",title:"Delete calendar",titleId:"cal-delete-title",closeAction:"cancel-delete-cal",size:"sm",body:`
              ${Bt()}
      ...
      ${ut}`,C=h==="calendars"?"my-calendars":h==="contacts"?"my-contacts":h==="tasks"?"tasks":h==="notes"?"notes":h==="files"?"files":"administration",ie=kr(),J=Sr(),et=rr(),_e=yr(),tt=h==="calendars"?x:h==="contacts"?Ht:h==="tasks"?ie:h==="notes"?J:h==="files"?et:_e,ua=h==="admin"?`<div class="tabs" role="tablist" aria-label="Administration sections">
        ...
        <section class="card">
      ...
      </div>`;const t=e.quotaBytes>0?`${Aa(e.usedBytes)} used · ${Aa(e.availableBytes)} free of ${Aa(e.quotaBytes)}`:`${Aa(e.usedBytes)} used · ${Aa(e.availableBytes)} free (no app quota)`,a=e.quotaBytes>0?Math.min(100,Math.round(100*e.usedBytes/e.quotaBytes)):0,r=ce.length,i=xe.length>0&&xe.every($=>ce.includes($.path)),m=r>0,n=r>0?`<div class="bulk-bar files-bulk-bar" role="toolbar" aria-label="Selected files">
            ...
            <div class="bulk-bar-actions">
              ...
              <button type="button" class="btn btn-small btn-danger" data-action="files-bulk-delete" ${d?"disabled":""}>Delete</button>
          ...
          </div>`:"",l=xe.length===0?'<tr><td colspan="5" class="muted">This folder is empty.</td></tr>':xe.map($=>{const x=ce.includes($.path),D=$.type==="dir"?"📁":"📄",L=$.type==="dir"?`<button type="button" class="files-name-btn" data-action="files-nav" data-path="${o($.path)}" ${d?"disabled":""}>
                      <span class="files-icon" aria-hidden="true">${D}</span>${o($.name)}
                    </button>`:`<span class="files-name"><span class="files-icon" aria-hidden="true">${D}</span>${o($.name)}</span>`,ae=$.type==="dir"?"—":Aa($.size);return`<tr class="files-row${x?" is-checked":""}" data-path="${o($.path)}" data-type="${$.type}">
                ...
                <td class="files-col-actions">
                  ...
                  <button type="button" class="btn btn-ghost btn-small" data-action="files-copy" data-path="${o($.path)}" ${d?"disabled":""}>Copy</button>
                  <button type="button" class="btn btn-ghost btn-small" data-action="files-move" data-path="${o($.path)}" ${d?"disabled":""}>Move</button>
                  <button type="button" class="btn btn-ghost btn-small" data-action="files-rename-open" data-path="${o($.path)}" data-name="${o($.name)}" ${d?"disabled":""}>Rename</button>
                  <button type="button" class="btn btn-ghost btn-small btn-danger-text" data-action="files-delete-open" data-path="${o($.path)}" data-name="${o($.name)}" ${d?"disabled":""}>Delete</button>
                </td>
              </tr>`}).join(""),u=Fe!==null?(()=>{const $=xe.find(D=>D.path===Fe),x=($==null?void 0:$.name)??"";return ke({id:"files-rename-modal",title:"Rename",titleId:"files-rename-title",closeAction:"files-rename-close",size:"sm",form:!0,formAttrs:'data-form="files-rename" id="files-rename-form"',body:`
                    <input type="hidden" name="path" value="${o(Fe)}" />
                    <label>New name
                      <input type="text" name="newName" value="${o(x)}" required maxlength="255" autocomplete="off" />
                    </label>`,footer:[{label:"Cancel",action:"files-rename-close",variant:"ghost"},{label:"Rename",type:"submit",variant:"primary",disabled:d}]})})():"",b=Ce!==null&&Ce.length>0?(()=>{const $=Ce,x=$.length>1,D=xe.find(T=>T.path===$[0]),L=x?`Delete ${$.length} items`:`Delete ${(D==null?void 0:D.type)==="dir"?"folder":"file"}`,ae=x?`<p style="margin:0 0 0.75rem">Delete <strong>${$.length}</strong> selected items? Folders are removed with their contents. This cannot be undone.</p>
                 <ul class="files-delete-list muted small">
                   ${$.slice(0,12).map(T=>{const Y=xe.find(H=>H.path===T);return`<li><span class="mono">${o((Y==null?void 0:Y.name)??T)}</span></li>`}).join("")}
                   ${$.length>12?`<li>…and ${$.length-12} more</li>`:""}
                 </ul>`:`<p style="margin:0">Delete <strong>${o((D==null?void 0:D.name)??$[0])}</strong>?${(D==null?void 0:D.type)==="dir"?" This removes the folder and everything inside it.":""}</p>`;return ke({id:"files-delete-modal",title:L,titleId:"files-delete-title",closeAction:"files-delete-close",size:"sm",body:ae,footer:[{label:"Cancel",action:"files-delete-close",variant:"ghost"},{label:"Delete",action:"files-delete-confirm",variant:"danger",disabled:d}]})})():"",y=fe!==null&&fe.paths.length>0?(()=>{const $=fe.op,x=fe.paths,D=x.length>1,L=xe.find(G=>G.path===x[0]),ae=(L==null?void 0:L.name)??Fa(x[0]),T=D?`${$==="copy"?"Copy":"Move"} ${x.length} items`:`${$==="copy"?"Copy":"Move"} ${(L==null?void 0:L.type)==="dir"?"folder":"file"}`,Y=At===""?"Home":At,H=ts(At,x);return ke({id:"files-transfer-modal",title:T,titleId:"files-transfer-title",closeAction:"files-transfer-close",size:"md",form:!0,formAttrs:'data-form="files-transfer"',body:`
                    ${D?`<p class="muted small" style="margin:0 0 0.75rem">${x.length} items will be ${$==="copy"?"copied":"moved"} into the destination folder (original names kept).</p>`:`<p class="muted small" style="margin:0 0 0.75rem"><span class="mono">${o(ae)}</span></p>`}
                    ...
                    ${D?"":`<label style="margin-top:0.85rem">New name <span class="muted">(optional)</span>
                          ...
                          </p>`}`,footer:[{label:"Cancel",action:"files-transfer-close",variant:"ghost"},{label:$==="copy"?"Copy":"Move",type:"submit",variant:"primary",disabled:d||H}]})})():"",v=dt?ke({id:"files-mkdir-modal",title:"New folder",titleId:"files-mkdir-title",closeAction:"files-mkdir-close",size:"sm",form:!0,formAttrs:'data-form="files-mkdir"',body:`
                <p class="muted small" style="margin:0 0 0.75rem">
                  Create a folder in
      ...
      ${v}
            ...
            title="${o(n)}${u?" — "+va(l):""}">
            ${o(n)}
          </button>`}).join("")}function ls(e){const t=Ve(e),a=(t==null?void 0:t.status)??"coming-soon",r=(t==null?void 0:t.label)??e,i=(t==null?void 0:t.summary)||"This area is not available in portal Administration yet.",m=va(a);return`<section class="card admin-coming-soon-card">
      ...
      with role <span class="badge badge-admin">Admin</span>.
      ...
      </section>`;else if(q){const i=q,m=i.services,n=i.links??{},l=e?`<span class="badge ${qa(e.status)}">${o(va(e.status))}</span>`:"",u=i.version?o(i.version):"—",b=i.git?o(i.git):"";a=`
        <section class="card admin-about-card">
          ...
          ${t}
        </section>`;const y=i.nbusers??i.users,v=i.nbcalendars??i.calendars,$=i.nbevents??i.events,x=i.nbbooks??i.addressBooks,D=i.nbcontacts??i.contacts;r=`
      ...
      ${r}`}function or(){const e=bt.trim().toLowerCase();return e?oe.filter(t=>t.username.toLowerCase().includes(e)||(t.displayname||"").toLowerCase().includes(e)||(t.email||"").toLowerCase().includes(e)):oe}function dr(){return ze?ke({id:"admin-user-create-modal",title:"Add user",titleId:"admin-user-create-title",closeAction:"admin-user-create-close",size:"sm",form:!0,formAttrs:'data-form="admin-user-create"',body:`
        ...
        <p class="flash flash-error">${o(qe)}</p>
          ...
          <td class="admin-user-actions">
            ...
            </label>`,footer:[{label:"Cancel",action:"admin-ab-close",variant:"ghost",disabled:d},{label:"Save",type:"submit",variant:"primary",disabled:d}]}):"",l=ve?ke({title:`Delete ${ve.kind==="calendar"?"calendar":"address book"}`,closeAction:"admin-resource-delete-close",size:"sm",body:`
          <p>Delete <strong>${o(ve.label)}</strong> for <span class="mono">${o(e.username)}</span>?</p>
          ${ve.kind==="addressbook"?`<label class="check-row"><input type="checkbox" data-action="admin-ab-force-toggle" ${ve.force?"checked":""} /> Force delete even if contacts exist</label>`:'<p class="muted small">Events on this calendar will be removed if this is the only instance.</p>'}`,footer:[{label:"Cancel",action:"admin-resource-delete-close",variant:"ghost"},{label:"Delete",action:"admin-resource-delete-confirm",variant:"danger",disabled:d}]}):"";return`<section class="card admin-user-detail">
      <div class="section-header">
    ...
    ${m}${n}${l}`}function pr(){const e=Ve("users");if(e&&e.available===!1)return ls("users");const t=or(),a=se&&oe.length===0?'<tr><td colspan="4" class="muted admin-table-empty">Loading users…</td></tr>':t.length===0?`<tr><td colspan="4" class="muted admin-table-empty">${te?o(te):bt.trim()?"No users match this filter.":"No users found."}</td></tr>`:t.map(r=>`<tr class="contact-table-row${B&&B.toLowerCase()===r.username.toLowerCase()?" is-selected":""}" data-action="admin-user-view" data-username="${o(r.username)}" tabindex="0" role="button">
      ...
      <section class="card">
        <div class="section-header">
          ...
          <div class="section-actions">
            ${e?`<span class="badge ${qa(e.status)}">${o(va(e.status))}</span>`:""}
      ...
      ${hr()}`}function hr(){if(!ba)return"";const e=yt.trim()==="CONFIRM";return ke({id:"admin-db-confirm-modal",title:"Confirm database change",titleId:"admin-db-confirm-title",closeAction:"admin-db-confirm-close",size:"sm",body:`
          ...
          </label>`,footer:[{label:"Cancel",action:"admin-db-confirm-close",variant:"ghost",disabled:d},{label:"Save database settings",action:"admin-db-confirm-save",variant:"danger",disabled:d||!e}]})}function yr(){return $e()?qt()?w==="users"?pr():w==="settings"?fr():w==="database"?gr():ir():`<section class="card admin-coming-soon-card">
          <div class="admin-coming-soon-head">
      ...
      <path d="M3.5 8.5 6.5 11.5 12.5 4.5" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        title="${o(L)}" aria-label="${o(L)}" ${d||n===0?"disabled":""}>${u}</button>`,y=i?`<div class="bulk-bar" style="margin-top:0.75rem">
          ...
          </div>`:"",v=j,$=Ot.map(D=>`<option value="${D.id}" ${v&&v.instanceId===D.id?"selected":""}>${o(D.displayname)}</option>`).join(""),x=v?`<div class="card">
            ${ye(X?v.parentUid?"New subtask":"New task":"Edit task","tasks")}
            <form class="stack" data-form="task" style="margin-top:1rem">
              ${X?`<label>Calendar
                    ...
                    </label>`:`<p class="muted small">Calendar: <strong>${o(v.calendarName)}</strong>${v.readOnly?" · read-only":""}</p>`}
              <label>Title
                <input type="text" name="summary" required maxlength="500" value="${o(v.summary)}" ${v.readOnly&&!X?"readonly":""} />
              </label>
              <label>Description
                <textarea name="description" rows="4" maxlength="20000" ${v.readOnly&&!X?"readonly":""}>${o(v.description)}</textarea>
              </label>
              <label>Parent task
                <select name="parentUid" ${v.readOnly&&!X?"disabled":""}>
                  ${wr(v,X)}
                </select>
              ...
              <div class="form-grid form-grid-2">
                <label>Status
                  <select name="status" ${v.readOnly&&!X?"disabled":""}>
                    ${["NEEDS-ACTION","IN-PROCESS","COMPLETED","CANCELLED"].map(D=>`<option value="${D}" ${v.status===D?"selected":""}>${o(e(D))}</option>`).join("")}
                  </select>
                </label>
                ${oa({field:"due",name:"due",label:"Due",value:Da(v.due),dateOnly:!1,disabled:!!(v.readOnly&&!X),allowClear:!0})}
              </div>
              <div class="form-grid form-grid-2">
                <label>Priority (0–9)
                  <input type="number" name="priority" min="0" max="9" value="${o(String(v.priority||0))}" ${v.readOnly&&!X?"readonly":""} />
                </label>
                <label>% complete
                  <input type="number" name="percent" min="0" max="100" value="${o(String(v.percent||0))}" ${v.readOnly&&!X?"readonly":""} />
                </label>
              </div>
              <div class="form-actions-row">
                ${X||v.canWrite?`<button type="submit" class="btn btn-primary" ${d?"disabled":""}>${X?"Create task":"Save task"}</button>`:""}
                ${!X&&v.canWrite?`<button type="button" class="btn btn-ghost" data-action="new-subtask" ${d?"disabled":""}>Add subtask</button>
    ...
    </div>`}function Sr(){const e=$a.length===0?`<tr class="contacts-empty-row"><td colspan="3" class="muted">${ns?"No notes match your search.":"No notes yet. Add one below."}</td></tr>`:$a.map(i=>{const m=ue(i.instanceId,i.uri),n=!we&&m===it?" is-selected":"",l=(i.description||"").replace(/\s+/g," ").slice(0,80);return`<tr class="contact-table-row${n}" data-action="select-note" data-instance="${i.instanceId}" data-uri="${o(i.uri)}" tabindex="0" role="button">
    ...
    <form class="stack" data-form="initialize">
      <label>Server timezone
        <select name="timezone" required ${Z?"disabled":""}>
          ${$n((s==null?void 0:s.timezone)||"UTC")}
        </select>
      </label>
      <h3 class="admin-subsection-title">DAV services</h3>
  ...
  </section>`}function ft(){const s=document.getElementById("app");if(!s)return;const c=(P==null?void 0:P.step)||"permissions";let g="";P?c==="permissions"?g=gl():c==="initialize"?g=hl():c==="database"?g=yl():c==="upgrade"?g=vl():c==="done"?g=$l():c==="locked"?g=wl():g=`<section class="card"><p>Unknown step: ${o(c)}</p></section>`:g='<section class="card"><p class="muted">Loading installer…</p></section>',s.innerHTML=`
