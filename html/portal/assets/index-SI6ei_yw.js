var Br=Object.defineProperty;var jr=(s,c,g)=>c in s?Br(s,c,{enumerable:!0,configurable:!0,writable:!0,value:g}):s[c]=g;var As=(s,c,g)=>jr(s,typeof c!="symbol"?c+"":c,g);(function(){const c=document.createElement("link").relList;if(c&&c.supports&&c.supports("modulepreload"))return;for(const w of document.querySelectorAll('link[rel="modulepreload"]'))h(w);new MutationObserver(w=>{for(const I of w)if(I.type==="childList")for(const U of I.addedNodes)U.tagName==="LINK"&&U.rel==="modulepreload"&&h(U)}).observe(document,{childList:!0,subtree:!0});function g(w){const I={};return w.integrity&&(I.integrity=w.integrity),w.referrerPolicy&&(I.referrerPolicy=w.referrerPolicy),w.crossOrigin==="use-credentials"?I.credentials="include":w.crossOrigin==="anonymous"?I.credentials="omit":I.credentials="same-origin",I}function h(w){if(w.ep)return;w.ep=!0;const I=g(w);fetch(w.href,I)}})();const nn={off:0,error:1,warn:2,info:3,debug:4};let Ma="off";const os="[angaradav-portal]";function zr(s){const c=(s||"off").toLowerCase().trim();return c==="error"||c==="warn"||c==="info"||c==="debug"||c==="off"?c:"off"}function Hr(s){return Ma=zr(s),Ma!=="off"&&console.info(os,`log level = ${Ma}`),Ma}function cn(s){return nn[Ma]>=nn[s]}function as(s,c,g,h){if(!cn(s))return;const w=[os,g];h!==void 0&&w.push(h),console[c](...w)}function Wr(s,c){cn("info")&&(c&&Object.keys(c).length>0?console.info(os,`event:${s}`,c):console.info(os,`event:${s}`))}const N={error(s,c){as("error","error",s,c)},warn(s,c){as("warn","warn",s,c)},info(s,c){as("info","info",s,c)},debug(s,c){as("debug","debug",s,c)},event:Wr};class qe extends Error{constructor(g,h,w={}){super(g);As(this,"status");As(this,"payload");this.status=h,this.payload=w}}let da="",ns=null,rs=null;function ls(s){da=s&&typeof s=="string"?s:""}function Jr(s){ns=s}function Yr(s){rs=s}function _s(s){if(!un(s))try{rs==null||rs()}catch{}}function un(s){return s==="/login"||s==="/ui"||s==="/logout"||s==="/install/status"||s.startsWith("/install/")}function is(s,c){if(!un(s)){ls("");try{ns==null||ns(c||"Session timed out. Please sign in again.")}catch{}}}async function x(s,c={}){const g=new Headers(c.headers);c.body&&!g.has("Content-Type")&&g.set("Content-Type","application/json");const h=(c.method||"GET").toUpperCase();h!=="GET"&&h!=="HEAD"&&h!=="OPTIONS"&&da&&g.set("X-CSRF-Token",da);const w=typeof performance<"u"?performance.now():Date.now();N.debug(`api → ${h} ${s}`);const I=await fetch(`/api${s}`,{...c,headers:g,credentials:"same-origin"});let U=null;const R=await I.text();if(R)try{U=JSON.parse(R)}catch{U={error:R}}const W=Math.round((typeof performance<"u"?performance.now():Date.now())-w);if(!I.ok){let ee=`Request failed (${I.status})`,ie={};if(U&&typeof U=="object"&&U!==null){const se=U;ie={...se},typeof se.error=="string"&&(ee=se.error)}else(I.status===500||I.status===504)&&(ee="Server error during import (often a timeout on large calendars). Try again — already imported events update faster.");throw I.status>=500?N.error(`api ← ${h} ${s} ${I.status} (${W}ms)`,ee):I.status!==401?N.warn(`api ← ${h} ${s} ${I.status} (${W}ms)`,ee):(N.debug(`api ← ${h} ${s} 401 (${W}ms)`),is(s,ee)),new qe(ee,I.status,ie)}return N.info(`api ← ${h} ${s} ${I.status} (${W}ms)`),_s(s),U}function st(s){return encodeURIComponent(s)}async function rn(s,c,g,h){const w=new Headers({"Content-Type":g,Accept:"application/x-ndjson, application/json;q=0.9"});da&&w.set("X-CSRF-Token",da);const I=typeof performance<"u"?performance.now():Date.now();N.debug(`api → POST ${s} (stream, ${g}, ${c.length} bytes)`);let U;try{U=await fetch(`/api${s}`,{method:"POST",headers:w,credentials:"same-origin",body:c})}catch(V){const G=V instanceof Error?V.message:"Network error";throw N.error(`api ← POST ${s} network fail`,G),new qe(`Import request failed to start (${G}). Check connectivity and container logs.`,0)}const R=(U.headers.get("Content-Type")||"").toLowerCase(),W=R.includes("ndjson")||R.includes("x-ndjson");if(!U.ok&&!W){let V=`Request failed (${U.status})`;try{const G=await U.json();G.error&&(V=G.error)}catch{}throw(U.status===504||U.status===502)&&(V="Gateway timeout during import. Pull the latest image (nginx 900s timeout) and recreate the container. Large calendars can take several minutes."),U.status===401?(N.debug(`api ← POST ${s} 401`,V),is(s,V)):N.warn(`api ← POST ${s} ${U.status}`,V),new qe(V,U.status)}if(!W&&U.ok){try{const V=await U.json();if(V&&typeof V.error=="string")throw new qe(V.error,U.status||500);if(V&&typeof V.imported=="number"&&typeof V.updated=="number")return N.info(`api ← POST ${s} json done`),V}catch(V){if(V instanceof qe)throw V}throw new qe("Unexpected import response from server",500)}if(!U.body)throw new qe("Import stream unavailable",500);const ee=U.body.getReader(),ie=new TextDecoder;let se="";const te={final:null,error:null,sawProgress:!1},mt=V=>{let G;try{G=JSON.parse(V)}catch{N.debug("import stream non-JSON line",V.slice(0,80));return}if(G.type==="progress"){te.sawProgress=!0;const Ie=Number(G.total)||0,je=Number(G.current)||0,Le=typeof G.percent=="number"?G.percent:Ie>0?Math.round(100*je/Ie):0;h==null||h({percent:Le,current:je,total:Ie,imported:Number(G.imported)||0,updated:Number(G.updated)||0,skipped:Number(G.skipped)||0})}else G.type==="done"&&G.result?te.final=G.result:G.type==="error"&&(te.error={message:G.error||"Import failed",status:G.status||500})};for(;;){const{done:V,value:G}=await ee.read();if(V)break;se+=ie.decode(G,{stream:!0});const Ie=se.split(`
`);se=Ie.pop()??"";for(const je of Ie){const Le=je.trim();Le&&mt(Le)}}se.trim()&&mt(se.trim());const B=Math.round((typeof performance<"u"?performance.now():Date.now())-I);if(te.error)throw te.error.status===401?(N.debug(`api ← POST ${s} stream 401 (${B}ms)`,te.error.message),is(s,te.error.message)):N.warn(`api ← POST ${s} stream error (${B}ms)`,te.error.message),new qe(te.error.message,te.error.status);if(!te.final)throw N.error(`api ← POST ${s} stream incomplete (${B}ms)`,{sawProgress:te.sawProgress}),new qe(te.sawProgress?"Import stopped before finishing (server crash, out of memory, or gateway timeout). On TrueNAS, set memory limit to at least 1G, pull latest image, and recreate the app.":"Import failed to start on the server. Check container logs and that you are on the latest image.",500);return N.info(`api ← POST ${s} stream done (${B}ms)`),_s(s),te.final}const A={ui:()=>x("/ui"),installStatus:async()=>{const s=await x("/install/status");return s&&typeof s=="object"&&"data"in s&&s.data?s.data:s},adminPing:()=>x("/admin/ping"),adminDashboard:()=>x("/admin/dashboard"),adminCapabilities:()=>x("/admin/capabilities"),adminUsers:()=>x("/admin/users"),adminUser:s=>x(`/admin/users/${encodeURIComponent(s)}`),adminCreateUser:s=>x("/admin/users",{method:"POST",body:JSON.stringify(s)}),adminUpdateUser:(s,c)=>x(`/admin/users/${encodeURIComponent(s)}`,{method:"PATCH",body:JSON.stringify(c)}),adminDeleteUser:(s,c=!0)=>x(`/admin/users/${encodeURIComponent(s)}`,{method:"DELETE",body:JSON.stringify({confirm:c})}),adminUserCalendars:s=>x(`/admin/users/${encodeURIComponent(s)}/calendars`),adminCreateUserCalendar:(s,c)=>x(`/admin/users/${encodeURIComponent(s)}/calendars`,{method:"POST",body:JSON.stringify(c)}),adminUpdateUserCalendar:(s,c,g)=>x(`/admin/users/${encodeURIComponent(s)}/calendars/${c}`,{method:"PATCH",body:JSON.stringify(g)}),adminDeleteUserCalendar:(s,c,g=!0)=>x(`/admin/users/${encodeURIComponent(s)}/calendars/${c}`,{method:"DELETE",body:JSON.stringify({confirm:g})}),adminUserAddressBooks:s=>x(`/admin/users/${encodeURIComponent(s)}/addressbooks`),adminCreateUserAddressBook:(s,c)=>x(`/admin/users/${encodeURIComponent(s)}/addressbooks`,{method:"POST",body:JSON.stringify(c)}),adminUpdateUserAddressBook:(s,c,g)=>x(`/admin/users/${encodeURIComponent(s)}/addressbooks/${c}`,{method:"PATCH",body:JSON.stringify(g)}),adminDeleteUserAddressBook:(s,c,g=!0,h=!1)=>x(`/admin/users/${encodeURIComponent(s)}/addressbooks/${c}`,{method:"DELETE",body:JSON.stringify({confirm:g,force:h})}),adminSystemSettings:()=>x("/admin/settings/system"),adminUpdateSystemSettings:s=>x("/admin/settings/system",{method:"PATCH",body:JSON.stringify(s)}),adminResetToDefault:(s=!0,c="")=>x("/admin/settings/reset-to-default",{method:"POST",body:JSON.stringify({confirm:s,password:c})}),adminDatabaseSettings:()=>x("/admin/settings/database"),adminTestDatabaseConnection:s=>x("/admin/settings/database/test",{method:"POST",body:JSON.stringify(s)}),adminUpdateDatabaseSettings:s=>x("/admin/settings/database",{method:"PATCH",body:JSON.stringify(s)}),me:async()=>{var c;const s=await x("/me");return ls(s.csrfToken||((c=s.user)==null?void 0:c.csrfToken)),s},login:async(s,c)=>{var h;const g=await x("/login",{method:"POST",body:JSON.stringify({username:s,password:c})});return ls((h=g.user)==null?void 0:h.csrfToken),g},logout:async()=>{try{return await x("/logout",{method:"POST"})}finally{ls("")}},calendars:()=>x("/calendars"),createCalendar:s=>x("/calendars",{method:"POST",body:JSON.stringify(s)}),holidayCountries:()=>x("/holidays/countries"),updateCalendar:(s,c)=>x(`/calendars/${s}`,{method:"PATCH",body:JSON.stringify(c)}),deleteCalendar:s=>x(`/calendars/${s}`,{method:"DELETE"}),calendarEvents:(s,c,g)=>{const h=new URLSearchParams({from:c,to:g}).toString();return x(`/calendars/${s}/events?${h}`)},getEvent:(s,c)=>x(`/calendars/${s}/events/${st(c)}`),createEvent:(s,c)=>x(`/calendars/${s}/events`,{method:"POST",body:JSON.stringify(c)}),updateEvent:(s,c,g)=>x(`/calendars/${s}/events/${st(c)}`,{method:"PATCH",body:JSON.stringify(g)}),deleteEvent:(s,c)=>x(`/calendars/${s}/events/${st(c)}`,{method:"DELETE"}),exportCalendar:async s=>{const c=await fetch(`/api/calendars/${s}/export`,{credentials:"same-origin"});if(!c.ok){let U=`Export failed (${c.status})`;try{const R=await c.json();R.error&&(U=R.error)}catch{}throw new qe(U,c.status)}const g=c.headers.get("Content-Disposition")||"",h=/filename="([^"]+)"/i.exec(g),w=(h==null?void 0:h[1])||`calendar-${s}.ics`;return{blob:await c.blob(),filename:w}},importCalendar:(s,c,g)=>rn(`/calendars/${s}/import`,c,"text/calendar; charset=utf-8",g),directory:()=>x("/directory"),shares:s=>x(`/calendars/${s}/shares`),share:(s,c,g)=>x(`/calendars/${s}/shares`,{method:"POST",body:JSON.stringify({username:c,access:g})}),revoke:(s,c)=>x(`/calendars/${s}/shares`,{method:"DELETE",body:JSON.stringify({href:c})}),addressbooks:()=>x("/addressbooks"),createAddressBook:s=>x("/addressbooks",{method:"POST",body:JSON.stringify(s)}),updateAddressBook:(s,c)=>x(`/addressbooks/${s}`,{method:"PATCH",body:JSON.stringify(c)}),deleteAddressBook:(s,c=!1)=>x(`/addressbooks/${s}`,{method:"DELETE",body:JSON.stringify({force:c})}),exportAddressBook:async s=>{const c=await fetch(`/api/addressbooks/${s}/export`,{credentials:"same-origin"});if(!c.ok){let U=`Export failed (${c.status})`;try{const R=await c.json();R.error&&(U=R.error)}catch{}throw new qe(U,c.status)}const g=c.headers.get("Content-Disposition")||"",h=/filename="([^"]+)"/i.exec(g),w=(h==null?void 0:h[1])||`contacts-${s}.vcf`;return{blob:await c.blob(),filename:w}},importAddressBook:(s,c,g)=>rn(`/addressbooks/${s}/import`,c,"text/vcard; charset=utf-8",g),contacts:(s,c="")=>{const g=c.trim()?`?q=${encodeURIComponent(c.trim())}`:"";return x(`/addressbooks/${s}/contacts${g}`)},getContact:(s,c)=>x(`/addressbooks/${s}/contacts/${st(c)}`),createContact:(s,c)=>x(`/addressbooks/${s}/contacts`,{method:"POST",body:JSON.stringify(c)}),updateContact:(s,c,g)=>x(`/addressbooks/${s}/contacts/${st(c)}`,{method:"PATCH",body:JSON.stringify(g)}),deleteContact:(s,c)=>x(`/addressbooks/${s}/contacts/${st(c)}`,{method:"DELETE"}),exportContact:async(s,c)=>{const g=await fetch(`/api/addressbooks/${s}/contacts/${st(c)}/export`,{credentials:"same-origin"});if(!g.ok){let R=`Export failed (${g.status})`;try{const W=await g.json();W.error&&(R=W.error)}catch{}throw new qe(R,g.status)}const h=g.headers.get("Content-Disposition")||"",w=/filename="([^"]+)"/i.exec(h),I=(w==null?void 0:w[1])||"contact.vcf";return{blob:await g.blob(),filename:I}},contactPhotoUrl:(s,c)=>`/api/addressbooks/${s}/contacts/${st(c)}/photo`,tasks:(s={})=>{const c=new URLSearchParams;s.q&&c.set("q",s.q),s.sort&&c.set("sort",s.sort),s.order&&c.set("order",s.order);const g=c.toString()?`?${c}`:"";return x(`/tasks${g}`)},createTask:s=>x("/tasks",{method:"POST",body:JSON.stringify(s)}),updateTask:(s,c,g)=>x(`/tasks/${s}/${st(c)}`,{method:"PATCH",body:JSON.stringify(g)}),deleteTask:(s,c)=>x(`/tasks/${s}/${st(c)}`,{method:"DELETE"}),bulkTasks:s=>x("/tasks/bulk",{method:"POST",body:JSON.stringify(s)}),notes:(s={})=>{const c=new URLSearchParams;s.q&&c.set("q",s.q),s.sort&&c.set("sort",s.sort),s.order&&c.set("order",s.order);const g=c.toString()?`?${c}`:"";return x(`/notes${g}`)},createNote:s=>x("/notes",{method:"POST",body:JSON.stringify(s)}),updateNote:(s,c,g)=>x(`/notes/${s}/${st(c)}`,{method:"PATCH",body:JSON.stringify(g)}),deleteNote:(s,c)=>x(`/notes/${s}/${st(c)}`,{method:"DELETE"}),filesStatus:()=>x("/files"),filesList:(s="")=>{const c=new URLSearchParams;s&&c.set("path",s);const g=c.toString()?`?${c}`:"";return x(`/files/entries${g}`)},filesMkdir:(s,c)=>x("/files/mkdir",{method:"POST",body:JSON.stringify({path:s,name:c})}),filesUpload:async(s,c,g={})=>{const h=new URLSearchParams;s&&h.set("path",s),h.set("name",c.name),g.replace&&h.set("replace","1");const w=new Headers;da&&w.set("X-CSRF-Token",da);const I=new FormData;I.append("file",c,c.name),s&&I.append("path",s);const U=typeof performance<"u"?performance.now():Date.now();N.debug(`api → POST /files/upload path=${s||"/"} name=${c.name} size=${c.size}`);const R=await fetch(`/api/files/upload?${h}`,{method:"POST",headers:w,credentials:"same-origin",body:I}),W=await R.text();let ee=null;if(W)try{ee=JSON.parse(W)}catch{ee={error:W}}const ie=Math.round((typeof performance<"u"?performance.now():Date.now())-U);if(!R.ok){let se=`Upload failed (${R.status})`;throw ee&&typeof ee=="object"&&ee!==null&&"error"in ee&&typeof ee.error=="string"&&(se=ee.error),R.status===401?(N.debug(`api ← POST /files/upload 401 (${ie}ms)`,se),is("/files/upload",se)):R.status>=500?N.error(`api ← POST /files/upload ${R.status} (${ie}ms)`,se):N.warn(`api ← POST /files/upload ${R.status} (${ie}ms)`,se),new qe(se,R.status)}return N.info(`api ← POST /files/upload 200 (${ie}ms)`),_s("/files/upload"),ee},filesDownloadUrl:s=>{const c=new URLSearchParams;return c.set("path",s),`/api/files/download?${c}`},filesDelete:s=>x("/files/entry",{method:"DELETE",body:JSON.stringify({path:s})}),filesRename:(s,c)=>x("/files/rename",{method:"POST",body:JSON.stringify({path:s,newName:c})}),filesMove:(s,c,g)=>x("/files/move",{method:"POST",body:JSON.stringify({from:s,to:c,newName:g})}),filesCopy:(s,c={})=>x("/files/copy",{method:"POST",body:JSON.stringify({path:s,to:c.to,newName:c.newName})}),filesBulk:(s,c)=>x("/files/bulk",{method:"POST",body:JSON.stringify({op:s,paths:c})})},Kr=["UTC","Africa/Cairo","Africa/Johannesburg","America/Anchorage","America/Argentina/Buenos_Aires","America/Chicago","America/Denver","America/Edmonton","America/Halifax","America/Los_Angeles","America/Mexico_City","America/New_York","America/Sao_Paulo","America/Toronto","America/Vancouver","Asia/Dubai","Asia/Hong_Kong","Asia/Jerusalem","Asia/Kolkata","Asia/Seoul","Asia/Shanghai","Asia/Singapore","Asia/Tokyo","Australia/Melbourne","Australia/Sydney","Europe/Amsterdam","Europe/Berlin","Europe/London","Europe/Madrid","Europe/Moscow","Europe/Paris","Europe/Rome","Europe/Warsaw","Pacific/Auckland","Pacific/Honolulu"];let Sa=null;function Gr(){if(Sa)return Sa;try{const s=Intl;if(typeof s.supportedValuesOf=="function"){const c=s.supportedValuesOf("timeZone");if(Array.isArray(c)&&c.length>0)return Sa=[...c].sort((g,h)=>g.localeCompare(h)),Sa}}catch{}return Sa=[...Kr],Sa}function mn(s){const c=s||"UTC",g=Gr(),h=g.includes(c),w=g.map(I=>`<option value="${ln(I)}" ${I===c?"selected":""}>${on(I)}</option>`);return!h&&c&&w.unshift(`<option value="${ln(c)}" selected>${on(c)}</option>`),w.join("")}function ln(s){return s.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;")}function on(s){return s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function i(s){return s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function ia(s,c,g={}){if(!c)return"";const h=g.dismissible!==void 0?g.dismissible:g.dismissAction!==void 0,w=g.dismissAction??"flash-close",I=g.role??"status",U=g.className?` ${g.className}`:"",R=g.style?` style="${i(g.style)}"`:"",W=h?`<button type="button" class="flash-close" data-action="${i(w)}" aria-label="Dismiss message" title="Dismiss">×</button>`:"";return`<div class="flash flash-${i(s)}${U}" role="${i(I)}"${R}>
      <span class="flash-text">${i(c)}</span>
      ${W}
    </div>`}function Qr(s){return s==="sm"?" cal-modal-card-sm":s==="wide"?" cal-modal-card-wide":""}function Xr(s){return s==="danger"?"btn btn-danger":s==="ghost"?"btn btn-ghost":"btn btn-primary"}function pn(s){return s.map(g=>{const h=g.type??"button",w=Xr(g.variant),I=g.disabled?" disabled":"",U=g.id?` id="${i(g.id)}"`:"",R=g.action?` data-action="${i(g.action)}"`:"",W=g.attrs?` ${g.attrs}`:"";return`<button type="${h}" class="${w}"${R}${U}${W}${I}>${i(g.label)}</button>`}).join(`
`)}function De(s){const c=s.titleId||(s.id?`${s.id}-title`:`modal-title-${Math.random().toString(36).slice(2,9)}`),g=s.id?` id="${i(s.id)}"`:"",h=s.className?` ${s.className}`:"",w=s.rootAttrs?` ${s.rootAttrs}`:"",I=`${Qr(s.size)}${s.cardClassName?` ${s.cardClassName}`:""}`,U=s.closeAction,R=s.lockBackdrop?"":` data-action="${i(U)}"`,W=s.hideClose?"":`<button type="button" class="modal-close info-modal-close" data-action="${i(U)}" aria-label="Close">×</button>`;let ee="";s.footer!==void 0&&(ee=typeof s.footer=="string"?s.footer:pn(s.footer));const ie=ee?`<footer class="cal-modal-footer">${ee}</footer>`:"",se=`<div class="cal-modal-body">${s.body}</div>`;let te;return s.form?te=`<form class="stack"${s.formAttrs?` ${s.formAttrs}`:""}>
        ${se}
        ${ie}
      </form>`:te=`${se}
    ...
    <div class="info-modal" id="info-modal" hidden role="dialog" aria-modal="true" aria-labelledby="info-modal-title">
    ...
    </div>`}function ll(s){let c=null,g=null,h=tl(),w=al(),I=null,U=!1,R=null,W=null,ee=null,ie=[],se=!1,te=null,mt="",B=qs().adminUsername??null,V=null,G=!1,Ie=null,je=!1,Le=!1,pt=null,At=!1,Et=[],Nt=[],Da=!1,We=null,zt=null,nt=null,Ht=null,$e=null,Wt=null,Ra=!1,Ca=null,ca=!1,ft=!1,Je="",Jt=null,Va=!1,Aa=null,Yt="sqlite",ua=!1,bt="",ma=null,Oe=!1,pa=null,ne=[],Kt=[],Ba=[],F=null,K=[],Gt=[],ze=null,fe=!1,Me=!1,Re=null,Ye=null,Tt={y:new Date().getFullYear(),m:new Date().getMonth()},Qt=[],ms=!1,gt=!1,k=null,rt=!1,O=null,ja="",Ea=null,Ue=[],M=null,$t=[],Xt="",ce=null,q=null,be=!1,Ce=!1,Ke=!1,Ee=null,He=null,Ge=!1,d=!1,j=null,za=null,Ls=!1,fa={timeFormat:"auto",weekStart:"auto",logLevel:"off"},Qe=null,Os=900,Na=null,Zt=Zr,ps=!1,Ha=!1;function fs(e){if(!e)return;const t=(e.timeFormat||"auto").toLowerCase(),a=(e.weekStart||"auto").toLowerCase();fa={timeFormat:t==="12h"||t==="24h"?t:"auto",weekStart:a==="monday"||a==="sunday"?a:"auto",logLevel:e.logLevel||"off"},Hr(fa.logLevel),typeof e.sessionIdleSeconds=="number"&&Number.isFinite(e.sessionIdleSeconds)&&e.sessionIdleSeconds>0&&(Os=Math.floor(e.sessionIdleSeconds)),typeof e.version=="string"&&e.version.trim()!==""&&(Zt=e.version.trim())}function bs(){Na!==null&&(clearTimeout(Na),Na=null)}function gs(){if(bs(),!c)return;const e=Math.max(30,Os)*1e3;Na=setTimeout(()=>{Na=null,Rs("Your session timed out. Please sign in again.")},e)}function Wa(){bs(),yt(),j=null,c=null,ne=[],Gt=[],F=null,K=[],Kt=[],Ue=[],M=null,$t=[],ce=null,q=null,be=!1,Ce=!1,Ke=!1,Me=!1,fe=!1,Re=null,Ye=null,gt=!1,k=null,rt=!1,Qt=[],Ne=[],ga=[],xt=[],qt=[],Pe=null,lt=null,z=null,re=null,Q=!1,ke=!1,ye=[],ys=null,Fe="",Te=[],ya=!1,Se=null,ge=null,de=null,ht=!1,ue=[],Ee=null,He=null,Ge=!1,d=!1,Oe=!1,I=null,U=!1,R=null,W=null,ee=null,ie=[],se=!1,te=null,mt="",B=null,V=null,G=!1,Ie=null,je=!1,Le=!1,pt=null,At=!1,Et=[],Nt=[],Da=!1,We=null,zt=null,nt=null,Ht=null,$e=null,Wt=null,Ra=!1,Ca=null,ca=!1,ft=!1,Je="",Jt=null,Va=!1,Aa=null,Yt="sqlite",ua=!1,bt="",ma=null,_a()}function we(){return!!(c!=null&&c.isAdmin||(c==null?void 0:c.role)==="Admin")}function _t(){return we()?W===null?!0:W.uiEnabled!==!1:!1}function Ve(e){const t=W==null?void 0:W.pages;return t?t.find(a=>a.id===e)??null:null}function ba(e){switch(e){case"full":return"Full";case"read-only":return"Read-only";case"coming-soon":return"Coming soon";case"deferred":return"Unavailable";default:return e}}function Ta(e){return e==="full"||e==="read-only"?"badge-ok":e==="deferred"?"badge-off":"badge-soon"}function _a(){pa&&(document.removeEventListener("click",pa,!0),pa=null)}function gn(){_a(),pa=t=>{var l;const a=t.target;(l=a==null?void 0:a.closest)!=null&&l.call(a,".user-menu")||(Oe=!1,_a(),m())};const e=pa;setTimeout(()=>{Oe&&pa===e&&document.addEventListener("click",e,!0)},0)}function Us(){h==="admin"&&(!we()||!_t())&&(h="calendars",w="overview",dt(h))}async function Ps(e,t={}){if(!we()){await Ms("calendars",t);return}h="admin",w=e,e!=="users"?(B=null,V=null,Ie=null):t.username!==void 0&&(B=t.username,t.username||(V=null,Ie=null)),Oe=!1,dt("admin",e,B),N.event("tab",{tab:"admin",adminPage:e,user:B}),t.clearFlash!==!1&&E(),d=!0,m();try{if(await hs(),!_t()){h="calendars",dt("calendars"),b("info","Portal Administration UI is disabled.");return}const a=Ve(e);e==="overview"&&(a==null?void 0:a.available)!==!1?await Ja():e==="users"&&(a==null?void 0:a.available)!==!1?(await ea(),B&&(await wt(B),await ta(B))):e==="settings"&&(a==null?void 0:a.available)!==!1?await Ya():e==="database"&&(a==null?void 0:a.available)!==!1&&await Ka()}catch(a){N.warn("admin page load failed",a instanceof Error?a.message:a),b("error",a instanceof Error?a.message:"Failed to load")}finally{d=!1,m()}}async function hs(){var e;ee=null;try{W=(await A.adminCapabilities()).data,N.debug("admin.capabilities",{uiEnabled:W.uiEnabled,pages:((e=W.pages)==null?void 0:e.length)??0})}catch(t){ee=t instanceof Error?t.message:"Failed to load capabilities",W={uiEnabled:!0,portalAdminUrl:"/portal/#admin",pages:[{id:"overview",label:"Overview",status:"full",available:!0,portalUrl:"/portal/#admin",portalLabel:"Overview",summary:"Live counts and service flags."},{id:"settings",label:"System settings",status:"full",available:!0,portalUrl:"/portal/#admin/settings",portalLabel:"System settings",summary:"Edit system flags and admin password in the portal."},{id:"users",label:"Users",status:"full",available:!0,portalUrl:"/portal/#admin/users",portalLabel:"Users",summary:"Full DAV user CRUD plus calendars and address books."},{id:"database",label:"Database",status:"full",available:!0,portalUrl:"/portal/#admin/database",portalLabel:"Database",summary:"Connection settings; saves require typing CONFIRM."}]},N.warn("admin.capabilities fallback",ee)}}async function Ja(){U=!0,R=null;try{I=(await A.adminDashboard()).data,N.debug("admin.dashboard",{users:I.users,calendars:I.calendars})}catch(e){throw I=null,R=e instanceof Error?e.message:"Failed to load dashboard",e}finally{U=!1}}async function ea(){se=!0,te=null;try{ie=(await A.adminUsers()).users??[],N.debug("admin.users",{count:ie.length})}catch(e){throw ie=[],te=e instanceof Error?e.message:"Failed to load users",e}finally{se=!1}}async function wt(e){G=!0,Ie=null;try{const t=await A.adminUser(e);V=t.user,B=t.user.username,N.debug("admin.user",{username:t.user.username})}catch(t){throw V=null,Ie=t instanceof Error?t.message:"Failed to load user",t}finally{G=!1}}async function ta(e){Da=!0;try{const[t,a]=await Promise.all([A.adminUserCalendars(e),A.adminUserAddressBooks(e)]);Et=t.calendars??[],Nt=a.addressbooks??[]}catch(t){throw Et=[],Nt=[],t}finally{Da=!1}}async function Ya(){Ra=!0,Ca=null;try{Wt=(await A.adminSystemSettings()).data}catch(e){throw Wt=null,Ca=e instanceof Error?e.message:"Failed to load settings",e}finally{Ra=!1}}async function Ka(){Va=!0,Aa=null;try{const e=await A.adminDatabaseSettings();Jt=e.data,Yt=(e.data.backend||"sqlite").toLowerCase()==="pgsql"?"pgsql":"sqlite"}catch(e){throw Jt=null,Aa=e instanceof Error?e.message:"Failed to load database settings",e}finally{Va=!1}}async function hn(e){const t=new FormData(e),a=String(t.get("username")??"").trim(),l=String(t.get("displayname")??"").trim(),o=String(t.get("email")??"").trim(),p=String(t.get("password")??""),n=String(t.get("passwordConfirm")??"");if(!a||!l||!o||!p){b("error","Username, display name, email, and password are required"),m();return}if(p!==n){b("error","Password confirmation does not match"),m();return}d=!0,E(),m();try{const r=await A.adminCreateUser({username:a,displayname:l,email:o,password:p,passwordConfirm:n});N.event("admin.user.create",{username:r.user.username}),je=!1,B=r.user.username,V=r.user,dt("admin","users",r.user.username),await ea(),b("success",`Created user “${r.user.username}”`)}catch(r){b("error",r instanceof Error?r.message:"Create failed")}finally{d=!1,m()}}async function yn(e){var u,f;if(!B)return;const t=B,a=new FormData(e),l=String(a.get("displayname")??"").trim(),o=String(a.get("description")??"").trim(),p=String(a.get("calendarcolor")??"").trim(),n=((u=e.querySelector('input[name="todos"]'))==null?void 0:u.checked)??!1,r=((f=e.querySelector('input[name="notes"]'))==null?void 0:f.checked)??!1;d=!0,E(),m();try{if(We==="create"){const y=String(a.get("uri")??"").trim().toLowerCase();await A.adminCreateUserCalendar(t,{uri:y,displayname:l,description:o,calendarcolor:p||void 0,todos:n,notes:r}),b("success",`Created calendar “${l}”`)}else{const y=Number(a.get("instanceId"));await A.adminUpdateUserCalendar(t,y,{displayname:l,description:o,calendarcolor:p,todos:n,notes:r}),b("success",`Updated calendar “${l}”`)}We=null,zt=null,await ta(t),await wt(t)}catch(y){b("error",y instanceof Error?y.message:"Save failed")}finally{d=!1,m()}}async function vn(e){if(!B)return;const t=B,a=new FormData(e),l=String(a.get("displayname")??"").trim(),o=String(a.get("description")??"").trim();d=!0,E(),m();try{if(nt==="create"){const p=String(a.get("uri")??"").trim().toLowerCase();await A.adminCreateUserAddressBook(t,{uri:p,displayname:l,description:o}),b("success",`Created address book “${l}”`)}else{const p=Number(a.get("id"));await A.adminUpdateUserAddressBook(t,p,{displayname:l,description:o}),b("success",`Updated address book “${l}”`)}nt=null,Ht=null,await ta(t),await wt(t)}catch(p){b("error",p instanceof Error?p.message:"Save failed")}finally{d=!1,m()}}function Fs(e){const t=new FormData(e),a=String(t.get("backend")??Yt).toLowerCase()==="pgsql"?"pgsql":"sqlite",l={backend:a};return a==="sqlite"?l.sqlite_file=String(t.get("sqlite_file")??"").trim():(l.pgsql_host=String(t.get("pgsql_host")??"").trim(),l.pgsql_dbname=String(t.get("pgsql_dbname")??"").trim(),l.pgsql_username=String(t.get("pgsql_username")??"").trim(),l.pgsql_password=String(t.get("pgsql_password")??"")),l}function $n(e){ma=Fs(e),bt="",ua=!0,E(),m()}async function wn(e){if(e||(e=s.querySelector('[data-form="admin-database"]')),!e){b("error","Database form not found"),m();return}const t=Fs(e);d=!0,E(),m();try{const a=await A.adminTestDatabaseConnection(t);b("success",a.message||"Connection successful"),N.event("admin.database.test",{backend:a.backend})}catch(a){b("error",a instanceof Error?a.message:"Connection test failed")}finally{d=!1,m()}}async function kn(e){const t=new FormData(e),a=n=>{var r;return!!((r=e.querySelector(`input[name="${n}"]`))!=null&&r.checked)},l={cal_enabled:a("cal_enabled"),card_enabled:a("card_enabled"),tasks_enabled:a("tasks_enabled"),notes_enabled:a("notes_enabled"),files_enabled:a("files_enabled"),push_enabled:a("push_enabled"),portal_admin_ui_enabled:a("portal_admin_ui_enabled"),timezone:String(t.get("timezone")??"").trim(),invite_from:String(t.get("invite_from")??"").trim(),dav_auth_type:String(t.get("dav_auth_type")??"Digest"),files_storage_path:String(t.get("files_storage_path")??"").trim(),files_max_upload_mb:Number(t.get("files_max_upload_mb")??0),files_quota_mb:Number(t.get("files_quota_mb")??0),files_quarantine_days:Number(t.get("files_quarantine_days")??0),session_max_age_minutes:Number(t.get("session_max_age_minutes")??15),portal_log_level:String(t.get("portal_log_level")??"off"),portal_admin_users:String(t.get("portal_admin_users")??"").trim(),push_external_url:String(t.get("push_external_url")??"").trim(),push_log_level:String(t.get("push_log_level")??"off")},o=String(t.get("admin_password")??""),p=String(t.get("admin_password_confirm")??"");(o!==""||p!=="")&&(l.admin_password=o,l.admin_password_confirm=p),d=!0,E(),m();try{Wt=(await A.adminUpdateSystemSettings(l)).data,N.event("admin.settings.save"),b("success","System settings saved")}catch(n){b("error",n instanceof Error?n.message:"Save failed")}finally{d=!1,m()}}async function Sn(e){const t=new FormData(e),a=String(t.get("username")??"").trim(),l=String(t.get("displayname")??"").trim(),o=String(t.get("email")??"").trim(),p=String(t.get("password")??""),n=String(t.get("passwordConfirm")??"");if(!a){b("error","Username is required"),m();return}if(!l||!o){b("error","Display name and email are required"),m();return}if(p!==""||n!==""){if(p===""||n===""){b("error","Password and confirmation are required to change password"),m();return}if(p!==n){b("error","Password confirmation does not match"),m();return}}d=!0,E(),m();try{const r={displayname:l,email:o};p!==""&&(r.password=p,r.passwordConfirm=n);const u=await A.adminUpdateUser(a,r);N.event("admin.user.update",{username:u.user.username,passwordChanged:p!==""}),Le=!1,V=u.user,B=u.user.username,await ea(),b("success",p!==""?`Updated “${u.user.username}” (password changed)`:`Updated “${u.user.username}”`)}catch(r){b("error",r instanceof Error?r.message:"Update failed")}finally{d=!1,m()}}async function Ms(e,t={}){if(e==="admin"&&(!we()||!_t())&&(we()&&W&&!W.uiEnabled&&b("info","Portal Administration UI is disabled (portal_admin_ui_enabled)."),e="calendars"),e==="admin"){await Ps(w||"overview",{...t,username:w==="users"?B:null});return}h=e,Oe=!1,dt(e),N.event("tab",{tab:e}),e!=="calendars"&&(fe=!1,Re=null),e!=="contacts"&&(Ye=null),t.clearFlash!==!1&&E(),d=!0,m();try{e==="contacts"&&M!==null?await Ot(M):e==="calendars"?await Ze():e==="tasks"?await Ut():e==="notes"?await va():e==="files"&&await kt()}catch(a){N.warn("tab load failed",a instanceof Error?a.message:a),b("error",a instanceof Error?a.message:"Failed to load")}finally{d=!1,m()}}async function kt(){ya=!0;try{N.debug("loadFiles",{path:Fe});const[e,t]=await Promise.all([A.filesStatus(),A.filesList(Fe).catch(a=>{if(a instanceof qe&&(a.status===503||a.status===404))return{path:Fe,entries:[]};throw a})]);if(ys=e,e.ready){Fe=t.path,Te=t.entries;const a=new Set(Te.map(l=>l.path));ue=ue.filter(l=>a.has(l))}else Te=[],ue=[];N.event("loadFiles",{path:Fe,count:Te.length,enabled:e.enabled,ready:e.ready})}finally{ya=!1}}function Rs(e){if(!ps){if(!c){bs();return}ps=!0;try{N.event("session.expired"),Wa(),Ha=!0,g={type:"info",message:e&&e.trim()?e:"Your session timed out. Please sign in again."},m()}finally{ps=!1}}}let Ne=[],ga=[],xt=[],qt=[],Ga="",Qa="",It="due",St="asc",ha="dtstart",aa="desc",Pe=null,lt=null,z=null,re=null,Q=!1,ke=!1,ye=[],ys=null,Fe="",Te=[],ya=!1,Se=null,ge=null,de=null,ht=!1,ue=[];function b(e,t){Ha&&e==="error"||(e!=="error"&&(Ha=!1),g={type:e,message:t})}function E(){g=null,Ha=!1}function Dn(e){const t=String(e.step||"");t==="upgrade"||t==="initialize"||t==="permissions"||t==="database"?(ze={step:t,message:e.message||(t==="upgrade"?"Complete the upgrade wizard before signing in.":"Complete setup before signing in."),installUrl:e.installUrl||"/portal/install/",productVersion:e.productVersion,configuredVersion:e.configuredVersion??null},typeof e.productVersion=="string"&&e.productVersion.trim()!==""&&(Zt=e.productVersion.trim())):ze=null}function Cn(e){if(!(e instanceof qe)||e.status!==503)return!1;const t=typeof e.payload.code=="string"?e.payload.code:"";return t!=="upgrade_required"&&t!=="not_configured"&&t!=="admin_password_missing"?!1:(ze={step:t==="upgrade_required"?"upgrade":"initialize",message:e.message,installUrl:typeof e.payload.installUrl=="string"?e.payload.installUrl:"/portal/install/",productVersion:typeof e.payload.productVersion=="string"?e.payload.productVersion:void 0,configuredVersion:typeof e.payload.configuredVersion=="string"?e.payload.configuredVersion:null},ze.productVersion&&(Zt=ze.productVersion),!0)}async function An(){var e,t,a,l;N.event("bootstrap.start"),Jr(o=>{Rs(/timed\s*out|session expired/i.test(o)?o:"Your session timed out. Please sign in again.")}),Yr(()=>{gs()});try{const o=await A.installStatus();Dn(o)}catch(o){N.debug("bootstrap: /api/install/status failed",o instanceof Error?o.message:o)}try{const o=await A.ui();fs(o.ui),typeof o.version=="string"&&o.version.trim()!==""?Zt=o.version.trim():o.ui&&typeof o.ui.version=="string"&&o.ui.version.trim()!==""&&(Zt=o.ui.version.trim()),ze==null||ze.step}catch(o){N.debug("bootstrap: /api/ui failed",o instanceof Error?o.message:o),Cn(o)}if(ze&&ze.step!=="done"&&ze.step!=="locked"){Wa(),N.event("bootstrap.installGate",{step:ze.step}),m();return}try{const o=await A.me();if(c=o.user,fs(o.ui),typeof o.version=="string"&&o.version.trim()!==""&&(Zt=o.version.trim()),N.event("bootstrap.session",{username:(c==null?void 0:c.username)??null}),gs(),we())try{await hs()}catch(p){N.warn("admin.capabilities bootstrap",p instanceof Error?p.message:p)}if(Us(),dt(h,w),await Xe(),h==="admin"&&we()&&_t())try{w==="overview"&&((e=Ve("overview"))==null?void 0:e.available)!==!1?await Ja():w==="users"&&((t=Ve("users"))==null?void 0:t.available)!==!1?(await ea(),B&&(await wt(B),await ta(B))):w==="settings"&&((a=Ve("settings"))==null?void 0:a.available)!==!1?await Ya():w==="database"&&((l=Ve("database"))==null?void 0:l.available)!==!1&&await Ka()}catch(p){N.warn("admin bootstrap load",p instanceof Error?p.message:p)}}catch(o){o instanceof qe&&o.status===401?(Wa(),/timed\s*out|session expired/i.test(o.message)&&b("info",o.message),N.event("bootstrap.anonymous")):(N.error("bootstrap failed",o instanceof Error?o.message:o),b("error",o instanceof Error?o.message:"Failed to load"))}m()}async function Xe(){N.debug("loadHome");const[e,t,a]=await Promise.all([A.calendars(),A.directory().catch(()=>({users:[]})),A.addressbooks()]);if(ne=e.calendars,Kt=t.users,Ue=a.addressbooks,N.event("loadHome",{calendars:ne.length,addressBooks:Ue.length,directory:Kt.length}),Ba.length===0)try{Ba=(await A.holidayCountries()).countries}catch{Ba=[]}if(K=K.filter(l=>ne.some(o=>o.id===l)),F!==null&&!ne.some(l=>l.id===F)&&(F=null,Gt=[],fe=!1,Re=null),K.length===0){const l=Vs();l?(K=[l.id],F=l.id):ne.length>0&&(K=[ne[0].id],F=ne[0].id)}F===null&&K.length>0&&(F=K[0]),F!==null&&fe?await xa(F):F!==null&&(Gt=[]),h==="calendars"&&await Ze(),M!==null&&!Ue.some(l=>l.id===M)&&(M=null,$t=[],ce=null,q=null,be=!1),Ye!==null&&!Ue.some(l=>l.id===Ye)&&(Ye=null),M===null&&Ue.length>0&&(M=Ue[0].id),M!==null&&h==="contacts"&&await Ot(M),h==="tasks"&&await Ut(),h==="notes"&&await va(),h==="files"&&await kt()}async function xa(e){Gt=(await A.shares(e)).shares}function Vs(){const e=ne.filter(a=>a.canShare);if(e.length===0)return null;const t=a=>{const l=a.uri.toLowerCase(),o=a.displayname.toLowerCase();return l==="default"||o==="default"||o==="default calendar"};return e.find(t)??e[0]??null}function he(e){const t=e.getFullYear(),a=String(e.getMonth()+1).padStart(2,"0"),l=String(e.getDate()).padStart(2,"0");return`${t}-${a}-${l}`}function En(e,t){const a=new Date(e,t,1),l=new Date(e,t+1,0);return{from:he(a),to:he(l)}}function vs(e){if(/^\d{4}-\d{2}-\d{2}$/.test(e)){const[a,l,o]=e.split("-").map(Number);return new Date(a,l-1,o)}const t=new Date(e);if(Number.isNaN(t.getTime())){const[a,l,o]=e.slice(0,10).split("-").map(Number);return new Date(a,(l||1)-1,o||1)}return new Date(t.getFullYear(),t.getMonth(),t.getDate())}function Nn(e){const t=vs(e.start);if(!e.end)return[he(t)];let a=vs(e.end);if(!e.allDay&&!/^\d{4}-\d{2}-\d{2}$/.test(e.end)){const r=new Date(e.end);!Number.isNaN(r.getTime())&&r.getHours()===0&&r.getMinutes()===0&&r.getSeconds()===0&&r.getTime()>new Date(e.start).getTime()&&(a=new Date(a.getFullYear(),a.getMonth(),a.getDate()-1))}if(a<t)return[he(t)];const l=[],o=new Date(t.getFullYear(),t.getMonth(),t.getDate()),p=new Date(a.getFullYear(),a.getMonth(),a.getDate());let n=0;for(;o<=p&&n++<370;)l.push(he(o)),o.setDate(o.getDate()+1);return l.length?l:[he(t)]}function $s(e,t){const a=e.slice(0,10),l=(t||a).slice(0,10);if(a===l){const $=Ia(a);return{start:$.start,end:$.end}}const[o,p,n]=a.split("-").map(Number),[r,u,f]=l.split("-").map(Number),y=Lt(new Date(o,p-1,n,9,0,0,0)),v=Lt(new Date(r,u-1,f,17,0,0,0));return{start:y,end:v}}function Tn(e,t){const a=sa(e);let l=t?sa(t):a;if(t&&!/^\d{4}-\d{2}-\d{2}$/.test(t)){const o=new Date(t);if(!Number.isNaN(o.getTime())&&o.getHours()===0&&o.getMinutes()===0&&o.getTime()>new Date(e).getTime()){const p=vs(t);p.setDate(p.getDate()-1),l=he(p)}}return{start:a,end:l}}async function Ze(){const e=K.filter(l=>ne.some(o=>o.id===l));if(e.length===0){Qt=[];return}const{from:t,to:a}=En(Tt.y,Tt.m);ms=!0,N.debug("loadMonthEvents",{selectedIds:e,from:t,to:a});try{const o=(await Promise.all(e.map(async p=>(await A.calendarEvents(p,t,a)).events.map(r=>({...r,instanceId:p}))))).flat();o.sort((p,n)=>{const r=p.start||"",u=n.start||"";return r!==u?r<u?-1:1:(p.summary||"").localeCompare(n.summary||"")}),Qt=o,N.event("monthEvents.loaded",{calendarIds:e,count:Qt.length,from:t,to:a})}catch(l){Qt=[],N.warn("loadMonthEvents failed",l instanceof Error?l.message:l)}finally{ms=!1}}function _n(e){const t=ne.find(a=>a.id===e);return t!=null&&t.color?t.color.length>=7?t.color.slice(0,7):t.color:"#3B82F6"}function xn(e){K.includes(e)?(K=K.filter(t=>t!==e),F===e&&(F=K[0]??null)):(K=[...K,e],F=e)}function qn(e,t){return new Date(e,t,1).toLocaleString(void 0,{month:"long",year:"numeric"})}function In(e){const t=e.summary||"(No title)";if(e.allDay||/^\d{4}-\d{2}-\d{2}$/.test(e.start))return t;const a=new Date(e.start);return Number.isNaN(a.getTime())?t:`${a.toLocaleTimeString(void 0,ws())} ${t}`}function Ln(){const e=ne.filter(T=>K.includes(T.id)),t=e.length===0?"No calendar selected":e.length===1?e[0].displayname:`${e.length} calendars`,a=Tt.y,l=Tt.m,o=new Date(a,l,1),p=ks(),n=(o.getDay()-p+7)%7,r=new Date(a,l+1,0).getDate(),u=new Date(a,l,0).getDate(),y=he(new Date),v=Bs(),$=new Map;for(const T of Qt)for(const Y of Nn(T)){const H=$.get(Y)??[];H.push(T),$.set(Y,H)}const _=[],D=Math.ceil((n+r)/7)*7;for(let T=0;T<D;T++){let Y,H=!0,X;T<n?(Y=u-n+T+1,H=!1,X=new Date(a,l-1,Y)):T>=n+r?(Y=T-(n+r)+1,H=!1,X=new Date(a,l+1,Y)):(Y=T-n+1,X=new Date(a,l,Y));const pe=he(X),Ae=pe===y,_e=H?$.get(pe)??[]:[],vt=Ea===pe?50:3,it=_e.slice(0,vt),Dt=_e.length-it.length,Be=it.map(J=>{var le;const et=J.instanceId,xe=In(J),tt=_n(et),oa=((le=ne.find(at=>at.id===et))==null?void 0:le.displayname)||"",S=oa?`${xe} · ${oa}`:xe;return`<button type="button" class="month-event${J.allDay?"":" is-timed"}" title="${i(S)}" style="--ev-color:${i(tt)}"
            data-action="open-event" data-instance="${et}" data-uri="${i(J.uri)}" ${d?"disabled":""}>${i(xe)}</button>`}).join(""),Rt=Dt>0?`<button type="button" class="month-event-more" data-action="open-event-day" data-day="${i(pe)}" title="Show all events this day" ${d?"disabled":""}>+${Dt} more</button>`:"",Vt=!H&&(Y===1||T===n+r)?X.toLocaleString(void 0,{month:"short",day:"numeric"}):String(Y),C=F!==null?ne.find(J=>J.id===F)??null:null,oe=!!(C&&!C.readOnly&&(C.canShare||C.access==="readwrite"));_.push(`<div class="month-cell${H?"":" is-outside"}${Ae?" is-today":""}${oe?" is-clickable":""}"${oe?` data-action="new-event-day" data-day="${i(pe)}" role="button" tabindex="0" title="Add event on ${i(pe)}"`:""}>
        <div class="month-daynum${Ae?" is-today-num":""}">${i(Vt)}</div>
        <div class="month-events">${Be}${Rt}</div>
      </div>`)}const L=e.length===0?ne.length===0?'<p class="muted small month-empty-hint">No calendars yet — create one on the left, or wait for someone to share with you.</p>':'<p class="muted small month-empty-hint">Check one or more calendars on the left to view events.</p>':ms?'<p class="muted small month-empty-hint">Loading events…</p>':"",ae=e.slice(0,6).map(T=>{const Y=T.color&&T.color.length>=7?T.color.slice(0,7):T.color||"#3B82F6";return`<span class="cal-swatch" style="background:${i(Y)};margin-top:0" title="${i(T.displayname)}"></span>`}).join("");return`<section class="card month-cal-card">
      ...
      ${f&&!r?Fn(t,o,p,u):""}
    </div>`}function Ss(e){var t;return e==="start"?String((k==null?void 0:k.start)||""):e==="end"?String((k==null?void 0:k.end)||""):e==="until"?((t=k==null?void 0:k.repeat)==null?void 0:t.until)||sa(k==null?void 0:k.start)||he(new Date):e==="due"?$a(z==null?void 0:z.due):e==="dtstart"?$a(re==null?void 0:re.dtstart):e==="bulk-due"?ja:e==="birthday"?String((q==null?void 0:q.birthday)||""):""}function ot(e,t){if(e==="start"&&k){k={...k,start:t||""};return}if(e==="end"&&k){k={...k,end:t};return}if(e==="until"&&k){k={...k,repeat:{...k.repeat??Xa(),until:t,endMode:"until"}};return}if(e==="due"&&z){if(t===null||t==="")z={...z,due:null};else if(/^\d{4}-\d{2}-\d{2}$/.test(t))z={...z,due:new Date(t+"T00:00:00").toISOString()};else{const a=new Date((t.length===16,t));z={...z,due:Number.isNaN(a.getTime())?t:a.toISOString()}}return}if(e==="dtstart"&&re){if(t===null||t==="")re={...re,dtstart:null};else if(/^\d{4}-\d{2}-\d{2}$/.test(t))re={...re,dtstart:new Date(t+"T00:00:00").toISOString()};else{const a=new Date((t.length===16,t));re={...re,dtstart:Number.isNaN(a.getTime())?t:a.toISOString()}}return}if(e==="birthday"&&q){q={...q,birthday:t&&/^\d{4}-\d{2}-\d{2}/.test(t)?t.slice(0,10):null};return}e==="bulk-due"&&(ja=t||"")}function Fn(e,t,a,l){const o=qa(t),p=(O==null?void 0:O.viewY)??Number(o.date.slice(0,4)),n=(O==null?void 0:O.viewM)??Number(o.date.slice(5,7))-1,r=ks(),u=Bs(),y=(new Date(p,n,1).getDay()-r+7)%7,v=new Date(p,n+1,0).getDate(),$=new Date(p,n,0).getDate(),_=o.date,D=o.hm,L=new Date(p,n,1).toLocaleString(void 0,{month:"long",year:"numeric"}),ae=[],T=Math.ceil((y+v)/7)*7;for(let H=0;H<T;H++){let X,pe,Ae=!1;H<y?(X=$-y+H+1,pe=new Date(p,n-1,X),Ae=!0):H>=y+v?(X=H-(y+v)+1,pe=new Date(p,n+1,X),Ae=!0):(X=H-y+1,pe=new Date(p,n,X));const _e=he(pe),vt=_e===_,it=_e===he(new Date);ae.push(`<button type="button" class="dt-day${Ae?" is-outside":""}${vt?" is-selected":""}${it?" is-today":""}" data-action="dt-pick-day" data-dt-field="${e}" data-day="${i(_e)}">${X}</button>`)}const Y=a?"":`<div class="dt-times" role="listbox" aria-label="Time">
    ...
    </div>`}function Mn(){s.querySelectorAll(".dt-field.is-open").forEach(e=>{const t=e.querySelector(".dt-trigger"),a=e.querySelector(".dt-popover");if(!t||!a)return;const l=t.getBoundingClientRect(),o=8;a.style.position="fixed",a.style.visibility="hidden",a.style.top="0",a.style.left="0";const p=a.offsetWidth||320,n=a.offsetHeight||300;let r=l.bottom+6;r+n>window.innerHeight-o&&(r=Math.max(o,l.top-n-6));let u=l.left;u+p>window.innerWidth-o&&(u=Math.max(o,window.innerWidth-p-o)),u<o&&(u=o),a.style.top=`${Math.round(r)}px`,a.style.left=`${Math.round(u)}px`,a.style.right="auto",a.style.visibility="visible",a.style.zIndex="200"})}function Xa(){return{freq:"",interval:1,until:null,count:null,byDay:[],endMode:"never"}}function Rn(e){return e.endMode==="until"||e.endMode==="count"||e.endMode==="never"?e.endMode:e.until?"until":e.count?"count":"never"}function Vn(){if(!gt||!k)return"";const e=k,t=e.repeat??Xa(),a=(t.freq||"").toUpperCase(),l=ne.filter(_=>_.canShare||_.access==="readwrite"),o=ne.filter(_=>_.id===e.instanceId?!0:_.readOnly?!1:_.canShare||_.access==="readwrite").map(_=>`<option value="${_.id}" ${_.id===e.instanceId?"selected":""}>${i(_.displayname)}</option>`).join(""),p=e.readOnly||!e.canWrite;let n,r;if(e.allDay)n=sa(e.start),r=sa(e.end);else{const _=e.start||"",D=e.end||"";if(/^\d{4}-\d{2}-\d{2}$/.test(_)){const L=$s(_,D||null);n=L.start,r=L.end||""}else n=$a(e.start),r=$a(e.end)}const u=[{code:"MO",label:"Mon"},{code:"TU",label:"Tue"},{code:"WE",label:"Wed"},{code:"TH",label:"Thu"},{code:"FR",label:"Fri"},{code:"SA",label:"Sat"},{code:"SU",label:"Sun"}],f=new Set((t.byDay||[]).map(_=>_.toUpperCase())),y=Rn(t),v=!!a&&y==="until",$=t.until||(y==="until"?sa(e.start)||he(new Date):"");return`<div class="cal-modal" id="event-edit-modal" role="dialog" aria-modal="true" aria-labelledby="event-modal-title">
          ...
          <form class="stack" data-form="edit-event">
            ...
            <fieldset class="event-repeat" ${p?"disabled":""}>
              ...
              ${a==="WEEKLY"?`<div class="event-byday" role="group" aria-label="Days of week">
                      ${u.map(_=>`<label class="checkbox event-byday-item">
                              <input type="checkbox" name="repeatByDay" value="${_.code}" ${f.has(_.code)?"checked":""} />
                              ${_.label}
                            </label>`).join("")}
                    </div>`:""}
            ...
            <div class="form-actions-row" style="margin-top:0.5rem">
              ${p?"":`<button type="submit" class="btn btn-primary" ${d?"disabled":""}>${rt?"Create event":"Save event"}</button>
    ...
    </div>`}function Bn(e,t){const a=ne.find(l=>l.id===t);return{uri:"",instanceId:t,calendarId:(a==null?void 0:a.calendarId)??0,calendarName:(a==null?void 0:a.displayname)??"Calendar",calendarUri:(a==null?void 0:a.uri)??"",uid:"",summary:"",description:"",location:"",start:e,end:e,allDay:!0,hasRrule:!1,repeat:Xa(),readOnly:!1,canWrite:!0}}async function Ot(e){$t=(await A.contacts(e,Xt)).contacts,ce!==null&&!$t.some(a=>a.uri===ce)&&(ce=null,be||(q=null,Ee=null,He=null,Ge=!1))}async function Ut(){const e=await A.tasks({q:Ga,sort:It,order:St});Ne=e.tasks,xt=e.calendars;const t=new Set(Ne.map(a=>me(a.instanceId,a.uri)));ye=ye.filter(a=>t.has(a)),Pe!==null&&!Ne.some(a=>`${a.instanceId}|${a.uri}`===Pe)&&(Pe=null,Q||(z=null))}async function va(){const e=await A.notes({q:Qa,sort:ha,order:aa});ga=e.notes,qt=e.calendars,lt!==null&&!ga.some(t=>`${t.instanceId}|${t.uri}`===lt)&&(lt=null,ke||(re=null))}function me(e,t){return`${e}|${t}`}function zs(e){if(!e)return"—";try{const t=new Date(e);return Number.isNaN(t.getTime())?e:t.toLocaleString(void 0,{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return e}}function $a(e){if(!e)return"";try{const t=new Date(e);if(Number.isNaN(t.getTime()))return"";const a=l=>String(l).padStart(2,"0");return`${t.getFullYear()}-${a(t.getMonth()+1)}-${a(t.getDate())}T${a(t.getHours())}:${a(t.getMinutes())}`}catch{return""}}function Pt(e,t,a,l,o,p=""){const n=a===t,r=n?l==="asc"?" ▲":" ▼":"";return`<th class="${`sortable-th${n?" is-sorted":""}${p?" "+p:""}`}" data-action="sort-${o}" data-sort="${i(t)}" role="columnheader" tabindex="0">${i(e)}${r}</th>`}async function jn(e){if(M===null)return;const t=await A.getContact(M,e);ce=e,be=!1;const a=t.contact;q={...a,emails:Array.isArray(a.emails)?a.emails:[],phones:Array.isArray(a.phones)?a.phones:[],custom:Array.isArray(a.custom)?a.custom:[],address:a.address??Hs(),birthday:a.birthday??null},Ee=a.photoDataUri??(a.hasPhoto&&M!==null?`${A.contactPhotoUrl(M,e)}?t=${Date.now()}`:null),He=null,Ge=!1,Ce=!0}function zn(){be=!0,ce=null,Ce=!0,q={uri:"",displayname:"",firstname:"",lastname:"",fullname:"",org:"",title:"",emails:[""],phones:[{type:"cell",value:""}],address:{street:"",city:"",region:"",postal:"",country:""},birthday:null,url:"",note:"",custom:[],hasPhoto:!1,photoDataUri:null},Ee=null,He=null,Ge=!1}function Hs(){return{street:"",city:"",region:"",postal:"",country:""}}function Hn(e){return new Promise((t,a)=>{const l=new FileReader;l.onload=()=>{const o=String(l.result??""),p=o.indexOf(",");t(p>=0?o.slice(p+1):o)},l.onerror=()=>a(new Error("Failed to read photo file")),l.readAsDataURL(e)})}function Ws(e,t={}){const a=!!c&&h==="admin"&&we()&&_t(),p=`
      <img class="brand-logo" src="/logo.png" width="28" height="28" alt="" aria-hidden="true" />
      <span class="brand-text">Angara<span class="brand-dav">DAV</span><span class="brand-sep" aria-hidden="true"> · </span><span class="${a?"brand-portal brand-portal-admin":"brand-portal brand-portal-user"}">${i(a?"Administration Portal":"User Portal")}</span></span>`,n=c?i(c.displayname||c.username):"",r=_t()?`<button type="button" class="user-menu-item${h==="admin"?" is-active":""}" role="menuitem" data-action="tab" data-tab="admin">
              Administration
          ...
          <a class="brand" href="/portal/">${p}</a>
        </nav>`,$=!(fe||Me||Re!==null||Ye!==null||gt||Ce||Ke)?Ft():"",_=t.tabs&&t.tabs.trim()!==""?`<div class="tabs-bar" role="presentation">
      ...
      ${Wn()}`}function Ft(){return g?ia(g.type,g.message,{dismissible:!0}):""}function Js(e){return!Number.isFinite(e)||e<0?"":e<1024?`${e} B`:e<1024*1024?`${(e/1024).toFixed(1)} KB`:`${(e/(1024*1024)).toFixed(1)} MB`}function wa(e){const t=Math.max(0,Math.floor(e)),a=Math.floor(t/60),l=t%60;return a>0?`${a}m ${l}s`:`${l}s`}function yt(){za!==null&&(clearInterval(za),za=null)}function Ys(){yt(),za=setInterval(()=>{if(!j||j.phase==="done"||j.phase==="error"){yt();return}j={...j,elapsedSec:Math.floor((Date.now()-j.startedAt)/1e3)},j.phase==="processing"&&Qs(j)},1e3)}function Mt(e,t={}){j&&(j={...j,phase:e,elapsedSec:Math.floor((Date.now()-j.startedAt)/1e3),...t},m())}function Ks(){yt(),j=null,m()}function Gs(e){!j||j.phase==="done"||j.phase==="error"||(j={...j,phase:"processing",processPercent:e.percent,processCurrent:e.current,processTotal:e.total,processImported:e.imported,processUpdated:e.updated,processSkipped:e.skipped,elapsedSec:Math.floor((Date.now()-j.startedAt)/1e3)},Qs(j))}function Qs(e){const t=s.querySelector("[data-import-status-line]"),a=s.querySelector(".import-progress-bar"),l=s.querySelector(".import-progress-track"),o=s.querySelector("[data-import-counts]"),p=e.kind==="calendar"?"items":"contacts";let n;if(e.phase==="processing"&&e.processTotal>0)n=`Importing ${e.processCurrent.toLocaleString()} / ${e.processTotal.toLocaleString()} ${p} (${e.processPercent??0}%) · ${wa(e.elapsedSec)}`;else if(e.phase==="processing")n=`Importing on server… ${wa(e.elapsedSec)}`;else return;t&&(t.textContent=n),o&&(o.textContent=`${e.processImported} new · ${e.processUpdated} updated${e.processSkipped?` · ${e.processSkipped} skipped`:""}`),a&&e.processPercent!==null&&(a.classList.remove("is-indeterminate"),a.style.width=`${Math.min(100,Math.max(0,e.processPercent))}%`),l&&e.processPercent!==null&&(l.setAttribute("aria-valuenow",String(e.processPercent)),l.removeAttribute("aria-valuetext"))}function Wn(){if(!j)return"";const e=j,t=e.phase!=="done"&&e.phase!=="error",a=e.kind==="calendar"?"calendar (.ics)":"contacts (.vcf)",l=e.phase==="done"?"Import finished":e.phase==="error"?"Import failed":"Importing…",o=(()=>{const r=[{id:"reading",label:"Reading file"},{id:"uploading",label:"Uploading to server"},{id:"processing",label:"Importing on server"}],f={reading:0,uploading:1,processing:2,done:3,error:2}[e.phase]??0;return r.map((y,v)=>{let $="pending";return e.phase==="done"||v<f?$="done":v===f&&($=(e.phase==="error","active")),`<li class="import-step import-step-${$}"><span class="import-step-icon" aria-hidden="true">${$==="done"?"✓":$==="active"?"●":"○"}</span> ${i(y.label)}</li>`}).join("")})();let p="";if(t){let r=null;e.phase==="reading"&&e.readPercent!==null?r=Math.min(100,Math.max(0,e.readPercent)):e.phase==="processing"&&e.processPercent!==null&&(r=Math.min(100,Math.max(0,e.processPercent)));const u=r===null?"import-progress-bar is-indeterminate":"import-progress-bar",f=r!==null?` style="width:${r}%"`:"",y=e.kind==="calendar"?"items":"contacts";let v;e.phase==="reading"?v=e.readPercent!==null?`Reading file… ${e.readPercent}%`:"Reading file…":e.phase==="uploading"?v="Uploading to server…":e.processTotal>0?v=`Importing ${e.processCurrent.toLocaleString()} / ${e.processTotal.toLocaleString()} ${y} (${e.processPercent??0}%) · ${wa(e.elapsedSec)}`:v=`Importing on server… ${wa(e.elapsedSec)}`;const $=e.phase==="processing"&&e.processTotal>0?`<p class="muted small" data-import-counts style="margin:0 0 0.5rem">${e.processImported} new · ${e.processUpdated} updated${e.processSkipped?` · ${e.processSkipped} skipped`:""}</p>`:'<p class="muted small" data-import-counts style="margin:0 0 0.5rem;display:none"></p>';p=`
        ...
        <div class="card auth-card">
      ...
      </div>`,{auth:!0})}function Jn(){if(!c){Zs();return}const e=ne.filter(S=>S.canShare),t=ne.filter(S=>!S.canShare),a=ne.find(S=>S.id===F)??null,l=e.map(S=>{const le=K.includes(S.id),at=le?" is-selected":"",Pa=S.id===F?" is-primary":"",Ds=S.color?`<span class="cal-swatch" style="background:${i(S.color)}"></span>`:'<span class="cal-swatch cal-swatch-empty"></span>',Cs=Es(S.access)+(S.readOnly?'<span class="badge">read-only</span>':"")+(S.holidaysCountry?`<span class="badge badge-admin">holidays ${i(S.holidaysCountry)}</span>`:"");return`<div class="cal-row${at}${Pa}" data-action="select-cal" data-id="${S.id}" role="button" tabindex="0" title="Toggle on the month grid">
        ...
        </div>`}).join(""),o=t.map(S=>{const le=K.includes(S.id),at=le?" is-selected":"",Pa=S.id===F?" is-primary":"",Ds=S.color?`<span class="cal-swatch" style="background:${i(S.color)}"></span>`:'<span class="cal-swatch cal-swatch-empty"></span>',Cs=S.access==="readwrite"?"Shared with you · full access — check to show events; click to set as primary for new events":"Shared with you · read-only — check to show events";return`<div class="cal-row${at}${Pa}" data-action="select-cal" data-id="${S.id}" role="button" tabindex="0" title="${i(Cs)}">
          <label class="cal-row-check" title="Show events on the month grid" onclick="event.stopPropagation()">
            <input type="checkbox" data-action="toggle-cal" data-id="${S.id}" ${le?"checked":""} ${d?"disabled":""} />
          </label>
          ${Ds}
          <span class="cal-row-text">
            ...
            <span class="muted small">${S.access==="readwrite"?"Shared · full access":"Shared · read-only"}</span>
                ...
                </section>`,footer:[{label:"Close",action:"close-cal-modal",variant:"ghost"}]}):"",y=Re!==null?ne.find(S=>S.id===Re&&S.canShare)??null:null,v=y?De({id:"cal-delete-modal",title:"Delete calendar",titleId:"cal-delete-title",closeAction:"cancel-delete-cal",size:"sm",body:`
              ${Ft()}
      ...
      ${f}
      ${v}
      ${Vn()}`,D=Ue.map(S=>`<div class="cal-row${S.id===M?" is-selected":""}" data-action="select-ab" data-id="${S.id}" role="button" tabindex="0">
          ...
          <span class="cal-row-actions">
            ...
            <button type="button" class="btn btn-small btn-danger" data-action="delete-ab" data-id="${S.id}" ${d?"disabled":""}>Delete</button>
        ...
        </div>`).join(""),L=Ue.find(S=>S.id===M)??null,ae=$t.length===0?`<tr class="contacts-empty-row"><td colspan="4" class="muted">${Xt?"No contacts match your search.":"No contacts yet. Add one or import a .vcf file."}</td></tr>`:$t.map(S=>{const le=!be&&S.uri===ce?" is-selected":"",at=i((S.displayname||"?").slice(0,1).toUpperCase()),Pa=S.hasPhoto&&M!==null?`<img class="contact-avatar" src="${i(A.contactPhotoUrl(M,S.uri))}" alt="" loading="lazy" data-avatar-fallback="${at}" />`:`<span class="contact-avatar contact-avatar-fallback" aria-hidden="true">${at}</span>`;return`<tr class="contact-table-row${le}" data-action="select-contact" data-uri="${i(S.uri)}" tabindex="0" role="button">
              ...
              </tr>`}).join(""),T=q,Y=Array.isArray(T==null?void 0:T.emails)&&T.emails.length>0?T.emails:[""],H=Array.isArray(T==null?void 0:T.phones)&&T.phones.length>0?T.phones:[{type:"cell",value:""}],X=(T==null?void 0:T.address)??Hs(),pe=Y.map((S,le)=>`<div class="multi-row" data-multi="email" data-idx="${le}">
          ...
          <button type="button" class="btn btn-ghost btn-small" data-action="remove-phone" data-idx="${le}" ${H.length<=1?"disabled":""} title="Remove">×</button>
        </div>`).join(""),_e=Array.isArray(T==null?void 0:T.custom)?T.custom:[],vt=_e.length===0?'<p class="muted small" style="margin:0 0 0.5rem">No custom fields yet. Labels and values can use any language (e.g. Супруг, 日本語).</p>':_e.map((S,le)=>`<div class="multi-row multi-row-custom" data-multi="custom" data-idx="${le}">
              ...
              </div>`).join(""),it=Ce&&T&&L?`<div class="cal-modal" id="contact-edit-modal" role="dialog" aria-modal="true" aria-labelledby="contact-modal-title">
            ...
            <div class="cal-modal-card cal-modal-card-wide">
              ...
              <div class="cal-modal-body">
                ...
                <form class="stack" data-form="contact">
                  <div class="contact-photo-row">
                    <div class="contact-photo-preview">
                      ${Ee?`<img src="${i(Ee)}" alt="Contact photo" />`:`<span class="contact-avatar contact-avatar-fallback contact-avatar-lg" aria-hidden="true">${i((T.fullname||T.firstname||"?").slice(0,1).toUpperCase())}</span>`}
                    </div>
                  ...
                  <div class="form-grid form-grid-2">
                    <label>First name
                      <input type="text" name="firstname" value="${i(T.firstname)}" maxlength="200" autocomplete="off" />
                    </label>
                    <label>Last name
                      <input type="text" name="lastname" value="${i(T.lastname)}" maxlength="200" autocomplete="off" />
                    </label>
                  </div>
                  <label>Full name
                    <input type="text" name="fullname" value="${i(T.fullname)}" maxlength="200" placeholder="Auto from first/last if empty" autocomplete="off" />
                  </label>
                  <div class="form-grid form-grid-2">
                    <label>Organization
                      <input type="text" name="org" value="${i(T.org)}" maxlength="200" autocomplete="off" />
                    </label>
                    <label>Title
                      <input type="text" name="title" value="${i(T.title)}" maxlength="200" autocomplete="off" />
                    </label>
                  </div>
                  ...
                  <label>Website
                    <input type="url" name="url" value="${i(T.url)}" maxlength="500" placeholder="https://" autocomplete="off" />
                  </label>
                  ${na({field:"birthday",name:"birthday",label:"Birthday",value:T.birthday||"",dateOnly:!0,allowClear:!0})}
                  <fieldset class="fieldset fieldset-custom">
                    <legend>Custom fields</legend>
                    ${vt}
                    <button type="button" class="btn btn-ghost btn-small" data-action="add-custom" ${_e.length>=30?"disabled":""}>+ Custom field</button>
                  </fieldset>
                  <label>Notes
                    <textarea name="note" rows="3" maxlength="4000">${i(T.note)}</textarea>
                  </label>
                  <div class="form-actions-row form-actions-wrap">
                    <button type="submit" class="btn btn-primary" ${d?"disabled":""}>${be?"Create contact":"Save contact"}</button>
                    ${!be&&T.uri?`<button type="button" class="btn" data-action="export-contact" ${d?"disabled":""}>Export .vcf</button>`:""}
                    ...
                    ${!be&&T.uri?`<span class="muted small mono">${i(T.uri)}</span>`:""}
                  </div>
                </form>
              </div>
            </div>
          </div>`:"",Dt=Ke&&L?De({id:"ab-edit-modal",title:"Address book details",titleId:"ab-modal-title",closeAction:"close-ab-modal",body:`
                ${Ft()}
                <section>
                  <p class="muted small mono" style="margin:0">
                ...
                </section>`,footer:[{label:"Close",action:"close-ab-modal",variant:"ghost"}]}):"",Be=Ye!==null?Ue.find(S=>S.id===Ye)??null:null,Rt=Be?De({id:"ab-delete-modal",title:"Delete address book",titleId:"ab-delete-title",closeAction:"cancel-delete-ab",size:"sm",body:`
              ${Ft()}
      ...
      ${Rt}
      ${Dt}
      ${it}`,C=h==="calendars"?"my-calendars":h==="contacts"?"my-contacts":h==="tasks"?"tasks":h==="notes"?"notes":h==="files"?"files":"administration",oe=pr(),J=fr(),et=Gn(),xe=dr(),tt=h==="calendars"?_:h==="contacts"?Vt:h==="tasks"?oe:h==="notes"?J:h==="files"?et:xe,oa=h==="admin"?`<div class="tabs" role="tablist" aria-label="Administration sections">
          ${Qn()}
        ...
        <section class="card">
          ...
          <p class="muted small">DAV path: <span class="mono">${i(e.davPath)}</span></p>
      ...
      </div>`;const t=e.quotaBytes>0?`${ka(e.usedBytes)} used · ${ka(e.availableBytes)} free of ${ka(e.quotaBytes)}`:`${ka(e.usedBytes)} used · ${ka(e.availableBytes)} free (no app quota)`,a=e.quotaBytes>0?Math.min(100,Math.round(100*e.usedBytes/e.quotaBytes)):0,l=ue.length,o=Te.length>0&&Te.every($=>ue.includes($.path)),p=l>0,n=l>0?`<div class="bulk-bar files-bulk-bar" role="toolbar" aria-label="Selected files">
            ...
            <div class="bulk-bar-actions">
          ...
          </div>`:"",r=Te.length===0?'<tr><td colspan="5" class="muted">This folder is empty.</td></tr>':Te.map($=>{const _=ue.includes($.path),D=$.type==="dir"?"📁":"📄",L=$.type==="dir"?`<button type="button" class="files-name-btn" data-action="files-nav" data-path="${i($.path)}" ${d?"disabled":""}>
                      <span class="files-icon" aria-hidden="true">${D}</span>${i($.name)}
                    </button>`:`<span class="files-name"><span class="files-icon" aria-hidden="true">${D}</span>${i($.name)}</span>`,ae=$.type==="dir"?"—":ka($.size);return`<tr class="files-row${_?" is-checked":""}" data-path="${i($.path)}" data-type="${$.type}">
                <td class="files-col-check">
                  <input type="checkbox" data-action="files-toggle" data-path="${i($.path)}"
                    ${_?"checked":""} ${d?"disabled":""}
              ...
              </tr>`}).join(""),u=Se!==null?(()=>{const $=Te.find(D=>D.path===Se),_=($==null?void 0:$.name)??"";return De({id:"files-rename-modal",title:"Rename",titleId:"files-rename-title",closeAction:"files-rename-close",size:"sm",form:!0,formAttrs:'data-form="files-rename" id="files-rename-form"',body:`
                    <input type="hidden" name="path" value="${i(Se)}" />
                    <label>New name
                      <input type="text" name="newName" value="${i(_)}" required maxlength="255" autocomplete="off" />
                    </label>`,footer:[{label:"Cancel",action:"files-rename-close",variant:"ghost"},{label:"Rename",type:"submit",variant:"primary",disabled:d}]})})():"",f=ge!==null&&ge.length>0?(()=>{const $=ge,_=$.length>1,D=Te.find(T=>T.path===$[0]),L=_?`Delete ${$.length} items`:`Delete ${(D==null?void 0:D.type)==="dir"?"folder":"file"}`,ae=_?`<p style="margin:0 0 0.75rem">Delete <strong>${$.length}</strong> selected items? Folders are removed with their contents. This cannot be undone.</p>
                 <ul class="files-delete-list muted small">
                   ${$.slice(0,12).map(T=>{const Y=Te.find(H=>H.path===T);return`<li><span class="mono">${i((Y==null?void 0:Y.name)??T)}</span></li>`}).join("")}
                   ${$.length>12?`<li>…and ${$.length-12} more</li>`:""}
                 </ul>`:`<p style="margin:0">Delete <strong>${i((D==null?void 0:D.name)??$[0])}</strong>?${(D==null?void 0:D.type)==="dir"?" This removes the folder and everything inside it.":""}</p>`;return De({id:"files-delete-modal",title:L,titleId:"files-delete-title",closeAction:"files-delete-close",size:"sm",body:ae,footer:[{label:"Cancel",action:"files-delete-close",variant:"ghost"},{label:"Delete",action:"files-delete-confirm",variant:"danger",disabled:d}]})})():"",y=de!==null&&de.paths.length>0?(()=>{const $=de.op,_=de.paths,D=_.length>1,L=Te.find(H=>H.path===_[0]),ae=(L==null?void 0:L.name)??Za(_[0]),T=D?`${$==="copy"?"Copy":"Move"} ${_.length} items`:`${$==="copy"?"Copy":"Move"} ${(L==null?void 0:L.type)==="dir"?"folder":"file"}`,Y=Fe;return De({id:"files-transfer-modal",title:T,titleId:"files-transfer-title",closeAction:"files-transfer-close",size:"sm",form:!0,formAttrs:'data-form="files-transfer"',body:`
                    ${D?`<p class="muted small" style="margin:0 0 0.75rem">${_.length} items will be ${$==="copy"?"copied":"moved"} into the destination folder (original names kept).</p>`:`<p class="muted small" style="margin:0 0 0.75rem"><span class="mono">${i(ae)}</span></p>`}
      ...
      <section class="card files-panel">
        ...
        <div class="table-wrap files-table-wrap">
          <table class="files-table">
            ...
            <tbody>
              ${ya&&Te.length===0?'<tr><td colspan="5" class="muted">Loading…</td></tr>':r}
            </tbody>
          </table>
        </div>
      </section>
      ...
      <div class="admin-coming-soon-head">
        <span class="badge ${Ta(a)}">${i(p)}</span>
      ...
      with role <span class="badge badge-admin">Admin</span>.
    </p>`;let a="",l="";if(U&&!I)l='<section class="card"><p class="muted">Loading overview…</p></section>';else if(R&&!I)l=`<section class="card">
      ...
      </section>`;else if(I){const o=I,p=o.services,n=o.links??{},r=e?`<span class="badge ${Ta(e.status)}">${i(ba(e.status))}</span>`:"",u=o.version?i(o.version):"—",f=o.git?i(o.git):"";a=`
        <section class="card admin-about-card">
          ...
          ${t}
        </section>`;const y=o.nbusers??o.users,v=o.nbcalendars??o.calendars,$=o.nbevents??o.events,_=o.nbbooks??o.addressBooks,D=o.nbcontacts??o.contacts;l=`
      ...
      ${l}`}function Zn(){const e=mt.trim().toLowerCase();return e?ie.filter(t=>t.username.toLowerCase().includes(e)||(t.displayname||"").toLowerCase().includes(e)||(t.email||"").toLowerCase().includes(e)):ie}function er(){return je?De({id:"admin-user-create-modal",title:"Add user",titleId:"admin-user-create-title",closeAction:"admin-user-create-close",size:"sm",form:!0,formAttrs:'data-form="admin-user-create"',body:`
          ...
          <p class="muted small">Username <span class="mono">${i(e.username)}</span> cannot be changed. Leave password fields empty to keep the current password.</p>
            ...
            </label>`,footer:[{label:"Cancel",action:"admin-user-edit-close",variant:"ghost",disabled:d},{label:"Save changes",type:"submit",variant:"primary",disabled:d}]})}function ar(){if(!pt)return"";const e=pt,t=V&&V.username.toLowerCase()===e.toLowerCase()?V:ie.find(l=>l.username.toLowerCase()===e.toLowerCase())??null,a=t?`${t.displayname||t.username} (${t.username})`:e;return De({id:"admin-user-delete-modal",title:"Delete user",titleId:"admin-user-delete-title",closeAction:"admin-user-delete-close",size:"sm",body:`
          ...
          ${ss({action:"admin-user-delete-toggle",label:"I understand and want to delete this user",checked:At,disabled:d,style:"admin"})}`,footer:[{label:"Cancel",action:"admin-user-delete-close",variant:"ghost",disabled:d},{label:"Delete permanently",action:"admin-user-delete-confirm",variant:"danger",disabled:d||!At,attrs:`data-username="${i(e)}"`}]})}function sr(){if(!B)return"";if(G&&!V)return`<section class="card admin-user-detail">
        <p class="muted">Loading user <span class="mono">${i(B)}</span>…</p>
      </section>`;if(Ie&&!V)return`<section class="card admin-user-detail">
        <div class="section-header">
          <h2>User detail</h2>
          <button type="button" class="btn btn-ghost btn-small" data-action="admin-user-close">Close</button>
        </div>
        <p class="flash flash-error">${i(Ie)}</p>
      </section>`;if(!V)return"";const e=V,t=Da&&Et.length===0?'<tr><td colspan="5" class="muted">Loading calendars…</td></tr>':Et.length===0?'<tr><td colspan="5" class="muted">No calendars.</td></tr>':Et.map(u=>`<tr>
    ...
    ${p}${n}${r}`}function nr(){const e=Ve("users");if(e&&e.available===!1)return es("users");const t=Zn(),a=se&&ie.length===0?'<tr><td colspan="4" class="muted admin-table-empty">Loading users…</td></tr>':t.length===0?`<tr><td colspan="4" class="muted admin-table-empty">${te?i(te):mt.trim()?"No users match this filter.":"No users found."}</td></tr>`:t.map(l=>`<tr class="contact-table-row${B&&B.toLowerCase()===l.username.toLowerCase()?" is-selected":""}" data-action="admin-user-view" data-username="${i(l.username)}" tabindex="0" role="button">
      ...
      <section class="card">
        <div class="section-header">
          ...
          <div class="section-actions">
            ${e?`<span class="badge ${Ta(e.status)}">${i(ba(e.status))}</span>`:""}
      ...
      ${er()}
      ${tr()}
      ${ar()}`}function rr(){const e=Ve("settings");if(e&&e.available===!1)return es("settings");if(Ra&&!Wt)return'<section class="card"><p class="muted">Loading system settings…</p></section>';if(Ca&&!Wt)return`<section class="card">
      ...
      ${ir()}`}function ir(){if(!ua)return"";const e=bt.trim()==="CONFIRM";return De({id:"admin-db-confirm-modal",title:"Confirm database change",titleId:"admin-db-confirm-title",closeAction:"admin-db-confirm-close",size:"sm",body:`
          ...
          </label>`,footer:[{label:"Cancel",action:"admin-db-confirm-close",variant:"ghost",disabled:d},{label:"Save database settings",action:"admin-db-confirm-save",variant:"danger",disabled:d||!e}]})}function dr(){return we()?_t()?w==="users"?nr():w==="settings"?rr():w==="database"?or():Xn():`<section class="card admin-coming-soon-card">
          <div class="admin-coming-soon-head">
      ...
      <path d="M3.5 8.5 6.5 11.5 12.5 4.5" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        title="${i(L)}" aria-label="${i(L)}" ${d||n===0?"disabled":""}>${u}</button>`,y=o?`<div class="bulk-bar" style="margin-top:0.75rem">
          ...
          </div>`:"",v=z,$=xt.map(D=>`<option value="${D.id}" ${v&&v.instanceId===D.id?"selected":""}>${i(D.displayname)}</option>`).join(""),_=v?`<div class="card">
      ...
      <section class="card contacts-main-card items-list-card">
        ...
        <div class="contact-toolbar" style="margin-top:0.75rem">
          ...
          <button type="button" class="btn btn-primary" data-action="new-task" ${d||xt.length===0?"disabled":""}>Add task</button>
        </div>
        ${y}
        ${xt.length===0?'<p class="muted small" style="margin-top:0.75rem">No writable calendars with tasks (VTODO) enabled. Create a calendar under <strong>Calendar</strong> (system Tasks setting must be on).</p>':""}
          ...
          <table class="contacts-table">
            <thead>
              <tr>
                ...
                ${Pt("Title","summary",It,St,"task","col-task-title")}
                ${Pt("Status","status",It,St,"task","col-task-status")}
                ${Pt("Due","due",It,St,"task","col-task-due")}
                ${Pt("Calendar","calendar",It,St,"task","col-task-cal")}
                ${Pt("%","percent",It,St,"task","col-task-pct")}
              </tr>
            </thead>
            <tbody>${r}</tbody>
          </table>
      ...
      <section class="stack items-edit-panel">
        ${_}
              ...
              </tr>`}).join(""),t=re,a=qt.map(o=>`<option value="${o.id}" ${t&&t.instanceId===o.id?"selected":""}>${i(o.displayname)}</option>`).join(""),l=t?`<div class="card">
            ${ve(ke?"New note":"Edit note","notes")}
            <form class="stack" data-form="note" style="margin-top:1rem">
              ${ke?`<label>Calendar
                      <select name="instanceId" required ${qt.length===0?"disabled":""}>
                        <option value="">${qt.length?"Select calendar…":"No writable calendars"}</option>
                        ${a}
                      </select>
      ...
      <section class="card contacts-main-card items-list-card">
        ...
        <div class="contact-toolbar" style="margin-top:0.75rem">
          ...
          <button type="button" class="btn btn-primary" data-action="new-note" ${d||qt.length===0?"disabled":""}>Add note</button>
        </div>
        ${qt.length===0?'<p class="muted small" style="margin-top:0.75rem">No writable calendars with notes (VJOURNAL) enabled. Enable Notes in Admin settings and ensure calendars include VJOURNAL.</p>':""}
        <div class="contacts-table-wrap items-table-wrap" style="margin-top:0.75rem">
          <table class="contacts-table">
            <thead>
              <tr>
    ...
    </div>`}function br(){const e=s.querySelector(".contacts-table-wrap"),t=s.querySelector(".contacts-ab-list"),a=s.querySelector(".calendars-owned-list");return{windowX:window.scrollX,windowY:window.scrollY,tableTop:(e==null?void 0:e.scrollTop)??null,abListTop:(t==null?void 0:t.scrollTop)??null,calListTop:(a==null?void 0:a.scrollTop)??null}}function gr(e){requestAnimationFrame(()=>{requestAnimationFrame(()=>{if(window.scrollTo(e.windowX,e.windowY),e.tableTop!==null){const t=s.querySelector(".contacts-table-wrap");t&&(t.scrollTop=e.tableTop)}if(e.abListTop!==null){const t=s.querySelector(".contacts-ab-list");t&&(t.scrollTop=e.abListTop)}if(e.calListTop!==null){const t=s.querySelector(".calendars-owned-list");t&&(t.scrollTop=e.calListTop)}})})}function m(){const e=br();c?Jn():Zs(),hr(),gr(e),requestAnimationFrame(()=>{var t;Mn(),(t=s.querySelector(".dt-time.is-selected"))==null||t.scrollIntoView({block:"center"})})}function tn(e){const t=e.querySelector('input[name="color_picker"]'),a=e.querySelector('input[name="color"]');!t||!a||(t.addEventListener("input",()=>{a.value=t.value.toUpperCase()}),a.addEventListener("change",()=>{let l=a.value.trim();l&&!l.startsWith("#")&&(l=`#${l}`),/^#[0-9A-Fa-f]{6}/.test(l)&&(t.value=l.slice(0,7),a.value=l.toUpperCase())}))}function hr(){s.querySelectorAll("[data-action]").forEach(C=>{C.addEventListener("click",oe=>{const J=oe.target.closest("[data-action]");((J==null?void 0:J.dataset.action)==="info"||(J==null?void 0:J.dataset.action)==="info-close")&&(oe.preventDefault(),oe.stopPropagation()),qr(oe)})}),_a(),Oe&&gn(),s.querySelectorAll("tr.contact-table-row[data-action], .cal-row[data-action], .month-cell[data-action]").forEach(C=>{C.addEventListener("keydown",oe=>{(oe.key==="Enter"||oe.key===" ")&&(oe.preventDefault(),C.click())})});const e=s.querySelector("#delete-cal-confirm"),t=s.querySelector("#delete-cal-submit");e==null||e.addEventListener("change",()=>{t&&(t.disabled=!e.checked||d)});const a=s.querySelector("#delete-ab-confirm"),l=s.querySelector("#delete-ab-submit");a==null||a.addEventListener("change",()=>{l&&(l.disabled=!a.checked||d)}),s.querySelectorAll("img.contact-avatar[data-avatar-fallback]").forEach(C=>{C.addEventListener("error",()=>{const oe=C.dataset.avatarFallback||"?",J=document.createElement("span");J.className="contact-avatar contact-avatar-fallback",J.setAttribute("aria-hidden","true"),J.textContent=oe,C.replaceWith(J)})}),Ls||(document.addEventListener("keydown",C=>{if(C.key==="Escape"){if(j&&(j.phase==="done"||j.phase==="error")){Ks();return}if(!j){if(Oe){Oe=!1,_a(),m();return}if(Se!==null||ge!==null||de!==null||ht){Se=null,ge=null,de=null,ht=!1,m();return}an()}}}),Ls=!0);const o=s.querySelector('[data-form="login"]');o==null||o.addEventListener("submit",C=>{C.preventDefault(),Sr(o)});const p=s.querySelector('[data-form="files-rename"]');p==null||p.addEventListener("submit",C=>{C.preventDefault(),Dr(p)});const n=s.querySelector('[data-form="files-transfer"]');n==null||n.addEventListener("submit",C=>{C.preventDefault(),Ar(n)});const r=s.querySelector('[data-form="files-mkdir"]');r==null||r.addEventListener("submit",C=>{C.preventDefault(),Cr(r)}),ht&&requestAnimationFrame(()=>{var C;(C=r==null?void 0:r.querySelector('input[name="name"]'))==null||C.focus()}),s.querySelectorAll('input[type="file"][data-action="files-upload"]').forEach(C=>{C.addEventListener("change",()=>{Er(C)})}),s.querySelectorAll('input[data-action="files-select-all"][data-indeterminate="1"]').forEach(C=>{C.indeterminate=!0});const u=s.querySelector('[data-form="share"]');u==null||u.addEventListener("submit",C=>{C.preventDefault(),Nr(u)});const f=s.querySelector('[data-form="edit-cal"]');f&&(tn(f),f.addEventListener("submit",C=>{C.preventDefault(),_r(f)}));const y=s.querySelector('[data-form="edit-event"]');y==null||y.addEventListener("submit",C=>{C.preventDefault(),Tr(y)}),s.querySelectorAll('select[data-action="event-repeat-freq"], select[data-action="event-repeat-end"]').forEach(C=>{C.addEventListener("change",()=>{if(!k)return;const oe=s.querySelector('[data-form="edit-event"]');if(!oe)return;const J=new FormData(oe),et=oe.querySelector('input[name="allDay"]'),xe=Ua(J);xe.endMode==="until"&&!xe.until&&(xe.until=sa(String(J.get("start")??k.start??""))||he(new Date)),k={...k,summary:String(J.get("summary")??k.summary),description:String(J.get("description")??k.description),location:String(J.get("location")??k.location),instanceId:Number(J.get("instanceId"))||k.instanceId,allDay:(et==null?void 0:et.checked)??k.allDay,start:String(J.get("start")??k.start??""),end:String(J.get("end")??k.end??"")||null,repeat:xe,hasRrule:!!String(J.get("repeatFreq")??"").trim()},xe.freq&&xe.endMode==="until"&&(O==null?void 0:O.field)==="end"&&(O=null),m(),xe.endMode==="until"&&requestAnimationFrame(()=>{var oa;const tt=s.querySelector('input[name="repeatUntil"]');tt==null||tt.focus();try{(oa=tt==null?void 0:tt.showPicker)==null||oa.call(tt)}catch{}})})});const v=s.querySelector('[data-form="create-cal"]');v&&(tn(v),v.addEventListener("submit",C=>{C.preventDefault(),xr(v)}));const $=s.querySelector('[data-form="create-ab"]');$==null||$.addEventListener("submit",C=>{C.preventDefault(),Pr($)});const _=s.querySelector('[data-form="edit-ab"]');_==null||_.addEventListener("submit",C=>{C.preventDefault(),Fr(_)});const D=s.querySelector('[data-form="contact"]');D==null||D.addEventListener("submit",C=>{C.preventDefault(),Ur(D)});const L=s.querySelector('[data-form="task"]');if(L==null||L.addEventListener("submit",C=>{C.preventDefault(),vr(L)}),L){const C=L.querySelector('select[name="instanceId"]');C==null||C.addEventListener("change",()=>{if(!Q||!z)return;const oe=Number(C.value);if(!Number.isFinite(oe)||oe<=0)return;const J=new FormData(L),et=String(J.get("due")??"").trim();z={...z,instanceId:oe,parentUid:z.parentUid&&Ne.some(xe=>xe.uid===z.parentUid&&xe.instanceId===oe)?z.parentUid:null,summary:String(J.get("summary")??""),description:String(J.get("description")??""),status:String(J.get("status")??"NEEDS-ACTION"),due:et?new Date(et).toISOString():null,priority:Number(J.get("priority")??0),percent:Number(J.get("percent")??0)},m()})}const ae=s.querySelector('[data-form="note"]');ae==null||ae.addEventListener("submit",C=>{C.preventDefault(),$r(ae)});const T=s.querySelector('input[data-action="contact-search"]');T==null||T.addEventListener("input",()=>{Qe&&clearTimeout(Qe),Qe=setTimeout(()=>{Xt=T.value,M!==null&&(async()=>{try{await Ot(M),m()}catch(C){b("error",C instanceof Error?C.message:"Search failed"),m()}})()},250)});const Y=s.querySelector('input[data-action="task-search"]');Y==null||Y.addEventListener("input",()=>{Qe&&clearTimeout(Qe),Qe=setTimeout(()=>{Ga=Y.value,(async()=>{try{await Ut(),m()}catch(C){b("error",C instanceof Error?C.message:"Search failed"),m()}})()},250)});const H=s.querySelector('input[data-action="admin-users-search"]');H==null||H.addEventListener("input",()=>{Qe&&clearTimeout(Qe),Qe=setTimeout(()=>{mt=H.value,m()},150)});const X=s.querySelector('[data-form="admin-user-create"]');X==null||X.addEventListener("submit",C=>{C.preventDefault(),hn(X)});const pe=s.querySelector('[data-form="admin-user-edit"]');pe==null||pe.addEventListener("submit",C=>{C.preventDefault(),Sn(pe)});const Ae=s.querySelector('[data-form="admin-cal"]');Ae==null||Ae.addEventListener("submit",C=>{C.preventDefault(),yn(Ae)});const _e=s.querySelector('[data-form="admin-ab"]');_e==null||_e.addEventListener("submit",C=>{C.preventDefault(),vn(_e)});const vt=s.querySelector('[data-form="admin-settings"]');vt==null||vt.addEventListener("submit",C=>{C.preventDefault(),kn(vt)});const it=s.querySelector('[data-form="admin-database"]');it==null||it.addEventListener("submit",C=>{C.preventDefault(),$n(it)});const Dt=s.querySelector('select[data-action="admin-db-backend"]');Dt==null||Dt.addEventListener("change",()=>{Yt=Dt.value==="pgsql"?"pgsql":"sqlite",m()});const Be=s.querySelector('input[data-action="admin-db-confirm-input"]');Be==null||Be.addEventListener("input",()=>{bt=Be.value;const C=s.querySelector('[data-action="admin-db-confirm-save"]');C&&(C.disabled=d||bt.trim()!=="CONFIRM")});const Rt=s.querySelector('input[data-action="admin-reset-password"]');Rt==null||Rt.addEventListener("input",()=>{Je=Rt.value;const C=s.querySelector('[data-action="admin-reset-confirm"]');C&&(C.disabled=d||!ft||Je.trim()==="")});const Vt=s.querySelector('input[data-action="note-search"]');Vt==null||Vt.addEventListener("input",()=>{Qe&&clearTimeout(Qe),Qe=setTimeout(()=>{Qa=Vt.value,(async()=>{try{await va(),m()}catch(C){b("error",C instanceof Error?C.message:"Search failed"),m()}})()},250)}),Ir(),kr(),wr()}async function yr(e){var o,p;const t=en();if(t.length===0){b("error","No writable tasks selected"),m();return}const a=t.map(n=>({instanceId:n.instanceId,uri:n.uri}));if(e==="bulk-task-delete"){if(!confirm(`Delete ${t.length} task${t.length===1?"":"s"}? CalDAV clients will sync the removal.`))return;d=!0,E(),m();try{const n=await A.bulkTasks({op:"delete",items:a});ye=[],Pe&&t.some(r=>me(r.instanceId,r.uri)===Pe)&&(Pe=null,z=null,Q=!1),await Ut(),n.failed>0?b("error",`Deleted ${n.ok}, failed ${n.failed}${n.errors[0]?`: ${n.errors[0]}`:""}`):b("success",`Deleted ${n.ok} task${n.ok===1?"":"s"}`)}catch(n){b("error",n instanceof Error?n.message:"Bulk delete failed")}finally{d=!1,m()}return}let l={};if(e==="bulk-task-status"){const n=s.querySelector("#bulk-task-status"),r=((o=n==null?void 0:n.value)==null?void 0:o.trim())??"";if(!r){b("error","Choose a status to apply"),m();return}l={status:r}}else if(e==="bulk-task-due"){const n=ja.trim();if(!n){b("error","Choose a due date to apply"),m();return}const r=/^\d{4}-\d{2}-\d{2}$/.test(n)?new Date(n+"T00:00:00"):new Date((n.length===16,n));if(Number.isNaN(r.getTime())){b("error","Invalid due date"),m();return}l={due:r.toISOString()}}else if(e==="bulk-task-clear-due")l={due:null};else if(e==="bulk-task-percent"){const n=s.querySelector("#bulk-task-percent"),r=((p=n==null?void 0:n.value)==null?void 0:p.trim())??"";if(r===""){b("error","Enter a percent complete (0–100)"),m();return}const u=Number(r);if(!Number.isFinite(u)||u<0||u>100){b("error","Percent must be between 0 and 100"),m();return}l={percent:Math.round(u)}}d=!0,E(),m();try{const n=await A.bulkTasks({op:"update",items:a,fields:l});if(await Ut(),z&&!Q){const u=me(z.instanceId,z.uri),f=Ne.find(y=>me(y.instanceId,y.uri)===u);f&&(z={...f})}const r=e==="bulk-task-status"?"status":e==="bulk-task-due"||e==="bulk-task-clear-due"?"due date":"percent";n.failed>0?b("error",`Updated ${r} on ${n.ok}, failed ${n.failed}${n.errors[0]?`: ${n.errors[0]}`:""}`):b("success",`Updated ${r} on ${n.ok} task${n.ok===1?"":"s"}`)}catch(n){b("error",n instanceof Error?n.message:"Bulk update failed")}finally{d=!1,m()}}async function vr(e){const t=new FormData(e),a=String(t.get("summary")??"").trim(),l=String(t.get("description")??"").trim(),o=String(t.get("status")??"NEEDS-ACTION"),p=String(t.get("due")??"").trim(),n=p?new Date(p).toISOString():null,r=Number(t.get("priority")??0),u=Number(t.get("percent")??0),f=String(t.get("parentUid")??"").trim(),y=f===""?null:f;d=!0,E(),m();try{if(Q){const v=Number(t.get("instanceId"));if(!Number.isFinite(v)||v<=0)throw new Error("Select a calendar");const $=await A.createTask({instanceId:v,summary:a,description:l,status:o,due:n,priority:r,percent:u,parentUid:y});Q=!1,Pe=me($.task.instanceId,$.task.uri),z=$.task,b("success",y?"Subtask created":"Task created")}else if(z){const v=await A.updateTask(z.instanceId,z.uri,{summary:a,description:l,status:o,due:n,priority:r,percent:u,parentUid:y});z=v.task,Pe=me(v.task.instanceId,v.task.uri),b("success","Task saved")}await Ut()}catch(v){b("error",v instanceof Error?v.message:"Save failed")}finally{d=!1,m()}}async function $r(e){const t=new FormData(e),a=String(t.get("summary")??"").trim(),l=String(t.get("description")??"").trim(),o=String(t.get("dtstart")??"").trim(),p=o?new Date(o).toISOString():null;d=!0,E(),m();try{if(ke){const n=Number(t.get("instanceId"));if(!Number.isFinite(n)||n<=0)throw new Error("Select a calendar");const r=await A.createNote({instanceId:n,summary:a,description:l,dtstart:p});ke=!1,lt=me(r.note.instanceId,r.note.uri),re=r.note,b("success","Note created")}else if(re){const n=await A.updateNote(re.instanceId,re.uri,{summary:a,description:l,dtstart:p});re=n.note,lt=me(n.note.instanceId,n.note.uri),b("success","Note saved")}await va()}catch(n){b("error",n instanceof Error?n.message:"Save failed")}finally{d=!1,m()}}function wr(){const e=s.querySelector('input[data-action="contact-photo"]');e&&e.addEventListener("change",()=>{(async()=>{var a;const t=(a=e.files)==null?void 0:a[0];if(e.value="",!!t){if(t.size>2.5*1024*1024){b("error","Photo is too large (max ~2 MB)"),m();return}try{const l=await Hn(t);He=l,Ee=`data:${t.type||"image/jpeg"};base64,${l}`,Ge=!1,m()}catch(l){b("error",l instanceof Error?l.message:"Failed to read photo"),m()}}})()})}function kr(){const e=s.querySelector('[data-form="create-cal"]');if(!e)return;const t=e.querySelector('input[name="holidays"]'),a=e.querySelector("#holidays-country-wrap"),l=e.querySelector('input[name="displayname"]'),o=e.querySelector('input[name="readOnly"]');if(!t||!a)return;const p=()=>{const n=t.checked;a.hidden=!n,l&&(l.required=!n,n&&!l.value.trim()?l.placeholder="Auto: Holidays (XX)":n||(l.placeholder="Work")),n&&o&&(o.checked=!0)};t.addEventListener("change",p),p()}async function Sr(e){var o,p,n,r;const t=new FormData(e),a=String(t.get("username")??""),l=String(t.get("password")??"");d=!0,E(),m(),N.event("login.attempt",{username:a});try{const u=await A.login(a,l);if(c=u.user,fs(u.ui),N.event("login.ok",{username:(c==null?void 0:c.username)??a}),gs(),we())try{await hs()}catch(f){N.warn("admin.capabilities login",f instanceof Error?f.message:f)}if(Us(),dt(h,w),await Xe(),h==="admin"&&we()&&_t())try{w==="overview"&&((o=Ve("overview"))==null?void 0:o.available)!==!1?await Ja():w==="users"&&((p=Ve("users"))==null?void 0:p.available)!==!1?(await ea(),B&&(await wt(B),await ta(B))):w==="settings"&&((n=Ve("settings"))==null?void 0:n.available)!==!1?await Ya():w==="database"&&((r=Ve("database"))==null?void 0:r.available)!==!1&&await Ka()}catch(f){N.warn("admin login load",f instanceof Error?f.message:f)}b("success","Signed in")}catch(u){N.warn("login.failed",u instanceof Error?u.message:u),b("error",u instanceof Error?u.message:"Login failed")}finally{d=!1,m()}}async function Dr(e){const t=new FormData(e),a=String(t.get("path")??""),l=String(t.get("newName")??"").trim();if(!a||!l){b("error","Name is required"),m();return}d=!0,E(),m();try{await A.filesRename(a,l),N.event("files.rename",{path:a,newName:l}),Se=null,await kt(),b("success",`Renamed to “${l}”`)}catch(o){b("error",o instanceof Error?o.message:"Rename failed")}finally{d=!1,m()}}async function Cr(e){const t=new FormData(e),a=String(t.get("name")??"").trim();if(!a){b("error","Folder name is required"),m();return}d=!0,E(),m();try{await A.filesMkdir(Fe,a),N.event("files.mkdir",{path:Fe,name:a}),ht=!1,await kt(),b("success",`Created folder “${a}”`)}catch(l){b("error",l instanceof Error?l.message:"Could not create folder")}finally{d=!1,m()}}async function Ar(e){if(!de||de.paths.length===0)return;const t=new FormData(e),a=String(t.get("toPath")??"").trim().replace(/^\/+|\/+$/g,""),l=String(t.get("newName")??"").trim(),o=de.op,p=[...de.paths],n=p.length>1;d=!0,E(),m();let r=0;const u=[];try{for(const y of p)try{if(o==="copy"){const v=Za(y),$=n||!l||l===v?void 0:l,_=await A.filesCopy(y,{to:a,newName:$});N.event("files.copy",{path:y,to:_.entry.path})}else{const v=Za(y),$=n||!l||l===v?void 0:l;await A.filesMove(y,a,$),N.event("files.move",{path:y,to:a})}r+=1}catch(v){u.push(`${Za(y)}: ${v instanceof Error?v.message:"failed"}`)}de=null,ue=[],await kt();const f=o==="copy"?"Copied":"Moved";r>0&&u.length===0?b("success",r===1?`${f} 1 item`:`${f} ${r} items`):r>0?b("info",`${f} ${r}; ${u.length} failed. ${u[0]}`):b("error",u[0]||`${o==="copy"?"Copy":"Move"} failed`)}catch(f){b("error",f instanceof Error?f.message:"Operation failed")}finally{d=!1,m()}}async function Er(e){const t=e.files;if(!t||t.length===0)return;const a=Array.from(t);e.value="",d=!0,E(),m();let l=0;const o=[];try{for(const p of a)try{await A.filesUpload(Fe,p,{replace:!0}),N.event("files.upload",{path:Fe,name:p.name,size:p.size}),l+=1}catch(n){o.push(`${p.name}: ${n instanceof Error?n.message:"failed"}`)}await kt(),l>0&&o.length===0?b("success",l===1?"Uploaded 1 file":`Uploaded ${l} files`):l>0?b("info",`Uploaded ${l}; ${o.length} failed. ${o[0]}`):b("error",o[0]||"Upload failed")}catch(p){b("error",p instanceof Error?p.message:"Upload failed")}finally{d=!1,m()}}async function Nr(e){if(F===null)return;const t=new FormData(e),a=String(t.get("username")??""),l=String(t.get("access")??"read");fe=!0,d=!0,E(),m();try{await A.share(F,a,l),await xa(F),b("success",`Shared with ${a}`)}catch(o){b("error",o instanceof Error?o.message:"Share failed")}finally{d=!1,m()}}function Oa(e){if(!k)return;const t=new FormData(e),a=e.querySelector('input[name="allDay"]');k={...k,summary:String(t.get("summary")??k.summary),description:String(t.get("description")??k.description),location:String(t.get("location")??k.location),instanceId:Number(t.get("instanceId"))||k.instanceId,allDay:(a==null?void 0:a.checked)??k.allDay,start:String(t.get("start")??k.start??""),end:String(t.get("end")??k.end??"")||null,repeat:Ua(t),hasRrule:!!String(t.get("repeatFreq")??"").trim()}}function Ua(e){const t=String(e.get("repeatFreq")??"").trim().toUpperCase();if(!t)return{freq:"",interval:1,until:null,count:null,byDay:[],endMode:"never"};const a=Math.max(1,Math.min(99,Number(e.get("repeatInterval")??1)||1)),l=String(e.get("repeatEndMode")??"never"),o=l==="until"||l==="count"?l:"never";let p=null,n=null;if(o==="until"){const u=String(e.get("repeatUntil")??"").trim();p=u?u.slice(0,10):null}else if(o==="count"){const u=Number(e.get("repeatCount")??0);n=Number.isFinite(u)&&u>0?Math.min(999,Math.round(u)):10}const r=e.getAll("repeatByDay").map(u=>String(u).toUpperCase()).filter(Boolean);return{freq:t,interval:a,until:p,count:n,byDay:r,endMode:o}}async function Tr(e){if(!k||!k.canWrite)return;const t=new FormData(e),a=String(t.get("summary")??"").trim(),l=String(t.get("description")??"").trim(),o=String(t.get("location")??"").trim(),p=t.get("allDay")==="on",n=String(t.get("start")??"").trim(),r=String(t.get("end")??"").trim(),u=Number(t.get("instanceId"))||k.instanceId,f=Ua(t);if(!a){b("error","Title is required"),m();return}if(!n){b("error","Start is required"),m();return}let y,v;if(p)y=n.slice(0,10),v=r?r.slice(0,10):y;else if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const L=$s(n,r||null);y=new Date(L.start).toISOString(),v=L.end?new Date(L.end).toISOString():null}else y=new Date(n).toISOString(),v=r?new Date(r).toISOString():null;const $=k.instanceId,_=k.uri,D=rt;d=!0,E(),gt=!0,m(),N.event(D?"event.create":"event.update",{instanceId:u,uri:D?null:_,allDay:p,summary:a});try{const L={summary:a,description:l,location:o,allDay:p,start:y,end:v,instanceId:u,repeat:f},ae=D?await A.createEvent(u,L):await A.updateEvent($,_,L);(F===null||ae.event.instanceId!==F)&&(F=ae.event.instanceId),await Ze(),gt=!1,k=null,rt=!1,O=null,N.event(D?"event.created":"event.saved",{uri:ae.event.uri,instanceId:ae.event.instanceId}),b("success",D?"Event created":"Event saved")}catch(L){N.warn("event.save failed",L instanceof Error?L.message:L),b("error",L instanceof Error?L.message:"Save failed")}finally{d=!1,m()}}async function _r(e){if(F===null)return;const t=new FormData(e),a=String(t.get("displayname")??"").trim(),l=String(t.get("description")??""),o=String(t.get("color")??"").trim();d=!0,E(),m();try{const p=await A.updateCalendar(F,{displayname:a,description:l,color:o});fe=!0,await Xe(),F=p.calendar.id,await xa(F),await Ze(),b("success","Calendar updated")}catch(p){b("error",p instanceof Error?p.message:"Update failed")}finally{d=!1,m()}}async function xr(e){const t=new FormData(e),a=String(t.get("displayname")??"").trim(),l=String(t.get("description")??""),o=String(t.get("color")??"").trim(),p=t.get("holidays")==="on",n=String(t.get("holidayCountry")??"").trim(),r=t.get("readOnly")==="on";if(Me=!0,p&&!n){b("error","Select a country for the holidays calendar"),m();return}if(!p&&!a){b("error","Display name is required"),m();return}d=!0,E(),m();try{const u=await A.createCalendar({displayname:a,description:l,color:o,holidays:p,holidayCountry:p?n:void 0,readOnly:r});F=u.calendar.id,K.includes(u.calendar.id)||(K=[...K,u.calendar.id]),Me=!1,await Xe();let f=`Created “${u.calendar.displayname}”`;const y=u.holidayImport??u.calendar.holidayImport;y&&(f+=`. Holidays imported: ${Ns(y)}.`),r&&(f+=" Calendar is read-only."),b("success",f)}catch(u){Me=!0,b("error",u instanceof Error?u.message:"Create failed")}finally{d=!1,m()}}async function qr(e){var l,o,p;const t=e.target.closest("[data-action]");if(!t)return;const a=t.dataset.action;if(a&&N.debug(`action:${a}`,{id:t.dataset.id,tab:t.dataset.tab,uri:t.dataset.uri}),a==="close-import-progress"){j&&(j.phase==="done"||j.phase==="error")&&Ks();return}if(a==="logout"){d=!0,N.event("logout");try{await A.logout()}catch{}Wa(),E(),m();return}if(a==="select-cal"||a==="toggle-cal"){const n=Number(t.dataset.id);if(!Number.isFinite(n))return;xn(n),d=!0,E(),m();try{await Ze()}catch(r){b("error",r instanceof Error?r.message:"Failed to load calendar")}finally{d=!1,m()}return}if(a==="edit-cal"){const n=Number(t.dataset.id);if(!Number.isFinite(n)||!ne.find(u=>u.id===n&&u.canShare))return;F=n,K.includes(n)||(K=[...K,n]),fe=!0,Re=null,d=!0,E(),m();try{await xa(n),await Ze()}catch(u){b("error",u instanceof Error?u.message:"Failed to open calendar")}finally{d=!1,m()}return}if(a==="close-cal-modal"){fe=!1,m();return}if(a==="open-create-cal-modal"){Me=!0,fe=!1,Re=null,E(),m();return}if(a==="close-create-cal-modal"){Me=!1,E(),m();return}if(a==="delete-cal"){const n=Number(t.dataset.id);if(!Number.isFinite(n)||!ne.find(u=>u.id===n&&u.canShare))return;Re=n,fe=!1,E(),m();return}if(a==="cancel-delete-cal"){Re=null,m();return}if(a==="confirm-delete-cal"){const n=Number(t.dataset.id),r=s.querySelector("#delete-cal-confirm");if(!Number.isFinite(n)||!(r!=null&&r.checked))return;d=!0,E(),m();try{if(await A.deleteCalendar(n),F===n&&(F=null),K=K.filter(u=>u!==n),Re=null,fe=!1,Gt=[],Qt=[],await Xe(),F===null){const u=Vs();u?(F=u.id,K.includes(u.id)||(K=[...K,u.id]),await Ze()):K.length>0&&(F=K[0],await Ze())}b("success","Calendar deleted")}catch(u){b("error",u instanceof Error?u.message:"Delete failed")}finally{d=!1,m()}return}if(a==="month-today"){const n=new Date;Tt={y:n.getFullYear(),m:n.getMonth()},Ea=null,d=!0,m();try{await Ze()}finally{d=!1,m()}return}if(a==="month-prev"||a==="month-next"){const n=a==="month-prev"?-1:1,r=new Date(Tt.y,Tt.m+n,1);Tt={y:r.getFullYear(),m:r.getMonth()},Ea=null,d=!0,m();try{await Ze()}finally{d=!1,m()}return}if(a==="open-event"){e.stopPropagation();const n=Number(t.dataset.instance),r=t.dataset.uri??"";if(!Number.isFinite(n)||!r)return;d=!0,E(),m();try{const u=await A.getEvent(n,r);k={...u.event,repeat:u.event.repeat??Xa()},rt=!1,gt=!0,O=null,fe=!1,Re=null}catch(u){b("error",u instanceof Error?u.message:"Failed to open event")}finally{d=!1,m()}return}if(a==="open-event-day"){e.stopPropagation();const n=t.dataset.day??"";Ea=Ea===n?null:n,m();return}if(a==="new-event-day"){const n=e.target;if((l=n==null?void 0:n.closest)!=null&&l.call(n,".month-event, .month-event-more"))return;const r=t.dataset.day??"";if(!/^\d{4}-\d{2}-\d{2}$/.test(r))return;if(F===null){b("error","Select a calendar first"),m();return}const u=ne.find(f=>f.id===F);if(!u||u.readOnly||!(u.canShare||u.access==="readwrite")){b("error","This calendar is read-only"),m();return}rt=!0,k=Bn(r,F),gt=!0,O=null,fe=!1,Re=null,E(),m();return}if(a==="close-event-modal"){gt=!1,k=null,rt=!1,O=null,E(),m();return}if(a==="dt-open"){const n=t.dataset.dtField||"";if(!n)return;const r=s.querySelector('[data-form="edit-event"]');if(r&&k&&Oa(r),(O==null?void 0:O.field)===n)O=null;else{const u=t.dataset.dtDateOnly==="1",f=t.dataset.dtClear!=="0",y=t.dataset.dtName||n;let v=Ss(n);!v&&(n==="due"||n==="dtstart"||n==="bulk-due")&&(v=Ia().start);const $=qa(v||he(new Date)),[_,D]=$.date.split("-").map(Number);O={field:n,viewY:_,viewM:(D||1)-1,dateOnly:u,allowClear:f,name:y}}m();return}if(a==="dt-month-prev"||a==="dt-month-next"){if(!O)return;const n=a==="dt-month-prev"?-1:1,r=new Date(O.viewY,O.viewM+n,1);O={...O,viewY:r.getFullYear(),viewM:r.getMonth()},m();return}if(a==="dt-pick-day"){if(!O)return;const n=O.field,r=t.dataset.day??"";if(!/^\d{4}-\d{2}-\d{2}$/.test(r))return;const u=s.querySelector('[data-form="edit-event"]');u&&k&&Oa(u);const f=O.dateOnly;if(f)ot(n,r),O=null;else{const y=Ss(n),v=qa(y||Ia(r).start).hm;ot(n,`${r}T${v}`),O={...O,viewY:Number(r.slice(0,4)),viewM:Number(r.slice(5,7))-1}}if(n==="start"&&k&&!f&&k.end){const y=new Date(String(k.start)),v=new Date(String(k.end));!Number.isNaN(y.getTime())&&!Number.isNaN(v.getTime())&&v<=y&&ot("end",Lt(new Date(y.getTime()+3600*1e3)))}m();return}if(a==="dt-pick-time"){if(!O||O.dateOnly)return;const n=O.field,r=t.dataset.hm??"";if(!/^\d{2}:\d{2}$/.test(r))return;const u=s.querySelector('[data-form="edit-event"]');u&&k&&Oa(u);const f=Ss(n)||Ia().start,v=`${qa(f).date}T${r}`;if(ot(n,v),n==="start"&&k){k={...k,allDay:!1};const $=k.end?qa(String(k.end)):null,_=new Date(v);(!$||new Date(`${$.date}T${$.hm}`)<=_)&&ot("end",Lt(new Date(_.getTime()+3600*1e3)))}O=null,m();return}if(a==="dt-today"){if(!O)return;const n=O.field,r=s.querySelector('[data-form="edit-event"]');r&&k&&Oa(r);const u=he(new Date);if(O.dateOnly)ot(n,u);else{const f=Ia(u);n==="start"?(ot("start",f.start),k&&!k.end&&ot("end",f.end)):n==="end"?ot("end",f.end):ot(n,f.start)}O=null,m();return}if(a==="dt-clear"){if(!O||!O.allowClear)return;const n=O.field,r=s.querySelector('[data-form="edit-event"]');r&&k&&Oa(r),ot(n,null),O=null,m();return}if(a==="event-allday-toggle"){if(!k)return;const n=s.querySelector('[data-form="edit-event"]'),r=t.checked;if(n){const u=new FormData(n),f=String(u.get("start")??k.start??""),y=String(u.get("end")??k.end??"")||null;let v=f,$=y;if(r){const _=Tn(f,y);v=_.start,$=_.end}else{const _=f.slice(0,10),D=(y||f).slice(0,10),L=$s(_,D);v=L.start,$=L.end}k={...k,summary:String(u.get("summary")??k.summary),description:String(u.get("description")??k.description),location:String(u.get("location")??k.location),instanceId:Number(u.get("instanceId"))||k.instanceId,allDay:r,start:v,end:$,repeat:Ua(u)}}else k={...k,allDay:r};O=null,m();return}if(a==="event-repeat-freq"||a==="event-repeat-end"){if(!k)return;const n=s.querySelector('[data-form="edit-event"]');if(!n)return;const r=new FormData(n),u=n.querySelector('input[name="allDay"]'),f=Ua(r);k={...k,summary:String(r.get("summary")??k.summary),description:String(r.get("description")??k.description),location:String(r.get("location")??k.location),instanceId:Number(r.get("instanceId"))||k.instanceId,allDay:(u==null?void 0:u.checked)??k.allDay,start:String(r.get("start")??k.start??""),end:String(r.get("end")??k.end??"")||null,repeat:f,hasRrule:!!String(r.get("repeatFreq")??"").trim()},f.freq&&f.endMode==="until"&&(O==null?void 0:O.field)==="end"&&(O=null),m();return}if(a==="delete-event"){if(!k||!k.canWrite||rt||!confirm("Delete this event? CalDAV clients will sync the removal."))return;const n=k.instanceId,r=k.uri;d=!0,E(),m();try{await A.deleteEvent(n,r),gt=!1,k=null,await Ze(),b("success","Event deleted")}catch(u){b("error",u instanceof Error?u.message:"Delete failed")}finally{d=!1,m()}return}if(a==="info"){const n=t.dataset.info??"";Mr(n);return}if(a==="info-close"){an();return}if(a==="flash-close"){E(),m();return}if(a==="user-menu-toggle"){e.stopPropagation(),Oe=!Oe,m();return}if(a==="user-menu-close"){Oe&&(Oe=!1,m());return}if(a==="tab"){const n=xs(t.dataset.tab);n&&(n==="admin"&&(w="overview"),await Ms(n));return}if(a==="admin-page"){const n=ds(t.dataset.adminPage);n&&await Ps(n);return}if(a==="admin-refresh"){if(!we()||h!=="admin")return;d=!0,E(),m();try{await Ja(),b("success","Overview refreshed")}catch(n){b("error",n instanceof Error?n.message:"Refresh failed")}finally{d=!1,m()}return}if(a==="admin-users-refresh"){if(!we()||h!=="admin")return;d=!0,E(),m();try{await ea(),B&&await wt(B),b("success","Users refreshed")}catch(n){b("error",n instanceof Error?n.message:"Refresh failed")}finally{d=!1,m()}return}if(a==="admin-user-view"){const n=t.dataset.username??"";if(!n||!we())return;d=!0,E(),B=n,w="users",dt("admin","users",n),m();try{await wt(n),await ta(n)}catch(r){b("error",r instanceof Error?r.message:"Failed to load user")}finally{d=!1,m()}return}if(a==="admin-user-close"){B=null,V=null,Ie=null,Le=!1,dt("admin","users",null),m();return}if(a==="admin-user-create-open"){if(!we())return;je=!0,Le=!1,pt=null,E(),m();return}if(a==="admin-user-create-close"){je=!1,m();return}if(a==="admin-user-edit-open"){if(!we())return;const n=t.dataset.username??B??"";if(!n)return;d=!0,E(),je=!1,pt=null,B=n,w="users",dt("admin","users",n),m();try{(!V||V.username.toLowerCase()!==n.toLowerCase())&&await wt(n),Le=!0}catch(r){b("error",r instanceof Error?r.message:"Failed to load user")}finally{d=!1,m()}return}if(a==="admin-user-edit-close"){Le=!1,m();return}if(a==="admin-user-delete-open"){if(!we())return;const n=t.dataset.username??B??"";if(!n)return;pt=n,At=!1,je=!1,Le=!1,E(),m();return}if(a==="admin-user-delete-close"){pt=null,At=!1,m();return}if(a==="admin-user-delete-toggle"){At=!!t.checked,m();return}if(a==="admin-user-delete-confirm"){if(!we())return;const n=t.dataset.username??pt??"";if(!n||!At)return;d=!0,E(),m();try{await A.adminDeleteUser(n,!0),N.event("admin.user.delete",{username:n}),pt=null,At=!1,Le=!1,(B==null?void 0:B.toLowerCase())===n.toLowerCase()&&(B=null,V=null,Et=[],Nt=[],dt("admin","users",null)),await ea(),b("success",`Deleted user “${n}”`)}catch(r){b("error",r instanceof Error?r.message:"Delete failed")}finally{d=!1,m()}return}if(a==="admin-cal-create"){We="create",zt=null,m();return}if(a==="admin-cal-edit"){We="edit",zt=Number(t.dataset.id),m();return}if(a==="admin-cal-close"){We=null,zt=null,m();return}if(a==="admin-cal-delete"){$e={kind:"calendar",id:Number(t.dataset.id),label:t.dataset.label??"calendar"},m();return}if(a==="admin-ab-create"){nt="create",Ht=null,m();return}if(a==="admin-ab-edit"){nt="edit",Ht=Number(t.dataset.id),m();return}if(a==="admin-ab-close"){nt=null,Ht=null,m();return}if(a==="admin-ab-delete"){$e={kind:"addressbook",id:Number(t.dataset.id),label:t.dataset.label??"address book",force:!1},m();return}if(a==="admin-ab-force-toggle"){($e==null?void 0:$e.kind)==="addressbook"&&($e={...$e,force:!!t.checked},m());return}if(a==="admin-resource-delete-close"){$e=null,m();return}if(a==="admin-resource-delete-confirm"){if(!B||!$e)return;const n=B,r=$e;d=!0,E(),m();try{r.kind==="calendar"?await A.adminDeleteUserCalendar(n,r.id,!0):await A.adminDeleteUserAddressBook(n,r.id,!0,!!r.force),$e=null,await ta(n),await wt(n),b("success","Deleted")}catch(u){b("error",u instanceof Error?u.message:"Delete failed")}finally{d=!1,m()}return}if(a==="admin-settings-refresh"){d=!0,E(),m();try{await Ya(),b("success","Settings reloaded")}catch(n){b("error",n instanceof Error?n.message:"Reload failed")}finally{d=!1,m()}return}if(a==="admin-reset-open"){ca=!0,ft=!1,Je="",E(),m();return}if(a==="admin-reset-close"){ca=!1,ft=!1,Je="",m();return}if(a==="admin-reset-toggle"){ft=!!t.checked,m();return}if(a==="admin-reset-password"){Je=t.value;const n=s.querySelector('[data-action="admin-reset-confirm"]');n&&(n.disabled=d||!ft||Je.trim()==="");return}if(a==="admin-reset-confirm"){if(!ft)return;if(Je.trim()===""){b("error","Re-enter your password to confirm Reset to Default"),m();return}d=!0,E(),m();try{const n=await A.adminResetToDefault(!0,Je);N.event("admin.settings.reset-to-default"),ca=!1,ft=!1,Je="";const r=n.redirectUrl&&n.redirectUrl.startsWith("/")?n.redirectUrl:"/portal/install/";window.location.assign(r);return}catch(n){b("error",n instanceof Error?n.message:"Reset failed"),d=!1,m()}return}if(a==="admin-database-refresh"){d=!0,E(),m();try{await Ka(),b("success","Database settings reloaded")}catch(n){b("error",n instanceof Error?n.message:"Reload failed")}finally{d=!1,m()}return}if(a==="admin-db-backend"){Yt=t.value==="pgsql"?"pgsql":"sqlite",m();return}if(a==="admin-db-test"){const n=t.closest("form");wn(n);return}if(a==="admin-db-confirm-close"){ua=!1,bt="",ma=null,m();return}if(a==="admin-db-confirm-input"){bt=t.value,m();const r=s.querySelector('[data-action="admin-db-confirm-input"]');if(r){r.focus();const u=r.value.length;r.setSelectionRange(u,u)}return}if(a==="admin-db-confirm-save"){if(bt.trim()!=="CONFIRM"||!ma)return;d=!0,E(),m();try{const n={...ma,confirm:"CONFIRM"},r=await A.adminUpdateDatabaseSettings(n);Jt=r.data,ua=!1,bt="",ma=null,Yt=(r.data.backend||"sqlite").toLowerCase()==="pgsql"?"pgsql":"sqlite",N.event("admin.database.save",{backend:r.data.backend}),b("success","Database settings saved")}catch(n){b("error",n instanceof Error?n.message:"Database save failed")}finally{d=!1,m()}return}if(a==="files-nav"){Fe=t.dataset.path??"",Se=null,ge=null,de=null,ht=!1,ue=[],d=!0,E(),m();try{await kt()}catch(r){b("error",r instanceof Error?r.message:"Failed to open folder")}finally{d=!1,m()}return}if(a==="files-toggle"){e.stopPropagation();const n=t.dataset.path??"";if(!n)return;t.checked?ue.includes(n)||(ue=[...ue,n]):ue=ue.filter(u=>u!==n),m();return}if(a==="files-select-all"){e.stopPropagation(),ue=t.checked?Te.map(r=>r.path):[],m();return}if(a==="files-copy"){const n=t.dataset.path??"";if(!n)return;de={op:"copy",paths:[n]},Se=null,ge=null,m();return}if(a==="files-move"){const n=t.dataset.path??"";if(!n)return;de={op:"move",paths:[n]},Se=null,ge=null,m();return}if(a==="files-bulk-copy"){if(ue.length===0)return;de={op:"copy",paths:[...ue]},Se=null,ge=null,m();return}if(a==="files-bulk-move"){if(ue.length===0)return;de={op:"move",paths:[...ue]},Se=null,ge=null,m();return}if(a==="files-transfer-close"){de=null,m();return}if(a==="files-bulk-delete"){if(ue.length===0)return;ge=[...ue],Se=null,de=null,m();return}if(a==="files-refresh"){d=!0,E(),m();try{await kt(),b("success","Refreshed")}catch(n){b("error",n instanceof Error?n.message:"Refresh failed")}finally{d=!1,m()}return}if(a==="files-mkdir"){ht=!0,Se=null,ge=null,de=null,E(),m();return}if(a==="files-mkdir-close"){ht=!1,m();return}if(a==="files-rename-open"){Se=t.dataset.path??null,ge=null,de=null,m();return}if(a==="files-rename-close"){Se=null,m();return}if(a==="files-delete-open"){const n=t.dataset.path??"";ge=n?[n]:null,Se=null,de=null,m();return}if(a==="files-delete-close"){ge=null,m();return}if(a==="files-delete-confirm"){const n=ge?[...ge]:[];if(n.length===0)return;d=!0,E(),m();try{if(n.length===1)await A.filesDelete(n[0]),N.event("files.delete",{path:n[0]}),b("success","Deleted");else{const r=await A.filesBulk("delete",n);N.event("files.bulk-delete",{ok:r.ok,failed:r.failed}),r.failed===0?b("success",r.ok===1?"Deleted 1 item":`Deleted ${r.ok} items`):r.ok>0?b("info",`Deleted ${r.ok}; ${r.failed} failed. ${r.errors[0]||""}`):b("error",r.errors[0]||"Delete failed")}ge=null,ue=[],await kt()}catch(r){b("error",r instanceof Error?r.message:"Delete failed")}finally{d=!1,m()}return}if(a==="files-download"){N.event("files.download",{path:t.getAttribute("href")??""});return}if(a==="sort-task"||a==="sort-note"){const n=t.dataset.sort||"";if(!n)return;if(a==="sort-task"){It===n?St=St==="asc"?"desc":"asc":(It=n,St=n==="due"||n==="summary"?"asc":"desc"),d=!0,m();try{await Ut()}catch(r){b("error",r instanceof Error?r.message:"Sort failed")}finally{d=!1,m()}}else{ha===n?aa=aa==="asc"?"desc":"asc":(ha=n,aa="asc"),d=!0,m();try{await va()}catch(r){b("error",r instanceof Error?r.message:"Sort failed")}finally{d=!1,m()}}return}if(a==="select-task"){if(e.target.closest("[data-stop-row], .task-check"))return;const n=Number(t.dataset.instance),r=t.dataset.uri??"";if(!Number.isFinite(n)||!r)return;const u=Ne.find(f=>f.instanceId===n&&f.uri===r)??null;Q=!1,Pe=me(n,r),z=u?{...u}:null,E(),m();return}if(a==="task-check"){e.preventDefault(),e.stopPropagation();const n=Number(t.dataset.instance),r=t.dataset.uri??"";if(!Number.isFinite(n)||!r)return;const u=me(n,r),f=Ne.find(y=>me(y.instanceId,y.uri)===u);if(!f||!f.canWrite||f.readOnly)return;ye.includes(u)?ye=ye.filter(y=>y!==u):ye=[...ye,u],m();return}if(a==="task-select-all"){e.preventDefault();const n=Ne.filter(u=>u.canWrite&&!u.readOnly);n.length>0&&n.every(u=>ye.includes(me(u.instanceId,u.uri)))?ye=[]:ye=n.map(u=>me(u.instanceId,u.uri)),m();return}if(a==="bulk-task-clear"){ye=[],m();return}if(a==="bulk-task-status"||a==="bulk-task-due"||a==="bulk-task-clear-due"||a==="bulk-task-percent"||a==="bulk-task-delete"){yr(a);return}if(a==="select-note"){const n=Number(t.dataset.instance),r=t.dataset.uri??"";if(!Number.isFinite(n)||!r)return;const u=ga.find(f=>f.instanceId===n&&f.uri===r)??null;ke=!1,lt=me(n,r),re=u?{...u}:null,E(),m();return}if(a==="new-task"){Q=!0,Pe=null,z={uri:"",instanceId:((o=xt[0])==null?void 0:o.id)??0,calendarId:0,calendarName:"",calendarUri:"",uid:"",parentUid:null,summary:"",description:"",status:"NEEDS-ACTION",due:null,priority:0,percent:0,completed:null,lastmodified:0,readOnly:!1,canWrite:!0},E(),m();return}if(a==="new-subtask"){if(!z||Q||!z.uid||!z.canWrite)return;const n=z;Q=!0,Pe=null,z={uri:"",instanceId:n.instanceId,calendarId:n.calendarId,calendarName:n.calendarName,calendarUri:n.calendarUri,uid:"",parentUid:n.uid,summary:"",description:"",status:"NEEDS-ACTION",due:null,priority:0,percent:0,completed:null,lastmodified:0,readOnly:!1,canWrite:!0},E(),m();return}if(a==="new-note"){ke=!0,lt=null,re={uri:"",instanceId:((p=qt[0])==null?void 0:p.id)??0,calendarId:0,calendarName:"",calendarUri:"",summary:"",description:"",dtstart:new Date().toISOString(),lastmodified:0,readOnly:!1,canWrite:!0},E(),m();return}if(a==="cancel-task"){Q=!1,z=null,Pe=null,m();return}if(a==="cancel-note"){ke=!1,re=null,lt=null,m();return}if(a==="delete-task"){if(!z||Q||!confirm("Delete this task? CalDAV clients will sync the removal."))return;d=!0,E(),m();try{await A.deleteTask(z.instanceId,z.uri),Pe=null,z=null,await Ut(),b("success","Task deleted")}catch(n){b("error",n instanceof Error?n.message:"Delete failed")}finally{d=!1,m()}return}if(a==="delete-note"){if(!re||ke||!confirm("Delete this note? CalDAV clients will sync the removal."))return;d=!0,E(),m();try{await A.deleteNote(re.instanceId,re.uri),lt=null,re=null,await va(),b("success","Note deleted")}catch(n){b("error",n instanceof Error?n.message:"Delete failed")}finally{d=!1,m()}return}if(a==="select-ab"){const n=Number(t.dataset.id);if(!Number.isFinite(n))return;M=n,Ke=!1,ce=null,q=null,be=!1,Ce=!1,Xt="",$t=[],Ee=null,He=null,Ge=!1,E(),d=!0,m();try{await Ot(n)}catch(r){b("error",r instanceof Error?r.message:"Failed to load contacts")}finally{d=!1,m()}return}if(a==="edit-ab"){e.stopPropagation();const n=Number(t.dataset.id);if(!Number.isFinite(n)||!Ue.find(f=>f.id===n))return;const u=M!==n;M=n,Ke=!0,Ce=!1,E(),u&&(ce=null,q=null,be=!1,Xt="",$t=[],Ee=null,He=null,Ge=!1),d=!0,m();try{u&&await Ot(n)}catch(f){b("error",f instanceof Error?f.message:"Failed to open address book")}finally{d=!1,m()}return}if(a==="close-ab-modal"){Ke=!1,m();return}if(a==="select-contact"){const n=t.dataset.uri??"";if(!n)return;E();try{await jn(n)}catch(r){b("error",r instanceof Error?r.message:"Failed to load contact")}m();return}if(a==="new-contact"){if(M===null)return;zn(),E(),m();return}if(a==="cancel-contact"||a==="close-contact-modal"){be=!1,Ce=!1,q=null,ce=null,Ee=null,He=null,Ge=!1,O=null,E(),m();return}if(a==="add-email"||a==="add-phone"||a==="add-custom"){if(!q)return;ts(),Array.isArray(q.emails)||(q.emails=[""]),Array.isArray(q.phones)||(q.phones=[{type:"cell",value:""}]),Array.isArray(q.custom)||(q.custom=[]),a==="add-email"?q.emails.length<10&&q.emails.push(""):a==="add-phone"?q.phones.length<10&&q.phones.push({type:"other",value:""}):q.custom.length<30&&q.custom.push({label:"",value:""}),m();return}if(a==="remove-email"){if(!q)return;ts();const n=Number(t.dataset.idx);if(!Number.isFinite(n))return;const r=Array.isArray(q.emails)?q.emails:[""];q.emails=r.filter((u,f)=>f!==n),q.emails.length===0&&(q.emails=[""]),m();return}if(a==="remove-phone"){if(!q)return;ts();const n=Number(t.dataset.idx);if(!Number.isFinite(n))return;const r=Array.isArray(q.phones)?q.phones:[{type:"cell",value:""}];q.phones=r.filter((u,f)=>f!==n),q.phones.length===0&&(q.phones=[{type:"cell",value:""}]),m();return}if(a==="remove-custom"){if(!q)return;ts();const n=Number(t.dataset.idx);if(!Number.isFinite(n))return;q.custom=(Array.isArray(q.custom)?q.custom:[]).filter((r,u)=>u!==n),m();return}if(a==="remove-photo"){Ee=null,He=null,Ge=!0,q&&(q.hasPhoto=!1),m();return}if(a==="delete-contact"){if(M===null||!ce||!confirm("Delete this contact? CardDAV clients will sync the removal."))return;d=!0,E(),Ce=!0,m();try{await A.deleteContact(M,ce),ce=null,q=null,be=!1,Ce=!1,O=null,Ee=null,await Xe(),b("success","Contact deleted")}catch(n){b("error",n instanceof Error?n.message:"Delete failed")}finally{d=!1,m()}return}if(a==="delete-ab"){e.stopPropagation();const n=Number(t.dataset.id??M);if(!Number.isFinite(n)||!Ue.find(u=>u.id===n))return;Ye=n,Ke=!1,Ce=!1,E(),m();return}if(a==="cancel-delete-ab"){Ye=null,m();return}if(a==="confirm-delete-ab"){const n=Number(t.dataset.id),r=s.querySelector("#delete-ab-confirm");if(!Number.isFinite(n)||!(r!=null&&r.checked))return;const u=Ue.find(y=>y.id===n);if(!u)return;const f=(u.cardCount??0)>0;d=!0,E(),m();try{await A.deleteAddressBook(n,f),M===n&&(M=null,$t=[],q=null,ce=null,be=!1),Ye=null,Ke=!1,Ce=!1,await Xe(),M===null&&Ue.length>0&&(M=Ue[0].id,await Ot(M)),b("success","Address book deleted")}catch(y){b("error",y instanceof Error?y.message:"Delete failed")}finally{d=!1,m()}return}if(a==="export-ab"){if(M===null)return;Ke=!0,d=!0,E(),m();try{const{blob:n,filename:r}=await A.exportAddressBook(M),u=URL.createObjectURL(n),f=document.createElement("a");f.href=u,f.download=r,f.click(),URL.revokeObjectURL(u),b("success",`Exported ${r}`)}catch(n){b("error",n instanceof Error?n.message:"Export failed")}finally{d=!1,m()}return}if(a==="export-contact"){if(M===null||!ce||be)return;Ce=!0,d=!0,E(),m();try{const{blob:n,filename:r}=await A.exportContact(M,ce),u=URL.createObjectURL(n),f=document.createElement("a");f.href=u,f.download=r,f.click(),URL.revokeObjectURL(u),b("success",`Exported ${r}`)}catch(n){b("error",n instanceof Error?n.message:"Export failed")}finally{d=!1,m()}return}if(a==="revoke"){const n=t.dataset.href??"";if(!n||F===null||!confirm("Revoke access for this user?"))return;fe=!0,d=!0,E(),m();try{await A.revoke(F,n),await xa(F),b("success","Share revoked")}catch(r){b("error",r instanceof Error?r.message:"Revoke failed")}finally{d=!1,m()}return}if(a==="export-cal"){if(F===null)return;fe=!0,d=!0,E(),m();try{const{blob:n,filename:r}=await A.exportCalendar(F),u=URL.createObjectURL(n),f=document.createElement("a");f.href=u,f.download=r,f.click(),URL.revokeObjectURL(u),b("success",`Exported ${r}`)}catch(n){b("error",n instanceof Error?n.message:"Export failed")}finally{d=!1,m()}}}function Ir(){const e=s.querySelector('input[data-action="import-cal"]');e&&e.addEventListener("change",()=>{Rr(e)});const t=s.querySelector('input[data-action="import-create-cal"]');t&&t.addEventListener("change",()=>{Vr(t)});const a=s.querySelector('input[data-action="import-ab"]');a&&a.addEventListener("change",()=>{Lr(a)})}async function Lr(e){var l;if(M===null)return;const t=(l=e.files)==null?void 0:l[0];if(e.value="",!t)return;const a=M;Ke=!0,d=!0,E(),yt(),j={kind:"contacts",fileName:t.name,fileSizeLabel:Js(t.size),phase:"reading",readPercent:0,processPercent:null,processCurrent:0,processTotal:0,processImported:0,processUpdated:0,processSkipped:0,startedAt:Date.now(),elapsedSec:0,resultMessage:null,ok:null},Ys(),m();try{const o=await Xs(t,r=>{if(!j||j.phase!=="reading")return;j={...j,readPercent:r};const u=s.querySelector(".import-progress-bar"),f=s.querySelector("[data-import-status-line]");u&&r!==null&&(u.classList.remove("is-indeterminate"),u.style.width=`${r}%`),f&&r!==null&&(f.textContent=`Reading file… ${r}%`)});Mt("uploading",{readPercent:100}),Mt("processing",{processPercent:0}),N.event("import.contacts.start",{file:t.name,bytes:t.size,abId:a});const p=await A.importAddressBook(a,o,r=>{Gs(r)}),n=Ns(p);await Xe(),M===a&&await Ot(a),yt(),Mt("done",{ok:!0,resultMessage:`${n} (from “${t.name}”)`}),b("success",`Import finished for “${t.name}”: ${n}.`)}catch(o){const p=o instanceof Error?o.message:"Import failed";yt(),Mt("error",{ok:!1,resultMessage:p}),b("error",p)}finally{d=!1,m()}}function ts(){if(!q)return;const e=s.querySelector('[data-form="contact"]');if(!e)return;const t=new FormData(e);q.firstname=String(t.get("firstname")??""),q.lastname=String(t.get("lastname")??""),q.fullname=String(t.get("fullname")??""),q.org=String(t.get("org")??""),q.title=String(t.get("title")??""),q.url=String(t.get("url")??""),q.note=String(t.get("note")??"");const a=String(t.get("birthday")??"").trim();q.birthday=a&&/^\d{4}-\d{2}-\d{2}/.test(a)?a.slice(0,10):null,q.address={street:String(t.get("street")??""),city:String(t.get("city")??""),region:String(t.get("region")??""),postal:String(t.get("postal")??""),country:String(t.get("country")??"")};const l=[];let o=0;for(;t.has(`email_${o}`);)l.push(String(t.get(`email_${o}`)??"")),o++;l.length&&(q.emails=l);const p=[];for(o=0;t.has(`phone_value_${o}`);)p.push({type:String(t.get(`phone_type_${o}`)??"other"),value:String(t.get(`phone_value_${o}`)??"")}),o++;p.length&&(q.phones=p);const n=[];for(o=0;t.has(`custom_label_${o}`)||t.has(`custom_value_${o}`);)n.push({label:String(t.get(`custom_label_${o}`)??""),value:String(t.get(`custom_value_${o}`)??"")}),o++;q.custom=n}function Or(e){const t=new FormData(e),a=[];let l=0;for(;t.has(`email_${l}`);){const r=String(t.get(`email_${l}`)??"").trim();r&&a.push(r),l++}const o=[];for(l=0;t.has(`phone_value_${l}`);){const r=String(t.get(`phone_value_${l}`)??"").trim();r&&o.push({type:String(t.get(`phone_type_${l}`)??"other"),value:r}),l++}const p=[];for(l=0;t.has(`custom_label_${l}`)||t.has(`custom_value_${l}`);){const r=String(t.get(`custom_label_${l}`)??"").trim(),u=String(t.get(`custom_value_${l}`)??"").trim();(r||u)&&p.push({label:r,value:u}),l++}const n={firstname:String(t.get("firstname")??"").trim(),lastname:String(t.get("lastname")??"").trim(),fullname:String(t.get("fullname")??"").trim(),org:String(t.get("org")??"").trim(),title:String(t.get("title")??"").trim(),emails:a,phones:o,address:{street:String(t.get("street")??"").trim(),city:String(t.get("city")??"").trim(),region:String(t.get("region")??"").trim(),postal:String(t.get("postal")??"").trim(),country:String(t.get("country")??"").trim()},url:String(t.get("url")??"").trim(),note:String(t.get("note")??"").trim(),birthday:(()=>{const r=String(t.get("birthday")??"").trim();return r&&/^\d{4}-\d{2}-\d{2}/.test(r)?r.slice(0,10):null})(),custom:p};return Ge?n.removePhoto=!0:He&&(n.photoBase64=He),n}async function Ur(e){if(M===null)return;const t=Or(e);d=!0,E(),Ce=!0,m();try{if(be){const a=await A.createContact(M,t);be=!1,ce=a.contact.uri,q=null,Ce=!1,Ee=null,He=null,Ge=!1,O=null,b("success","Contact created")}else ce&&(ce=(await A.updateContact(M,ce,t)).contact.uri,q=null,Ce=!1,Ee=null,He=null,Ge=!1,O=null,b("success","Contact saved"));try{await Xe()}catch(a){if(console.error(a),M!==null)try{await Ot(M)}catch{}}}catch(a){b("error",a instanceof Error?a.message:"Save failed")}finally{d=!1,m()}}async function Pr(e){const t=new FormData(e),a=String(t.get("displayname")??"").trim(),l=String(t.get("description")??"").trim();if(a){d=!0,E(),m();try{const o=await A.createAddressBook({displayname:a,description:l});M=o.addressbook.id,ce=null,q=null,be=!1,Xt="",await Xe(),b("success",`Address book “${o.addressbook.displayname}” created`)}catch(o){b("error",o instanceof Error?o.message:"Create failed")}finally{d=!1,m()}}}async function Fr(e){if(M===null)return;const t=new FormData(e),a=String(t.get("displayname")??"").trim(),l=String(t.get("description")??"").trim();Ke=!0,d=!0,E(),m();try{await A.updateAddressBook(M,{displayname:a,description:l}),await Xe(),b("success","Address book updated")}catch(o){b("error",o instanceof Error?o.message:"Update failed")}finally{d=!1,m()}}function Mr(e){const t=nl[e];if(!t)return;const a=s.querySelector("#info-modal"),l=s.querySelector("#info-modal-title"),o=s.querySelector("#info-modal-body");if(!a||!l||!o)return;l.textContent=t.title,o.innerHTML=t.paragraphs.map(n=>`<p>${i(n)}</p>`).join(""),a.hidden=!1,document.body.classList.add("info-modal-open");const p=a.querySelector(".info-modal-close");p==null||p.focus()}function an(){const e=s.querySelector("#info-modal");e&&(e.hidden=!0,document.body.classList.remove("info-modal-open"))}async function Rr(e){var a;if(F===null)return;const t=(a=e.files)==null?void 0:a[0];e.value="",t&&(fe=!0,await sn(F,t,{keepEditModalOpen:!0}))}async function Vr(e){var f;const t=(f=e.files)==null?void 0:f[0];if(e.value="",!t)return;const a=s.querySelector('[data-form="create-cal"]'),l=a?new FormData(a):new FormData,o=l.get("holidays")==="on",p=l.get("readOnly")==="on";if(o){b("error","Turn off “Holidays calendar” to import a .ics file into a new calendar."),Me=!0,m();return}if(p){b("error","Turn off “Read-only” before importing — import cannot write to a read-only calendar."),Me=!0,m();return}let n=String(l.get("displayname")??"").trim();n||(n=t.name.replace(/\.ics$/i,"").trim()||"Imported calendar");const r=String(l.get("description")??""),u=String(l.get("color")??"").trim();d=!0,E(),Me=!0,m();try{const y=await A.createCalendar({displayname:n,description:r,color:u,readOnly:!1});F=y.calendar.id,Me=!1,await Xe(),b("success",`Created “${y.calendar.displayname}” — importing…`),await sn(y.calendar.id,t,{keepEditModalOpen:!1,successPrefix:`Calendar “${y.calendar.displayname}” created. `})}catch(y){const v=y instanceof Error?y.message:"Create or import failed";Me=!0,b("error",v),d=!1,m()}}async function sn(e,t,a={}){d=!0,E(),yt(),j={kind:"calendar",fileName:t.name,fileSizeLabel:Js(t.size),phase:"reading",readPercent:0,processPercent:null,processCurrent:0,processTotal:0,processImported:0,processUpdated:0,processSkipped:0,startedAt:Date.now(),elapsedSec:0,resultMessage:null,ok:null},Ys(),m();try{const l=await Xs(t,n=>{if(!j||j.phase!=="reading")return;j={...j,readPercent:n};const r=s.querySelector(".import-progress-bar"),u=s.querySelector("[data-import-status-line]");r&&n!==null&&(r.classList.remove("is-indeterminate"),r.style.width=`${n}%`),u&&n!==null&&(u.textContent=`Reading file… ${n}%`)});Mt("uploading",{readPercent:100}),Mt("processing",{processPercent:0}),N.event("import.calendar.start",{file:t.name,bytes:t.size,calId:e});const o=await A.importCalendar(e,l,n=>{Gs(n)}),p=Ns(o);F===e&&await Ze(),yt(),Mt("done",{ok:!0,resultMessage:`${p} (from “${t.name}”)`}),b("success",`${a.successPrefix||""}Import finished for “${t.name}”: ${p}.`)}catch(l){const o=l instanceof Error?l.message:"Import failed";yt(),Mt("error",{ok:!1,resultMessage:o}),b("error",o)}finally{a.keepEditModalOpen&&(fe=!0),d=!1,m()}}An()}let jt="",P=null,Z=!1,ct=null,Ct=null,Bt="sqlite",cs=!1;async function us(s,c={}){const g={Accept:"application/json",...c.headers};c.body&&(g["Content-Type"]="application/json"),jt&&c.method&&c.method!=="GET"&&(g["X-CSRF-Token"]=jt);const h=await fetch(`/api/install${s}`,{credentials:"same-origin",...c,headers:g});let w;try{w=await h.json()}catch{throw new Error(`Request failed (${h.status})`)}if(!h.ok)throw new Error(w.error||`Request failed (${h.status})`);return w&&typeof w=="object"&&"data"in w&&w.data!==void 0?w.data:w}async function Is(){var s;P=await us("/status"),jt=P.csrfToken||jt,((s=P.defaults)==null?void 0:s.backend)==="pgsql"?Bt="pgsql":Bt="sqlite"}function Fa(s,c,g){return`<label class="check-row"><input type="checkbox" name="${i(s)}" ${c?"checked":""} ${Z?"disabled":""} /> ${i(g)}</label>`}function ol(){const s=P==null?void 0:P.permissions;return`<section class="card">
  ...
  `,pl()}function pl(){var c,g,h,w,I,U;const s=document.getElementById("app");s&&((c=s.querySelector('[data-action="reload"]'))==null||c.addEventListener("click",()=>{fl()}),(g=s.querySelector('[data-action="backend-change"]'))==null||g.addEventListener("change",R=>{Bt=R.target.value==="pgsql"?"pgsql":"sqlite",ut()}),(h=s.querySelector('[data-action="upgrade-toggle"]'))==null||h.addEventListener("change",R=>{cs=!!R.target.checked,ut()}),(w=s.querySelector('[data-action="upgrade-run"]'))==null||w.addEventListener("click",()=>{hl()}),(I=s.querySelector('[data-form="initialize"]'))==null||I.addEventListener("submit",R=>{R.preventDefault(),bl(R.target)}),(U=s.querySelector('[data-form="database"]'))==null||U.addEventListener("submit",R=>{R.preventDefault(),gl(R.target)}))}async function fl(){Z=!0,ct=null,ut();try{await Is(),Ct=null}catch(s){ct=s instanceof Error?s.message:"Failed to load installer status"}finally{Z=!1,ut()}}async function bl(s){const c=new FormData(s),g=w=>{var I;return!!((I=s.querySelector(`input[name="${w}"]`))!=null&&I.checked)},h={timezone:String(c.get("timezone")??"").trim(),cal_enabled:g("cal_enabled"),card_enabled:g("card_enabled"),tasks_enabled:g("tasks_enabled"),notes_enabled:g("notes_enabled"),files_enabled:g("files_enabled"),dav_auth_type:String(c.get("dav_auth_type")??"Digest"),invite_from:String(c.get("invite_from")??"").trim(),session_max_age_minutes:Number(c.get("session_max_age_minutes")??15),admin_password:String(c.get("admin_password")??""),admin_password_confirm:String(c.get("admin_password_confirm")??"")};Z=!0,ct=null,Ct=null,ut();try{P=await us("/initialize",{method:"POST",body:JSON.stringify(h)}),jt=P.csrfToken||jt,Ct="Server settings saved. Configure the database next.",N.event("install.initialize")}catch(w){ct=w instanceof Error?w.message:"Initialize failed"}finally{Z=!1,ut()}}async function gl(s){const c=new FormData(s),g=String(c.get("backend")??Bt),h={backend:g,admin_password:String(c.get("admin_password")??""),admin_password_confirm:String(c.get("admin_password_confirm")??"")};g==="sqlite"?h.sqlite_file=String(c.get("sqlite_file")??"").trim():(h.pgsql_host=String(c.get("pgsql_host")??"").trim(),h.pgsql_dbname=String(c.get("pgsql_dbname")??"").trim(),h.pgsql_username=String(c.get("pgsql_username")??"").trim(),h.pgsql_password=String(c.get("pgsql_password")??"")),Z=!0,ct=null,Ct=null,ut();try{P=await us("/database",{method:"POST",body:JSON.stringify(h)}),jt=P.csrfToken||jt,Ct="Database configured. Installer is locked.",N.event("install.database"),P.completed||P.step}catch(w){ct=w instanceof Error?w.message:"Database setup failed"}finally{Z=!1,ut()}}async function hl(){if(cs){Z=!0,ct=null,Ct=null,ut();try{const s=await us("/upgrade",{method:"POST",body:JSON.stringify({confirm:!0})});Ct="Upgrade completed."+(s.messages&&s.messages.length?" "+s.messages.slice(0,3).join(" · "):""),N.event("install.upgrade"),await Is()}catch(s){ct=s instanceof Error?s.message:"Upgrade failed"}finally{Z=!1,ut()}}}async function yl(s){document.title="AngaraDAV · Setup",document.body.classList.add("layout-install"),s.innerHTML='<section class="card"><p class="muted">Loading installer…</p></section>';try{await Is()}catch(c){ct=c instanceof Error?c.message:"Failed to load installer"}ut()}const Ts=document.getElementById("app");if(!Ts)throw new Error("#app missing");const dn=window.location.pathname.replace(/\/+$/,"")||"/";dn==="/portal/install"||dn.endsWith("/portal/install")?yl(Ts):ll(Ts);
