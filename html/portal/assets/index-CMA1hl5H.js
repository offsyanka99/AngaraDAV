var Va=Object.defineProperty;var Ha=(e,t,a)=>t in e?Va(e,t,{enumerable:!0,configurable:!0,writable:!0,value:a}):e[t]=a;var dt=(e,t,a)=>Ha(e,typeof t!="symbol"?t+"":t,a);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function a(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(n){if(n.ep)return;n.ep=!0;const r=a(n);fetch(n.href,r)}})();const Vt={off:0,error:1,warn:2,info:3,debug:4};let Ee="off";const Ke="[angaradav-portal]";function za(e){const t=(e||"off").toLowerCase().trim();return t==="error"||t==="warn"||t==="info"||t==="debug"||t==="off"?t:"off"}function Ka(e){return Ee=za(e),Ee!=="off"&&console.info(Ke,`log level = ${Ee}`),Ee}function la(e){return Vt[Ee]>=Vt[e]}function Me(e,t,a,s){if(!la(e))return;const n=[Ke,a];s!==void 0&&n.push(s),console[t](...n)}function ja(e,t){la("info")&&(t&&Object.keys(t).length>0?console.info(Ke,`event:${e}`,t):console.info(Ke,`event:${e}`))}const w={error(e,t){Me("error","error",e,t)},warn(e,t){Me("warn","warn",e,t)},info(e,t){Me("info","info",e,t)},debug(e,t){Me("debug","debug",e,t)},event:ja};class M extends Error{constructor(a,s,n={}){super(a);dt(this,"status");dt(this,"payload");this.status=s,this.payload=n}}let le="",Re=null,Be=null;function Ve(e){le=e&&typeof e=="string"?e:""}function Wa(e){Re=e}function Ja(e){Be=e}function Ct(e){if(!da(e))try{Be==null||Be()}catch{}}function da(e){return e==="/login"||e==="/ui"||e==="/logout"||e==="/install/status"||e.startsWith("/install/")}function je(e,t){if(!da(e)){Ve("");try{Re==null||Re(t||"Session timed out. Please sign in again.")}catch{}}}async function E(e,t={}){const a=new Headers(t.headers);t.body&&!a.has("Content-Type")&&a.set("Content-Type","application/json");const s=(t.method||"GET").toUpperCase();s!=="GET"&&s!=="HEAD"&&s!=="OPTIONS"&&le&&a.set("X-CSRF-Token",le);const n=typeof performance<"u"?performance.now():Date.now();w.debug(`api → ${s} ${e}`);const r=await fetch(`/api${e}`,{...t,headers:a,credentials:"same-origin"});let i=null;const l=await r.text();if(l)try{i=JSON.parse(l)}catch{i={error:l}}const d=Math.round((typeof performance<"u"?performance.now():Date.now())-n);if(!r.ok){let c=`Request failed (${r.status})`,f={};if(i&&typeof i=="object"&&i!==null){const m=i;f={...m},typeof m.error=="string"&&(c=m.error)}else(r.status===500||r.status===504)&&(c="Server error during import (often a timeout on large calendars). Try again — already imported events update faster.");throw r.status>=500?w.error(`api ← ${s} ${e} ${r.status} (${d}ms)`,c):r.status!==401?w.warn(`api ← ${s} ${e} ${r.status} (${d}ms)`,c):(w.debug(`api ← ${s} ${e} 401 (${d}ms)`),je(e,c)),new M(c,r.status,f)}return w.info(`api ← ${s} ${e} ${r.status} (${d}ms)`),Ct(e),i}function R(e){return encodeURIComponent(e)}async function Ht(e,t,a,s){const n=new Headers({"Content-Type":a,Accept:"application/x-ndjson, application/json;q=0.9"});le&&n.set("X-CSRF-Token",le);const r=typeof performance<"u"?performance.now():Date.now();w.debug(`api → POST ${e} (stream, ${a}, ${t.length} bytes)`);let i;try{i=await fetch(`/api${e}`,{method:"POST",headers:n,credentials:"same-origin",body:t})}catch(p){const $=p instanceof Error?p.message:"Network error";throw w.error(`api ← POST ${e} network fail`,$),new M(`Import request failed to start (${$}). Check connectivity and container logs.`,0)}const l=(i.headers.get("Content-Type")||"").toLowerCase(),d=l.includes("ndjson")||l.includes("x-ndjson");if(!i.ok&&!d){let p=`Request failed (${i.status})`;try{const $=await i.json();$.error&&(p=$.error)}catch{}throw(i.status===504||i.status===502)&&(p="Gateway timeout during import. Pull the latest image (nginx 900s timeout) and recreate the container. Large calendars can take several minutes."),i.status===401?(w.debug(`api ← POST ${e} 401`,p),je(e,p)):w.warn(`api ← POST ${e} ${i.status}`,p),new M(p,i.status)}if(!d&&i.ok){try{const p=await i.json();if(p&&typeof p.error=="string")throw new M(p.error,i.status||500);if(p&&typeof p.imported=="number"&&typeof p.updated=="number")return w.info(`api ← POST ${e} json done`),p}catch(p){if(p instanceof M)throw p}throw new M("Unexpected import response from server",500)}if(!i.body)throw new M("Import stream unavailable",500);const c=i.body.getReader(),f=new TextDecoder;let m="";const u={final:null,error:null,sawProgress:!1},b=p=>{let $;try{$=JSON.parse(p)}catch{w.debug("import stream non-JSON line",p.slice(0,80));return}if($.type==="progress"){u.sawProgress=!0;const k=Number($.total)||0,h=Number($.current)||0,v=typeof $.percent=="number"?$.percent:k>0?Math.round(100*h/k):0;s==null||s({percent:v,current:h,total:k,imported:Number($.imported)||0,updated:Number($.updated)||0,skipped:Number($.skipped)||0})}else $.type==="done"&&$.result?u.final=$.result:$.type==="error"&&(u.error={message:$.error||"Import failed",status:$.status||500})};for(;;){const{done:p,value:$}=await c.read();if(p)break;m+=f.decode($,{stream:!0});const k=m.split(`
`);m=k.pop()??"";for(const h of k){const v=h.trim();v&&b(v)}}m.trim()&&b(m.trim());const y=Math.round((typeof performance<"u"?performance.now():Date.now())-r);if(u.error)throw u.error.status===401?(w.debug(`api ← POST ${e} stream 401 (${y}ms)`,u.error.message),je(e,u.error.message)):w.warn(`api ← POST ${e} stream error (${y}ms)`,u.error.message),new M(u.error.message,u.error.status);if(!u.final)throw w.error(`api ← POST ${e} stream incomplete (${y}ms)`,{sawProgress:u.sawProgress}),new M(u.sawProgress?"Import stopped before finishing (server crash, out of memory, or gateway timeout). On TrueNAS, set memory limit to at least 1G, pull latest image, and recreate the app.":"Import failed to start on the server. Check container logs and that you are on the latest image.",500);return w.info(`api ← POST ${e} stream done (${y}ms)`),Ct(e),u.final}const S={ui:()=>E("/ui"),installStatus:async()=>{const e=await E("/install/status");return e&&typeof e=="object"&&"data"in e&&e.data?e.data:e},adminPing:()=>E("/admin/ping"),adminDashboard:()=>E("/admin/dashboard"),adminCapabilities:()=>E("/admin/capabilities"),adminUsers:()=>E("/admin/users"),adminUser:e=>E(`/admin/users/${encodeURIComponent(e)}`),adminCreateUser:e=>E("/admin/users",{method:"POST",body:JSON.stringify(e)}),adminUpdateUser:(e,t)=>E(`/admin/users/${encodeURIComponent(e)}`,{method:"PATCH",body:JSON.stringify(t)}),adminDeleteUser:(e,t=!0)=>E(`/admin/users/${encodeURIComponent(e)}`,{method:"DELETE",body:JSON.stringify({confirm:t})}),adminUserCalendars:e=>E(`/admin/users/${encodeURIComponent(e)}/calendars`),adminCreateUserCalendar:(e,t)=>E(`/admin/users/${encodeURIComponent(e)}/calendars`,{method:"POST",body:JSON.stringify(t)}),adminUpdateUserCalendar:(e,t,a)=>E(`/admin/users/${encodeURIComponent(e)}/calendars/${t}`,{method:"PATCH",body:JSON.stringify(a)}),adminDeleteUserCalendar:(e,t,a=!0)=>E(`/admin/users/${encodeURIComponent(e)}/calendars/${t}`,{method:"DELETE",body:JSON.stringify({confirm:a})}),adminUserAddressBooks:e=>E(`/admin/users/${encodeURIComponent(e)}/addressbooks`),adminCreateUserAddressBook:(e,t)=>E(`/admin/users/${encodeURIComponent(e)}/addressbooks`,{method:"POST",body:JSON.stringify(t)}),adminUpdateUserAddressBook:(e,t,a)=>E(`/admin/users/${encodeURIComponent(e)}/addressbooks/${t}`,{method:"PATCH",body:JSON.stringify(a)}),adminDeleteUserAddressBook:(e,t,a=!0,s=!1)=>E(`/admin/users/${encodeURIComponent(e)}/addressbooks/${t}`,{method:"DELETE",body:JSON.stringify({confirm:a,force:s})}),adminSystemSettings:()=>E("/admin/settings/system"),adminUpdateSystemSettings:e=>E("/admin/settings/system",{method:"PATCH",body:JSON.stringify(e)}),adminResetToDefault:(e=!0,t="")=>E("/admin/settings/reset-to-default",{method:"POST",body:JSON.stringify({confirm:e,password:t})}),adminDatabaseSettings:()=>E("/admin/settings/database"),adminTestDatabaseConnection:e=>E("/admin/settings/database/test",{method:"POST",body:JSON.stringify(e)}),adminUpdateDatabaseSettings:e=>E("/admin/settings/database",{method:"PATCH",body:JSON.stringify(e)}),me:async()=>{var t;const e=await E("/me");return Ve(e.csrfToken||((t=e.user)==null?void 0:t.csrfToken)||""),e},login:async(e,t)=>{var s;const a=await E("/login",{method:"POST",body:JSON.stringify({username:e,password:t})});return Ve((s=a.user)==null?void 0:s.csrfToken),a},logout:async()=>{try{return await E("/logout",{method:"POST"})}finally{Ve("")}},calendars:()=>E("/calendars"),createCalendar:e=>E("/calendars",{method:"POST",body:JSON.stringify(e)}),holidayCountries:()=>E("/holidays/countries"),updateCalendar:(e,t)=>E(`/calendars/${e}`,{method:"PATCH",body:JSON.stringify(t)}),deleteCalendar:e=>E(`/calendars/${e}`,{method:"DELETE"}),calendarEvents:(e,t,a)=>{const s=new URLSearchParams({from:t,to:a}).toString();return E(`/calendars/${e}/events?${s}`)},getEvent:(e,t)=>E(`/calendars/${e}/events/${R(t)}`),createEvent:(e,t)=>E(`/calendars/${e}/events`,{method:"POST",body:JSON.stringify(t)}),updateEvent:(e,t,a)=>E(`/calendars/${e}/events/${R(t)}`,{method:"PATCH",body:JSON.stringify(a)}),deleteEvent:(e,t)=>E(`/calendars/${e}/events/${R(t)}`,{method:"DELETE"}),exportCalendar:async e=>{const t=await fetch(`/api/calendars/${e}/export`,{credentials:"same-origin"});if(!t.ok){let i=`Export failed (${t.status})`;try{const l=await t.json();l.error&&(i=l.error)}catch{}throw new M(i,t.status)}const a=t.headers.get("Content-Disposition")||"",s=/filename="([^"]+)"/i.exec(a),n=(s==null?void 0:s[1])||`calendar-${e}.ics`;return{blob:await t.blob(),filename:n}},importCalendar:(e,t,a)=>Ht(`/calendars/${e}/import`,t,"text/calendar; charset=utf-8",a),directory:()=>E("/directory"),shares:e=>E(`/calendars/${e}/shares`),share:(e,t,a)=>E(`/calendars/${e}/shares`,{method:"POST",body:JSON.stringify({username:t,access:a})}),revoke:(e,t)=>E(`/calendars/${e}/shares`,{method:"DELETE",body:JSON.stringify({href:t})}),addressbooks:()=>E("/addressbooks"),createAddressBook:e=>E("/addressbooks",{method:"POST",body:JSON.stringify(e)}),updateAddressBook:(e,t)=>E(`/addressbooks/${e}`,{method:"PATCH",body:JSON.stringify(t)}),deleteAddressBook:(e,t=!1)=>E(`/addressbooks/${e}`,{method:"DELETE",body:JSON.stringify({force:t})}),exportAddressBook:async e=>{const t=await fetch(`/api/addressbooks/${e}/export`,{credentials:"same-origin"});if(!t.ok){let i=`Export failed (${t.status})`;try{const l=await t.json();l.error&&(i=l.error)}catch{}throw new M(i,t.status)}const a=t.headers.get("Content-Disposition")||"",s=/filename="([^"]+)"/i.exec(a),n=(s==null?void 0:s[1])||`contacts-${e}.vcf`;return{blob:await t.blob(),filename:n}},importAddressBook:(e,t,a)=>Ht(`/addressbooks/${e}/import`,t,"text/vcard; charset=utf-8",a),contacts:(e,t="")=>{const a=t.trim()?`?q=${encodeURIComponent(t.trim())}`:"";return E(`/addressbooks/${e}/contacts${a}`)},getContact:(e,t)=>E(`/addressbooks/${e}/contacts/${R(t)}`),createContact:(e,t)=>E(`/addressbooks/${e}/contacts`,{method:"POST",body:JSON.stringify(t)}),updateContact:(e,t,a)=>E(`/addressbooks/${e}/contacts/${R(t)}`,{method:"PATCH",body:JSON.stringify(a)}),deleteContact:(e,t)=>E(`/addressbooks/${e}/contacts/${R(t)}`,{method:"DELETE"}),exportContact:async(e,t)=>{const a=await fetch(`/api/addressbooks/${e}/contacts/${R(t)}/export`,{credentials:"same-origin"});if(!a.ok){let l=`Export failed (${a.status})`;try{const d=await a.json();d.error&&(l=d.error)}catch{}throw new M(l,a.status)}const s=a.headers.get("Content-Disposition")||"",n=/filename="([^"]+)"/i.exec(s),r=(n==null?void 0:n[1])||"contact.vcf";return{blob:await a.blob(),filename:r}},contactPhotoUrl:(e,t)=>`/api/addressbooks/${e}/contacts/${R(t)}/photo`,tasks:(e={})=>{const t=new URLSearchParams;e.q&&t.set("q",e.q),e.sort&&t.set("sort",e.sort),e.order&&t.set("order",e.order);const a=t.toString()?`?${t}`:"";return E(`/tasks${a}`)},createTask:e=>E("/tasks",{method:"POST",body:JSON.stringify(e)}),updateTask:(e,t,a)=>E(`/tasks/${e}/${R(t)}`,{method:"PATCH",body:JSON.stringify(a)}),deleteTask:(e,t)=>E(`/tasks/${e}/${R(t)}`,{method:"DELETE"}),bulkTasks:e=>E("/tasks/bulk",{method:"POST",body:JSON.stringify(e)}),notes:(e={})=>{const t=new URLSearchParams;e.q&&t.set("q",e.q),e.sort&&t.set("sort",e.sort),e.order&&t.set("order",e.order);const a=t.toString()?`?${t}`:"";return E(`/notes${a}`)},createNote:e=>E("/notes",{method:"POST",body:JSON.stringify(e)}),updateNote:(e,t,a)=>E(`/notes/${e}/${R(t)}`,{method:"PATCH",body:JSON.stringify(a)}),deleteNote:(e,t)=>E(`/notes/${e}/${R(t)}`,{method:"DELETE"}),filesStatus:()=>E("/files"),filesList:(e="")=>{const t=new URLSearchParams;e&&t.set("path",e);const a=t.toString()?`?${t}`:"";return E(`/files/entries${a}`)},filesMkdir:(e,t)=>E("/files/mkdir",{method:"POST",body:JSON.stringify({path:e,name:t})}),filesUpload:(e,t,a={})=>{const s=new URLSearchParams;e&&s.set("path",e),s.set("name",t.name),a.replace&&s.set("replace","1");const n=new FormData;n.append("file",t,t.name),e&&n.append("path",e);const r=typeof performance<"u"?performance.now():Date.now();return w.debug(`api → POST /files/upload path=${e||"/"} name=${t.name} size=${t.size}`),new Promise((i,l)=>{const d=new XMLHttpRequest;d.open("POST",`/api/files/upload?${s}`),d.withCredentials=!0,le&&d.setRequestHeader("X-CSRF-Token",le),a.onProgress&&(d.upload.onprogress=c=>{var f,m;c.lengthComputable?(f=a.onProgress)==null||f.call(a,c.loaded,c.total):(m=a.onProgress)==null||m.call(a,c.loaded,t.size||c.loaded)}),d.onload=()=>{const c=Math.round((typeof performance<"u"?performance.now():Date.now())-r);let f=null;const m=d.responseText||"";if(m)try{f=JSON.parse(m)}catch{f={error:m}}const u=d.status;if(u<200||u>=300){let b=`Upload failed (${u||0})`;f&&typeof f=="object"&&f!==null&&"error"in f&&typeof f.error=="string"&&(b=f.error),u===401?(w.debug(`api ← POST /files/upload 401 (${c}ms)`,b),je("/files/upload",b)):u>=500?w.error(`api ← POST /files/upload ${u} (${c}ms)`,b):w.warn(`api ← POST /files/upload ${u} (${c}ms)`,b),l(new M(b,u||0));return}w.info(`api ← POST /files/upload 200 (${c}ms)`),Ct("/files/upload"),i(f)},d.onerror=()=>{const c=Math.round((typeof performance<"u"?performance.now():Date.now())-r);w.error(`api ← POST /files/upload network error (${c}ms)`),l(new M("Upload failed (network error)",0))},d.onabort=()=>{l(new M("Upload cancelled",0))},d.send(n)})},filesDownloadUrl:e=>{const t=new URLSearchParams;return t.set("path",e),`/api/files/download?${t}`},filesDelete:e=>E("/files/entry",{method:"DELETE",body:JSON.stringify({path:e})}),filesRename:(e,t)=>E("/files/rename",{method:"POST",body:JSON.stringify({path:e,newName:t})}),filesMove:(e,t,a)=>E("/files/move",{method:"POST",body:JSON.stringify({from:e,to:t,newName:a})}),filesCopy:(e,t={})=>E("/files/copy",{method:"POST",body:JSON.stringify({path:e,to:t.to,newName:t.newName})}),filesBulk:(e,t)=>E("/files/bulk",{method:"POST",body:JSON.stringify({op:e,paths:t})})},oa="angaradav-portal-tab",ca="angaradav-portal-admin-page",Ya="2.2.1",Ga="https://github.com/offsyanka99/AngaraDAV/tree/main/docs";function Qa(e){const t=new Date;return{user:null,flash:null,activeTab:e.activeTab,adminPage:e.adminPage,adminDashboard:null,adminDashboardLoading:!1,adminDashboardError:null,adminCapabilities:null,adminCapabilitiesError:null,adminUsers:[],adminUsersLoading:!1,adminUsersError:null,adminUsersQuery:"",adminSelectedUsername:e.adminSelectedUsername,adminUserDetail:null,adminUserDetailLoading:!1,adminUserDetailError:null,adminUserCreateOpen:!1,adminUserEditOpen:!1,adminUserDeleteUsername:null,adminUserDeleteConfirmChecked:!1,adminUserCalendars:[],adminUserAddressBooks:[],adminUserResourcesLoading:!1,adminCalModal:null,adminCalEditId:null,adminAbModal:null,adminAbEditId:null,adminResourceDelete:null,adminSystemSettings:null,adminSystemSettingsLoading:!1,adminSystemSettingsError:null,adminResetModalOpen:!1,adminResetConfirmChecked:!1,adminResetPassword:"",adminDatabaseSettings:null,adminDatabaseSettingsLoading:!1,adminDatabaseSettingsError:null,adminDbFormBackend:"sqlite",adminDbConfirmOpen:!1,adminDbConfirmText:"",adminDbPendingBody:null,userMenuOpen:!1,userMenuDocClick:null,calendars:[],directory:[],holidayCountries:[],selectedId:null,selectedIds:[],shares:[],installGate:null,calModalOpen:!1,createCalModalOpen:!1,deleteConfirmId:null,deleteAbConfirmId:null,monthCursor:{y:t.getFullYear(),m:t.getMonth()},monthEvents:[],monthEventsLoading:!1,eventModalOpen:!1,editingEvent:null,creatingEvent:!1,eventDtPicker:null,bulkDueValue:"",monthExpandDay:null,addressBooks:[],selectedAbId:null,contacts:[],contactSearch:"",selectedContactUri:null,editingContact:null,creatingContact:!1,contactModalOpen:!1,abModalOpen:!1,photoPreview:null,photoBase64Pending:null,removePhotoPending:!1,busy:!1,importProgress:null,importElapsedTimer:null,filesUploadProgress:null,filesUploadElapsedTimer:null,filesUploadMenuOpen:!1,filesUploadMenuDocClick:null,filesUploadDropActive:!1,escapeBound:!1,portalEventsBound:!1,portalUi:{timeFormat:"auto",weekStart:"auto",logLevel:"off"},searchTimer:null,sessionIdleSeconds:900,sessionIdleTimer:null,appVersion:Ya,handlingSessionExpiry:!1,suppressErrorFlashAfterExpiry:!1,tasks:[],notes:[],taskCalendars:[],noteCalendars:[],taskSearch:"",noteSearch:"",taskSort:"due",taskOrder:"asc",noteSort:"dtstart",noteOrder:"desc",selectedTaskKey:null,selectedNoteKey:null,editingTask:null,editingNote:null,creatingTask:!1,creatingNote:!1,checkedTaskKeys:[],filesStatus:null,filesPath:"",filesEntries:[],filesLoading:!1,filesRenamePath:null,filesDeletePaths:null,filesTransfer:null,filesTransferDest:"",filesTreeChildren:{},filesTreeExpanded:[],filesMkdirOpen:!1,checkedFilePaths:[],filesUploadConflict:null,confirmDelete:null,dtPickerDocClick:null}}function o(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function W(e,t,a={}){if(!t)return"";const s=a.dismissible!==void 0?a.dismissible:a.dismissAction!==void 0,n=a.dismissAction??"flash-close",r=a.role??"status",i=a.className?` ${a.className}`:"",l=a.style?` style="${o(a.style)}"`:"",d=s?`<button type="button" class="flash-close" data-action="${o(n)}" aria-label="Dismiss message" title="Dismiss">×</button>`:"";return`<div class="flash flash-${o(e)}${i}" role="${o(r)}"${l}>
      <span class="flash-text">${o(t)}</span>
      ${d}
    </div>`}function Xa(e){return e==="sm"?" cal-modal-card-sm":e==="wide"?" cal-modal-card-wide":""}function Za(e){return e==="danger"?"btn btn-danger":e==="ghost"?"btn btn-ghost":"btn btn-primary"}function Et(e){return e.map(a=>{const s=a.type??"button",n=Za(a.variant),r=a.disabled?" disabled":"",i=a.id?` id="${o(a.id)}"`:"",l=a.action?` data-action="${o(a.action)}"`:"",d=a.attrs?` ${a.attrs}`:"";return`<button type="${s}" class="${n}"${l}${i}${d}${r}>${o(a.label)}</button>`}).join(`
`)}function N(e){const t=e.titleId||(e.id?`${e.id}-title`:`modal-title-${Math.random().toString(36).slice(2,9)}`),a=e.id?` id="${o(e.id)}"`:"",s=e.className?` ${e.className}`:"",n=e.rootAttrs?` ${e.rootAttrs}`:"",r=`${Xa(e.size)}${e.cardClassName?` ${e.cardClassName}`:""}`,i=e.closeAction,l=e.lockBackdrop?"":` data-action="${o(i)}"`,d=e.hideClose?"":`<button type="button" class="modal-close info-modal-close" data-action="${o(i)}" aria-label="Close">×</button>`;let c="";e.footer!==void 0&&(c=typeof e.footer=="string"?e.footer:Et(e.footer));const f=c?`<footer class="cal-modal-footer">${c}</footer>`:"",m=`<div class="cal-modal-body">${e.body}</div>`;let u;return e.form?u=`<form class="stack"${e.formAttrs?` ${e.formAttrs}`:""}>
        ${m}
        ${f}
      </form>`:u=`${m}
    ...
    <div class="info-modal" id="info-modal" hidden role="dialog" aria-modal="true" aria-labelledby="info-modal-title">
    ...
    </div>`}function ot(e,t,a={},s){const n=!!e.user&&e.activeTab==="admin"&&de(e)&&Fe(e),l=`
      <img class="brand-logo" src="/logo.png" width="28" height="28" alt="" aria-hidden="true" />
      <span class="brand-text">Angara<span class="brand-dav">DAV</span><span class="brand-sep" aria-hidden="true"> · </span><span class="${n?"brand-portal brand-portal-admin":"brand-portal brand-portal-user"}">${o(n?"Administration Portal":"User Portal")}</span></span>`,d=e.user?o(e.user.displayname||e.user.username):"",c=Fe(e)?`<button type="button" class="user-menu-item${e.activeTab==="admin"?" is-active":""}" role="menuitem" data-action="tab" data-tab="admin">
              Administration
      ...
      <footer class="site-footer">
        <div class="container footer-inner">
          ...
          <span class="footer-sep" aria-hidden="true">·</span>
          <a href="${o(Ga)}" target="_blank" rel="noopener noreferrer">Docs</a>
        </div>
      ...
      ${s.renderFilesUploadProgressModal()}`}function $t(e){e.userMenuDocClick&&(document.removeEventListener("click",e.userMenuDocClick,!0),e.userMenuDocClick=null)}function un(e,t){$t(e),e.userMenuDocClick=s=>{var r;const n=s.target;(r=n==null?void 0:n.closest)!=null&&r.call(n,".user-menu")||(e.userMenuOpen=!1,$t(e),t())};const a=e.userMenuDocClick;setTimeout(()=>{e.userMenuOpen&&e.userMenuDocClick===a&&document.addEventListener("click",a,!0)},0)}function We(e){e.dtPickerDocClick&&(document.removeEventListener("click",e.dtPickerDocClick,!0),e.dtPickerDocClick=null)}function mn(e,t){if(We(e),!e.eventDtPicker)return;e.dtPickerDocClick=s=>{var r,i;const n=s.target;(r=n==null?void 0:n.closest)!=null&&r.call(n,".dt-field.is-open, .dt-popover, [data-dt-popover]")||(i=n==null?void 0:n.closest)!=null&&i.call(n,'[data-action="dt-open"]')||(e.eventDtPicker=null,We(e),t())};const a=e.dtPickerDocClick;setTimeout(()=>{e.eventDtPicker&&e.dtPickerDocClick===a&&document.addEventListener("click",a,!0)},0)}async function Z(e){e.state.filesLoading=!0;try{w.debug("loadFiles",{path:e.state.filesPath});const[t,a]=await Promise.all([S.filesStatus(),S.filesList(e.state.filesPath).catch(s=>{if(s instanceof M&&(s.status===503||s.status===404))return{path:e.state.filesPath,entries:[]};throw s})]);if(e.state.filesStatus=t,t.ready){e.state.filesPath=a.path,e.state.filesEntries=a.entries;const s=new Set(e.state.filesEntries.map(n=>n.path));e.state.checkedFilePaths=e.state.checkedFilePaths.filter(n=>s.has(n))}else e.state.filesEntries=[],e.state.checkedFilePaths=[];w.event("loadFiles",{path:e.state.filesPath,count:e.state.filesEntries.length,enabled:t.enabled,ready:t.ready})}finally{e.state.filesLoading=!1}}function pa(...e){return e.map(t=>t.replace(/\\/g,"/").replace(/^\/+|\/+$/g,"")).filter(Boolean).join("/")}function Ue(e){const t=e.replace(/\\/g,"/").split("/").filter(Boolean);return t[t.length-1]||e}function tt(e,t,a){for(const s of a)if(s&&(t===s||t.startsWith(`${s}/`)))return!0;return!1}function z(e){e.state.filesTransfer=null,e.state.filesTransferDest="",e.state.filesTreeChildren={},e.state.filesTreeExpanded=[]}async function xe(e,t,a){if(a.length===0)return;e.state.filesTransfer={op:t,paths:[...a]},e.state.filesTransferDest=e.state.filesPath,e.state.filesTreeChildren={};const s=new Set([""]);if(e.state.filesPath){const n=e.state.filesPath.split("/").filter(Boolean);let r="";for(const i of n)r=r?`${r}/${i}`:i,s.add(r)}e.state.filesTreeExpanded=[...s],e.state.filesRenamePath=null,e.state.filesDeletePaths=null,e.state.filesMkdirOpen=!1,e.state.filesUploadMenuOpen=!1,e.state.filesUploadMenuDocClick&&(document.removeEventListener("click",e.state.filesUploadMenuDocClick,!0),e.state.filesUploadMenuDocClick=null),e.clearFlash(),e.render(),await Promise.all([...s].map(n=>kt(e,n)))}async function kt(e,t){const a=e.state.filesTreeChildren[t];if(!(a&&a!=="error")){e.state.filesTreeChildren={...e.state.filesTreeChildren,[t]:"loading"},e.render();try{const n=(await S.filesList(t)).entries.filter(r=>r.type==="dir").slice().sort((r,i)=>r.name.localeCompare(i.name,void 0,{sensitivity:"base"}));if(!e.state.filesTransfer)return;e.state.filesTreeChildren={...e.state.filesTreeChildren,[t]:n}}catch(s){if(!e.state.filesTransfer)return;e.state.filesTreeChildren={...e.state.filesTreeChildren,[t]:"error"},w.warn("files.tree",{path:t||"/",error:s instanceof Error?s.message:String(s)})}e.render()}}function pn(e){if(!e.state.filesTransfer)return"";const t=e.state.filesTransfer.paths,a=[],s=(n,r)=>{const i=e.state.filesTransferDest===n,l=tt(e,n,t),d=e.state.filesTreeExpanded.includes(n),c=e.state.filesTreeChildren[n],f=Array.isArray(c),m=n===""||c==="loading"||c==="error"||!f||c.length>0,u=n===""?"Home":Ue(n),b=l?"Cannot use a selected item (or a folder inside it) as the destination":n===""?"File home host.root":n,y=d?"▾":"▸";if(a.push(`<div class="files-tree-row${i?" is-selected":""}${l?" is-blocked":""}" style="--depth:${r}" role="treeitem" aria-selected="${i}" aria-expanded="${d}" aria-disabled="${l}">
      ...
      <button type="button" class="files-tree-select${i?" is-selected":""}" data-action="files-tree-select" data-path="${o(n)}"
        ...
        </div>`);return}if(f){for(const p of c)s(p.path,r+1);c.length===0&&n===""&&a.push(`<div class="files-tree-status muted small" style="--depth:${r+1}">No subfolders yet — destination will be Home.</div>`)}}};return s("",0),`<div class="files-folder-tree" role="tree" aria-label="Destination folder">${a.join("")}</div>`}async function fn(e,t){if(!e.state.filesTransfer||e.state.filesTransfer.paths.length===0)return;const a=new FormData(t),s=(e.state.filesTransferDest||String(a.get("toPath")??"")).trim().replace(/^\/+|\/+$/g,""),n=String(a.get("newName")??"").trim(),r=e.state.filesTransfer.op,i=[...e.state.filesTransfer.paths],l=i.length>1;if(tt(e,s,i)){e.setFlash("error","Choose a different destination folder"),e.render();return}e.state.busy=!0,e.clearFlash(),e.render();let d=0;const c=[];try{for(const m of i)try{if(r==="copy"){const u=Ue(m),b=l||!n||n===u?void 0:n,y=await S.filesCopy(m,{to:s,newName:b});w.event("files.copy",{path:m,to:y.entry.path})}else{const u=Ue(m),b=l||!n||n===u?void 0:n;await S.filesMove(m,s,b),w.event("files.move",{path:m,to:s})}d+=1}catch(u){c.push(`${Ue(m)}: ${u instanceof Error?u.message:"failed"}`)}z(e),e.state.checkedFilePaths=[],await Z(e);const f=r==="copy"?"Copied":"Moved";d>0&&c.length===0?e.setFlash("success",d===1?`${f} 1 item`:`${f} ${d} items`):d>0?e.setFlash("info",`${f} ${d}; ${c.length} failed. ${c[0]}`):e.setFlash("error",c[0]||`${r==="copy"?"Copy":"Move"} failed`)}catch(f){e.setFlash("error",f instanceof Error?f.message:"Operation failed")}finally{e.state.busy=!1,e.render()}}function wt(...e){return e.map(t=>t.replace(/\\/g,"/").replace(/^\/+|\/+$/g,"")).filter(Boolean).join("/")}function fa(e){if(!e||typeof e!="object")return!1;const t=e.name;return t==="AbortError"||t==="NotAllowedError"}function ba(e,t=!0){return Array.from(e).map(s=>{const r=(s.webkitRelativePath||"").replace(/\\/g,"/").replace(/^\/+/,"")||s.name;return{file:s,relativePath:r||s.name}})}function bn(e){return new Promise((t,a)=>{const s=[],n=()=>{e.readEntries(r=>{if(!r.length){t(s);return}s.push(...r),n()},r=>a(r))};n()})}function gn(e){return new Promise((t,a)=>{e.file(t,a)})}async function ga(e,t){const a=wt(t,e.name);if(e.isFile)return[{file:await gn(e),relativePath:a||e.name}];if(e.isDirectory){const s=e.createReader(),n=await bn(s);if(n.length===0)return[{file:null,relativePath:a,isEmptyDir:!0}];const r=[];for(const i of n)r.push(...await ga(i,a));return r}return[]}async function*yn(e){const t=e;if(typeof t.values=="function"){for await(const a of t.values())yield a;return}if(typeof t.entries=="function")for await(const[,a]of t.entries())yield a}async function Tt(e,t){const a=wt(t,e.name),s=[];let n=0;for await(const r of yn(e))if(n+=1,r.kind==="file"){const i=await r.getFile();s.push({file:i,relativePath:wt(a,r.name)||i.name})}else r.kind==="directory"&&s.push(...await Tt(r,a));return n===0&&s.push({file:null,relativePath:a,isEmptyDir:!0}),s}async function vn(){const e=window;if(typeof e.showOpenFilePicker!="function")return{kind:"fallback"};try{const t=await e.showOpenFilePicker({multiple:!0});if(!t||t.length===0)return{kind:"cancel"};const a=[];for(const s of t){const n=await s.getFile();a.push({file:n,relativePath:n.name})}return{kind:"items",items:a}}catch(t){return fa(t)?{kind:"cancel"}:{kind:"fallback"}}}async function $n(){const e=window;if(typeof e.showDirectoryPicker!="function")return{kind:"fallback"};try{const t=await e.showDirectoryPicker({mode:"read"}),a=await Tt(t,"");return a.length===0?{kind:"cancel"}:{kind:"items",items:a}}catch(t){return fa(t)?{kind:"cancel"}:{kind:"fallback"}}}function ct(e){return e.replace(/\\/g,"/").replace(/^\/+/,"").replace(/\/+$/,"")}async function kn(e){var r,i;const t=e.items?Array.from(e.items):[],a=[];for(const l of t){if(l.kind!=="file")continue;const d=l;if(typeof d.getAsFileSystemHandle=="function")try{const c=await d.getAsFileSystemHandle();if(c){if(c.kind==="file"){const f=await c.getFile();a.push({file:f,relativePath:f.name})}else c.kind==="directory"&&a.push(...await Tt(c,""));continue}}catch{}if(typeof d.webkitGetAsEntry=="function"){const c=d.webkitGetAsEntry();if(c){a.push(...await ga(c,""));continue}}}const s=e.files&&e.files.length>0?ba(e.files,!0):[],n=new Map;for(const l of s){const d=ct(l.relativePath||((r=l.file)==null?void 0:r.name)||"");d&&n.set(d,l)}for(const l of a){const d=ct(l.relativePath||((i=l.file)==null?void 0:i.name)||"");if(!d&&l.isEmptyDir){const c=ct(l.relativePath);c&&n.set(c,l);continue}d&&n.set(d,l)}return Array.from(n.values())}function Le(e){if(!e)return!1;if(e.types&&typeof e.types.includes=="function")return e.types.includes("Files");try{for(let t=0;t<e.types.length;t++)if(e.types[t]==="Files")return!0}catch{}return!1}function ya(e,t=80){const a=String(e??"").replace(/\s+/g," ").trim();return a?a.length>t?`${a.slice(0,t-1)}…`:a:""}function te(e,t,a){const s=ya(t);return s?`${e} “${s}” ${a}`:`${e} ${a}`}function ut(e){const t=ya(e.displayname||e.fullname);return t||[e.firstname,e.lastname].map(s=>String(s??"").trim()).filter(Boolean).join(" ")||"Unnamed contact"}function va(e){if(!e)return"—";try{const t=new Date(e);return Number.isNaN(t.getTime())?e:t.toLocaleString(void 0,{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return e}}function Je(e){return!Number.isFinite(e)||e<0?"":e<1024?`${e} B`:e<1024*1024?`${(e/1024).toFixed(1)} KB`:`${(e/(1024*1024)).toFixed(1)} MB`}function H(e){const t=Math.max(0,Math.floor(e)),a=Math.floor(t/60),s=t%60;return a>0?`${a}m ${s}s`:`${s}s`}function fe(e){return!Number.isFinite(e)||e<0?"—":e<1024?`${e} B`:e<1024*1024?`${(e/1024).toFixed(1)} KB`:e<1024*1024*1024?`${(e/(1024*1024)).toFixed(1)} MB`:`${(e/(1024*1024*1024)).toFixed(2)} GB`}function wn(e){if(!e)return"—";try{return new Date(e*1e3).toLocaleString()}catch{return"—"}}function G(e,t,a,s,n,r=""){const i=a===t,l=i?s==="asc"?" ▲":" ▼":"";return`<th class="${`sortable-th${i?" is-sorted":""}${r?" "+r:""}`}" data-action="sort-${n}" data-sort="${o(t)}" role="columnheader" tabindex="0">${o(e)}${l}</th>`}function j(e){e.state.filesUploadMenuDocClick&&(document.removeEventListener("click",e.state.filesUploadMenuDocClick,!0),e.state.filesUploadMenuDocClick=null)}function hn(e){j(e),e.state.filesUploadMenuDocClick=a=>{var n;const s=a.target;(n=s==null?void 0:s.closest)!=null&&n.call(s,".files-upload-menu")||(e.state.filesUploadMenuOpen=!1,j(e),e.render())};const t=e.state.filesUploadMenuDocClick;setTimeout(()=>{e.state.filesUploadMenuOpen&&e.state.filesUploadMenuDocClick===t&&document.addEventListener("click",t,!0)},0)}function $e(e){e.state.filesUploadElapsedTimer!==null&&(clearInterval(e.state.filesUploadElapsedTimer),e.state.filesUploadElapsedTimer=null)}function Sn(e){$e(e),e.state.filesUploadElapsedTimer=setInterval(()=>{if(!e.state.filesUploadProgress||e.state.filesUploadProgress.phase==="done"||e.state.filesUploadProgress.phase==="error"){$e(e);return}e.state.filesUploadProgress={...e.state.filesUploadProgress,elapsedSec:Math.floor((Date.now()-e.state.filesUploadProgress.startedAt)/1e3)},ge(e,e.state.filesUploadProgress)},1e3)}function $a(e){$e(e),e.state.filesUploadProgress=null,e.render()}function ka(e,t){return t.bytesTotal>0?Math.min(100,Math.max(0,Math.round(100*t.bytesSent/t.bytesTotal))):t.totalFiles>0?Math.min(100,Math.max(0,Math.round(100*t.completedFiles/t.totalFiles))):null}function ge(e,t){if(!e.root.querySelector("[data-files-upload-progress]"))return;const a=e.root.querySelector(".files-upload-progress-bar"),s=e.root.querySelector(".files-upload-progress-track"),n=e.root.querySelector("[data-files-upload-status]"),r=e.root.querySelector("[data-files-upload-current]"),i=ka(e,t),l=t.phase==="uploading"?`Uploading ${t.completedFiles.toLocaleString()} / ${t.totalFiles.toLocaleString()} file${t.totalFiles===1?"":"s"}${t.failedFiles?` · ${t.failedFiles} failed`:""}${i!==null?` (${i}%)`:""} · ${H(t.elapsedSec)}`:(n==null?void 0:n.textContent)||"";n&&t.phase==="uploading"&&(n.textContent=l),r&&t.phase==="uploading"&&(r.textContent=t.currentName||"",r.title=t.currentName||""),a&&i!==null&&(a.classList.remove("is-indeterminate"),a.style.width=`${i}%`),s&&i!==null&&(s.setAttribute("aria-valuenow",String(i)),s.removeAttribute("aria-valuetext"))}function _e(e){if(!e.state.filesUploadProgress)return"";const t=e.state.filesUploadProgress,a=t.phase==="uploading",s=t.phase==="done"?"Upload finished":t.phase==="error"?"Upload failed":"Uploading…",n=ka(e,t),r=n===null?"files-upload-progress-bar is-indeterminate":"files-upload-progress-bar",i=n!==null?` style="width:${n}%"`:"";let l="";if(a){const c=`Uploading ${t.completedFiles.toLocaleString()} / ${t.totalFiles.toLocaleString()} file${t.totalFiles===1?"":"s"}${t.failedFiles?` · ${t.failedFiles} failed`:""}${n!==null?` (${n}%)`:""} · ${H(t.elapsedSec)}`,f=t.bytesTotal>0?`${Je(t.bytesSent)} / ${Je(t.bytesTotal)}`:"";l=`
      ...
      ${c}
      <p class="muted small" style="margin:0.75rem 0 0">After ${o(H(t.elapsedSec))}</p>`}const d=a?'<p class="muted small" style="margin:0">Please wait…</p>':Et([{label:"Close",action:"close-files-upload-progress",variant:"primary"}]);return N({title:s,titleId:"files-upload-progress-title",closeAction:"close-files-upload-progress",size:"sm",className:"import-progress-modal files-upload-progress-modal",cardClassName:"import-progress-card",rootAttrs:"data-files-upload-progress",hideClose:a,lockBackdrop:a,body:l,footer:d})}async function Jt(e,t,a,s){const n=a.replace(/\\/g,"/").split("/").map(i=>i.trim()).filter(Boolean);let r=t;for(const i of n){const l=pa(r,i);if(s.has(l)){r=l;continue}try{await S.filesMkdir(r,i),w.event("files.mkdir",{path:r,name:i,via:"upload-folder"})}catch(d){if(!(d instanceof M&&d.status===409))throw d}s.add(l),r=l}}function Dn(e,t){var s;const a=t==="files"?'input[type="file"][data-action="files-upload-pick-files"]':'input[type="file"][data-action="files-upload-pick-folder"]';(s=e.root.querySelector(a))==null||s.click()}async function Yt(e,t){if(e.state.busy||e.state.filesUploadProgress)return;e.state.filesUploadMenuOpen=!1,j(e),e.state.filesRenamePath=null,e.state.filesDeletePaths=null,z(e),e.state.filesMkdirOpen=!1;const a=t==="files"?vn:$n;try{const s=await a();if(s.kind==="cancel"){e.render();return}if(s.kind==="items"){if(s.items.length===0){e.setFlash("info",t==="folder"?"Folder is empty":"No files selected"),e.render();return}await Ut(e,s.items);return}e.render(),requestAnimationFrame(()=>{Dn(e,t)})}catch(s){e.setFlash("error",s instanceof Error?s.message:"Could not open picker"),e.render()}}function at(e,t){return`${e}\0${t}`}function Cn(e,t){return t.map(a=>{const s=a.file,n=(a.relativePath||s.name).replace(/\\/g,"/"),r=n.split("/").filter(Boolean),i=r.pop()||s.name,l=r.join("/"),d=pa(e,l);return{item:a,file:s,fileName:i,parentPath:d,displayName:n||i,relDir:l}})}function En(e){const t=new Set,a=[];for(const s of e){const n=at(s.parentPath,s.fileName);t.has(n)||(t.add(n),a.push(s))}return a}async function Pn(e,t){if(t.length===0)return[];const a=new Map;for(const n of t){const r=a.get(n.parentPath)??[];r.push(n),a.set(n.parentPath,r)}const s=[];for(const[n,r]of a){let i=new Map;try{const l=await S.filesList(n);i=new Map;for(const d of l.entries)(d.type==="file"||d.type==="dir")&&i.set(d.name,d.type)}catch{i=new Map}for(const l of r)i.has(l.fileName)&&s.push(l)}return s.sort((n,r)=>n.displayName.localeCompare(r.displayName)),s}const At=new WeakMap;function mt(e){e&&(At.delete(e.state),e.state.filesUploadConflict=null)}function ze(e,t){var f;const a=At.get(e.state),s=e.state.filesUploadConflict;if(t==="cancel"){mt(e),e.setFlash("info","Upload cancelled"),e.render();return}if(!a){e.state.filesUploadConflict=null,e.setFlash("error","Upload session expired — drop or choose the files again"),e.render();return}const n=new Set(((f=s==null?void 0:s.conflictKeys)!=null&&f.length?s.conflictKeys:a.conflictKeys)??[]);let r=a.planned,i=new Set,l=0;if(t==="overwrite")i=new Set(n);else{const m=[];for(const u of a.planned){const b=at(u.parentPath,u.fileName);n.has(b)?l+=1:m.push(u)}if(r=m,w.event("files.upload.skip_existing",{skipped:l,remaining:r.length,total:a.planned.length,conflictKeys:n.size}),r.length===0&&a.emptyDirs.length===0){mt(e),e.setFlash("info",l===1?"Nothing to upload — the selected file already exists":`Nothing to upload — all ${l} selected files already exist`),e.render();return}}const d=a.destBase,c=a.emptyDirs;mt(e),wa(e,r,c,d,i)}async function Ut(e,t){if(t.length===0||e.state.filesUploadProgress||e.state.filesUploadConflict)return;e.state.filesUploadMenuOpen=!1,j(e),e.state.filesUploadDropActive=!1;const a=t.filter(i=>i.file&&!i.isEmptyDir),s=t.filter(i=>i.isEmptyDir&&i.relativePath),n=e.state.filesPath,r=En(Cn(n,a));if(w.event("files.upload.plan",{destBase:n||"/",files:r.length,emptyDirs:s.length,sample:r.slice(0,5).map(i=>({display:i.displayName,parent:i.parentPath||"/",name:i.fileName}))}),r.length>0){e.state.busy=!0,e.clearFlash(),e.render();try{const i=await Pn(n,r);if(i.length>0){const l=i.map(d=>at(d.parentPath,d.fileName));At.set(e.state,{planned:r,emptyDirs:s,destBase:n,conflictKeys:l}),e.state.filesUploadConflict={names:i.map(d=>d.displayName),totalFiles:r.length,conflictCount:i.length,conflictKeys:l},w.event("files.upload.conflicts",{total:r.length,conflicts:i.length,names:i.slice(0,12).map(d=>d.displayName)}),e.state.busy=!1,e.render();return}}catch(i){e.state.busy=!1,e.setFlash("error",i instanceof Error?i.message:"Could not check existing files"),e.render();return}}await wa(e,r,s,n,new Set)}async function wa(e,t,a,s,n){var b,y;const r=t.reduce((p,$)=>p+($.file.size||0),0),i=Date.now(),l=t.length+a.length;e.state.filesUploadProgress={phase:"uploading",totalFiles:Math.max(t.length,1),completedFiles:0,failedFiles:0,currentName:((b=t[0])==null?void 0:b.displayName)||((y=a[0])==null?void 0:y.relativePath)||"",bytesTotal:r,bytesSent:0,startedAt:i,elapsedSec:0,resultMessage:null,errorSamples:[]},e.state.busy=!0,e.clearFlash(),Sn(e),e.render();let d=0;const c=[],f=new Set;let m=0,u=0;try{for(const k of a){const h=k.relativePath.replace(/\\/g,"/").replace(/^\/+|\/+$/g,"");if(h){e.state.filesUploadProgress&&(e.state.filesUploadProgress={...e.state.filesUploadProgress,currentName:h+"/",elapsedSec:Math.floor((Date.now()-i)/1e3)},ge(e,e.state.filesUploadProgress));try{await Jt(e,s,h,f)}catch(v){c.push(`${h}/: ${v instanceof Error?v.message:"failed"}`)}}}for(const k of t){const{file:h,fileName:v,parentPath:g,displayName:D,relDir:U}=k;e.state.filesUploadProgress&&(e.state.filesUploadProgress={...e.state.filesUploadProgress,currentName:D,bytesSent:m,elapsedSec:Math.floor((Date.now()-i)/1e3)},ge(e,e.state.filesUploadProgress));try{U&&await Jt(e,s,U,f);const I=n.has(at(g,v));await S.filesUpload(g,h,{replace:I,onProgress:(O,C)=>{if(!e.state.filesUploadProgress||e.state.filesUploadProgress.phase!=="uploading")return;const P=C>0?C:h.size;e.state.filesUploadProgress={...e.state.filesUploadProgress,currentName:D,bytesSent:m+Math.min(O,P||O),elapsedSec:Math.floor((Date.now()-i)/1e3)},ge(e,e.state.filesUploadProgress)}}),w.event("files.upload",{path:g,name:v,size:h.size,relativePath:D,replace:I}),d+=1,I&&(u+=1),m+=h.size||0,e.state.filesUploadProgress&&(e.state.filesUploadProgress={...e.state.filesUploadProgress,completedFiles:d,failedFiles:c.length,bytesSent:m},ge(e,e.state.filesUploadProgress))}catch(I){const O=`${D}: ${I instanceof Error?I.message:"failed"}`;c.push(O),m+=h.size||0,e.state.filesUploadProgress&&(e.state.filesUploadProgress={...e.state.filesUploadProgress,completedFiles:d,failedFiles:c.length,bytesSent:m,errorSamples:c.slice(0,12)},ge(e,e.state.filesUploadProgress))}}await Z(e),$e(e);const p=Math.floor((Date.now()-i)/1e3),$=t.length;if(d>0&&c.length===0){let k=d===1?"Uploaded 1 file":`Uploaded ${d} files`;u>0&&(k+=u===1?" (1 overwritten)":` (${u} overwritten)`),e.state.filesUploadProgress={phase:"done",totalFiles:Math.max($,1),completedFiles:d,failedFiles:0,currentName:"",bytesTotal:r,bytesSent:r,startedAt:i,elapsedSec:p,resultMessage:k,errorSamples:[]},e.setFlash("success",k)}else if(d>0){const k=`Uploaded ${d}; ${c.length} failed. ${c[0]}`;e.state.filesUploadProgress={phase:"done",totalFiles:Math.max($,1),completedFiles:d,failedFiles:c.length,currentName:"",bytesTotal:r,bytesSent:r,startedAt:i,elapsedSec:p,resultMessage:k,errorSamples:c.slice(0,12)},e.setFlash("info",k)}else if(l>0&&c.length===0&&a.length>0){const k=a.length===1?"Created 1 empty folder":`Created ${a.length} empty folders`;e.state.filesUploadProgress={phase:"done",totalFiles:1,completedFiles:0,failedFiles:0,currentName:"",bytesTotal:0,bytesSent:0,startedAt:i,elapsedSec:p,resultMessage:k,errorSamples:[]},e.setFlash("success",k)}else{const k=c[0]||"Upload failed";e.state.filesUploadProgress={phase:"error",totalFiles:Math.max($,1),completedFiles:0,failedFiles:c.length,currentName:"",bytesTotal:r,bytesSent:0,startedAt:i,elapsedSec:p,resultMessage:k,errorSamples:c.slice(0,12)},e.setFlash("error",k)}}catch(p){$e(e);const $=p instanceof Error?p.message:"Upload failed";e.state.filesUploadProgress={phase:"error",totalFiles:Math.max(t.length,1),completedFiles:d,failedFiles:Math.max(c.length,1),currentName:"",bytesTotal:r,bytesSent:m,startedAt:i,elapsedSec:Math.floor((Date.now()-i)/1e3),resultMessage:$,errorSamples:c.length?c.slice(0,12):[$]},e.setFlash("error",$)}finally{e.state.busy=!1,e.render()}}function Gt(e,t,a){const s=t.files;if(!s||s.length===0)return;const n=ba(s,a);t.value="",Ut(e,n)}function Tn(e,t){const a=t?t.split("/").filter(Boolean):[];let s="";const n=[`<button type="button" class="files-crumb" data-action="files-nav" data-path="" ${e.state.busy?"disabled":""}>Home</button>`];for(const r of a){s=s?`${s}/${r}`:r;const i=s;n.push('<span class="files-crumb-sep" aria-hidden="true">/</span>'),n.push(`<button type="button" class="files-crumb" data-action="files-nav" data-path="${o(i)}" ${e.state.busy?"disabled":""}>${o(r)}</button>`)}return`<nav class="files-breadcrumb" aria-label="Folder path">${n.join("")}</nav>`}function An(e){const t=e.state.filesStatus;if(!t)return`<div class="card"><p class="muted">${e.state.filesLoading||e.state.busy?"Loading…":"Unable to load file storage status."}</p></div>`;if(!t.enabled)return`<div class="portal-grid portal-grid-files">
      <section class="card">
        ${x("Files","files","h1")}
        ...
        <p class="muted small">DAV path: <span class="mono">${o(t.davPath)}</span></p>
          ...
          <div class="bulk-bar-actions">
            ...
            <button type="button" class="btn btn-small btn-danger" data-action="files-bulk-delete" ${e.state.busy?"disabled":""}>Delete</button>
              ...
              <td class="files-col-mtime hide-sm">${o(wn(v.mtime))}</td>
              <td class="files-col-actions">
                ...
                <button type="button" class="btn btn-ghost btn-small btn-danger-text" data-action="files-delete-open" data-path="${o(v.path)}" data-name="${o(v.name)}" ${e.state.busy?"disabled":""}>Delete</button>
            ...
            </tr>`}).join(""),u=e.state.filesRenamePath!==null?(()=>{const v=e.state.filesEntries.find(D=>D.path===e.state.filesRenamePath),g=(v==null?void 0:v.name)??"";return N({id:"files-rename-modal",title:"Rename",titleId:"files-rename-title",closeAction:"files-rename-close",size:"sm",form:!0,formAttrs:'data-form="files-rename" id="files-rename-form"',body:`
                  ...
                  </label>`,footer:[{label:"Cancel",action:"files-rename-close",variant:"ghost"},{label:"Rename",type:"submit",variant:"primary",disabled:e.state.busy}]})})():"",b=e.state.filesDeletePaths!==null&&e.state.filesDeletePaths.length>0?(()=>{const v=e.state.filesDeletePaths,g=v.length>1,D=e.state.filesEntries.find(O=>O.path===v[0]),U=g?`Delete ${v.length} items`:`Delete ${(D==null?void 0:D.type)==="dir"?"folder":"file"}`,I=g?`<p style="margin:0 0 0.75rem">Delete <strong>${v.length}</strong> selected items? Folders are removed with their contents. This cannot be undone.</p>
               <ul class="files-delete-list muted small">
                 ${v.slice(0,12).map(O=>{const C=e.state.filesEntries.find(P=>P.path===O);return`<li><span class="mono">${o((C==null?void 0:C.name)??O)}</span></li>`}).join("")}
                 ${v.length>12?`<li>…and ${v.length-12} more</li>`:""}
               </ul>`:`<p style="margin:0">Delete <strong>${o((D==null?void 0:D.name)??v[0])}</strong>?${(D==null?void 0:D.type)==="dir"?" This removes the folder and everything inside it.":""}</p>`;return N({id:"files-delete-modal",title:U,titleId:"files-delete-title",closeAction:"files-delete-close",size:"sm",body:I,footer:[{label:"Cancel",action:"files-delete-close",variant:"ghost"},{label:"Delete",action:"files-delete-confirm",variant:"danger",disabled:e.state.busy}]})})():"",y=e.state.filesTransfer!==null&&e.state.filesTransfer.paths.length>0?(()=>{const v=e.state.filesTransfer.op,g=e.state.filesTransfer.paths,D=g.length>1,U=e.state.filesEntries.find(A=>A.path===g[0]),I=(U==null?void 0:U.name)??Ue(g[0]),O=D?`${v==="copy"?"Copy":"Move"} ${g.length} items`:`${v==="copy"?"Copy":"Move"} ${(U==null?void 0:U.type)==="dir"?"folder":"file"}`,C=e.state.filesTransferDest===""?"Home":e.state.filesTransferDest,P=tt(e,e.state.filesTransferDest,g);return N({id:"files-transfer-modal",title:O,titleId:"files-transfer-title",closeAction:"files-transfer-close",size:"md",form:!0,formAttrs:'data-form="files-transfer"',body:`
                  ${D?`<p class="muted small" style="margin:0 0 0.75rem">${g.length} items will be ${v==="copy"?"copied":"moved"} into the destination folder (original names kept).</p>`:`<p class="muted small" style="margin:0 0 0.75rem"><span class="mono">${o(I)}</span></p>`}
                  ...
                  <div class="files-transfer-dest">
                    <div class="files-transfer-dest-head">
                      ...
                      <span class="muted small mono files-transfer-dest-value" title="${o(C)}">${o(C)}</span>
                    </div>
                    ${pn(e)}
              ...
              </label>`,footer:[{label:"Cancel",action:"files-mkdir-close",variant:"ghost"},{label:"Create",type:"submit",variant:"primary",disabled:e.state.busy}]}):"",$=e.state.filesUploadConflict?(()=>{const v=e.state.filesUploadConflict,g=v.conflictCount,D=Math.max(0,v.totalFiles-g),U=g===1?"1 file already exists in the destination.":`${g} of ${v.totalFiles} files already exist in the destination.`,I=D>0?D===1?" Skip existing keeps the other 1 new file.":` Skip existing keeps the other ${D} new files.`:" Skip existing cancels the upload (nothing new to send).",O=v.names.slice(0,12).map(P=>`<li><span class="mono">${o(P)}</span></li>`).join(""),C=v.names.length>12?`<li class="muted">…and ${v.names.length-12} more</li>`:"";return N({id:"files-upload-conflict-modal",title:g===1?"File already exists":"Files already exist",titleId:"files-upload-conflict-title",closeAction:"files-upload-conflict-cancel",size:"sm",body:`
    ...
    with role <span class="badge badge-admin">Admin</span>.
      ...
      <section class="card admin-about-card">
        ...
        ${a}
      </section>`;const u=i.nbusers??i.users,b=i.nbcalendars??i.calendars,y=i.nbevents??i.events,p=i.nbbooks??i.addressBooks,$=i.nbcontacts??i.contacts;n=`
    ...
    ${n}`}function xn(e){const t=e.state.adminUsersQuery.trim().toLowerCase();return t?e.state.adminUsers.filter(a=>a.username.toLowerCase().includes(t)||(a.displayname||"").toLowerCase().includes(t)||(a.email||"").toLowerCase().includes(t)):e.state.adminUsers}function Ln(e){return e.state.adminUserCreateOpen?N({id:"admin-user-create-modal",title:"Add user",titleId:"admin-user-create-title",closeAction:"admin-user-create-close",size:"sm",form:!0,formAttrs:'data-form="admin-user-create"',body:`
        ...
        <p class="muted small">Username <span class="mono">${o(t.username)}</span> cannot be changed. Leave password fields empty to keep the current password.</p>
          ...
          </label>`,footer:[{label:"Cancel",action:"admin-user-edit-close",variant:"ghost",disabled:e.state.busy},{label:"Save changes",type:"submit",variant:"primary",disabled:e.state.busy}]})}function qn(e){if(!e.state.adminUserDeleteUsername)return"";const t=e.state.adminUserDeleteUsername,a=e.state.adminUserDetail&&e.state.adminUserDetail.username.toLowerCase()===t.toLowerCase()?e.state.adminUserDetail:e.state.adminUsers.find(n=>n.username.toLowerCase()===t.toLowerCase())??null,s=a?`${a.displayname||a.username} (${a.username})`:t;return N({id:"admin-user-delete-modal",title:"Delete user",titleId:"admin-user-delete-title",closeAction:"admin-user-delete-close",size:"sm",body:`
  ...
  ${i}${l}${d}`}function Bn(e){const t=ae(e,"users");if(t&&t.available===!1)return nt(e,"users");const a=xn(e),s=e.state.adminUsersLoading&&e.state.adminUsers.length===0?'<tr><td colspan="4" class="muted admin-table-empty">Loading users…</td></tr>':a.length===0?`<tr><td colspan="4" class="muted admin-table-empty">${e.state.adminUsersError?o(e.state.adminUsersError):e.state.adminUsersQuery.trim()?"No users match this filter.":"No users found."}</td></tr>`:a.map(n=>`<tr class="contact-table-row${e.state.adminSelectedUsername&&e.state.adminSelectedUsername.toLowerCase()===n.username.toLowerCase()?" is-selected":""}" data-action="admin-user-view" data-username="${o(n.username)}" tabindex="0" role="button">
    ...
    ${Ln(e)}
    ${_n(e)}
    ${qn(e)}`}async function Vn(e,t){const a=new FormData(t),s=String(a.get("username")??"").trim(),n=String(a.get("displayname")??"").trim(),r=String(a.get("email")??"").trim(),i=String(a.get("password")??""),l=String(a.get("passwordConfirm")??"");if(!s||!n||!r||!i){e.setFlash("error","Username, display name, email, and password are required"),e.render();return}if(i!==l){e.setFlash("error","Password confirmation does not match"),e.render();return}e.state.busy=!0,e.clearFlash(),e.render();try{const d=await S.adminCreateUser({username:s,displayname:n,email:r,password:i,passwordConfirm:l});w.event("admin.user.create",{username:d.user.username}),e.state.adminUserCreateOpen=!1,e.state.adminSelectedUsername=d.user.username,e.state.adminUserDetail=d.user,e.persistTab("admin","users",d.user.username),await oe(e),e.setFlash("success",`Created user “${d.user.username}”`)}catch(d){e.setFlash("error",d instanceof Error?d.message:"Create failed")}finally{e.state.busy=!1,e.render()}}async function Hn(e,t){const a=new FormData(t),s=String(a.get("username")??"").trim(),n=String(a.get("displayname")??"").trim(),r=String(a.get("email")??"").trim(),i=String(a.get("password")??""),l=String(a.get("passwordConfirm")??"");if(!s){e.setFlash("error","Username is required"),e.render();return}if(!n||!r){e.setFlash("error","Display name and email are required"),e.render();return}if(i!==""||l!==""){if(i===""||l===""){e.setFlash("error","Password and confirmation are required to change password"),e.render();return}if(i!==l){e.setFlash("error","Password confirmation does not match"),e.render();return}}e.state.busy=!0,e.clearFlash(),e.render();try{const d={displayname:n,email:r};i!==""&&(d.password=i,d.passwordConfirm=l);const c=await S.adminUpdateUser(s,d);w.event("admin.user.update",{username:c.user.username,passwordChanged:i!==""}),e.state.adminUserEditOpen=!1,e.state.adminUserDetail=c.user,e.state.adminSelectedUsername=c.user.username,await oe(e),e.setFlash("success",i!==""?`Updated “${c.user.username}” (password changed)`:`Updated “${c.user.username}”`)}catch(d){e.setFlash("error",d instanceof Error?d.message:"Update failed")}finally{e.state.busy=!1,e.render()}}async function zn(e,t){var c,f;if(!e.state.adminSelectedUsername)return;const a=e.state.adminSelectedUsername,s=new FormData(t),n=String(s.get("displayname")??"").trim(),r=String(s.get("description")??"").trim(),i=String(s.get("calendarcolor")??"").trim(),l=((c=t.querySelector('input[name="todos"]'))==null?void 0:c.checked)??!1,d=((f=t.querySelector('input[name="notes"]'))==null?void 0:f.checked)??!1;e.state.busy=!0,e.clearFlash(),e.render();try{if(e.state.adminCalModal==="create"){const m=String(s.get("uri")??"").trim().toLowerCase();await S.adminCreateUserCalendar(a,{uri:m,displayname:n,description:r,calendarcolor:i||void 0,todos:l,notes:d}),e.setFlash("success",`Created calendar “${n}”`)}else{const m=Number(s.get("instanceId"));await S.adminUpdateUserCalendar(a,m,{displayname:n,description:r,calendarcolor:i,todos:l,notes:d}),e.setFlash("success",`Updated calendar “${n}”`)}e.state.adminCalModal=null,e.state.adminCalEditId=null,await ce(e,a),await K(e,a)}catch(m){e.setFlash("error",m instanceof Error?m.message:"Save failed")}finally{e.state.busy=!1,e.render()}}async function Kn(e,t){if(!e.state.adminSelectedUsername)return;const a=e.state.adminSelectedUsername,s=new FormData(t),n=String(s.get("displayname")??"").trim(),r=String(s.get("description")??"").trim();e.state.busy=!0,e.clearFlash(),e.render();try{if(e.state.adminAbModal==="create"){const i=String(s.get("uri")??"").trim().toLowerCase();await S.adminCreateUserAddressBook(a,{uri:i,displayname:n,description:r}),e.setFlash("success",`Created address book “${n}”`)}else{const i=Number(s.get("id"));await S.adminUpdateUserAddressBook(a,i,{displayname:n,description:r}),e.setFlash("success",`Updated address book “${n}”`)}e.state.adminAbModal=null,e.state.adminAbEditId=null,await ce(e,a),await K(e,a)}catch(i){e.setFlash("error",i instanceof Error?i.message:"Save failed")}finally{e.state.busy=!1,e.render()}}const jn=["UTC","Africa/Cairo","Africa/Johannesburg","America/Anchorage","America/Argentina/Buenos_Aires","America/Chicago","America/Denver","America/Edmonton","America/Halifax","America/Los_Angeles","America/Mexico_City","America/New_York","America/Sao_Paulo","America/Toronto","America/Vancouver","Asia/Dubai","Asia/Hong_Kong","Asia/Jerusalem","Asia/Kolkata","Asia/Seoul","Asia/Shanghai","Asia/Singapore","Asia/Tokyo","Australia/Melbourne","Australia/Sydney","Europe/Amsterdam","Europe/Berlin","Europe/London","Europe/Madrid","Europe/Moscow","Europe/Paris","Europe/Rome","Europe/Warsaw","Pacific/Auckland","Pacific/Honolulu"];let be=null;function Wn(){if(be)return be;try{const e=Intl;if(typeof e.supportedValuesOf=="function"){const t=e.supportedValuesOf("timeZone");if(Array.isArray(t)&&t.length>0)return be=[...t].sort((a,s)=>a.localeCompare(s)),be}}catch{}return be=[...jn],be}function Sa(e){const t=e||"UTC",a=Wn(),s=a.includes(t),n=a.map(r=>`<option value="${Qt(r)}" ${r===t?"selected":""}>${Xt(r)}</option>`);return!s&&t&&n.unshift(`<option value="${Qt(t)}" selected>${Xt(t)}</option>`),n.join("")}function Qt(e){return e.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;")}function Xt(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Jn(e){const t=ae(e,"settings");if(t&&t.available===!1)return nt(e,"settings");if(e.state.adminSystemSettingsLoading&&!e.state.adminSystemSettings)return'<section class="card"><p class="muted">Loading system settings…</p></section>';if(e.state.adminSystemSettingsError&&!e.state.adminSystemSettings)return`<section class="card">
    ...
    ${Yn(e)}`}function Yn(e){return e.state.adminResetModalOpen?N({id:"admin-reset-modal",title:"Reset to Default",titleId:"admin-reset-title",closeAction:"admin-reset-close",size:"sm",body:`
    ...
    ${es(e)}`}function es(e){if(!e.state.adminDbConfirmOpen)return"";const t=e.state.adminDbConfirmText.trim()==="CONFIRM";return N({id:"admin-db-confirm-modal",title:"Confirm database change",titleId:"admin-db-confirm-title",closeAction:"admin-db-confirm-close",size:"sm",body:`
        ...
        </label>`,footer:[{label:"Cancel",action:"admin-db-confirm-close",variant:"ghost",disabled:e.state.busy},{label:"Save database settings",action:"admin-db-confirm-save",variant:"danger",disabled:e.state.busy||!t}]})}async function Ca(e,t,a={}){if(!e.userIsAdmin()){await e.activateTab("calendars",a);return}e.state.activeTab="admin",e.state.adminPage=t,t!=="users"?(e.state.adminSelectedUsername=null,e.state.adminUserDetail=null,e.state.adminUserDetailError=null):a.username!==void 0&&(e.state.adminSelectedUsername=a.username,a.username||(e.state.adminUserDetail=null,e.state.adminUserDetailError=null)),e.state.userMenuOpen=!1,e.persistTab("admin",t,e.state.adminSelectedUsername),w.event("tab",{tab:"admin",adminPage:t,user:e.state.adminSelectedUsername}),a.clearFlash!==!1&&e.clearFlash(),e.state.busy=!0,e.render();try{if(await ht(e),!e.adminUiEnabled()){e.state.activeTab="calendars",e.persistTab("calendars"),e.setFlash("info","Portal Administration UI is disabled.");return}const s=ae(e,t);t==="overview"&&(s==null?void 0:s.available)!==!1?await Ye(e):t==="users"&&(s==null?void 0:s.available)!==!1?(await oe(e),e.state.adminSelectedUsername&&(await K(e,e.state.adminSelectedUsername),await ce(e,e.state.adminSelectedUsername))):t==="settings"&&(s==null?void 0:s.available)!==!1?await Ge(e):t==="database"&&(s==null?void 0:s.available)!==!1&&await Qe(e)}catch(s){w.warn("admin page load failed",s instanceof Error?s.message:s),e.setFlash("error",s instanceof Error?s.message:"Failed to load")}finally{e.state.busy=!1,e.render()}}function ts(e){return e.userIsAdmin()?e.adminUiEnabled()?e.state.adminPage==="users"?Bn(e):e.state.adminPage==="settings"?Jn(e):e.state.adminPage==="database"?Zn(e):Mn(e):`<section class="card admin-coming-soon-card">
        <div class="admin-coming-soon-head">
    ...
    </div>`}function us(e=document){e.querySelectorAll(".dt-field.is-open").forEach(t=>{const a=t.querySelector(".dt-trigger"),s=t.querySelector(".dt-popover");if(!a||!s)return;const n=a.getBoundingClientRect(),r=8;s.style.position="fixed",s.style.visibility="hidden",s.style.top="0",s.style.left="0";const i=s.offsetWidth||320,l=s.offsetHeight||300;let d=n.bottom+6;d+l>window.innerHeight-r&&(d=Math.max(r,n.top-l-6));let c=n.left;c+i>window.innerWidth-r&&(c=Math.max(r,window.innerWidth-i-r)),c<r&&(c=r),s.style.top=`${Math.round(d)}px`,s.style.left=`${Math.round(c)}px`,s.style.right="auto",s.style.visibility="visible",s.style.zIndex="200"})}async function Nt(e,t){const a=await S.shares(t);e.state.shares=a.shares}function ms(e){const t=e.state.calendars.filter(s=>s.canShare);if(t.length===0)return null;const a=s=>{const n=s.uri.toLowerCase(),r=s.displayname.toLowerCase();return n==="default"||r==="default"||r==="default calendar"};return t.find(a)??t[0]??null}async function st(e){const t=e.state.selectedIds.filter(n=>e.state.calendars.some(r=>r.id===n));if(t.length===0){e.state.monthEvents=[];return}const{from:a,to:s}=ss(e.state.monthCursor.y,e.state.monthCursor.m);e.state.monthEventsLoading=!0,w.debug("loadMonthEvents",{selectedIds:t,from:a,to:s});try{const r=(await Promise.all(t.map(async i=>(await S.calendarEvents(i,a,s)).events.map(d=>({...d,instanceId:i}))))).flat();r.sort((i,l)=>{const d=i.start||"",c=l.start||"";return d!==c?d<c?-1:1:(i.summary||"").localeCompare(l.summary||"")}),e.state.monthEvents=r,w.event("monthEvents.loaded",{calendarIds:t,count:e.state.monthEvents.length,from:a,to:s})}catch(n){e.state.monthEvents=[],w.warn("loadMonthEvents failed",n instanceof Error?n.message:n)}finally{e.state.monthEventsLoading=!1}}function ps(e,t){const a=e.state.calendars.find(s=>s.id===t);return a!=null&&a.color?a.color.length>=7?a.color.slice(0,7):a.color:"#3B82F6"}function fs(e,t){e.state.selectedIds.includes(t)?(e.state.selectedIds=e.state.selectedIds.filter(a=>a!==t),e.state.selectedId===t&&(e.state.selectedId=e.state.selectedIds[0]??null)):(e.state.selectedIds=[...e.state.selectedIds,t],e.state.selectedId=t)}function bs(e,t,a){return new Date(t,a,1).toLocaleString(void 0,{month:"long",year:"numeric"})}function gs(e,t){const a=t.summary||"(No title)";if(t.allDay||/^\d{4}-\d{2}-\d{2}$/.test(t.start))return a;const s=new Date(t.start);return Number.isNaN(s.getTime())?a:`${s.toLocaleTimeString(void 0,e.timeFormatOpts())} ${a}`}function ys(e){const t=e.state.calendars.filter(h=>e.state.selectedIds.includes(h.id)),a=t.length===0?"No calendar selected":t.length===1?t[0].displayname:`${t.length} calendars`,s=e.state.monthCursor.y,n=e.state.monthCursor.m,r=new Date(s,n,1),i=e.localeWeekStart(),l=(r.getDay()-i+7)%7,d=new Date(s,n+1,0).getDate(),c=new Date(s,n,0).getDate(),m=L(new Date),u=e.localeDowLabels(),b=new Map;for(const h of e.state.monthEvents)for(const v of rs(h)){const g=b.get(v)??[];g.push(h),b.set(v,g)}const y=[],p=Math.ceil((l+d)/7)*7;for(let h=0;h<p;h++){let v,g=!0,D;h<l?(v=c-l+h+1,g=!1,D=new Date(s,n-1,v)):h>=l+d?(v=h-(l+d)+1,g=!1,D=new Date(s,n+1,v)):(v=h-l+1,D=new Date(s,n,v));const U=L(D),I=U===m,O=g?b.get(U)??[]:[],C=e.state.monthExpandDay===U?50:3,P=O.slice(0,C),A=O.length-P.length,q=P.map(pe=>{var Bt;const it=pe.instanceId,lt=gs(e,pe),qa=ps(e,it),Rt=((Bt=e.state.calendars.find(Ba=>Ba.id===it))==null?void 0:Bt.displayname)||"",Ra=Rt?`${lt} · ${Rt}`:lt;return`<button type="button" class="month-event${pe.allDay?"":" is-timed"}" title="${o(Ra)}" style="--ev-color:${o(qa)}"
          data-action="open-event" data-instance="${it}" data-uri="${o(pe.uri)}" ${e.state.busy?"disabled":""}>${o(lt)}</button>`}).join(""),se=A>0?`<button type="button" class="month-event-more" data-action="open-event-day" data-day="${o(U)}" title="Show all events this day" ${e.state.busy?"disabled":""}>+${A} more</button>`:"",ue=!g&&(v===1||h===l+d)?D.toLocaleString(void 0,{month:"short",day:"numeric"}):String(v),me=e.state.selectedId!==null?e.state.calendars.find(pe=>pe.id===e.state.selectedId)??null:null,Ne=!!(me&&!me.readOnly&&(me.canShare||me.access==="readwrite"));y.push(`<div class="month-cell${g?"":" is-outside"}${I?" is-today":""}${Ne?" is-clickable":""}"${Ne?` data-action="new-event-day" data-day="${o(U)}" role="button" tabindex="0" title="Add event on ${o(U)}"`:""}>
      <div class="month-daynum${I?" is-today-num":""}">${o(ue)}</div>
      <div class="month-events">${q}${se}</div>
    </div>`)}const $=t.length===0?e.state.calendars.length===0?'<p class="muted small month-empty-hint">No calendars yet — create one on the left, or wait for someone to share with you.</p>':"":e.state.monthEventsLoading?'<p class="muted small month-empty-hint">Loading events…</p>':"",k=t.slice(0,6).map(h=>{const v=h.color&&h.color.length>=7?h.color.slice(0,7):h.color||"#3B82F6";return`<span class="cal-swatch" style="background:${o(v)};margin-top:0" title="${o(h.displayname)}"></span>`}).join("");return`<section class="card month-cal-card">
    <div class="month-cal-toolbar">
      ...
      <h2 class="month-cal-title">${o(bs(e,s,n))}</h2>
    ...
    <div class="cal-modal-card">
      ...
      <p class="muted small">Large imports can time out; try again — already-imported items update faster.</p>`;const l=a?'<p class="muted small" style="margin:0">Please wait…</p>':Et([{label:"Close",action:"close-import-progress",variant:"primary"}]);return N({title:n,titleId:"import-progress-title",closeAction:"close-import-progress",size:"sm",className:"import-progress-modal",cardClassName:"import-progress-card",rootAttrs:"data-import-progress",hideClose:a,lockBackdrop:a,body:i,footer:l})}function Ia(e,t,a){return new Promise((s,n)=>{const r=new FileReader;r.onprogress=i=>{i.lengthComputable&&i.total>0?a(Math.min(100,Math.round(i.loaded/i.total*100))):a(null)},r.onload=()=>s(String(r.result??"")),r.onerror=()=>n(r.error??new Error("Failed to read file")),r.readAsText(t)})}function Ss(e){const t=e.root.querySelector('input[data-action="import-cal"]');t&&t.addEventListener("change",()=>{Ds(e,t)});const a=e.root.querySelector('input[data-action="import-create-cal"]');a&&a.addEventListener("change",()=>{Cs(e,a)});const s=e.root.querySelector('input[data-action="import-ab"]');s&&s.addEventListener("change",()=>{e.onImportContacts(s)})}async function Ds(e,t){var s;if(e.state.selectedId===null)return;const a=(s=t.files)==null?void 0:s[0];t.value="",a&&(e.state.calModalOpen=!0,await Oa(e,e.state.selectedId,a,{keepEditModalOpen:!0}))}async function Cs(e,t){var f;const a=(f=t.files)==null?void 0:f[0];if(t.value="",!a)return;const s=e.root.querySelector('[data-form="create-cal"]'),n=s?new FormData(s):new FormData,r=n.get("holidays")==="on",i=n.get("readOnly")==="on";if(r){e.setFlash("error","Turn off “Holidays calendar” to import a .ics file into a new calendar."),e.state.createCalModalOpen=!0,e.render();return}if(i){e.setFlash("error","Turn off “Read-only” before importing — import cannot write to a read-only calendar."),e.state.createCalModalOpen=!0,e.render();return}let l=String(n.get("displayname")??"").trim();l||(l=a.name.replace(/\.ics$/i,"").trim()||"Imported calendar");const d=String(n.get("description")??""),c=String(n.get("color")??"").trim();e.state.busy=!0,e.clearFlash(),e.state.createCalModalOpen=!0,e.render();try{const m=await S.createCalendar({displayname:l,description:d,color:c,readOnly:!1});e.state.selectedId=m.calendar.id,e.state.createCalModalOpen=!1,await e.loadHome(),e.setFlash("success",`Created “${m.calendar.displayname}” — importing…`),await Oa(e,m.calendar.id,a,{keepEditModalOpen:!1,successPrefix:`Calendar “${m.calendar.displayname}” created. `})}catch(m){const u=m instanceof Error?m.message:"Create or import failed";e.state.createCalModalOpen=!0,e.setFlash("error",u),e.state.busy=!1,e.render()}}async function Oa(e,t,a,s={}){e.state.busy=!0,e.clearFlash(),ee(e),e.state.importProgress={kind:"calendar",fileName:a.name,fileSizeLabel:Je(a.size),phase:"reading",readPercent:0,processPercent:null,processCurrent:0,processTotal:0,processImported:0,processUpdated:0,processSkipped:0,startedAt:Date.now(),elapsedSec:0,resultMessage:null,ok:null},Aa(e),e.render();try{const n=await Ia(e,a,l=>{if(!e.state.importProgress||e.state.importProgress.phase!=="reading")return;e.state.importProgress={...e.state.importProgress,readPercent:l};const d=e.root.querySelector(".import-progress-bar"),c=e.root.querySelector("[data-import-status-line]");d&&l!==null&&(d.classList.remove("is-indeterminate"),d.style.width=`${l}%`),c&&l!==null&&(c.textContent=`Reading file… ${l}%`)});Te(e,"uploading",{readPercent:100}),Te(e,"processing",{processPercent:0}),w.event("import.calendar.start",{file:a.name,bytes:a.size,calId:t});const r=await S.importCalendar(t,n,l=>{Ua(e,l)}),i=e.formatImportResult(r);e.state.selectedId===t&&await st(e),ee(e),Te(e,"done",{ok:!0,resultMessage:`${i} (from “${a.name}”)`}),e.setFlash("success",`${s.successPrefix||""}Import finished for “${a.name}”: ${i}.`)}catch(n){const r=n instanceof Error?n.message:"Import failed";ee(e),Te(e,"error",{ok:!1,resultMessage:r}),e.setFlash("error",r)}finally{s.keepEditModalOpen&&(e.state.calModalOpen=!0),e.state.busy=!1,e.render()}}async function Es(e,t){if(e.state.selectedId===null)return;const a=new FormData(t),s=String(a.get("username")??"").trim(),n=String(a.get("access")??"read");if(!s){e.setFlash("error","Select a user to share with"),e.render();return}e.state.calModalOpen=!0,e.state.busy=!0,e.clearFlash(),e.render();try{await S.share(e.state.selectedId,s,n),await Nt(e,e.state.selectedId),e.setFlash("success",`Shared with ${s}`)}catch(r){e.setFlash("error",r instanceof Error?r.message:"Share failed")}finally{e.state.busy=!1,e.render()}}async function Ps(e,t){if(!e.state.editingEvent||!e.state.editingEvent.canWrite)return;const a=new FormData(t),s=String(a.get("summary")??"").trim(),n=String(a.get("description")??"").trim(),r=String(a.get("location")??"").trim(),i=a.get("allDay")==="on",l=String(a.get("start")??"").trim(),d=String(a.get("end")??"").trim(),c=Number(a.get("instanceId"))||e.state.editingEvent.instanceId,f=Ie(a);if(!s){e.setFlash("error","Title is required"),e.render();return}if(!l){e.setFlash("error","Start is required"),e.render();return}let m,u;if(i)m=l.slice(0,10),u=d?d.slice(0,10):m;else if(/^\d{4}-\d{2}-\d{2}$/.test(l)){const $=Ot(l,d||null);m=new Date($.start).toISOString(),u=$.end?new Date($.end).toISOString():null}else m=new Date(l).toISOString(),u=d?new Date(d).toISOString():null;const b=e.state.editingEvent.instanceId,y=e.state.editingEvent.uri,p=e.state.creatingEvent;e.state.busy=!0,e.clearFlash(),e.state.eventModalOpen=!0,e.render(),w.event(p?"event.create":"event.update",{instanceId:c,uri:p?null:y,allDay:i,summary:s});try{const $={summary:s,description:n,location:r,allDay:i,start:m,end:u,instanceId:c,repeat:f},k=p?await S.createEvent(c,$):await S.updateEvent(b,y,$);(e.state.selectedId===null||k.event.instanceId!==e.state.selectedId)&&(e.state.selectedId=k.event.instanceId),await st(e),e.state.eventModalOpen=!1,e.state.editingEvent=null,e.state.creatingEvent=!1,e.state.eventDtPicker=null,w.event(p?"event.created":"event.saved",{uri:k.event.uri,instanceId:k.event.instanceId}),e.setFlash("success",te("Event",k.event.summary||s,p?"created":"saved"))}catch($){w.warn("event.save failed",$ instanceof Error?$.message:$),e.setFlash("error",$ instanceof Error?$.message:"Save failed")}finally{e.state.busy=!1,e.render()}}async function Ts(e,t){if(e.state.selectedId===null)return;const a=new FormData(t),s=String(a.get("displayname")??"").trim(),n=String(a.get("description")??""),r=String(a.get("color")??"").trim();e.state.busy=!0,e.clearFlash(),e.render();try{const i=await S.updateCalendar(e.state.selectedId,{displayname:s,description:n,color:r});e.state.calModalOpen=!0,await e.loadHome(),e.state.selectedId=i.calendar.id,await Nt(e,e.state.selectedId),await st(e),e.setFlash("success","Calendar updated")}catch(i){e.setFlash("error",i instanceof Error?i.message:"Update failed")}finally{e.state.busy=!1,e.render()}}async function As(e,t){const a=new FormData(t),s=String(a.get("displayname")??"").trim(),n=String(a.get("description")??""),r=String(a.get("color")??"").trim(),i=a.get("holidays")==="on",l=String(a.get("holidayCountry")??"").trim(),d=a.get("readOnly")==="on";if(e.state.createCalModalOpen=!0,i&&!l){e.setFlash("error","Select a country for the holidays calendar"),e.render();return}if(!i&&!s){e.setFlash("error","Display name is required"),e.render();return}e.state.busy=!0,e.clearFlash(),e.render();try{const c=await S.createCalendar({displayname:s,description:n,color:r,holidays:i,holidayCountry:i?l:void 0,readOnly:d});e.state.selectedId=c.calendar.id,e.state.selectedIds.includes(c.calendar.id)||(e.state.selectedIds=[...e.state.selectedIds,c.calendar.id]),e.state.createCalModalOpen=!1,await e.loadHome();let f=`Created “${c.calendar.displayname}”`;const m=c.holidayImport??c.calendar.holidayImport;m&&(f+=`. Holidays imported: ${e.formatImportResult(m)}.`),d&&(f+=" Calendar is read-only."),e.setFlash("success",f)}catch(c){e.state.createCalModalOpen=!0,e.setFlash("error",c instanceof Error?c.message:"Create failed")}finally{e.state.busy=!1,e.render()}}function Us(e){const t=e.root.querySelector('[data-form="create-cal"]');if(!t)return;const a=t.querySelector('input[name="holidays"]'),s=t.querySelector("#holidays-country-wrap"),n=t.querySelector('input[name="displayname"]'),r=t.querySelector('input[name="readOnly"]');if(!a||!s)return;const i=()=>{const l=a.checked;s.hidden=!l,n&&(n.required=!l,l&&!n.value.trim()?n.placeholder="Auto: Holidays (XX)":l||(n.placeholder="Work")),l&&r&&(r.checked=!0)};a.addEventListener("change",i),i()}function Zt(e){const{state:t}=e,a=t.calendars.filter(p=>p.canShare),s=t.calendars.filter(p=>!p.canShare),n=t.calendars.find(p=>p.id===t.selectedId)??null,r=a.map(p=>{const $=t.selectedIds.includes(p.id),k=$?" is-selected":"",h=p.id===t.selectedId?" is-primary":"",v=p.color?`<span class="cal-swatch" style="background:${o(p.color)}"></span>`:'<span class="cal-swatch cal-swatch-empty"></span>',g=e.accessBadge(p.access)+(p.readOnly?'<span class="badge">read-only</span>':"")+(p.holidaysCountry?`<span class="badge badge-admin">holidays ${o(p.holidaysCountry)}</span>`:"");return`<div class="cal-row${k}${h}" data-action="select-cal" data-id="${p.id}" role="button" tabindex="0" title="Toggle on the month grid">
      ...
      </div>`}).join(""),i=s.map(p=>{const $=t.selectedIds.includes(p.id),k=$?" is-selected":"",h=p.id===t.selectedId?" is-primary":"",v=p.color?`<span class="cal-swatch" style="background:${o(p.color)}"></span>`:'<span class="cal-swatch cal-swatch-empty"></span>',g=p.access==="readwrite"?"Shared with you · full access — check to show events; click to set as primary for new events":"Shared with you · read-only — check to show events";return`<div class="cal-row${k}${h}" data-action="select-cal" data-id="${p.id}" role="button" tabindex="0" title="${o(g)}">
        <label class="cal-row-check" title="Show events on the month grid" onclick="event.stopPropagation()">
          <input type="checkbox" data-action="toggle-cal" data-id="${p.id}" ${$?"checked":""} ${t.busy?"disabled":""} />
        </label>
        ${v}
        ...
        <span class="cal-row-actions">
          <button type="button" class="btn btn-small" data-action="export-cal" data-id="${p.id}" ${t.busy?"disabled":""} title="Export as .ics">Export</button>
            ...
            </tr>`).join(""),c=n!=null&&n.color&&n.color.length>=7?n.color.slice(0,7):"#3B82F6",f=!!(n&&n.readOnly),m=t.calModalOpen&&n&&n.canShare?N({id:"cal-edit-modal",title:"Calendar details",titleId:"cal-modal-title",closeAction:"close-cal-modal",body:`
              ${e.renderFlashBanner()}
              <section>
    ...
    ${e.renderEventModal()}`}function Ae(e){if(!e.state.editingContact)return;const t=e.root.querySelector('[data-form="contact"]');if(!t)return;const a=new FormData(t);e.state.editingContact.firstname=String(a.get("firstname")??""),e.state.editingContact.lastname=String(a.get("lastname")??""),e.state.editingContact.fullname=String(a.get("fullname")??""),e.state.editingContact.org=String(a.get("org")??""),e.state.editingContact.title=String(a.get("title")??""),e.state.editingContact.url=String(a.get("url")??""),e.state.editingContact.note=String(a.get("note")??"");const s=String(a.get("birthday")??"").trim();e.state.editingContact.birthday=s&&/^\d{4}-\d{2}-\d{2}/.test(s)?s.slice(0,10):null,e.state.editingContact.address={street:String(a.get("street")??""),city:String(a.get("city")??""),region:String(a.get("region")??""),postal:String(a.get("postal")??""),country:String(a.get("country")??"")};const n=[];let r=0;for(;a.has(`email_${r}`);)n.push(String(a.get(`email_${r}`)??"")),r++;n.length&&(e.state.editingContact.emails=n);const i=[];for(r=0;a.has(`phone_value_${r}`);)i.push({type:String(a.get(`phone_type_${r}`)??"other"),value:String(a.get(`phone_value_${r}`)??"")}),r++;i.length&&(e.state.editingContact.phones=i);const l=[];for(r=0;a.has(`custom_label_${r}`)||a.has(`custom_value_${r}`);)l.push({label:String(a.get(`custom_label_${r}`)??""),value:String(a.get(`custom_value_${r}`)??"")}),r++;e.state.editingContact.custom=l}function Fs(e,t){const a=new FormData(t),s=[];let n=0;for(;a.has(`email_${n}`);){const d=String(a.get(`email_${n}`)??"").trim();d&&s.push(d),n++}const r=[];for(n=0;a.has(`phone_value_${n}`);){const d=String(a.get(`phone_value_${n}`)??"").trim();d&&r.push({type:String(a.get(`phone_type_${n}`)??"other"),value:d}),n++}const i=[];for(n=0;a.has(`custom_label_${n}`)||a.has(`custom_value_${n}`);){const d=String(a.get(`custom_label_${n}`)??"").trim(),c=String(a.get(`custom_value_${n}`)??"").trim();(d||c)&&i.push({label:d,value:c}),n++}const l={firstname:String(a.get("firstname")??"").trim(),lastname:String(a.get("lastname")??"").trim(),fullname:String(a.get("fullname")??"").trim(),org:String(a.get("org")??"").trim(),title:String(a.get("title")??"").trim(),emails:s,phones:r,address:{street:String(a.get("street")??"").trim(),city:String(a.get("city")??"").trim(),region:String(a.get("region")??"").trim(),postal:String(a.get("postal")??"").trim(),country:String(a.get("country")??"").trim()},url:String(a.get("url")??"").trim(),note:String(a.get("note")??"").trim(),birthday:(()=>{const d=String(a.get("birthday")??"").trim();return d&&/^\d{4}-\d{2}-\d{2}/.test(d)?d.slice(0,10):null})(),custom:i};return e.state.removePhotoPending?l.removePhoto=!0:e.state.photoBase64Pending&&(l.photoBase64=e.state.photoBase64Pending),l}function Y(e){const{state:t,root:a}=e,s=a.querySelector('[data-form="edit-event"]');s&&t.editingEvent&&e.syncEditingEventFromForm(s);const n=a.querySelector('[data-form="task"]');n&&t.editingTask&&e.syncEditingTaskFromForm(n);const r=a.querySelector('[data-form="note"]');r&&t.editingNote&&e.syncEditingNoteFromForm(r),t.editingContact&&Ae(e.contactsHost)}async function Is(e,t,a,s){var c,f,m;const{state:n,root:r,render:i,setFlash:l,clearFlash:d}=e;if(t==="select-cal"||t==="toggle-cal"){const u=Number(a.dataset.id);if(!Number.isFinite(u))return!0;e.toggleCalendarSelected(u),n.busy=!0,d(),i();try{await e.loadMonthEvents()}catch(b){l("error",b instanceof Error?b.message:"Failed to load calendar")}finally{n.busy=!1,i()}return!0}if(t==="edit-cal"){const u=Number(a.dataset.id);if(!Number.isFinite(u)||!n.calendars.find(y=>y.id===u&&y.canShare))return!0;n.selectedId=u,n.selectedIds.includes(u)||(n.selectedIds=[...n.selectedIds,u]),n.calModalOpen=!0,n.deleteConfirmId=null,n.busy=!0,d(),i();try{await e.loadShares(u),await e.loadMonthEvents()}catch(y){l("error",y instanceof Error?y.message:"Failed to open calendar")}finally{n.busy=!1,i()}return!0}if(t==="close-cal-modal")return n.calModalOpen=!1,i(),!0;if(t==="open-create-cal-modal")return n.createCalModalOpen=!0,n.calModalOpen=!1,n.deleteConfirmId=null,d(),i(),!0;if(t==="close-create-cal-modal")return n.createCalModalOpen=!1,d(),i(),!0;if(t==="delete-cal"){const u=Number(a.dataset.id);return!Number.isFinite(u)||!n.calendars.find(y=>y.id===u&&y.canShare)||(n.deleteConfirmId=u,n.calModalOpen=!1,d(),i()),!0}if(t==="cancel-delete-cal")return n.deleteConfirmId=null,i(),!0;if(t==="confirm-delete-cal"){const u=Number(a.dataset.id),b=r.querySelector("#delete-cal-confirm");if(!Number.isFinite(u)||!(b!=null&&b.checked))return!0;n.busy=!0,d(),i();try{if(await S.deleteCalendar(u),n.selectedId===u&&(n.selectedId=null),n.selectedIds=n.selectedIds.filter(y=>y!==u),n.deleteConfirmId=null,n.calModalOpen=!1,n.shares=[],n.monthEvents=[],await e.loadHome(),n.selectedId===null){const y=e.pickDefaultCalendar();y?(n.selectedId=y.id,n.selectedIds.includes(y.id)||(n.selectedIds=[...n.selectedIds,y.id]),await e.loadMonthEvents()):n.selectedIds.length>0&&(n.selectedId=n.selectedIds[0],await e.loadMonthEvents())}l("success","Calendar deleted")}catch(y){l("error",y instanceof Error?y.message:"Delete failed")}finally{n.busy=!1,i()}return!0}if(t==="month-today"){const u=new Date;n.monthCursor={y:u.getFullYear(),m:u.getMonth()},n.monthExpandDay=null,n.busy=!0,i();try{await e.loadMonthEvents()}finally{n.busy=!1,i()}return!0}if(t==="month-prev"||t==="month-next"){const u=t==="month-prev"?-1:1,b=new Date(n.monthCursor.y,n.monthCursor.m+u,1);n.monthCursor={y:b.getFullYear(),m:b.getMonth()},n.monthExpandDay=null,n.busy=!0,i();try{await e.loadMonthEvents()}finally{n.busy=!1,i()}return!0}if(t==="open-event"){s.stopPropagation();const u=Number(a.dataset.instance),b=a.dataset.uri??"";if(!Number.isFinite(u)||!b)return!0;n.busy=!0,d(),i();try{const y=await S.getEvent(u,b);n.editingEvent={...y.event,repeat:y.event.repeat??e.defaultRepeat()},n.creatingEvent=!1,n.eventModalOpen=!0,n.eventDtPicker=null,n.calModalOpen=!1,n.deleteConfirmId=null}catch(y){l("error",y instanceof Error?y.message:"Failed to open event")}finally{n.busy=!1,i()}return!0}if(t==="open-event-day"){s.stopPropagation();const u=a.dataset.day??"";return n.monthExpandDay=n.monthExpandDay===u?null:u,i(),!0}if(t==="new-event-day"){const u=s.target;if((c=u==null?void 0:u.closest)!=null&&c.call(u,".month-event, .month-event-more"))return!0;const b=a.dataset.day??"";if(!/^\d{4}-\d{2}-\d{2}$/.test(b))return!0;if(n.selectedId===null)return l("error","Select a calendar first"),i(),!0;const y=n.calendars.find(p=>p.id===n.selectedId);return!y||y.readOnly||!(y.canShare||y.access==="readwrite")?(l("error","This calendar is read-only"),i(),!0):(n.creatingEvent=!0,n.editingEvent=e.blankEventForDay(b,n.selectedId),n.eventModalOpen=!0,n.eventDtPicker=null,n.calModalOpen=!1,n.deleteConfirmId=null,d(),i(),!0)}if(t==="close-event-modal")return n.eventModalOpen=!1,n.editingEvent=null,n.creatingEvent=!1,n.eventDtPicker=null,d(),i(),!0;if(t==="dt-open"){const u=a.dataset.dtField||"";if(!u)return!0;if(Y(e),((f=n.eventDtPicker)==null?void 0:f.field)===u)n.eventDtPicker=null;else{const b=a.dataset.dtDateOnly==="1",y=a.dataset.dtClear!=="0",p=a.dataset.dtName||u;let $=e.getDtFieldCurrentValue(u);!$&&(u==="due"||u==="dtstart"||u==="bulk-due")&&($=Pe().start);const k=ye($||L(new Date)),[h,v]=k.date.split("-").map(Number);n.eventDtPicker={field:u,viewY:h,viewM:(v||1)-1,dateOnly:b,allowClear:y,name:p}}return i(),!0}if(t==="dt-month-prev"||t==="dt-month-next"){if(!n.eventDtPicker)return!0;Y(e);const u=t==="dt-month-prev"?-1:1,b=new Date(n.eventDtPicker.viewY,n.eventDtPicker.viewM+u,1);return n.eventDtPicker={...n.eventDtPicker,viewY:b.getFullYear(),viewM:b.getMonth()},i(),!0}if(t==="dt-set-month"){if(!n.eventDtPicker)return!0;Y(e);const b=Number(a.value);return!Number.isFinite(b)||b<0||b>11||(n.eventDtPicker={...n.eventDtPicker,viewM:b},i()),!0}if(t==="dt-set-year"){if(!n.eventDtPicker)return!0;Y(e);const b=Number(a.value);return!Number.isFinite(b)||b<1||b>9999||(n.eventDtPicker={...n.eventDtPicker,viewY:b},i()),!0}if(t==="dt-pick-day"){if(!n.eventDtPicker)return!0;const u=n.eventDtPicker.field,b=a.dataset.day??"";if(!/^\d{4}-\d{2}-\d{2}$/.test(b))return!0;Y(e);const y=n.eventDtPicker.dateOnly;if(y)e.setDtFieldValue(u,b),n.eventDtPicker=null;else{const p=e.getDtFieldCurrentValue(u),$=ye(p||Pe(b).start).hm;e.setDtFieldValue(u,`${b}T${$}`),n.eventDtPicker={...n.eventDtPicker,viewY:Number(b.slice(0,4)),viewM:Number(b.slice(5,7))-1}}if(u==="start"&&n.editingEvent&&!y&&n.editingEvent.end){const p=new Date(String(n.editingEvent.start)),$=new Date(String(n.editingEvent.end));!Number.isNaN(p.getTime())&&!Number.isNaN($.getTime())&&$<=p&&e.setDtFieldValue("end",Q(new Date(p.getTime()+3600*1e3)))}return i(),!0}if(t==="dt-pick-time"){if(!n.eventDtPicker||n.eventDtPicker.dateOnly)return!0;const u=n.eventDtPicker.field,b=a.dataset.hm??"";if(!/^\d{2}:\d{2}$/.test(b))return!0;Y(e);const y=e.getDtFieldCurrentValue(u)||Pe().start,$=`${ye(y).date}T${b}`;if(e.setDtFieldValue(u,$),u==="start"&&n.editingEvent){n.editingEvent={...n.editingEvent,allDay:!1};const k=n.editingEvent.end?ye(String(n.editingEvent.end)):null,h=new Date($);(!k||new Date(`${k.date}T${k.hm}`)<=h)&&e.setDtFieldValue("end",Q(new Date(h.getTime()+3600*1e3)))}return n.eventDtPicker=null,i(),!0}if(t==="dt-today"){if(!n.eventDtPicker)return!0;const u=n.eventDtPicker.field;Y(e);const b=L(new Date);if(n.eventDtPicker.dateOnly)e.setDtFieldValue(u,b);else{const y=Pe(b);u==="start"?(e.setDtFieldValue("start",y.start),n.editingEvent&&!n.editingEvent.end&&e.setDtFieldValue("end",y.end)):u==="end"?e.setDtFieldValue("end",y.end):e.setDtFieldValue(u,y.start)}return n.eventDtPicker=null,i(),!0}if(t==="dt-clear"){if(!n.eventDtPicker||!n.eventDtPicker.allowClear)return!0;const u=n.eventDtPicker.field;return Y(e),e.setDtFieldValue(u,null),n.eventDtPicker=null,i(),!0}if(t==="event-allday-toggle"){if(!n.editingEvent)return!0;const u=r.querySelector('[data-form="edit-event"]'),b=a.checked;if(u){const y=new FormData(u),p=String(y.get("start")??n.editingEvent.start??""),$=String(y.get("end")??n.editingEvent.end??"")||null;let k=p,h=$;if(b){const v=os(p,$);k=v.start,h=v.end}else{const v=p.slice(0,10),g=($||p).slice(0,10),D=Ot(v,g);k=D.start,h=D.end}n.editingEvent={...n.editingEvent,summary:String(y.get("summary")??n.editingEvent.summary),description:String(y.get("description")??n.editingEvent.description),location:String(y.get("location")??n.editingEvent.location),instanceId:Number(y.get("instanceId"))||n.editingEvent.instanceId,allDay:b,start:k,end:h,repeat:Ie(y)}}else n.editingEvent={...n.editingEvent,allDay:b};return n.eventDtPicker=null,i(),!0}if(t==="event-repeat-freq"||t==="event-repeat-end"){if(!n.editingEvent)return!0;const u=r.querySelector('[data-form="edit-event"]');if(!u)return!0;const b=new FormData(u),y=u.querySelector('input[name="allDay"]'),p=Ie(b);return n.editingEvent={...n.editingEvent,summary:String(b.get("summary")??n.editingEvent.summary),description:String(b.get("description")??n.editingEvent.description),location:String(b.get("location")??n.editingEvent.location),instanceId:Number(b.get("instanceId"))||n.editingEvent.instanceId,allDay:(y==null?void 0:y.checked)??n.editingEvent.allDay,start:String(b.get("start")??n.editingEvent.start??""),end:String(b.get("end")??n.editingEvent.end??"")||null,repeat:p,hasRrule:!!String(b.get("repeatFreq")??"").trim()},p.freq&&p.endMode==="until"&&((m=n.eventDtPicker)==null?void 0:m.field)==="end"&&(n.eventDtPicker=null),i(),!0}if(t==="delete-event"){if(!n.editingEvent||!n.editingEvent.canWrite||n.creatingEvent)return!0;const u=String(n.editingEvent.summary||"this event").trim()||"this event";return n.confirmDelete={scope:"event",title:"Delete event",message:`Delete “${u}”?`,detail:"CalDAV clients will sync the removal. This cannot be undone."},i(),!0}if(t==="revoke"){const u=a.dataset.href??"";return!u||n.selectedId===null||(n.confirmDelete={scope:"revoke-share",title:"Revoke share",message:"Revoke access for this user?",detail:"They will lose this calendar until you share it again.",href:u},i()),!0}if(t==="export-cal"){s.stopPropagation();const u=a.dataset.id,b=u!==void 0&&u!==""?Number(u):n.selectedId;if(b===null||Number.isNaN(b))return!0;n.busy=!0,d(),i();try{const{blob:y,filename:p}=await S.exportCalendar(b),$=await e.saveBlobAsFile(y,p);$==="cancelled"?l("info","Export cancelled"):$==="saved"?l("success",`Saved ${p}`):l("success",`Download started: ${p}`)}catch(y){l("error",y instanceof Error?y.message:"Export failed")}finally{n.busy=!1,i()}return!0}return!1}async function Na(e){const t=await S.notes({q:e.state.noteSearch,sort:e.state.noteSort,order:e.state.noteOrder});e.state.notes=t.notes,e.state.noteCalendars=t.calendars,e.state.selectedNoteKey!==null&&!e.state.notes.some(a=>`${a.instanceId}|${a.uri}`===e.state.selectedNoteKey)&&(e.state.selectedNoteKey=null,e.state.creatingNote||(e.state.editingNote=null))}function _(e,t){return`${e}|${t}`}function Os(e){const t=e.state.notes.length===0?`<tr class="contacts-empty-row"><td colspan="3" class="muted">${e.state.noteSearch?"No notes match your search.":"No notes yet. Add one below."}</td></tr>`:e.state.notes.map(r=>{const i=_(r.instanceId,r.uri),l=!e.state.creatingNote&&i===e.state.selectedNoteKey?" is-selected":"",d=(r.description||"").replace(/\s+/g," ").slice(0,80);return`<tr class="contact-table-row${l}" data-action="select-note" data-instance="${r.instanceId}" data-uri="${o(r.uri)}" tabindex="0" role="button">
              ...
              <td class="col-note-date muted small">${o(va(r.dtstart))}</td>
    ...
    <section class="stack items-edit-panel">
      ${n}
  ...
  </div>`}function Ns(e,t){if(!e.state.editingNote)return;const a=new FormData(t),s=String(a.get("dtstart")??"").trim(),n=a.get("instanceId"),r=n!==null&&String(n)!==""?Number(n):e.state.editingNote.instanceId;e.state.editingNote={...e.state.editingNote,instanceId:Number.isFinite(r)&&r>0?r:e.state.editingNote.instanceId,summary:String(a.get("summary")??e.state.editingNote.summary),description:String(a.get("description")??e.state.editingNote.description),dtstart:s?new Date(s).toISOString():null}}async function Ms(e,t){const a=new FormData(t),s=String(a.get("summary")??"").trim(),n=String(a.get("description")??"").trim(),r=String(a.get("dtstart")??"").trim(),i=r?new Date(r).toISOString():null;e.state.busy=!0,e.clearFlash(),e.render();try{if(e.state.creatingNote){const l=Number(a.get("instanceId"));if(!Number.isFinite(l)||l<=0)throw new Error("Select a calendar");const d=await S.createNote({instanceId:l,summary:s,description:n,dtstart:i});e.state.creatingNote=!1,e.state.selectedNoteKey=_(d.note.instanceId,d.note.uri),e.state.editingNote=d.note,e.setFlash("success",te("Note",d.note.summary||s,"created"))}else if(e.state.editingNote){const l=await S.updateNote(e.state.editingNote.instanceId,e.state.editingNote.uri,{summary:s,description:n,dtstart:i});e.state.editingNote=l.note,e.state.selectedNoteKey=_(l.note.instanceId,l.note.uri),e.setFlash("success",te("Note",l.note.summary||s,"saved"))}await Na(e)}catch(l){e.setFlash("error",l instanceof Error?l.message:"Save failed")}finally{e.state.busy=!1,e.render()}}async function xs(e,t,a,s){var d;const{state:n,render:r,setFlash:i,clearFlash:l}=e;if(t==="sort-note"){const c=a.dataset.sort||"";if(!c)return!0;n.noteSort===c?n.noteOrder=n.noteOrder==="asc"?"desc":"asc":(n.noteSort=c,n.noteOrder="asc"),n.busy=!0,r();try{await e.loadNotes()}catch(f){i("error",f instanceof Error?f.message:"Sort failed")}finally{n.busy=!1,r()}return!0}if(t==="select-note"){const c=Number(a.dataset.instance),f=a.dataset.uri??"";if(!Number.isFinite(c)||!f)return!0;const m=n.notes.find(u=>u.instanceId===c&&u.uri===f)??null;return n.creatingNote=!1,n.selectedNoteKey=e.itemKey(c,f),n.editingNote=m?{...m}:null,l(),r(),!0}if(t==="new-note")return n.creatingNote=!0,n.selectedNoteKey=null,n.editingNote={uri:"",instanceId:((d=n.noteCalendars[0])==null?void 0:d.id)??0,calendarId:0,calendarName:"",calendarUri:"",summary:"",description:"",dtstart:new Date().toISOString(),lastmodified:0,readOnly:!1,canWrite:!0},l(),r(),!0;if(t==="cancel-note")return n.creatingNote=!1,n.editingNote=null,n.selectedNoteKey=null,r(),!0;if(t==="delete-note"){if(!n.editingNote||n.creatingNote)return!0;const c=String(n.editingNote.summary||"this note").trim()||"this note";return n.confirmDelete={scope:"note",title:"Delete note",message:`Delete “${c}”?`,detail:"CalDAV clients will sync the removal. This cannot be undone."},r(),!0}return!1}async function Xe(e){const t=await S.tasks({q:e.state.taskSearch,sort:e.state.taskSort,order:e.state.taskOrder});e.state.tasks=t.tasks,e.state.taskCalendars=t.calendars;const a=new Set(e.state.tasks.map(s=>_(s.instanceId,s.uri)));e.state.checkedTaskKeys=e.state.checkedTaskKeys.filter(s=>a.has(s)),e.state.selectedTaskKey!==null&&!e.state.tasks.some(s=>`${s.instanceId}|${s.uri}`===e.state.selectedTaskKey)&&(e.state.selectedTaskKey=null,e.state.creatingTask||(e.state.editingTask=null))}function Ls(e,t){const a=new Map;for(const f of t)f.uid&&a.set(f.uid,f);const s=new Map(t.map((f,m)=>[_(f.instanceId,f.uri),m])),n=new Map,r=[];for(const f of t){const m=f.parentUid;if(m&&a.has(m)&&m!==f.uid){const u=n.get(m)??[];u.push(f),n.set(m,u)}else r.push(f)}const i=(f,m)=>(s.get(_(f.instanceId,f.uri))??0)-(s.get(_(m.instanceId,m.uri))??0);r.sort(i);for(const[,f]of n)f.sort(i);const l=[],d=new Set,c=(f,m)=>{const u=f.uid||_(f.instanceId,f.uri);if(!d.has(u)){d.add(u),l.push({task:f,depth:Math.min(m,8)});for(const b of n.get(f.uid)??[])c(b,m+1);d.delete(u)}};for(const f of r)c(f,0);for(const f of t)l.some(m=>m.task===f)||l.push({task:f,depth:0});return l}function _s(e,t){const a=new Set([t]);if(!t)return a;let s=!0;for(;s;){s=!1;for(const n of e.state.tasks)n.parentUid&&a.has(n.parentUid)&&n.uid&&!a.has(n.uid)&&(a.add(n.uid),s=!0)}return a}function qs(e,t,a){const s=t.instanceId,n=a||!t.uid?new Set:_s(e,t.uid),r=e.state.tasks.filter(d=>d.uid&&d.instanceId===s&&!n.has(d.uid)&&d.uid!==t.uid),i=t.parentUid||"",l=['<option value="">None (top-level)</option>',...r.map(d=>`<option value="${o(d.uid)}" ${d.uid===i?"selected":""}>${o(d.summary||d.uid)}</option>`)];if(i&&!r.some(d=>d.uid===i)){const d=e.state.tasks.find(c=>c.uid===i);l.push(`<option value="${o(i)}" selected>${o((d==null?void 0:d.summary)||i)} (current)</option>`)}return l.join("")}function Ma(e){const t=new Set(e.state.checkedTaskKeys);return e.state.tasks.filter(a=>t.has(_(a.instanceId,a.uri))&&a.canWrite&&!a.readOnly)}function Rs(e){const t=p=>({"NEEDS-ACTION":"To do","IN-PROCESS":"In progress",COMPLETED:"Done",CANCELLED:"Cancelled"})[p]||p,a=Ls(e,e.state.tasks),s=e.state.tasks.filter(p=>p.canWrite&&!p.readOnly).map(p=>_(p.instanceId,p.uri)),n=s.length>0&&s.every(p=>e.state.checkedTaskKeys.includes(p)),r=e.state.checkedTaskKeys.length>0,l=Ma(e).length,d=e.state.tasks.length===0?`<tr class="contacts-empty-row"><td colspan="6" class="muted">${e.state.taskSearch?"No tasks match your search.":"No tasks yet. Add one below."}</td></tr>`:a.map(({task:p,depth:$})=>{const k=_(p.instanceId,p.uri),h=!e.state.creatingTask&&k===e.state.selectedTaskKey?" is-selected":"",v=e.state.checkedTaskKeys.includes(k),g=p.status==="COMPLETED"?"badge-ok":p.status==="CANCELLED"?"":"badge-admin",D=$>0?` style="--task-depth:${$}"`:"",U=$>0?'<span class="task-subtask-marker" aria-hidden="true">↳</span>':"",I=p.canWrite&&!p.readOnly;return`<tr class="contact-table-row task-row${$>0?" is-subtask":""}${h}${v?" is-checked":""}" data-action="select-task" data-instance="${p.instanceId}" data-uri="${o(p.uri)}" tabindex="0" role="button"${D}>
              ...
              <td class="col-task-due muted small">${o(va(p.due))}</td>
      ...
      ${y}
  ...
  </div>`}function Bs(e,t){if(!e.state.editingTask)return;const a=new FormData(t),s=String(a.get("due")??"").trim(),n=a.get("instanceId"),r=n!==null&&String(n)!==""?Number(n):e.state.editingTask.instanceId,i=String(a.get("parentUid")??"").trim();e.state.editingTask={...e.state.editingTask,instanceId:Number.isFinite(r)&&r>0?r:e.state.editingTask.instanceId,summary:String(a.get("summary")??e.state.editingTask.summary),description:String(a.get("description")??e.state.editingTask.description),status:String(a.get("status")??e.state.editingTask.status),due:s?new Date(s).toISOString():null,priority:Number(a.get("priority")??e.state.editingTask.priority??0),percent:Number(a.get("percent")??e.state.editingTask.percent??0),parentUid:i===""?null:i}}async function Vs(e,t){var r,i;const a=Ma(e);if(a.length===0){e.setFlash("error","No writable tasks selected"),e.render();return}const s=a.map(l=>({instanceId:l.instanceId,uri:l.uri}));if(t==="bulk-task-delete"){e.state.busy=!0,e.clearFlash(),e.render();try{const l=await S.bulkTasks({op:"delete",items:s});e.state.checkedTaskKeys=[],e.state.selectedTaskKey&&a.some(d=>_(d.instanceId,d.uri)===e.state.selectedTaskKey)&&(e.state.selectedTaskKey=null,e.state.editingTask=null,e.state.creatingTask=!1),await Xe(e),l.failed>0?e.setFlash("error",`Deleted ${l.ok}, failed ${l.failed}${l.errors[0]?`: ${l.errors[0]}`:""}`):e.setFlash("success",`Deleted ${l.ok} task${l.ok===1?"":"s"}`)}catch(l){e.setFlash("error",l instanceof Error?l.message:"Bulk delete failed")}finally{e.state.busy=!1,e.render()}return}let n={};if(t==="bulk-task-status"){const l=e.root.querySelector("#bulk-task-status"),d=((r=l==null?void 0:l.value)==null?void 0:r.trim())??"";if(!d){e.setFlash("error","Choose a status to apply"),e.render();return}n={status:d}}else if(t==="bulk-task-due"){const l=e.state.bulkDueValue.trim();if(!l){e.setFlash("error","Choose a due date to apply"),e.render();return}const d=/^\d{4}-\d{2}-\d{2}$/.test(l)?new Date(l+"T00:00:00"):new Date((l.length===16,l));if(Number.isNaN(d.getTime())){e.setFlash("error","Invalid due date"),e.render();return}n={due:d.toISOString()}}else if(t==="bulk-task-clear-due")n={due:null};else if(t==="bulk-task-percent"){const l=e.root.querySelector("#bulk-task-percent"),d=((i=l==null?void 0:l.value)==null?void 0:i.trim())??"";if(d===""){e.setFlash("error","Enter a percent complete (0–100)"),e.render();return}const c=Number(d);if(!Number.isFinite(c)||c<0||c>100){e.setFlash("error","Percent must be between 0 and 100"),e.render();return}n={percent:Math.round(c)}}e.state.busy=!0,e.clearFlash(),e.render();try{const l=await S.bulkTasks({op:"update",items:s,fields:n});if(await Xe(e),e.state.editingTask&&!e.state.creatingTask){const c=_(e.state.editingTask.instanceId,e.state.editingTask.uri),f=e.state.tasks.find(m=>_(m.instanceId,m.uri)===c);f&&(e.state.editingTask={...f})}const d=t==="bulk-task-status"?"status":t==="bulk-task-due"||t==="bulk-task-clear-due"?"due date":"percent";l.failed>0?e.setFlash("error",`Updated ${d} on ${l.ok}, failed ${l.failed}${l.errors[0]?`: ${l.errors[0]}`:""}`):e.setFlash("success",`Updated ${d} on ${l.ok} task${l.ok===1?"":"s"}`)}catch(l){e.setFlash("error",l instanceof Error?l.message:"Bulk update failed")}finally{e.state.busy=!1,e.render()}}async function Hs(e,t){const a=new FormData(t),s=String(a.get("summary")??"").trim(),n=String(a.get("description")??"").trim(),r=String(a.get("status")??"NEEDS-ACTION"),i=String(a.get("due")??"").trim(),l=i?new Date(i).toISOString():null,d=Number(a.get("priority")??0),c=Number(a.get("percent")??0),f=String(a.get("parentUid")??"").trim(),m=f===""?null:f;e.state.busy=!0,e.clearFlash(),e.render();try{if(e.state.creatingTask){const u=Number(a.get("instanceId"));if(!Number.isFinite(u)||u<=0)throw new Error("Select a calendar");const b=await S.createTask({instanceId:u,summary:s,description:n,status:r,due:l,priority:d,percent:c,parentUid:m});e.state.creatingTask=!1,e.state.selectedTaskKey=_(b.task.instanceId,b.task.uri),e.state.editingTask=b.task,e.setFlash("success",te(m?"Subtask":"Task",b.task.summary||s,"created"))}else if(e.state.editingTask){const u=await S.updateTask(e.state.editingTask.instanceId,e.state.editingTask.uri,{summary:s,description:n,status:r,due:l,priority:d,percent:c,parentUid:m});e.state.editingTask=u.task,e.state.selectedTaskKey=_(u.task.instanceId,u.task.uri),e.setFlash("success",te("Task",u.task.summary||s,"saved"))}await Xe(e)}catch(u){e.setFlash("error",u instanceof Error?u.message:"Save failed")}finally{e.state.busy=!1,e.render()}}async function zs(e,t,a,s){var d;const{state:n,render:r,setFlash:i,clearFlash:l}=e;if(t==="sort-task"){const c=a.dataset.sort||"";if(!c)return!0;n.taskSort===c?n.taskOrder=n.taskOrder==="asc"?"desc":"asc":(n.taskSort=c,n.taskOrder=c==="due"||c==="summary"?"asc":"desc"),n.busy=!0,r();try{await e.loadTasks()}catch(f){i("error",f instanceof Error?f.message:"Sort failed")}finally{n.busy=!1,r()}return!0}if(t==="select-task"){if(s.target.closest("[data-stop-row], .task-check"))return!0;const c=Number(a.dataset.instance),f=a.dataset.uri??"";if(!Number.isFinite(c)||!f)return!0;const m=n.tasks.find(u=>u.instanceId===c&&u.uri===f)??null;return n.creatingTask=!1,n.selectedTaskKey=e.itemKey(c,f),n.editingTask=m?{...m}:null,l(),r(),!0}if(t==="task-check"){s.preventDefault(),s.stopPropagation();const c=Number(a.dataset.instance),f=a.dataset.uri??"";if(!Number.isFinite(c)||!f)return!0;const m=e.itemKey(c,f),u=n.tasks.find(b=>e.itemKey(b.instanceId,b.uri)===m);return!u||!u.canWrite||u.readOnly||(n.checkedTaskKeys.includes(m)?n.checkedTaskKeys=n.checkedTaskKeys.filter(b=>b!==m):n.checkedTaskKeys=[...n.checkedTaskKeys,m],r()),!0}if(t==="task-select-all"){s.preventDefault();const c=n.tasks.filter(m=>m.canWrite&&!m.readOnly);return c.length>0&&c.every(m=>n.checkedTaskKeys.includes(e.itemKey(m.instanceId,m.uri)))?n.checkedTaskKeys=[]:n.checkedTaskKeys=c.map(m=>e.itemKey(m.instanceId,m.uri)),r(),!0}if(t==="bulk-task-clear")return n.checkedTaskKeys=[],r(),!0;if(t==="bulk-task-status"||t==="bulk-task-due"||t==="bulk-task-clear-due"||t==="bulk-task-percent"||t==="bulk-task-delete"){if(t==="bulk-task-delete"){const c=n.checkedTaskKeys.length;return c===0?(i("error","No tasks selected"),r(),!0):(n.confirmDelete={scope:"bulk-task",title:c===1?"Delete task":`Delete ${c} tasks`,message:c===1?"Delete the selected task?":`Delete ${c} selected tasks?`,detail:"CalDAV clients will sync the removal. This cannot be undone.",count:c},r(),!0)}return e.runBulkTaskAction(t),!0}if(t==="new-task")return n.creatingTask=!0,n.selectedTaskKey=null,n.editingTask={uri:"",instanceId:((d=n.taskCalendars[0])==null?void 0:d.id)??0,calendarId:0,calendarName:"",calendarUri:"",uid:"",parentUid:null,summary:"",description:"",status:"NEEDS-ACTION",due:null,priority:0,percent:0,completed:null,lastmodified:0,readOnly:!1,canWrite:!0},l(),r(),!0;if(t==="new-subtask"){if(!n.editingTask||n.creatingTask||!n.editingTask.uid||!n.editingTask.canWrite)return!0;const c=n.editingTask;return n.creatingTask=!0,n.selectedTaskKey=null,n.editingTask={uri:"",instanceId:c.instanceId,calendarId:c.calendarId,calendarName:c.calendarName,calendarUri:c.calendarUri,uid:"",parentUid:c.uid,summary:"",description:"",status:"NEEDS-ACTION",due:null,priority:0,percent:0,completed:null,lastmodified:0,readOnly:!1,canWrite:!0},l(),r(),!0}if(t==="cancel-task")return n.creatingTask=!1,n.editingTask=null,n.selectedTaskKey=null,r(),!0;if(t==="delete-task"){if(!n.editingTask||n.creatingTask)return!0;const c=String(n.editingTask.summary||"this task").trim()||"this task";return n.confirmDelete={scope:"task",title:"Delete task",message:`Delete “${c}”?`,detail:"CalDAV clients will sync the removal. This cannot be undone."},r(),!0}return!1}async function xt(e,t){const a=await S.contacts(t,e.state.contactSearch);e.state.contacts=a.contacts,e.state.selectedContactUri!==null&&!e.state.contacts.some(s=>s.uri===e.state.selectedContactUri)&&(e.state.selectedContactUri=null,e.state.creatingContact||(e.state.editingContact=null,e.state.photoPreview=null,e.state.photoBase64Pending=null,e.state.removePhotoPending=!1))}async function Ks(e,t){if(e.state.selectedAbId===null)return;const a=await S.getContact(e.state.selectedAbId,t);e.state.selectedContactUri=t,e.state.creatingContact=!1;const s=a.contact;e.state.editingContact={...s,emails:Array.isArray(s.emails)?s.emails:[],phones:Array.isArray(s.phones)?s.phones:[],custom:Array.isArray(s.custom)?s.custom:[],address:s.address??xa(),birthday:s.birthday??null},e.state.photoPreview=s.photoDataUri??(s.hasPhoto&&e.state.selectedAbId!==null?`${S.contactPhotoUrl(e.state.selectedAbId,t)}?t=${Date.now()}`:null),e.state.photoBase64Pending=null,e.state.removePhotoPending=!1,e.state.contactModalOpen=!0}function js(e){e.state.creatingContact=!0,e.state.selectedContactUri=null,e.state.contactModalOpen=!0,e.state.editingContact={uri:"",displayname:"",firstname:"",lastname:"",fullname:"",org:"",title:"",emails:[""],phones:[{type:"cell",value:""}],address:{street:"",city:"",region:"",postal:"",country:""},birthday:null,url:"",note:"",custom:[],hasPhoto:!1,photoDataUri:null},e.state.photoPreview=null,e.state.photoBase64Pending=null,e.state.removePhotoPending=!1}function xa(e){return{street:"",city:"",region:"",postal:"",country:""}}function Ws(e,t){return new Promise((a,s)=>{const n=new FileReader;n.onload=()=>{const r=String(n.result??""),i=r.indexOf(",");a(i>=0?r.slice(i+1):r)},n.onerror=()=>s(new Error("Failed to read photo file")),n.readAsDataURL(t)})}function Js(e){const t=e.root.querySelector('input[data-action="contact-photo"]');t&&t.addEventListener("change",()=>{(async()=>{var s;const a=(s=t.files)==null?void 0:s[0];if(t.value="",!!a){if(a.size>2.5*1024*1024){e.setFlash("error","Photo is too large (max ~2 MB)"),e.render();return}try{const n=await Ws(e,a);e.state.photoBase64Pending=n,e.state.photoPreview=`data:${a.type||"image/jpeg"};base64,${n}`,e.state.removePhotoPending=!1,e.render()}catch(n){e.setFlash("error",n instanceof Error?n.message:"Failed to read photo"),e.render()}}})()})}async function Ys(e,t){var n;if(e.state.selectedAbId===null)return;const a=(n=t.files)==null?void 0:n[0];if(t.value="",!a)return;const s=e.state.selectedAbId;e.state.abModalOpen=!0,e.state.busy=!0,e.clearFlash(),e.stopImportElapsedTimer(),e.state.importProgress={kind:"contacts",fileName:a.name,fileSizeLabel:Je(a.size),phase:"reading",readPercent:0,processPercent:null,processCurrent:0,processTotal:0,processImported:0,processUpdated:0,processSkipped:0,startedAt:Date.now(),elapsedSec:0,resultMessage:null,ok:null},e.startImportElapsedTimer(),e.render();try{const r=await e.readFileTextWithProgress(a,d=>{if(!e.state.importProgress||e.state.importProgress.phase!=="reading")return;e.state.importProgress={...e.state.importProgress,readPercent:d};const c=e.root.querySelector(".import-progress-bar"),f=e.root.querySelector("[data-import-status-line]");c&&d!==null&&(c.classList.remove("is-indeterminate"),c.style.width=`${d}%`),f&&d!==null&&(f.textContent=`Reading file… ${d}%`)});e.setImportPhase("uploading",{readPercent:100}),e.setImportPhase("processing",{processPercent:0}),w.event("import.contacts.start",{file:a.name,bytes:a.size,abId:s});const i=await S.importAddressBook(s,r,d=>{e.applyServerImportProgress(d)}),l=e.formatImportResult(i);await e.loadHome(),e.state.selectedAbId===s&&await xt(e,s),e.stopImportElapsedTimer(),e.setImportPhase("done",{ok:!0,resultMessage:`${l} (from “${a.name}”)`}),e.setFlash("success",`Import finished for “${a.name}”: ${l}.`)}catch(r){const i=r instanceof Error?r.message:"Import failed";e.stopImportElapsedTimer(),e.setImportPhase("error",{ok:!1,resultMessage:i}),e.setFlash("error",i)}finally{e.state.busy=!1,e.render()}}async function Gs(e,t){if(e.state.selectedAbId===null)return;const a=Fs(e,t),s=ut(a);e.state.busy=!0,e.clearFlash(),e.state.contactModalOpen=!0,e.render();try{if(e.state.creatingContact){const n=await S.createContact(e.state.selectedAbId,a);e.state.creatingContact=!1,e.state.selectedContactUri=n.contact.uri,e.state.editingContact=null,e.state.contactModalOpen=!1,e.state.photoPreview=null,e.state.photoBase64Pending=null,e.state.removePhotoPending=!1,e.state.eventDtPicker=null,e.setFlash("success",te("Contact",ut(n.contact)||s,"created"))}else if(e.state.selectedContactUri){const n=await S.updateContact(e.state.selectedAbId,e.state.selectedContactUri,a);e.state.selectedContactUri=n.contact.uri,e.state.editingContact=null,e.state.contactModalOpen=!1,e.state.photoPreview=null,e.state.photoBase64Pending=null,e.state.removePhotoPending=!1,e.state.eventDtPicker=null,e.setFlash("success",te("Contact",ut(n.contact)||s,"saved"))}try{await e.loadHome()}catch(n){if(console.error(n),e.state.selectedAbId!==null)try{await xt(e,e.state.selectedAbId)}catch{}}}catch(n){e.setFlash("error",n instanceof Error?n.message:"Save failed")}finally{e.state.busy=!1,e.render()}}async function Qs(e,t){const a=new FormData(t),s=String(a.get("displayname")??"").trim(),n=String(a.get("description")??"").trim();if(s){e.state.busy=!0,e.clearFlash(),e.render();try{const r=await S.createAddressBook({displayname:s,description:n});e.state.selectedAbId=r.addressbook.id,e.state.selectedContactUri=null,e.state.editingContact=null,e.state.creatingContact=!1,e.state.contactSearch="",await e.loadHome(),e.setFlash("success",`Address book “${r.addressbook.displayname}” created`)}catch(r){e.setFlash("error",r instanceof Error?r.message:"Create failed")}finally{e.state.busy=!1,e.render()}}}async function Xs(e,t){if(e.state.selectedAbId===null)return;const a=new FormData(t),s=String(a.get("displayname")??"").trim(),n=String(a.get("description")??"").trim();e.state.abModalOpen=!0,e.state.busy=!0,e.clearFlash(),e.render();try{await S.updateAddressBook(e.state.selectedAbId,{displayname:s,description:n}),await e.loadHome(),e.setFlash("success",te("Address book",s,"updated"))}catch(r){e.setFlash("error",r instanceof Error?r.message:"Update failed")}finally{e.state.busy=!1,e.render()}}function Zs(e){const{state:t}=e,a=t.addressBooks.map(k=>`<div class="cal-row${k.id===t.selectedAbId?" is-selected":""}" data-action="select-ab" data-id="${k.id}" role="button" tabindex="0">
        ...
        <span class="cal-row-actions">
          ...
          <button type="button" class="btn btn-small btn-danger" data-action="delete-ab" data-id="${k.id}" ${t.busy?"disabled":""}>Delete</button>
            ...
            </tr>`}).join(""),r=t.editingContact,i=Array.isArray(r==null?void 0:r.emails)&&r.emails.length>0?r.emails:[""],l=Array.isArray(r==null?void 0:r.phones)&&r.phones.length>0?r.phones:[{type:"cell",value:""}],d=(r==null?void 0:r.address)??e.emptyAddress(),c=i.map((k,h)=>`<div class="multi-row" data-multi="email" data-idx="${h}">
        <input type="email" name="email_${h}" value="${o(k??"")}" placeholder="email@example.com" autocomplete="off" />
        <button type="button" class="btn btn-ghost btn-small" data-action="remove-email" data-idx="${h}" ${i.length<=1?"disabled":""} title="Remove">×</button>
      </div>`).join(""),f=l.map((k,h)=>`<div class="multi-row multi-row-phone" data-multi="phone" data-idx="${h}">
        <select name="phone_type_${h}" aria-label="Phone type">
          ${["cell","work","home","other"].map(v=>`<option value="${v}" ${((k==null?void 0:k.type)??"other")===v?"selected":""}>${v}</option>`).join("")}
        </select>
        <input type="tel" name="phone_value_${h}" value="${o((k==null?void 0:k.value)??"")}" placeholder="+1…" autocomplete="off" />
        <button type="button" class="btn btn-ghost btn-small" data-action="remove-phone" data-idx="${h}" ${l.length<=1?"disabled":""} title="Remove">×</button>
      </div>`).join(""),m=Array.isArray(r==null?void 0:r.custom)?r.custom:[],u=m.length===0?'<p class="muted small" style="margin:0 0 0.5rem">No custom fields yet. Labels and values can use any language (e.g. Супруг, 日本語).</p>':m.map((k,h)=>`<div class="multi-row multi-row-custom" data-multi="custom" data-idx="${h}">
              <input type="text" name="custom_label_${h}" value="${o(k.label||"")}" placeholder="Label (any language)" maxlength="64" autocomplete="off" aria-label="Custom field label" />
              <input type="text" name="custom_value_${h}" value="${o(k.value||"")}" placeholder="Value" maxlength="2000" autocomplete="off" aria-label="Custom field value" />
              <button type="button" class="btn btn-ghost btn-small" data-action="remove-custom" data-idx="${h}" title="Remove">×</button>
    ...
    ${b}`}async function er(e,t,a,s){var c,f;const{state:n,root:r,render:i,setFlash:l,clearFlash:d}=e;if(t==="select-ab"){const m=Number(a.dataset.id);if(!Number.isFinite(m))return!0;n.selectedAbId=m,n.abModalOpen=!1,n.selectedContactUri=null,n.editingContact=null,n.creatingContact=!1,n.contactModalOpen=!1,n.contactSearch="",n.contacts=[],n.photoPreview=null,n.photoBase64Pending=null,n.removePhotoPending=!1,d(),n.busy=!0,i();try{await e.loadContacts(m)}catch(u){l("error",u instanceof Error?u.message:"Failed to load contacts")}finally{n.busy=!1,i()}return!0}if(t==="edit-ab"){s.stopPropagation();const m=Number(a.dataset.id);if(!Number.isFinite(m)||!n.addressBooks.find(y=>y.id===m))return!0;const b=n.selectedAbId!==m;n.selectedAbId=m,n.abModalOpen=!0,n.contactModalOpen=!1,d(),b&&(n.selectedContactUri=null,n.editingContact=null,n.creatingContact=!1,n.contactSearch="",n.contacts=[],n.photoPreview=null,n.photoBase64Pending=null,n.removePhotoPending=!1),n.busy=!0,i();try{b&&await e.loadContacts(m)}catch(y){l("error",y instanceof Error?y.message:"Failed to open address book")}finally{n.busy=!1,i()}return!0}if(t==="close-ab-modal")return n.abModalOpen=!1,i(),!0;if(t==="select-contact"){const m=a.dataset.uri??"";if(!m)return!0;d();try{await e.openContact(m)}catch(u){l("error",u instanceof Error?u.message:"Failed to load contact")}return i(),!0}if(t==="new-contact")return n.selectedAbId===null||(e.startNewContact(),d(),i()),!0;if(t==="cancel-contact"||t==="close-contact-modal")return n.creatingContact=!1,n.contactModalOpen=!1,n.editingContact=null,n.selectedContactUri=null,n.photoPreview=null,n.photoBase64Pending=null,n.removePhotoPending=!1,n.eventDtPicker=null,d(),i(),!0;if(t==="add-email"||t==="add-phone"||t==="add-custom")return n.editingContact&&(Ae(e.contactsHost),Array.isArray(n.editingContact.emails)||(n.editingContact.emails=[""]),Array.isArray(n.editingContact.phones)||(n.editingContact.phones=[{type:"cell",value:""}]),Array.isArray(n.editingContact.custom)||(n.editingContact.custom=[]),t==="add-email"?n.editingContact.emails.length<10&&n.editingContact.emails.push(""):t==="add-phone"?n.editingContact.phones.length<10&&n.editingContact.phones.push({type:"other",value:""}):n.editingContact.custom.length<30&&n.editingContact.custom.push({label:"",value:""}),i()),!0;if(t==="remove-email"){if(!n.editingContact)return!0;Ae(e.contactsHost);const m=Number(a.dataset.idx);if(!Number.isFinite(m))return!0;const u=Array.isArray(n.editingContact.emails)?n.editingContact.emails:[""];return n.editingContact.emails=u.filter((b,y)=>y!==m),n.editingContact.emails.length===0&&(n.editingContact.emails=[""]),i(),!0}if(t==="remove-phone"){if(!n.editingContact)return!0;Ae(e.contactsHost);const m=Number(a.dataset.idx);if(!Number.isFinite(m))return!0;const u=Array.isArray(n.editingContact.phones)?n.editingContact.phones:[{type:"cell",value:""}];return n.editingContact.phones=u.filter((b,y)=>y!==m),n.editingContact.phones.length===0&&(n.editingContact.phones=[{type:"cell",value:""}]),i(),!0}if(t==="remove-custom"){if(!n.editingContact)return!0;Ae(e.contactsHost);const m=Number(a.dataset.idx);return Number.isFinite(m)&&(n.editingContact.custom=(Array.isArray(n.editingContact.custom)?n.editingContact.custom:[]).filter((u,b)=>b!==m),i()),!0}if(t==="remove-photo")return n.photoPreview=null,n.photoBase64Pending=null,n.removePhotoPending=!0,n.editingContact&&(n.editingContact.hasPhoto=!1),i(),!0;if(t==="delete-contact"){if(n.selectedAbId===null||!n.selectedContactUri)return!0;const m=String(((c=n.editingContact)==null?void 0:c.fullname)||((f=n.editingContact)==null?void 0:f.displayname)||"this contact").trim()||"this contact";return n.confirmDelete={scope:"contact",title:"Delete contact",message:`Delete “${m}”?`,detail:"CardDAV clients will sync the removal. This cannot be undone."},i(),!0}if(t==="delete-ab"){s.stopPropagation();const m=Number(a.dataset.id??n.selectedAbId);return!Number.isFinite(m)||!n.addressBooks.find(b=>b.id===m)||(n.deleteAbConfirmId=m,n.abModalOpen=!1,n.contactModalOpen=!1,d(),i()),!0}if(t==="cancel-delete-ab")return n.deleteAbConfirmId=null,i(),!0;if(t==="confirm-delete-ab"){const m=Number(a.dataset.id),u=r.querySelector("#delete-ab-confirm");if(!Number.isFinite(m)||!(u!=null&&u.checked))return!0;const b=n.addressBooks.find(p=>p.id===m);if(!b)return!0;const y=(b.cardCount??0)>0;n.busy=!0,d(),i();try{await S.deleteAddressBook(m,y),n.selectedAbId===m&&(n.selectedAbId=null,n.contacts=[],n.editingContact=null,n.selectedContactUri=null,n.creatingContact=!1),n.deleteAbConfirmId=null,n.abModalOpen=!1,n.contactModalOpen=!1,await e.loadHome(),n.selectedAbId===null&&n.addressBooks.length>0&&(n.selectedAbId=n.addressBooks[0].id,await e.loadContacts(n.selectedAbId)),l("success","Address book deleted")}catch(p){l("error",p instanceof Error?p.message:"Delete failed")}finally{n.busy=!1,i()}return!0}if(t==="export-ab"){s.stopPropagation();const m=a.dataset.id,u=m!==void 0&&m!==""?Number(m):n.selectedAbId;if(u===null||Number.isNaN(u))return!0;n.busy=!0,d(),i();try{const{blob:b,filename:y}=await S.exportAddressBook(u),p=await e.saveBlobAsFile(b,y);p==="cancelled"?l("info","Export cancelled"):p==="saved"?l("success",`Saved ${y}`):l("success",`Download started: ${y}`)}catch(b){l("error",b instanceof Error?b.message:"Export failed")}finally{n.busy=!1,i()}return!0}if(t==="export-contact"){if(n.selectedAbId===null||!n.selectedContactUri||n.creatingContact)return!0;n.contactModalOpen=!0,n.busy=!0,d(),i();try{const{blob:m,filename:u}=await S.exportContact(n.selectedAbId,n.selectedContactUri),b=await e.saveBlobAsFile(m,u);b==="cancelled"?l("info","Export cancelled"):b==="saved"?l("success",`Saved ${u}`):l("success",`Download started: ${u}`)}catch(m){l("error",m instanceof Error?m.message:"Export failed")}finally{n.busy=!1,i()}return!0}return!1}function Lt(e){return e==="calendars"||e==="contacts"||e==="tasks"||e==="notes"||e==="files"||e==="admin"?e:null}function La(e){return e==="overview"||e==="users"||e==="settings"||e==="database"?e:null}function _t(){const e=(typeof location<"u"?location.hash:"").replace(/^#/,"").split(/[?&]/)[0].replace(/^\/+/,"");if(!e)return{tab:null,adminPage:null,adminUsername:null};if(e==="admin"||e.startsWith("admin/")){const t=e.split("/").filter(Boolean),a=t[1]??"overview",s=La(a)??"overview";let n=null;if(s==="users"&&t[2])try{n=decodeURIComponent(t[2])}catch{n=t[2]}return{tab:"admin",adminPage:s,adminUsername:n}}return{tab:Lt(e),adminPage:null,adminUsername:null}}function tr(){const e=_t().tab;if(e)return e;try{const t=Lt(sessionStorage.getItem(oa));if(t)return t}catch{}return"calendars"}function ar(){const e=_t().adminPage;if(e)return e;try{const t=La(sessionStorage.getItem(ca));if(t)return t}catch{}return"overview"}function nr(e,t=null){return e==="overview"?"#admin":e==="users"&&t?`#admin/users/${encodeURIComponent(t)}`:`#admin/${e}`}function pt(e,t="overview",a=null){try{sessionStorage.setItem(oa,e),e==="admin"&&sessionStorage.setItem(ca,t)}catch{}if(typeof history>"u"||typeof location>"u")return;const s=e==="admin"?nr(t,a):`#${e}`;location.hash!==s&&history.replaceState(null,"",`${location.pathname}${location.search}${s}`)}function ea(e){return e==="readwrite"?'<span class="badge badge-admin">full access</span>':e==="read"?'<span class="badge">read-only</span>':e==="owner"?'<span class="badge badge-ok">owner</span>':`<span class="badge">${o(e)}</span>`}function ft(e){const t=[`${e.imported} new`,`${e.updated} updated`];return e.skipped>0&&t.push(`${e.skipped} skipped`),t.join(", ")}function sr(e){const{state:t,root:a}=e;if(!t.user){e.renderLogin();return}let s;switch(t.activeTab){case"calendars":s=Zt(e);break;case"contacts":s=Zs(e);break;case"tasks":s=e.renderTasksTab();break;case"notes":s=e.renderNotesTab();break;case"files":s=e.renderFilesTab();break;case"admin":s=e.renderAdminSection();break;default:s=Zt(e)}const n=t.activeTab==="calendars"?"my-calendars":t.activeTab==="contacts"?"my-contacts":t.activeTab==="tasks"?"tasks":t.activeTab==="notes"?"notes":t.activeTab==="files"?"files":"administration",r=t.activeTab==="admin"?`<div class="tabs" role="tablist" aria-label="Administration sections">
          ...
          <button type="button" class="info-btn tab-info" data-action="info"
            data-info="${n}"
            aria-label="About this tab" title="About this tab"><span aria-hidden="true">i</span></button>
        </div>`;a.innerHTML=e.shell(s,{tabs:r}),document.body.classList.toggle("cal-modal-open",t.calModalOpen||t.createCalModalOpen||t.deleteConfirmId!==null||t.deleteAbConfirmId!==null||t.eventModalOpen||t.contactModalOpen||t.abModalOpen||t.importProgress!==null||t.filesUploadProgress!==null||t.filesRenamePath!==null||t.filesDeletePaths!==null||t.filesTransfer!==null||t.filesMkdirOpen||t.filesUploadConflict!==null||t.confirmDelete!==null||t.adminUserCreateOpen||t.adminUserEditOpen||t.adminUserDeleteUsername!==null||t.adminResetModalOpen||t.adminDbConfirmOpen||t.adminCalModal!==null||t.adminAbModal!==null||t.adminResourceDelete!==null),document.body.classList.toggle("layout-contacts",t.activeTab==="contacts"),document.body.classList.toggle("layout-calendars",t.activeTab==="calendars"),document.body.classList.toggle("layout-tasks",t.activeTab==="tasks"||t.activeTab==="notes"),document.body.classList.toggle("layout-files",t.activeTab==="files"),document.body.classList.toggle("layout-admin",t.activeTab==="admin")}async function rr(e,t,a,s){const{state:n,render:r,clearFlash:i,setFlash:l}=e;if(t==="confirm-delete-cancel")return Wt(n),r(),!0;if(t==="confirm-delete-ok"){const d=n.confirmDelete;if(!d)return r(),!0;const c=d.scope;if(Wt(n),c==="event"){if(!n.editingEvent||!n.editingEvent.canWrite||n.creatingEvent)return r(),!0;const f=n.editingEvent.instanceId,m=n.editingEvent.uri;n.busy=!0,i(),r();try{await S.deleteEvent(f,m),n.eventModalOpen=!1,n.editingEvent=null,await e.loadMonthEvents(),l("success","Event deleted")}catch(u){l("error",u instanceof Error?u.message:"Delete failed")}finally{n.busy=!1,r()}return!0}if(c==="task"){if(!n.editingTask||n.creatingTask)return r(),!0;n.busy=!0,i(),r();try{await S.deleteTask(n.editingTask.instanceId,n.editingTask.uri),n.selectedTaskKey=null,n.editingTask=null,await e.loadTasks(),l("success","Task deleted")}catch(f){l("error",f instanceof Error?f.message:"Delete failed")}finally{n.busy=!1,r()}return!0}if(c==="note"){if(!n.editingNote||n.creatingNote)return r(),!0;n.busy=!0,i(),r();try{await S.deleteNote(n.editingNote.instanceId,n.editingNote.uri),n.selectedNoteKey=null,n.editingNote=null,await e.loadNotes(),l("success","Note deleted")}catch(f){l("error",f instanceof Error?f.message:"Delete failed")}finally{n.busy=!1,r()}return!0}if(c==="contact"){if(n.selectedAbId===null||!n.selectedContactUri)return r(),!0;n.busy=!0,i(),n.contactModalOpen=!0,r();try{await S.deleteContact(n.selectedAbId,n.selectedContactUri),n.selectedContactUri=null,n.editingContact=null,n.creatingContact=!1,n.contactModalOpen=!1,n.eventDtPicker=null,n.photoPreview=null,await e.loadHome(),l("success","Contact deleted")}catch(f){l("error",f instanceof Error?f.message:"Delete failed")}finally{n.busy=!1,r()}return!0}if(c==="bulk-task")return await e.runBulkTaskAction("bulk-task-delete"),!0;if(c==="revoke-share"){const f=d.href??"";if(!f||n.selectedId===null)return r(),!0;n.calModalOpen=!0,n.busy=!0,i(),r();try{await S.revoke(n.selectedId,f),await e.loadShares(n.selectedId),l("success","Share revoked")}catch(m){l("error",m instanceof Error?m.message:"Revoke failed")}finally{n.busy=!1,r()}return!0}return r(),!0}if(t==="close-import-progress")return n.importProgress&&(n.importProgress.phase==="done"||n.importProgress.phase==="error")&&e.closeImportProgress(),!0;if(t==="logout"){n.busy=!0,w.event("logout");try{await S.logout()}catch{}return e.clearPortalSessionState(),i(),r(),!0}if(t==="info"){const d=a.dataset.info??"";return e.openInfoModal(d),!0}if(t==="info-close")return e.closeInfoModal(),!0;if(t==="flash-close")return i(),r(),!0;if(t==="user-menu-toggle")return s.stopPropagation(),n.userMenuOpen=!n.userMenuOpen,r(),!0;if(t==="user-menu-close")return n.userMenuOpen&&(n.userMenuOpen=!1,r()),!0;if(t==="tab"){const d=Lt(a.dataset.tab);return d&&(d==="admin"&&(n.adminPage="overview"),await e.activateTab(d)),!0}return!1}async function ta(e,t){const a=t.target.closest("[data-action]");if(!a)return;const s=a.dataset.action;s&&(w.debug(`action:${s}`,{id:a.dataset.id,tab:a.dataset.tab,uri:a.dataset.uri}),!await rr(e,s,a,t)&&(s.startsWith("admin-")&&await ns(e.adminHost,s,a)||(s.startsWith("files-")||s==="close-files-upload-progress")&&await In(e.filesHost,s,a,t)||await Is(e,s,a,t)||await zs(e,s,a,t)||await xs(e,s,a)||await er(e,s,a,t)))}function ir(e){const{state:t,root:a,render:s,setFlash:n}=e;a.querySelectorAll("[data-action]").forEach(C=>{C.addEventListener("click",P=>{const A=P.target.closest("[data-action]");((A==null?void 0:A.dataset.action)==="info"||(A==null?void 0:A.dataset.action)==="info-close")&&(P.preventDefault(),P.stopPropagation()),ta(e,P)})}),a.querySelectorAll('select[data-action="dt-set-month"], select[data-action="dt-set-year"]').forEach(C=>{C.addEventListener("change",P=>{P.stopPropagation(),ta(e,P)}),C.addEventListener("click",P=>{P.stopPropagation()})}),e.unbindUserMenuOutside(),t.userMenuOpen&&e.bindUserMenuOutside(),We(t),t.eventDtPicker&&mn(t,s),e.unbindFilesUploadMenuOutside(),t.filesUploadMenuOpen&&e.bindFilesUploadMenuOutside(),a.querySelectorAll("tr.contact-table-row[data-action], .cal-row[data-action], .month-cell[data-action]").forEach(C=>{C.addEventListener("keydown",P=>{(P.key==="Enter"||P.key===" ")&&(P.preventDefault(),C.click())})});const r=a.querySelector("#delete-cal-confirm"),i=a.querySelector("#delete-cal-submit");r==null||r.addEventListener("change",()=>{i&&(i.disabled=!r.checked||t.busy)});const l=a.querySelector("#delete-ab-confirm"),d=a.querySelector("#delete-ab-submit");l==null||l.addEventListener("change",()=>{d&&(d.disabled=!l.checked||t.busy)}),a.querySelectorAll("img.contact-avatar[data-avatar-fallback]").forEach(C=>{C.addEventListener("error",()=>{const P=C.dataset.avatarFallback||"?",A=document.createElement("span");A.className="contact-avatar contact-avatar-fallback",A.setAttribute("aria-hidden","true"),A.textContent=P,C.replaceWith(A)})});const c=a.querySelector('[data-form="login"]');c==null||c.addEventListener("submit",C=>{C.preventDefault(),e.onLogin(c)});const f=a.querySelector('[data-form="share"]');f==null||f.addEventListener("submit",C=>{C.preventDefault(),e.onShare(f)});const m=a.querySelector('[data-form="edit-event"]');m==null||m.addEventListener("submit",C=>{C.preventDefault(),e.onSaveEvent(m)}),m==null||m.querySelectorAll('select[name="repeatFreq"], select[name="repeatEndMode"]').forEach(C=>{C.addEventListener("change",()=>{if(!t.editingEvent)return;const P=new FormData(m);t.editingEvent={...t.editingEvent,repeat:Ie(P),hasRrule:!!String(P.get("repeatFreq")??"").trim()},s()})});const u=a.querySelector('[data-form="edit-cal"]');u==null||u.addEventListener("submit",C=>{C.preventDefault(),e.onEditCal(u)}),u&&e.bindColorPair(u);const b=a.querySelector('[data-form="create-cal"]');b==null||b.addEventListener("submit",C=>{C.preventDefault(),e.onCreateCal(b)}),b&&e.bindColorPair(b);const y=a.querySelector('[data-form="contact"]');y==null||y.addEventListener("submit",C=>{C.preventDefault(),e.onSaveContact(y)});const p=a.querySelector('[data-form="create-ab"]');p==null||p.addEventListener("submit",C=>{C.preventDefault(),e.onCreateAb(p)});const $=a.querySelector('[data-form="edit-ab"]');$==null||$.addEventListener("submit",C=>{C.preventDefault(),e.onEditAb($)});const k=a.querySelector('[data-form="task"]');if(k==null||k.addEventListener("submit",C=>{C.preventDefault(),e.onSaveTask(k)}),k){const C=k.querySelector('select[name="instanceId"]');C==null||C.addEventListener("change",()=>{if(!t.creatingTask||!t.editingTask)return;const P=Number(C.value);if(!Number.isFinite(P)||P<=0)return;e.syncEditingTaskFromForm(k);const A=t.editingTask.parentUid;t.editingTask={...t.editingTask,instanceId:P,parentUid:A&&t.tasks.some(q=>q.uid===A&&q.instanceId===P)?A:null},s()})}const h=a.querySelector('[data-form="note"]');if(h==null||h.addEventListener("submit",C=>{C.preventDefault(),e.onSaveNote(h)}),h){const C=h.querySelector('select[name="instanceId"]');C==null||C.addEventListener("change",()=>{if(!t.creatingNote||!t.editingNote)return;const P=Number(C.value);!Number.isFinite(P)||P<=0||(e.syncEditingNoteFromForm(h),t.editingNote={...t.editingNote,instanceId:P},s())})}const v=a.querySelector('input[data-action="contact-search"]');v==null||v.addEventListener("input",()=>{t.searchTimer&&clearTimeout(t.searchTimer),t.searchTimer=setTimeout(()=>{t.contactSearch=v.value,(async()=>{try{t.selectedAbId!==null&&await e.loadContacts(t.selectedAbId),s()}catch(C){n("error",C instanceof Error?C.message:"Search failed"),s()}})()},250)});const g=a.querySelector('input[data-action="task-search"]');g==null||g.addEventListener("input",()=>{t.searchTimer&&clearTimeout(t.searchTimer),t.searchTimer=setTimeout(()=>{t.taskSearch=g.value,(async()=>{try{await e.loadTasks(),s()}catch(C){n("error",C instanceof Error?C.message:"Search failed"),s()}})()},250)});const D=a.querySelector('input[data-action="note-search"]');D==null||D.addEventListener("input",()=>{t.searchTimer&&clearTimeout(t.searchTimer),t.searchTimer=setTimeout(()=>{t.noteSearch=D.value,(async()=>{try{await e.loadNotes(),s()}catch(C){n("error",C instanceof Error?C.message:"Search failed"),s()}})()},250)});const U=a.querySelector('select[data-action="admin-db-backend"]');U==null||U.addEventListener("change",()=>{t.adminDbFormBackend=U.value==="pgsql"?"pgsql":"sqlite",s()});const I=a.querySelector('input[data-action="admin-db-confirm-input"]');I==null||I.addEventListener("input",()=>{t.adminDbConfirmText=I.value;const C=a.querySelector('[data-action="admin-db-confirm-save"]');C&&(C.disabled=t.busy||t.adminDbConfirmText.trim()!=="CONFIRM")});const O=a.querySelector('input[data-action="admin-reset-password"]');O==null||O.addEventListener("input",()=>{t.adminResetPassword=O.value;const C=a.querySelector('[data-action="admin-reset-confirm"]');C&&(C.disabled=t.busy||!t.adminResetConfirmChecked||t.adminResetPassword.trim()==="")}),ha(e.filesHost),Ea(e.adminHost),e.bindImportInput(),e.bindHolidaysToggle(),e.bindContactPhotoInput()}const aa=new WeakMap;function lr(e){if(aa.has(e.root)){w.debug("portalEvents: already bound for root");return}aa.set(e.root,!0),e.state.portalEventsBound=!0,e.state.escapeBound=!0;const{root:t}=e;t.addEventListener("click",a=>dr(e,a)),t.addEventListener("submit",a=>or(e,a)),t.addEventListener("change",a=>cr(e,a)),t.addEventListener("input",a=>ur(e,a)),t.addEventListener("keydown",a=>mr(e,a)),document.addEventListener("keydown",a=>pr(e,a)),w.event("portalEvents.registered")}function dr(e,t){var s,n;const a=(n=(s=t.target)==null?void 0:s.closest)==null?void 0:n.call(s,"[data-action]");!a||!e.root.contains(a)||w.debug("portalEvents.click.scaffold",{action:a.dataset.action})}function or(e,t){var s,n;const a=(n=(s=t.target)==null?void 0:s.closest)==null?void 0:n.call(s,"form[data-form]");!a||!e.root.contains(a)||w.debug("portalEvents.submit.scaffold",{form:a.dataset.form})}function cr(e,t){var n;const a=t.target;if(!a||!e.root.contains(a))return;const s=(n=a.closest("[data-action]"))==null?void 0:n.dataset.action;s&&w.debug("portalEvents.change.scaffold",{action:s})}function ur(e,t){var n;const a=t.target;if(!a||!e.root.contains(a))return;const s=(n=a.closest("[data-action]"))==null?void 0:n.dataset.action;s&&w.debug("portalEvents.input.scaffold",{action:s})}function mr(e,t){var s,n;if(t.key!=="Enter"&&t.key!==" ")return;const a=(n=(s=t.target)==null?void 0:s.closest)==null?void 0:n.call(s,"tr.contact-table-row[data-action], .cal-row[data-action], .month-cell[data-action]");!a||!e.root.contains(a)||w.debug("portalEvents.keydown.scaffold",{action:a.dataset.action})}function pr(e,t){if(t.key!=="Escape")return;const{state:a,render:s}=e;if(a.importProgress&&(a.importProgress.phase==="done"||a.importProgress.phase==="error")){e.closeImportProgress();return}if(!a.importProgress){if(a.filesUploadProgress&&(a.filesUploadProgress.phase==="done"||a.filesUploadProgress.phase==="error")){e.closeFilesUploadProgress();return}if(!a.filesUploadProgress){if(a.filesUploadMenuOpen){a.filesUploadMenuOpen=!1,e.unbindFilesUploadMenuOutside(),s();return}if(a.userMenuOpen){a.userMenuOpen=!1,e.unbindUserMenuOutside(),s();return}if(a.filesUploadConflict!==null){ze(e.filesHost,"cancel");return}if(a.filesRenamePath!==null||a.filesDeletePaths!==null||a.filesTransfer!==null||a.filesMkdirOpen){a.filesRenamePath=null,a.filesDeletePaths=null,e.resetFilesTransferTree(),a.filesMkdirOpen=!1,s();return}if(a.confirmDelete){a.confirmDelete=null,s();return}if(e.closeInfoModal(),a.eventDtPicker){a.eventDtPicker=null,We(a),s();return}if(a.eventModalOpen){a.eventModalOpen=!1,a.editingEvent=null,a.creatingEvent=!1,a.eventDtPicker=null,s();return}if(a.contactModalOpen){a.contactModalOpen=!1,a.editingContact=null,a.creatingContact=!1,a.photoPreview=null,a.photoBase64Pending=null,a.removePhotoPending=!1,s();return}if(a.abModalOpen){a.abModalOpen=!1,s();return}if(a.calModalOpen||a.createCalModalOpen||a.deleteConfirmId!==null||a.deleteAbConfirmId!==null){a.calModalOpen=!1,a.createCalModalOpen=!1,a.deleteConfirmId=null,a.deleteAbConfirmId=null,s();return}if(a.adminUserCreateOpen||a.adminUserEditOpen||a.adminUserDeleteUsername!==null){a.adminUserCreateOpen=!1,a.adminUserEditOpen=!1,a.adminUserDeleteUsername=null,s();return}if(a.adminResetModalOpen){a.adminResetModalOpen=!1,s();return}if(a.adminDbConfirmOpen){a.adminDbConfirmOpen=!1,a.adminDbConfirmText="",a.adminDbPendingBody=null,s();return}(a.adminCalModal!==null||a.adminAbModal!==null||a.adminResourceDelete!==null)&&(a.adminCalModal=null,a.adminAbModal=null,a.adminResourceDelete=null,s())}}}function bt(e){const{state:t}=e;t.activeTab==="admin"&&(!e.userIsAdmin()||!e.adminUiEnabled())&&(t.activeTab="calendars",t.adminPage="overview",e.persistTab(t.activeTab))}async function fr(e,t,a={}){return Ca(e.adminHost,t,a)}async function na(e,t,a={}){const{state:s,render:n,setFlash:r,clearFlash:i}=e;if(t==="admin"&&(!e.userIsAdmin()||!e.adminUiEnabled())&&(e.userIsAdmin()&&s.adminCapabilities&&!s.adminCapabilities.uiEnabled&&r("info","Portal Administration UI is disabled (portal_admin_ui_enabled)."),t="calendars"),t==="admin"){await e.activateAdminPage(s.adminPage||"overview",{...a,username:s.adminPage==="users"?s.adminSelectedUsername:null});return}s.activeTab=t,s.userMenuOpen=!1,e.persistTab(t),w.event("tab",{tab:t}),t!=="calendars"&&(s.calModalOpen=!1,s.deleteConfirmId=null),t!=="contacts"&&(s.deleteAbConfirmId=null),a.clearFlash!==!1&&i(),s.busy=!0,n();try{t==="contacts"&&s.selectedAbId!==null?await e.loadContacts(s.selectedAbId):t==="calendars"?await e.loadMonthEvents():t==="tasks"?await e.loadTasks():t==="notes"?await e.loadNotes():t==="files"&&await e.loadFiles()}catch(l){w.warn("tab load failed",l instanceof Error?l.message:l),r("error",l instanceof Error?l.message:"Failed to load")}finally{s.busy=!1,n()}}async function Se(e){const{state:t}=e;w.debug("loadHome");const[a,s,n]=await Promise.all([S.calendars(),S.directory().catch(()=>({users:[]})),S.addressbooks()]);if(t.calendars=a.calendars,t.directory=s.users,t.addressBooks=n.addressbooks,w.event("loadHome",{calendars:t.calendars.length,addressBooks:t.addressBooks.length,directory:t.directory.length}),t.holidayCountries.length===0)try{const r=await S.holidayCountries();t.holidayCountries=r.countries}catch{t.holidayCountries=[]}if(t.selectedIds=t.selectedIds.filter(r=>t.calendars.some(i=>i.id===r)),t.selectedId!==null&&!t.calendars.some(r=>r.id===t.selectedId)&&(t.selectedId=null,t.shares=[],t.calModalOpen=!1,t.deleteConfirmId=null),t.selectedIds.length===0){const r=e.pickDefaultCalendar();r?(t.selectedIds=[r.id],t.selectedId=r.id):t.calendars.length>0&&(t.selectedIds=[t.calendars[0].id],t.selectedId=t.calendars[0].id)}t.selectedId===null&&t.selectedIds.length>0&&(t.selectedId=t.selectedIds[0]),t.selectedId!==null&&t.calModalOpen?await e.loadShares(t.selectedId):t.selectedId!==null&&(t.shares=[]),t.activeTab==="calendars"&&await e.loadMonthEvents(),t.selectedAbId!==null&&!t.addressBooks.some(r=>r.id===t.selectedAbId)&&(t.selectedAbId=null,t.contacts=[],t.selectedContactUri=null,t.editingContact=null,t.creatingContact=!1),t.deleteAbConfirmId!==null&&!t.addressBooks.some(r=>r.id===t.deleteAbConfirmId)&&(t.deleteAbConfirmId=null),t.selectedAbId===null&&t.addressBooks.length>0&&(t.selectedAbId=t.addressBooks[0].id),t.selectedAbId!==null&&t.activeTab==="contacts"&&await e.loadContacts(t.selectedAbId),t.activeTab==="tasks"&&await e.loadTasks(),t.activeTab==="notes"&&await e.loadNotes(),t.activeTab==="files"&&await e.loadFiles()}function br(e){const{state:t}=e;return Ft(t.portalUi.timeFormat)}function gr(e){const{state:t}=e;return It(t.portalUi.weekStart)}function yr(e){const{state:t}=e;return Pa(t.portalUi.weekStart)}function _a(e,t,a){const{state:s}=e;return ls(t,a,s.portalUi.timeFormat)}function vr(e,t,a,s,n){var c,f;const{state:r}=e,i=ye(a),l=((c=r.eventDtPicker)==null?void 0:c.viewY)??Number(i.date.slice(0,4)),d=((f=r.eventDtPicker)==null?void 0:f.viewM)??Number(i.date.slice(5,7))-1;return cs({field:t,value:a,dateOnly:s,allowClear:n,viewY:l,viewM:d,weekStart:r.portalUi.weekStart,timeFormat:r.portalUi.timeFormat})}function gt(e){us(e.root)}function De(e,t){var b;const{state:a}=e,{field:s,name:n,label:r,value:i,dateOnly:l=!1,required:d,disabled:c,allowClear:f=!0}=t,m=((b=a.eventDtPicker)==null?void 0:b.field)===s,u=_a(e,i,l);return`<div class="dt-field${m?" is-open":""}" data-dt-id="${o(s)}">
    ...
    ${m&&!c?vr(e,s,i,l,f):""}
  </div>`}function sa(e,t){var s,n,r,i,l,d,c,f;const{state:a}=e;return t==="start"?String(((s=a.editingEvent)==null?void 0:s.start)||""):t==="end"?String(((n=a.editingEvent)==null?void 0:n.end)||""):t==="until"?((i=(r=a.editingEvent)==null?void 0:r.repeat)==null?void 0:i.until)||ve((l=a.editingEvent)==null?void 0:l.start)||L(new Date):t==="due"?ke((d=a.editingTask)==null?void 0:d.due):t==="dtstart"?ke((c=a.editingNote)==null?void 0:c.dtstart):t==="bulk-due"?a.bulkDueValue:t==="birthday"?String(((f=a.editingContact)==null?void 0:f.birthday)||""):""}function ra(e,t,a){const{state:s}=e;if(t==="start"&&s.editingEvent){s.editingEvent={...s.editingEvent,start:a||""};return}if(t==="end"&&s.editingEvent){s.editingEvent={...s.editingEvent,end:a};return}if(t==="until"&&s.editingEvent){s.editingEvent={...s.editingEvent,repeat:{...s.editingEvent.repeat??e.defaultRepeat(),until:a,endMode:"until"}};return}if(t==="due"&&s.editingTask){if(a===null||a==="")s.editingTask={...s.editingTask,due:null};else if(/^\d{4}-\d{2}-\d{2}$/.test(a))s.editingTask={...s.editingTask,due:new Date(a+"T00:00:00").toISOString()};else{const n=new Date((a.length===16,a));s.editingTask={...s.editingTask,due:Number.isNaN(n.getTime())?a:n.toISOString()}}return}if(t==="dtstart"&&s.editingNote){if(a===null||a==="")s.editingNote={...s.editingNote,dtstart:null};else if(/^\d{4}-\d{2}-\d{2}$/.test(a))s.editingNote={...s.editingNote,dtstart:new Date(a+"T00:00:00").toISOString()};else{const n=new Date((a.length===16,a));s.editingNote={...s.editingNote,dtstart:Number.isNaN(n.getTime())?a:n.toISOString()}}return}if(t==="birthday"&&s.editingContact){s.editingContact={...s.editingContact,birthday:a&&/^\d{4}-\d{2}-\d{2}/.test(a)?a.slice(0,10):null};return}t==="bulk-due"&&(s.bulkDueValue=a||"")}function $r(e,t){const{root:a}=e,s=on[t];if(!s)return;const n=a.querySelector("#info-modal"),r=a.querySelector("#info-modal-title"),i=a.querySelector("#info-modal-body");if(!n||!r||!i)return;r.textContent=s.title,i.innerHTML=s.paragraphs.map(d=>`<p>${o(d)}</p>`).join(""),n.hidden=!1,document.body.classList.add("info-modal-open");const l=n.querySelector(".info-modal-close");l==null||l.focus()}function kr(e){const{root:t}=e,a=t.querySelector("#info-modal");a&&(a.hidden=!0,document.body.classList.remove("info-modal-open"))}async function wr(e,t){const a=window;if(typeof a.showSaveFilePicker=="function")try{const i=await(await a.showSaveFilePicker({suggestedName:t})).createWritable();try{await i.write(e)}finally{await i.close()}return"saved"}catch(r){if(r instanceof DOMException&&r.name==="AbortError")return"cancelled"}const s=URL.createObjectURL(e),n=document.createElement("a");return n.href=s,n.download=t,n.rel="noopener",n.style.display="none",document.body.appendChild(n),n.click(),window.setTimeout(()=>{URL.revokeObjectURL(s),n.remove()},6e4),"started"}function hr(e){const t=e.querySelector('input[name="color_picker"]'),a=e.querySelector('input[name="color"]');!t||!a||(t.addEventListener("input",()=>{a.value=t.value.toUpperCase()}),a.addEventListener("change",()=>{let s=a.value.trim();s&&!s.startsWith("#")&&(s=`#${s}`),/^#[0-9A-Fa-f]{6}/.test(s)&&(t.value=s.slice(0,7),a.value=s.toUpperCase())}))}function Sr(e){const t=Qa({activeTab:tr(),adminPage:ar(),adminSelectedUsername:_t().adminUsername??null});let a,s,n,r,i,l,d;function c(g,D){ua(t,g,D)}function f(){en(t)}function m(){const g=Kt(e);t.user?sr(d):zt(e,t,(D,U)=>ot(t,D,U,{renderImportProgressModal:()=>qe(n),renderFilesUploadProgressModal:()=>_e(a)})),ir(d),jt(e,g),requestAnimationFrame(()=>{var D;gt(d),(D=e.querySelector(".dt-time.is-selected"))==null||D.scrollIntoView({block:"center"})})}function u(){ee(n)}function b(){$e(a)}function y(){z(a)}function p(){$t(t)}function $(){j(a)}function k(){tn(t,{stopImportElapsedTimer:u,stopFilesUploadElapsedTimer:b,resetFilesTransferTree:y,unbindUserMenuOutside:p,unbindFilesUploadMenuOutside:$})}function h(g){an(t,{message:g,clearSession:k,render:m})}function v(){return{state:t,render:m,handleSessionExpired:h,clearPortalSessionState:k,normalizeActiveTab:()=>bt(d),persistTab:pt,loadHome:()=>Se(d),loadAdminCapabilities:()=>ht(s),loadAdminDashboard:()=>Ye(s),loadAdminUsers:()=>oe(s),loadAdminUserDetail:g=>K(s,g),loadAdminUserResources:g=>ce(s,g),loadAdminSystemSettings:()=>Ge(s),loadAdminDatabaseSettings:()=>Qe(s),adminPageMeta:g=>ae(s,g),setFlash:c,clearFlash:f}}a={state:t,root:e,render:m,setFlash:c,clearFlash:f},s={state:t,root:e,render:m,setFlash:c,clearFlash:f,userIsAdmin:()=>de(t),adminUiEnabled:()=>Fe(t),persistTab:pt,activateTab:(g,D)=>na(d,g,D),loadHome:()=>Se(d),normalizeActiveTab:()=>bt(d)},n={state:t,root:e,render:m,setFlash:c,clearFlash:f,localeWeekStart:()=>gr(d),localeDowLabels:()=>yr(d),formatDtDisplay:(g,D)=>_a(d,g,D),timeFormatOpts:()=>br(d),renderPortalDateTimeField:g=>De(d,g),getDtFieldCurrentValue:g=>sa(d,g),setDtFieldValue:(g,D)=>ra(d,g,D),positionDtPopovers:()=>gt(d),renderFlashBanner:()=>yt(t),accessBadge:ea,formatImportResult:ft,loadHome:()=>Se(d),onImportContacts:g=>Ys(l,g)},r={state:t,root:e,render:m,setFlash:c,clearFlash:f,renderPortalDateTimeField:g=>De(d,g)},i={state:t,root:e,render:m,setFlash:c,clearFlash:f,renderPortalDateTimeField:g=>De(d,g)},l={state:t,root:e,render:m,setFlash:c,clearFlash:f,renderPortalDateTimeField:g=>De(d,g),stopImportElapsedTimer:()=>ee(n),startImportElapsedTimer:()=>Aa(n),setImportPhase:(g,D)=>Te(n,g,D),applyServerImportProgress:g=>Ua(n,g),readFileTextWithProgress:(g,D)=>Ia(n,g,D),formatImportResult:ft,loadHome:()=>Se(d)},d={state:t,root:e,render:m,setFlash:c,clearFlash:f,filesHost:a,adminHost:s,calendarsHost:n,notesHost:r,tasksHost:i,contactsHost:l,clearPortalSessionState:k,userIsAdmin:()=>de(t),adminUiEnabled:()=>Fe(t),normalizeActiveTab:()=>bt(d),persistTab:pt,activateTab:(g,D)=>na(d,g,D),activateAdminPage:(g,D)=>fr(d,g,D),loadHome:()=>Se(d),handleSessionExpired:h,loadFiles:()=>Z(a),loadShares:g=>Nt(n,g),loadMonthEvents:()=>st(n),loadContacts:g=>xt(l,g),loadTasks:()=>Xe(i),loadNotes:()=>Na(r),loadAdminCapabilities:()=>ht(s),loadAdminDashboard:()=>Ye(s),loadAdminUsers:()=>oe(s),loadAdminUserDetail:g=>K(s,g),loadAdminUserResources:g=>ce(s,g),loadAdminSystemSettings:()=>Ge(s),loadAdminDatabaseSettings:()=>Qe(s),adminPageMeta:g=>ae(s,g),pickDefaultCalendar:()=>ms(n),toggleCalendarSelected:g=>fs(n,g),blankEventForDay:(g,D)=>ks(n,g,D),defaultRepeat:()=>Mt(),itemKey:_,openContact:g=>Ks(l,g),startNewContact:()=>js(l),emptyAddress:()=>xa(),syncEditingEventFromForm:g=>ws(n,g),syncEditingTaskFromForm:g=>Bs(i,g),syncEditingNoteFromForm:g=>Ns(r,g),runBulkTaskAction:g=>Vs(i,g),shell:(g,D)=>ot(t,g,D,{renderImportProgressModal:()=>qe(n),renderFilesUploadProgressModal:()=>_e(a)}),renderLogin:()=>zt(e,t,(g,D)=>ot(t,g,D,{renderImportProgressModal:()=>qe(n),renderFilesUploadProgressModal:()=>_e(a)})),renderFlashBanner:()=>yt(t),renderMonthGrid:()=>ys(n),renderEventModal:()=>$s(n),renderImportProgressModal:()=>qe(n),renderFilesUploadProgressModal:()=>_e(a),renderTasksTab:()=>Rs(i),renderNotesTab:()=>Os(r),renderFilesTab:()=>An(a),renderAdminSection:()=>ts(s),adminSubnavButtons:()=>Nn(s),renderPortalDateTimeField:g=>De(d,g),getDtFieldCurrentValue:g=>sa(d,g),setDtFieldValue:(g,D)=>ra(d,g,D),positionDtPopovers:()=>gt(d),accessBadge:ea,formatImportResult:ft,closeImportProgress:()=>hs(n),closeFilesUploadProgress:()=>$a(a),resetFilesTransferTree:y,stopImportElapsedTimer:u,stopFilesUploadElapsedTimer:b,unbindUserMenuOutside:p,bindUserMenuOutside:()=>un(t,m),unbindFilesUploadMenuOutside:$,bindFilesUploadMenuOutside:()=>hn(a),onLogin:g=>ln(g,v()),onShare:g=>Es(n,g),onSaveEvent:g=>Ps(n,g),onEditCal:g=>Ts(n,g),onCreateCal:g=>As(n,g),onSaveContact:g=>Gs(l,g),onCreateAb:g=>Qs(l,g),onEditAb:g=>Xs(l,g),onSaveTask:g=>Hs(i,g),onSaveNote:g=>Ms(r,g),bindColorPair:hr,bindImportInput:()=>Ss(n),bindHolidaysToggle:()=>Us(n),bindContactPhotoInput:()=>Js(l),bindFilesDom:()=>ha(a),bindAdminDom:()=>Ea(s),saveBlobAsFile:wr,openInfoModal:g=>$r(d,g),closeInfoModal:()=>kr(d),captureScroll:()=>Kt(e),restoreScroll:g=>jt(e,g)},lr(d),rn(v())}let ne="",T=null,F=!1,B=null,J=null,X="sqlite",Ze=!1;async function rt(e,t={}){const a={Accept:"application/json",...t.headers};t.body&&(a["Content-Type"]="application/json"),ne&&t.method&&t.method!=="GET"&&(a["X-CSRF-Token"]=ne);const s=await fetch(`/api/install${e}`,{credentials:"same-origin",...t,headers:a});let n;try{n=await s.json()}catch{throw new Error(`Request failed (${s.status})`)}if(!s.ok)throw new Error(n.error||`Request failed (${s.status})`);return n&&typeof n=="object"&&"data"in n&&n.data!==void 0?n.data:n}async function qt(){var e;T=await rt("/status"),ne=T.csrfToken||ne,((e=T.defaults)==null?void 0:e.backend)==="pgsql"?X="pgsql":X="sqlite"}function Ce(e,t,a){return`<label class="check-row"><input type="checkbox" name="${o(e)}" ${t?"checked":""} ${F?"disabled":""} /> ${o(a)}</label>`}function Dr(){const e=T==null?void 0:T.permissions;return`<section class="card">
  ...
  </section>`}function Cr(){const e=T==null?void 0:T.defaults;return`<section class="card">
    ...
    <form class="stack" data-form="initialize">
      <label>Server timezone
        <select name="timezone" required ${F?"disabled":""}>
          ${Sa((e==null?void 0:e.timezone)||"UTC")}
        </select>
      </label>
      <h3 class="admin-subsection-title">DAV services</h3>
  ...
  </section>`}function Er(){const e=T==null?void 0:T.defaults,t=(T==null?void 0:T.pdoDrivers)||[],a=t.includes("sqlite"),s=t.includes("pgsql");return`<section class="card">
    <h2>Database</h2>
  ...
  </section>`}function Tr(){return`<section class="card">
  ...
  </section>`}function Ar(){return`<section class="card card-danger-zone">
  ...
  </section>`}function V(){const e=document.getElementById("app");if(!e)return;const t=(T==null?void 0:T.step)||"permissions";let a="";T?t==="permissions"?a=Dr():t==="initialize"?a=Cr():t==="database"?a=Er():t==="upgrade"?a=Pr():t==="done"?a=Tr():t==="locked"?a=Ar():a=`<section class="card"><p>Unknown step: ${o(t)}</p></section>`:a='<section class="card"><p class="muted">Loading installer…</p></section>',e.innerHTML=`
  ...
  `,Ur()}function Ur(){var t,a,s,n,r,i;const e=document.getElementById("app");e&&((t=e.querySelector('[data-action="reload"]'))==null||t.addEventListener("click",()=>{Fr()}),(a=e.querySelector('[data-action="backend-change"]'))==null||a.addEventListener("change",l=>{X=l.target.value==="pgsql"?"pgsql":"sqlite",V()}),(s=e.querySelector('[data-action="upgrade-toggle"]'))==null||s.addEventListener("change",l=>{Ze=!!l.target.checked,V()}),(n=e.querySelector('[data-action="upgrade-run"]'))==null||n.addEventListener("click",()=>{Nr()}),(r=e.querySelector('[data-form="initialize"]'))==null||r.addEventListener("submit",l=>{l.preventDefault(),Ir(l.target)}),(i=e.querySelector('[data-form="database"]'))==null||i.addEventListener("submit",l=>{l.preventDefault(),Or(l.target)}))}async function Fr(){F=!0,B=null,V();try{await qt(),J=null}catch(e){B=e instanceof Error?e.message:"Failed to load installer status"}finally{F=!1,V()}}async function Ir(e){const t=new FormData(e),a=n=>{var r;return!!((r=e.querySelector(`input[name="${n}"]`))!=null&&r.checked)},s={timezone:String(t.get("timezone")??"").trim(),cal_enabled:a("cal_enabled"),card_enabled:a("card_enabled"),tasks_enabled:a("tasks_enabled"),notes_enabled:a("notes_enabled"),files_enabled:a("files_enabled"),dav_auth_type:String(t.get("dav_auth_type")??"Digest"),invite_from:String(t.get("invite_from")??"").trim(),session_max_age_minutes:Number(t.get("session_max_age_minutes")??15),admin_password:String(t.get("admin_password")??""),admin_password_confirm:String(t.get("admin_password_confirm")??"")};F=!0,B=null,J=null,V();try{T=await rt("/initialize",{method:"POST",body:JSON.stringify(s)}),ne=T.csrfToken||ne,J="Server settings saved. Configure the database next.",w.event("install.initialize")}catch(n){B=n instanceof Error?n.message:"Initialize failed"}finally{F=!1,V()}}async function Or(e){const t=new FormData(e),a=String(t.get("backend")??X),s={backend:a,admin_password:String(t.get("admin_password")??""),admin_password_confirm:String(t.get("admin_password_confirm")??"")};a==="sqlite"?s.sqlite_file=String(t.get("sqlite_file")??"").trim():(s.pgsql_host=String(t.get("pgsql_host")??"").trim(),s.pgsql_dbname=String(t.get("pgsql_dbname")??"").trim(),s.pgsql_username=String(t.get("pgsql_username")??"").trim(),s.pgsql_password=String(t.get("pgsql_password")??"")),F=!0,B=null,J=null,V();try{T=await rt("/database",{method:"POST",body:JSON.stringify(s)}),ne=T.csrfToken||ne,J="Database configured. Installer is locked.",w.event("install.database"),T.completed||T.step}catch(n){B=n instanceof Error?n.message:"Database setup failed"}finally{F=!1,V()}}async function Nr(){if(Ze){F=!0,B=null,J=null,V();try{const e=await rt("/upgrade",{method:"POST",body:JSON.stringify({confirm:!0})});J="Upgrade completed."+(e.messages&&e.messages.length?" "+e.messages.slice(0,3).join(" · "):""),w.event("install.upgrade"),await qt()}catch(e){B=e instanceof Error?e.message:"Upgrade failed"}finally{F=!1,V()}}}async function Mr(e){document.title="AngaraDAV · Setup",document.body.classList.add("layout-install"),e.innerHTML='<section class="card"><p class="muted">Loading installer…</p></section>';try{await qt()}catch(t){B=t instanceof Error?t.message:"Failed to load installer"}V()}const Dt=document.getElementById("app");if(!Dt)throw new Error("#app missing");const ia=window.location.pathname.replace(/\/+$/,"")||"/";ia==="/portal/install"||ia.endsWith("/portal/install")?Mr(Dt):Sr(Dt);
