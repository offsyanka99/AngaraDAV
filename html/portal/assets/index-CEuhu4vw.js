...
`)}function ke(s){const d=s.titleId||(s.id?`${s.id}-title`:`modal-title-${Math.random().toString(36).slice(2,9)}`),g=s.id?` id="${i(s.id)}"`:"",h=s.className?` ${s.className}`:"",k=s.rootAttrs?` ${s.rootAttrs}`:"",q=`${Rr(s.size)}${s.cardClassName?` ${s.cardClassName}`:""}`,O=s.closeAction,R=s.lockBackdrop?"":` data-action="${i(O)}"`,W=s.hideClose?"":`<button type="button" class="modal-close info-modal-close" data-action="${i(O)}" aria-label="Close">×</button>`;let X="";s.footer!==void 0&&(X=typeof s.footer=="string"?s.footer:rn(s.footer));const ce=X?`<footer class="cal-modal-footer">${X}</footer>`:"",de=`<div class="cal-modal-body">${s.body}</div>`;let Z;return s.form?Z=`<form class="stack"${s.formAttrs?` ${s.formAttrs}`:""}>
    ...
    </div>`}function Xa(s){const d=s.style==="checkbox"?"checkbox":"admin-delete-confirm",g=s.style==="checkbox"?' style="margin-top:1rem"':"",h=s.id?` id="${i(s.id)}"`:"",k=s.checked?" checked":"",q=s.disabled?" disabled":"";return`<label class="${d}"${g}>
            <input type="checkbox"${h} data-action="${i(s.action)}"${k}${q} />
            ${i(s.label)}
    ...
    <div class="info-modal" id="info-modal" hidden role="dialog" aria-modal="true" aria-labelledby="info-modal-title">
      ...
      <span class="brand-text">Angara<span class="brand-dav">DAV</span><span class="brand-sep" aria-hidden="true"> · </span><span class="${a?"brand-portal brand-portal-admin":"brand-portal brand-portal-user"}">${i(a?"Administration Portal":"User Portal")}</span></span>`,n=d?i(d.displayname||d.username):"",r=Et()?`<button type="button" class="user-menu-item${h==="admin"?" is-active":""}" role="menuitem" data-action="tab" data-tab="admin">
          ...
          <a class="brand" href="/portal/">${p}</a>
        </nav>`,w=!(ue||Me||Fe!==null||He!==null||dt||De||We)?Ot():"",x=e.tabs&&e.tabs.trim()!==""?`<div class="tabs-bar" role="presentation">
      ...
      ${f}
      ${v}
      ${xn()}`,S=qe.map(D=>`<div class="cal-row${D.id===F?" is-selected":""}" data-action="select-ab" data-id="${D.id}" role="button" tabindex="0">
          ...
          <button type="button" class="btn btn-ghost btn-small" data-action="remove-phone" data-idx="${be}" ${H.length<=1?"disabled":""} title="Remove">×</button>
              ...
              </div>`).join(""),at=De&&I&&L?`<div class="cal-modal" id="contact-edit-modal" role="dialog" aria-modal="true" aria-labelledby="contact-modal-title">
            ...
            <div class="cal-modal-card cal-modal-card-wide">
              ...
              <div class="cal-modal-body">
          ...
          </div>`:"",$t=We&&L?ke({id:"ab-edit-modal",title:"Address book details",titleId:"ab-modal-title",closeAction:"close-ab-modal",body:`
                ${Ot()}
                <section>
                  <p class="muted small mono" style="margin:0">
                ...
                </section>`,footer:[{label:"Close",action:"close-ab-modal",variant:"ghost"}]}):"",Ve=He!==null?qe.find(D=>D.id===He)??null:null,Pt=Ve?ke({id:"ab-delete-modal",title:"Delete address book",titleId:"ab-delete-title",closeAction:"cancel-delete-ab",size:"sm",body:`
              ${Ot()}
      ...
      ${at}`,se=h==="calendars"?"my-calendars":h==="contacts"?"my-contacts":h==="tasks"?"tasks":h==="notes"?"notes":h==="files"?"files":"administration",J=tr(),pt=ar(),Ue=Mn(),ft=Qn(),Ga=h==="calendars"?x:h==="contacts"?A:h==="tasks"?J:h==="notes"?pt:h==="files"?Ue:ft,Tr=h==="admin"?`<div class="tabs" role="tablist" aria-label="Administration sections">
        ...
        <section class="card">
          ...
          </div>`:"",r=Ne.length===0?'<tr><td colspan="5" class="muted">This folder is empty.</td></tr>':Ne.map(w=>{const x=le.includes(w.path),S=w.type==="dir"?"📁":"📄",L=w.type==="dir"?`<button type="button" class="files-name-btn" data-action="files-nav" data-path="${i(w.path)}" ${c?"disabled":""}>
    ...
    </p>`;let a="",l="";if(O&&!q)l='<section class="card"><p class="muted">Loading overview…</p></section>';else if(R&&!q)l=`<section class="card">
      ...
      </section>`;else if(q){const o=q,p=o.services,n=o.links??{},r=t?`<span class="badge ${Da(t.status)}">${i(da(t.status))}</span>`:"",u=o.version?i(o.version):"—",f=o.git?i(o.git):"";a=`
    ...
    </svg>`,f=(S,L)=>`<button type="button" class="btn btn-small bulk-apply-btn" data-action="${S}"
        title="${i(L)}" aria-label="${i(L)}" ${c||n===0?"disabled":""}>${u}</button>`,y=o?`<div class="bulk-bar" style="margin-top:0.75rem">
          ...
          <table class="contacts-table">
            <thead>
              <tr>
                ...
                ${Lt("Title","summary",Tt,vt,"task","col-task-title")}
                ${Lt("Status","status",Tt,vt,"task","col-task-status")}
                ${Lt("Due","due",Tt,vt,"task","col-task-due")}
                ${Lt("Calendar","calendar",Tt,vt,"task","col-task-cal")}
                ${Lt("%","percent",Tt,vt,"task","col-task-pct")}
              </tr>
            </thead>
            <tbody>${r}</tbody>
          </table>
      ...
      <section class="card contacts-main-card items-list-card">
        ...
        <div class="contacts-table-wrap items-table-wrap" style="margin-top:0.75rem">
          <table class="contacts-table">
            <thead>
              <tr>
                ${Lt("Title","summary",ua,Qt,"note","col-note-title")}
                ${Lt("Date","dtstart",ua,Qt,"note","col-note-date")}
                ${Lt("Calendar","calendar",ua,Qt,"note","col-note-cal")}
              </tr>
            </thead>
            <tbody>${t}</tbody>
          </table>
    ...
    <form class="stack" data-form="initialize">
      ...
      <h3 class="admin-subsection-title">DAV services</h3>
      ${qa("cal_enabled",(s==null?void 0:s.cal_enabled)!==!1,"Enable CalDAV")}
      ${qa("card_enabled",(s==null?void 0:s.card_enabled)!==!1,"Enable CardDAV")}
      ${qa("tasks_enabled",(s==null?void 0:s.tasks_enabled)!==!1,"Enable Tasks (VTODO)")}
      ${qa("notes_enabled",!!(s!=null&&s.notes_enabled),"Enable Notes (VJOURNAL)")}
      ${qa("files_enabled",!!(s!=null&&s.files_enabled),"Enable WebDAV file storage")}
      <label>WebDAV authentication type
