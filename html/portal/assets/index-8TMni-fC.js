...
`)}function U(e){const t=e.titleId||(e.id?`${e.id}-title`:`modal-title-${Math.random().toString(36).slice(2,9)}`),a=e.id?` id="${d(e.id)}"`:"",s=e.className?` ${e.className}`:"",n=e.rootAttrs?` ${e.rootAttrs}`:"",r=`${Za(e.size)}${e.cardClassName?` ${e.cardClassName}`:""}`,i=e.closeAction,l=e.lockBackdrop?"":` data-action="${d(i)}"`,o=e.hideClose?"":`<button type="button" class="modal-close info-modal-close" data-action="${d(i)}" aria-label="Close">×</button>`;let c="";e.footer!==void 0&&(c=typeof e.footer=="string"?e.footer:Tt(e.footer));const m=c?`<footer class="cal-modal-footer">${c}</footer>`:"",p=`<div class="cal-modal-body">${e.body}</div>`;let u;return e.form?u=`<form class="stack"${e.formAttrs?` ${e.formAttrs}`:""}>
    ...
    <div class="info-modal" id="info-modal" hidden role="dialog" aria-modal="true" aria-labelledby="info-modal-title">
      ...
      <span class="brand-text">Angara<span class="brand-dav">DAV</span><span class="brand-sep" aria-hidden="true"> · </span><span class="${n?"brand-portal brand-portal-admin":"brand-portal brand-portal-user"}">${d(n?"Administration Portal":"User Portal")}</span></span>`,o=e.user?d(e.user.displayname||e.user.username):"",c=Ue(e)?`<button type="button" class="user-menu-item${e.activeTab==="admin"?" is-active":""}" role="menuitem" data-action="tab" data-tab="admin">
              Administration
            </button>`:"",m=n?`<button type="button" class="user-menu-item" role="menuitem" data-action="tab" data-tab="calendars">
              User portal
            </button>`:"",p=e.user?`<div class="user-menu${e.userMenuOpen?" is-open":""}">
      ...
      <button type="button" class="files-tree-select${i?" is-selected":""}" data-action="files-tree-select" data-path="${d(n)}"
        ...
        </div>`);return}if(m){for(const f of c)s(f.path,r+1);c.length===0&&n===""&&a.push(`<div class="files-tree-status muted small" style="--depth:${r+1}">No subfolders yet — destination will be Home.</div>`)}}};return s("",0),`<div class="files-folder-tree" role="tree" aria-label="Destination folder">${a.join("")}</div>`}async function bn(e,t){if(!e.state.filesTransfer||e.state.filesTransfer.paths.length===0)return;const a=new FormData(t),s=(e.state.filesTransferDest||String(a.get("toPath")??"")).trim().replace(/^\/+|\/+$/g,""),n=String(a.get("newName")??"").trim(),r=e.state.filesTransfer.op,i=[...e.state.filesTransfer.paths],l=i.length>1;if(at(e,s,i)){e.setFlash("error","Choose a different destination folder"),e.render();return}e.state.busy=!0,e.clearFlash(),e.render();let o=0;const c=[];try{for(const p of i)try{if(r==="copy"){const u=Fe(p),b=l||!n||n===u?void 0:n,y=await h.filesCopy(p,{to:s,newName:b});w.event("files.copy",{path:p,to:y.entry.path})}else{const u=Fe(p),b=l||!n||n===u?void 0:n;await h.filesMove(p,s,b),w.event("files.move",{path:p,to:s})}o+=1}catch(u){c.push(`${Fe(p)}: ${u instanceof Error?u.message:"failed"}`)}z(e),e.state.checkedFilePaths=[],await X(e);const m=r==="copy"?"Copied":"Moved";o>0&&c.length===0?e.setFlash("success",o===1?`${m} 1 item`:`${m} ${o} items`):o>0?e.setFlash("info",`${m} ${o}; ${c.length} failed. ${c[0]}`):e.setFlash("error",c[0]||`${r==="copy"?"Copy":"Move"} failed`)}catch(m){e.setFlash("error",m instanceof Error?m.message:"Operation failed")}finally{e.state.busy=!1,e.render()}}function ht(...e){return e.map(t=>t.replace(/\\/g,"/").replace(/^\/+|\/+$/g,"")).filter(Boolean).join("/")}function fa(e){if(!e||typeof e!="object")return!1;const t=e.name;return t==="AbortError"||t==="NotAllowedError"}function ba(e,t=!0){return Array.from(e).map(s=>{const r=(s.webkitRelativePath||"").replace(/\\/g,"/").replace(/^\/+/,"")||s.name;return{file:s,relativePath:r||s.name}})}function gn(e){return new Promise((t,a)=>{const s=[],n=()=>{e.readEntries(r=>{if(!r.length){t(s);return}s.push(...r),n()},r=>a(r))};n()})}function yn(e){return new Promise((t,a)=>{e.file(t,a)})}async function ga(e,t){const a=ht(t,e.name);if(e.isFile)return[{file:await yn(e),relativePath:a||e.name}];if(e.isDirectory){const s=e.createReader(),n=await gn(s);if(n.length===0)return[{file:null,relativePath:a,isEmptyDir:!0}];const r=[];for(const i of n)r.push(...await ga(i,a));return r}return[]}async function*vn(e){const t=e;if(typeof t.values=="function"){for await(const a of t.values())yield a;return}if(typeof t.entries=="function")for await(const[,a]of t.entries())yield a}async function At(e,t){const a=ht(t,e.name),s=[];let n=0;for await(const r of vn(e))if(n+=1,r.kind==="file"){const i=await r.getFile();s.push({file:i,relativePath:ht(a,r.name)||i.name})}else r.kind==="directory"&&s.push(...await At(r,a));return n===0&&s.push({file:null,relativePath:a,isEmptyDir:!0}),s}async function $n(){const e=window;if(typeof e.showOpenFilePicker!="function")return{kind:"fallback"};try{const t=await e.showOpenFilePicker({multiple:!0});if(!t||t.length===0)return{kind:"cancel"};const a=[];for(const s of t){const n=await s.getFile();a.push({file:n,relativePath:n.name})}return{kind:"items",items:a}}catch(t){return fa(t)?{kind:"cancel"}:{kind:"fallback"}}}async function kn(){const e=window;if(typeof e.showDirectoryPicker!="function")return{kind:"fallback"};try{const t=await e.showDirectoryPicker({mode:"read"}),a=await At(t,"");return a.length===0?{kind:"cancel"}:{kind:"items",items:a}}catch(t){return fa(t)?{kind:"cancel"}:{kind:"fallback"}}}function ut(e){return e.replace(/\\/g,"/").replace(/^\/+/,"").replace(/\/+$/,"")}async function wn(e){var r,i;const t=e.items?Array.from(e.items):[],a=[];for(const l of t){if(l.kind!=="file")continue;const o=l;if(typeof o.getAsFileSystemHandle=="function")try{const c=await o.getAsFileSystemHandle();if(c){if(c.kind==="file"){const m=await c.getFile();a.push({file:m,relativePath:m.name})}else c.kind==="directory"&&a.push(...await At(c,""));continue}}catch{}if(typeof o.webkitGetAsEntry=="function"){const c=o.webkitGetAsEntry();if(c){a.push(...await ga(c,""));continue}}}const s=e.files&&e.files.length>0?ba(e.files,!0):[],n=new Map;for(const l of s){const o=ut(l.relativePath||((r=l.file)==null?void 0:r.name)||"");o&&n.set(o,l)}for(const l of a){const o=ut(l.relativePath||((i=l.file)==null?void 0:i.name)||"");if(!o&&l.isEmptyDir){const c=ut(l.relativePath);c&&n.set(c,l);continue}o&&n.set(o,l)}return Array.from(n.values())}function hn(e){if(!e)return!1;if(e.types&&typeof e.types.includes=="function")return e.types.includes("Files");try{for(let t=0;t<e.types.length;t++)if(e.types[t]==="Files")return!0}catch{}return!1}function ya(e,t=80){const a=String(e??"").replace(/\s+/g," ").trim();return a?a.length>t?`${a.slice(0,t-1)}…`:a:""}function te(e,t,a){const s=ya(t);return s?`${e} “${s}” ${a}`:`${e} ${a}`}function mt(e){const t=ya(e.displayname||e.fullname);return t||[e.firstname,e.lastname].map(s=>String(s??"").trim()).filter(Boolean).join(" ")||"Unnamed contact"}function va(e){if(!e)return"—";try{const t=new Date(e);return Number.isNaN(t.getTime())?e:t.toLocaleString(void 0,{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return e}}function Ye(e){return!Number.isFinite(e)||e<0?"":e<1024?`${e} B`:e<1024*1024?`${(e/1024).toFixed(1)} KB`:`${(e/(1024*1024)).toFixed(1)} MB`}function H(e){const t=Math.max(0,Math.floor(e)),a=Math.floor(t/60),s=t%60;return a>0?`${a}m ${s}s`:`${s}s`}function fe(e){return!Number.isFinite(e)||e<0?"—":e<1024?`${e} B`:e<1024*1024?`${(e/1024).toFixed(1)} KB`:e<1024*1024*1024?`${(e/(1024*1024)).toFixed(1)} MB`:`${(e/(1024*1024*1024)).toFixed(2)} GB`}function Sn(e){if(!e)return"—";try{return new Date(e*1e3).toLocaleString()}catch{return"—"}}function Y(e,t,a,s,n,r=""){const i=a===t,l=i?s==="asc"?" ▲":" ▼":"";return`<th class="${`sortable-th${i?" is-sorted":""}${r?" "+r:""}`}" data-action="sort-${n}" data-sort="${d(t)}" role="columnheader" tabindex="0">${d(e)}${l}</th>`}function Z(e){e.state.filesUploadMenuDocClick&&(document.removeEventListener("click",e.state.filesUploadMenuDocClick,!0),e.state.filesUploadMenuDocClick=null)}function Dn(e){Z(e),e.state.filesUploadMenuDocClick=a=>{var n;const s=a.target;(n=s==null?void 0:s.closest)!=null&&n.call(s,".files-upload-menu")||(e.state.filesUploadMenuOpen=!1,Z(e),e.render())};const t=e.state.filesUploadMenuDocClick;setTimeout(()=>{e.state.filesUploadMenuOpen&&e.state.filesUploadMenuDocClick===t&&document.addEventListener("click",t,!0)},0)}function $e(e){e.state.filesUploadElapsedTimer!==null&&(clearInterval(e.state.filesUploadElapsedTimer),e.state.filesUploadElapsedTimer=null)}function Cn(e){$e(e),e.state.filesUploadElapsedTimer=setInterval(()=>{if(!e.state.filesUploadProgress||e.state.filesUploadProgress.phase==="done"||e.state.filesUploadProgress.phase==="error"){$e(e);return}e.state.filesUploadProgress={...e.state.filesUploadProgress,elapsedSec:Math.floor((Date.now()-e.state.filesUploadProgress.startedAt)/1e3)},ge(e,e.state.filesUploadProgress)},1e3)}function $a(e){$e(e),e.state.filesUploadProgress=null,e.render()}function ka(e,t){return t.bytesTotal>0?Math.min(100,Math.max(0,Math.round(100*t.bytesSent/t.bytesTotal))):t.totalFiles>0?Math.min(100,Math.max(0,Math.round(100*t.completedFiles/t.totalFiles))):null}function ge(e,t){if(!e.root.querySelector("[data-files-upload-progress]"))return;const a=e.root.querySelector(".files-upload-progress-bar"),s=e.root.querySelector(".files-upload-progress-track"),n=e.root.querySelector("[data-files-upload-status]"),r=e.root.querySelector("[data-files-upload-current]"),i=ka(e,t),l=t.phase==="uploading"?`Uploading ${t.completedFiles.toLocaleString()} / ${t.totalFiles.toLocaleString()} file${t.totalFiles===1?"":"s"}${t.failedFiles?` · ${t.failedFiles} failed`:""}${i!==null?` (${i}%)`:""} · ${H(t.elapsedSec)}`:(n==null?void 0:n.textContent)||"";n&&t.phase==="uploading"&&(n.textContent=l),r&&t.phase==="uploading"&&(r.textContent=t.currentName||"",r.title=t.currentName||""),a&&i!==null&&(a.classList.remove("is-indeterminate"),a.style.width=`${i}%`),s&&i!==null&&(s.setAttribute("aria-valuenow",String(i)),s.removeAttribute("aria-valuetext"))}function Le(e){if(!e.state.filesUploadProgress)return"";const t=e.state.filesUploadProgress,a=t.phase==="uploading",s=t.phase==="done"?"Upload finished":t.phase==="error"?"Upload failed":"Uploading…",n=ka(e,t),r=n===null?"files-upload-progress-bar is-indeterminate":"files-upload-progress-bar",i=n!==null?` style="width:${n}%"`:"";let l="";if(a){const c=`Uploading ${t.completedFiles.toLocaleString()} / ${t.totalFiles.toLocaleString()} file${t.totalFiles===1?"":"s"}${t.failedFiles?` · ${t.failedFiles} failed`:""}${n!==null?` (${n}%)`:""} · ${H(t.elapsedSec)}`,m=t.bytesTotal>0?`${Ye(t.bytesSent)} / ${Ye(t.bytesTotal)}`:"";l=`
      ...
      <p class="muted small" style="margin:0">Took ${d(H(t.elapsedSec))}</p>`;else{const c=t.errorSamples.length>0?`<ul class="files-upload-error-list muted small">${t.errorSamples.slice(0,8).map(m=>`<li>${d(m)}</li>`).join("")}${t.errorSamples.length>8?`<li>…and ${t.errorSamples.length-8} more</li>`:""}</ul>`:"";l=`
      ${j("error",t.resultMessage||"Upload failed.",{className:"import-result",style:"margin:0 0 1rem"})}
      ${c}
      <p class="muted small" style="margin:0.75rem 0 0">After ${d(H(t.elapsedSec))}</p>`}const o=a?'<p class="muted small" style="margin:0">Please wait…</p>':Tt([{label:"Close",action:"close-files-upload-progress",variant:"primary"}]);return U({title:s,titleId:"files-upload-progress-title",closeAction:"close-files-upload-progress",size:"sm",className:"import-progress-modal files-upload-progress-modal",cardClassName:"import-progress-card",rootAttrs:"data-files-upload-progress",hideClose:a,lockBackdrop:a,body:l,footer:o})}async function Yt(e,t,a,s){const n=a.replace(/\\/g,"/").split("/").map(i=>i.trim()).filter(Boolean);let r=t;for(const i of n){const l=pa(r,i);if(s.has(l)){r=l;continue}try{await h.filesMkdir(r,i),w.event("files.mkdir",{path:r,name:i,via:"upload-folder"})}catch(o){if(!(o instanceof O&&o.status===409))throw o}s.add(l),r=l}}function En(e,t){var s;const a=t==="files"?'input[type="file"][data-action="files-upload-pick-files"]':'input[type="file"][data-action="files-upload-pick-folder"]';(s=e.root.querySelector(a))==null||s.click()}async function Gt(e,t){if(e.state.busy||e.state.filesUploadProgress)return;e.state.filesUploadMenuOpen=!1,Z(e),e.state.filesRenamePath=null,e.state.filesDeletePaths=null,z(e),e.state.filesMkdirOpen=!1;const a=t==="files"?$n:kn;try{const s=await a();if(s.kind==="cancel"){e.render();return}if(s.kind==="items"){if(s.items.length===0){e.setFlash("info",t==="folder"?"Folder is empty":"No files selected"),e.render();return}await Ut(e,s.items);return}e.render(),requestAnimationFrame(()=>{En(e,t)})}catch(s){e.setFlash("error",s instanceof Error?s.message:"Could not open picker"),e.render()}}function nt(e,t){return`${e}\0${t}`}function Tn(e,t){return t.map(a=>{const s=a.file,n=(a.relativePath||s.name).replace(/\\/g,"/"),r=n.split("/").filter(Boolean),i=r.pop()||s.name,l=r.join("/"),o=pa(e,l);return{item:a,file:s,fileName:i,parentPath:o,displayName:n||i,relDir:l}})}function Pn(e){const t=new Set,a=[];for(const s of e){const n=nt(s.parentPath,s.fileName);t.has(n)||(t.add(n),a.push(s))}return a}async function An(e,t){if(t.length===0)return[];const a=new Map;for(const n of t){const r=a.get(n.parentPath)??[];r.push(n),a.set(n.parentPath,r)}const s=[];for(const[n,r]of a){let i=new Map;try{const l=await h.filesList(n);i=new Map;for(const o of l.entries)(o.type==="file"||o.type==="dir")&&i.set(o.name,o.type)}catch{i=new Map}for(const l of r)i.has(l.fileName)&&s.push(l)}return s.sort((n,r)=>n.displayName.localeCompare(r.displayName)),s}const Ft=new WeakMap;function pt(e){e&&(Ft.delete(e.state),e.state.filesUploadConflict=null)}function Ke(e,t){var m;const a=Ft.get(e.state),s=e.state.filesUploadConflict;if(t==="cancel"){pt(e),e.setFlash("info","Upload cancelled"),e.render();return}if(!a){e.state.filesUploadConflict=null,e.setFlash("error","Upload session expired — drop or choose the files again"),e.render();return}const n=new Set(((m=s==null?void 0:s.conflictKeys)!=null&&m.length?s.conflictKeys:a.conflictKeys)??[]);let r=a.planned,i=new Set,l=0;if(t==="overwrite")i=new Set(n);else{const p=[];for(const u of a.planned){const b=nt(u.parentPath,u.fileName);n.has(b)?l+=1:p.push(u)}if(r=p,w.event("files.upload.skip_existing",{skipped:l,remaining:r.length,total:a.planned.length,conflictKeys:n.size}),r.length===0&&a.emptyDirs.length===0){pt(e),e.setFlash("info",l===1?"Nothing to upload — the selected file already exists":`Nothing to upload — all ${l} selected files already exist`),e.render();return}}const o=a.destBase,c=a.emptyDirs;pt(e),wa(e,r,c,o,i)}async function Ut(e,t){if(t.length===0||e.state.filesUploadProgress||e.state.filesUploadConflict)return;e.state.filesUploadMenuOpen=!1,Z(e),e.state.filesUploadDropActive=!1;const a=t.filter(i=>i.file&&!i.isEmptyDir),s=t.filter(i=>i.isEmptyDir&&i.relativePath),n=e.state.filesPath,r=Pn(Tn(n,a));if(w.event("files.upload.plan",{destBase:n||"/",files:r.length,emptyDirs:s.length,sample:r.slice(0,5).map(i=>({display:i.displayName,parent:i.parentPath||"/",name:i.fileName}))}),r.length>0){e.state.busy=!0,e.clearFlash(),e.render();try{const i=await An(n,r);if(i.length>0){const l=i.map(o=>nt(o.parentPath,o.fileName));Ft.set(e.state,{planned:r,emptyDirs:s,destBase:n,conflictKeys:l}),e.state.filesUploadConflict={names:i.map(o=>o.displayName),totalFiles:r.length,conflictCount:i.length,conflictKeys:l},w.event("files.upload.conflicts",{total:r.length,conflicts:i.length,names:i.slice(0,12).map(o=>o.displayName)}),e.state.busy=!1,e.render();return}}catch(i){e.state.busy=!1,e.setFlash("error",i instanceof Error?i.message:"Could not check existing files"),e.render();return}}await wa(e,r,s,n,new Set)}async function wa(e,t,a,s,n){var b,y;const r=t.reduce((f,$)=>f+($.file.size||0),0),i=Date.now(),l=t.length+a.length;e.state.filesUploadProgress={phase:"uploading",totalFiles:Math.max(t.length,1),completedFiles:0,failedFiles:0,currentName:((b=t[0])==null?void 0:b.displayName)||((y=a[0])==null?void 0:y.relativePath)||"",bytesTotal:r,bytesSent:0,startedAt:i,elapsedSec:0,resultMessage:null,errorSamples:[]},e.state.busy=!0,e.clearFlash(),Cn(e),e.render();let o=0;const c=[],m=new Set;let p=0,u=0;try{for(const k of a){const S=k.relativePath.replace(/\\/g,"/").replace(/^\/+|\/+$/g,"");if(S){e.state.filesUploadProgress&&(e.state.filesUploadProgress={...e.state.filesUploadProgress,currentName:S+"/",elapsedSec:Math.floor((Date.now()-i)/1e3)},ge(e,e.state.filesUploadProgress));try{await Yt(e,s,S,m)}catch(v){c.push(`${S}/: ${v instanceof Error?v.message:"failed"}`)}}}for(const k of t){const{file:S,fileName:v,parentPath:g,displayName:D,relDir:P}=k;e.state.filesUploadProgress&&(e.state.filesUploadProgress={...e.state.filesUploadProgress,currentName:D,bytesSent:p,elapsedSec:Math.floor((Date.now()-i)/1e3)},ge(e,e.state.filesUploadProgress));try{P&&await Yt(e,s,P,m);const F=n.has(nt(g,v));await h.filesUpload(g,S,{replace:F,onProgress:(I,_)=>{if(!e.state.filesUploadProgress||e.state.filesUploadProgress.phase!=="uploading")return;const T=_>0?_:S.size;e.state.filesUploadProgress={...e.state.filesUploadProgress,currentName:D,bytesSent:p+Math.min(I,T||I),elapsedSec:Math.floor((Date.now()-i)/1e3)},ge(e,e.state.filesUploadProgress)}}),w.event("files.upload",{path:g,name:v,size:S.size,relativePath:D,replace:F}),o+=1,F&&(u+=1),p+=S.size||0,e.state.filesUploadProgress&&(e.state.filesUploadProgress={...e.state.filesUploadProgress,completedFiles:o,failedFiles:c.length,bytesSent:p},ge(e,e.state.filesUploadProgress))}catch(F){const I=`${D}: ${F instanceof Error?F.message:"failed"}`;c.push(I),p+=S.size||0,e.state.filesUploadProgress&&(e.state.filesUploadProgress={...e.state.filesUploadProgress,completedFiles:o,failedFiles:c.length,bytesSent:p,errorSamples:c.slice(0,12)},ge(e,e.state.filesUploadProgress))}}await X(e),$e(e);const f=Math.floor((Date.now()-i)/1e3),$=t.length;if(o>0&&c.length===0){let k=o===1?"Uploaded 1 file":`Uploaded ${o} files`;u>0&&(k+=u===1?" (1 overwritten)":` (${u} overwritten)`),e.state.filesUploadProgress={phase:"done",totalFiles:Math.max($,1),completedFiles:o,failedFiles:0,currentName:"",bytesTotal:r,bytesSent:r,startedAt:i,elapsedSec:f,resultMessage:k,errorSamples:[]},e.setFlash("success",k)}else if(o>0){const k=`Uploaded ${o}; ${c.length} failed. ${c[0]}`;e.state.filesUploadProgress={phase:"done",totalFiles:Math.max($,1),completedFiles:o,failedFiles:c.length,currentName:"",bytesTotal:r,bytesSent:r,startedAt:i,elapsedSec:f,resultMessage:k,errorSamples:c.slice(0,12)},e.setFlash("info",k)}else if(l>0&&c.length===0&&a.length>0){const k=a.length===1?"Created 1 empty folder":`Created ${a.length} empty folders`;e.state.filesUploadProgress={phase:"done",totalFiles:1,completedFiles:0,failedFiles:0,currentName:"",bytesTotal:0,bytesSent:0,startedAt:i,elapsedSec:f,resultMessage:k,errorSamples:[]},e.setFlash("success",k)}else{const k=c[0]||"Upload failed";e.state.filesUploadProgress={phase:"error",totalFiles:Math.max($,1),completedFiles:0,failedFiles:c.length,currentName:"",bytesTotal:r,bytesSent:0,startedAt:i,elapsedSec:f,resultMessage:k,errorSamples:c.slice(0,12)},e.setFlash("error",k)}}catch(f){$e(e);const $=f instanceof Error?f.message:"Upload failed";e.state.filesUploadProgress={phase:"error",totalFiles:Math.max(t.length,1),completedFiles:o,failedFiles:Math.max(c.length,1),currentName:"",bytesTotal:r,bytesSent:p,startedAt:i,elapsedSec:Math.floor((Date.now()-i)/1e3),resultMessage:$,errorSamples:c.length?c.slice(0,12):[$]},e.setFlash("error",$)}finally{e.state.busy=!1,e.render()}}function Qt(e,t,a){const s=t.files;if(!s||s.length===0)return;const n=ba(s,a);t.value="",Ut(e,n)}function Fn(e,t){const a=t?t.split("/").filter(Boolean):[];let s="";const n=[`<button type="button" class="files-crumb" data-action="files-nav" data-path="" ${e.state.busy?"disabled":""}>Home</button>`];for(const r of a){s=s?`${s}/${r}`:r;const i=s;n.push('<span class="files-crumb-sep" aria-hidden="true">/</span>'),n.push(`<button type="button" class="files-crumb" data-action="files-nav" data-path="${d(i)}" ${e.state.busy?"disabled":""}>${d(r)}</button>`)}return`<nav class="files-breadcrumb" aria-label="Folder path">${n.join("")}</nav>`}function Un(e){const t=e.state.filesStatus;if(!t)return`<div class="card"><p class="muted">${e.state.filesLoading||e.state.busy?"Loading…":"Unable to load file storage status."}</p></div>`;if(!t.enabled)return`<div class="portal-grid portal-grid-files">
      <section class="card">
        ${M("Files","files","h1")}
      ...
      <section class="card">
        ...
        <p class="muted small">DAV path: <span class="mono">${d(t.davPath)}</span></p>
          ...
          <div class="bulk-bar-actions">
            ...
            <button type="button" class="btn btn-small btn-danger" data-action="files-bulk-delete" ${e.state.busy?"disabled":""}>Delete</button>
        ...
        </div>`:"",m=(()=>{if(e.state.filesLoading&&e.state.filesEntries.length===0)return"Loading…";if(e.state.filesEntries.length===0)return"0 items";const v=[];l>0&&v.push(`${l} folder${l===1?"":"s"}`),o>0&&v.push(`${o} file${o===1?"":"s"}`);const g=`${e.state.filesEntries.length} item${e.state.filesEntries.length===1?"":"s"}`;return v.length===2?`${g} · ${v.join(", ")}`:v[0]??g})(),p=e.state.filesEntries.length===0?'<tr><td colspan="5" class="muted">This folder is empty.</td></tr>':e.state.filesEntries.map(v=>{const g=e.state.checkedFilePaths.includes(v.path),D=v.type==="dir"?"📁":"📄",P=v.type==="dir"?`<button type="button" class="files-name-btn" data-action="files-nav" data-path="${d(v.path)}" ${e.state.busy?"disabled":""}>
              ...
              <td class="files-col-mtime hide-sm">${d(Sn(v.mtime))}</td>
    ...
    <section class="card files-panel${e.state.filesUploadDropActive?" is-dragover":""}" data-files-drop-target>
      ...
      <div class="table-wrap files-table-wrap">
        <table class="files-table">
          ...
          <tbody>
            ${e.state.filesLoading&&e.state.filesEntries.length===0?'<tr><td colspan="5" class="muted">Loading…</td></tr>':p}
          </tbody>
        </table>
      </div>
      <div class="files-status-bar muted small" role="status" aria-live="polite">
        ${n>0?`${n} of ${e.state.filesEntries.length} selected`:d(m)}
      </div>
    </section>
    ${u}
    ${b}
    ...
    </section>`;else if(e.state.adminDashboard){const i=e.state.adminDashboard,l=i.services,o=i.links??{},c=t?`<span class="badge ${Oe(e,t.status)}">${d(we(e,t.status))}</span>`:"",m=i.version?d(i.version):"—",p=i.git?d(i.git):"";s=`
        ...
        <div class="admin-about-grid">
          <div>
            ...
            <p>
              ...
              ${p?`<span class="mono muted small"> (${p})</span>`:""}
            </p>
    ...
    ${n}`}function _n(e){const t=e.state.adminUsersQuery.trim().toLowerCase();return t?e.state.adminUsers.filter(a=>a.username.toLowerCase().includes(t)||(a.displayname||"").toLowerCase().includes(t)||(a.email||"").toLowerCase().includes(t)):e.state.adminUsers}function Rn(e){return e.state.adminUserCreateOpen?U({id:"admin-user-create-modal",title:"Add user",titleId:"admin-user-create-title",closeAction:"admin-user-create-close",size:"sm",form:!0,formAttrs:'data-form="admin-user-create"',body:`
        ...
        <p class="muted small">Username <span class="mono">${d(t.username)}</span> cannot be changed. Leave password fields empty to keep the current password.</p>
          ...
          </label>`,footer:[{label:"Cancel",action:"admin-user-edit-close",variant:"ghost",disabled:e.state.busy},{label:"Save changes",type:"submit",variant:"primary",disabled:e.state.busy}]})}function Bn(e){if(!e.state.adminUserDeleteUsername)return"";const t=e.state.adminUserDeleteUsername,a=e.state.adminUserDetail&&e.state.adminUserDetail.username.toLowerCase()===t.toLowerCase()?e.state.adminUserDetail:e.state.adminUsers.find(n=>n.username.toLowerCase()===t.toLowerCase())??null,s=a?`${a.displayname||a.username} (${a.username})`:t;return U({id:"admin-user-delete-modal",title:"Delete user",titleId:"admin-user-delete-title",closeAction:"admin-user-delete-close",size:"sm",body:`
  ...
  ${i}${l}${o}`}function Hn(e){const t=ae(e,"users");if(t&&t.available===!1)return st(e,"users");const a=_n(e),s=e.state.adminUsersLoading&&e.state.adminUsers.length===0?'<tr><td colspan="4" class="muted admin-table-empty">Loading users…</td></tr>':a.length===0?`<tr><td colspan="4" class="muted admin-table-empty">${e.state.adminUsersError?d(e.state.adminUsersError):e.state.adminUsersQuery.trim()?"No users match this filter.":"No users found."}</td></tr>`:a.map(n=>`<tr class="contact-table-row${e.state.adminSelectedUsername&&e.state.adminSelectedUsername.toLowerCase()===n.username.toLowerCase()?" is-selected":""}" data-action="admin-user-view" data-username="${d(n.username)}" tabindex="0" role="button">
    ...
    ${Qn(e)}`}function Qn(e){return e.state.adminResetModalOpen?U({id:"admin-reset-modal",title:"Reset to Default",titleId:"admin-reset-title",closeAction:"admin-reset-close",size:"sm",body:`
    ...
    ${as(e)}`}function as(e){if(!e.state.adminDbConfirmOpen)return"";const t=e.state.adminDbConfirmText.trim()==="CONFIRM";return U({id:"admin-db-confirm-modal",title:"Confirm database change",titleId:"admin-db-confirm-title",closeAction:"admin-db-confirm-close",size:"sm",body:`
      ...
      <div class="dt-popover-inner${s?" is-date-only":""}">
        <div class="dt-cal">
          ...
          <div class="dt-dow-row">${p.map(T=>`<span class="dt-dow">${d(T)}</span>`).join("")}</div>
    ...
    </div>`}function ps(e=document){e.querySelectorAll(".dt-field.is-open").forEach(t=>{const a=t.querySelector(".dt-trigger"),s=t.querySelector(".dt-popover");if(!a||!s)return;const n=a.getBoundingClientRect(),r=8;s.style.position="fixed",s.style.visibility="hidden",s.style.top="0",s.style.left="0";const i=s.offsetWidth||320,l=s.offsetHeight||300;let o=n.bottom+6;o+l>window.innerHeight-r&&(o=Math.max(r,n.top-l-6));let c=n.left;c+i>window.innerWidth-r&&(c=Math.max(r,window.innerWidth-i-r)),c<r&&(c=r),s.style.top=`${Math.round(o)}px`,s.style.left=`${Math.round(c)}px`,s.style.right="auto",s.style.visibility="visible",s.style.zIndex="200"})}async function Nt(e,t){const a=await h.shares(t);e.state.shares=a.shares}function fs(e){const t=e.state.calendars.filter(s=>s.canShare);if(t.length===0)return null;const a=s=>{const n=s.uri.toLowerCase(),r=s.displayname.toLowerCase();return n==="default"||r==="default"||r==="default calendar"};return t.find(a)??t[0]??null}async function rt(e){const t=e.state.selectedIds.filter(n=>e.state.calendars.some(r=>r.id===n));if(t.length===0){e.state.monthEvents=[];return}const{from:a,to:s}=is(e.state.monthCursor.y,e.state.monthCursor.m);e.state.monthEventsLoading=!0,w.debug("loadMonthEvents",{selectedIds:t,from:a,to:s});try{const r=(await Promise.all(t.map(async i=>(await h.calendarEvents(i,a,s)).events.map(o=>({...o,instanceId:i}))))).flat();r.sort((i,l)=>{const o=i.start||"",c=l.start||"";return o!==c?o<c?-1:1:(i.summary||"").localeCompare(l.summary||"")}),e.state.monthEvents=r,w.event("monthEvents.loaded",{calendarIds:t,count:e.state.monthEvents.length,from:a,to:s})}catch(n){e.state.monthEvents=[],w.warn("loadMonthEvents failed",n instanceof Error?n.message:n)}finally{e.state.monthEventsLoading=!1}}function bs(e,t){const a=e.state.calendars.find(s=>s.id===t);return a!=null&&a.color?a.color.length>=7?a.color.slice(0,7):a.color:"#3B82F6"}function gs(e,t){e.state.selectedIds.includes(t)?(e.state.selectedIds=e.state.selectedIds.filter(a=>a!==t),e.state.selectedId===t&&(e.state.selectedId=e.state.selectedIds[0]??null)):(e.state.selectedIds=[...e.state.selectedIds,t],e.state.selectedId=t)}function ys(e,t,a){return new Date(t,a,1).toLocaleString(void 0,{month:"long",year:"numeric"})}function vs(e,t){const a=t.summary||"(No title)";if(t.allDay||/^\d{4}-\d{2}-\d{2}$/.test(t.start))return a;const s=new Date(t.start);return Number.isNaN(s.getTime())?a:`${s.toLocaleTimeString(void 0,e.timeFormatOpts())} ${a}`}function $s(e){const t=e.state.calendars.filter(S=>e.state.selectedIds.includes(S.id)),a=t.length===0?"No calendar selected":t.length===1?t[0].displayname:`${t.length} calendars`,s=e.state.monthCursor.y,n=e.state.monthCursor.m,r=new Date(s,n,1),i=e.localeWeekStart(),l=(r.getDay()-i+7)%7,o=new Date(s,n+1,0).getDate(),c=new Date(s,n,0).getDate(),p=N(new Date),u=e.localeDowLabels(),b=new Map;for(const S of e.state.monthEvents)for(const v of ls(S)){const g=b.get(v)??[];g.push(S),b.set(v,g)}const y=[],f=Math.ceil((l+o)/7)*7;for(let S=0;S<f;S++){let v,g=!0,D;S<l?(v=c-l+S+1,g=!1,D=new Date(s,n-1,v)):S>=l+o?(v=S-(l+o)+1,g=!1,D=new Date(s,n+1,v)):(v=S-l+1,D=new Date(s,n,v));const P=N(D),F=P===p,I=g?b.get(P)??[]:[],_=e.state.monthExpandDay===P?50:3,T=I.slice(0,_),x=I.length-T.length,V=T.map(pe=>{var Vt;const lt=pe.instanceId,ot=vs(e,pe),qa=bs(e,lt),Bt=((Vt=e.state.calendars.find(Va=>Va.id===lt))==null?void 0:Vt.displayname)||"",Ba=Bt?`${ot} · ${Bt}`:ot;return`<button type="button" class="month-event${pe.allDay?"":" is-timed"}" title="${d(Ba)}" style="--ev-color:${d(qa)}"
          ...
          <fieldset class="event-repeat" ${i?"disabled":""}>
            ...
            ${s==="WEEKLY"?`<div class="event-byday" role="group" aria-label="Days of week">
                    ${c.map(y=>`<label class="checkbox event-byday-item">
                            <input type="checkbox" name="repeatByDay" value="${y.code}" ${m.has(y.code)?"checked":""} />
                            ${y.label}
                          </label>`).join("")}
                  </div>`:""}
            ${s?`<div class="form-grid form-grid-2" style="margin-top:0.5rem">
                    <label>Ends
                      <select name="repeatEndMode" data-action="event-repeat-end">
                        <option value="never" ${p==="never"?"selected":""}>Never</option>
                        <option value="until" ${p==="until"?"selected":""}>On date</option>
                        <option value="count" ${p==="count"?"selected":""}>After count</option>
                      </select>
                    </label>
                    ${p==="until"?e.renderPortalDateTimeField({field:"until",name:"repeatUntil",label:"Until",value:b,dateOnly:!0,disabled:i,allowClear:!0}):p==="count"?`<label>Occurrences
      ...
      <p class="muted small">Large imports can time out; try again — already-imported items update faster.</p>`;const l=a?'<p class="muted small" style="margin:0">Please wait…</p>':Tt([{label:"Close",action:"close-import-progress",variant:"primary"}]);return U({title:n,titleId:"import-progress-title",closeAction:"close-import-progress",size:"sm",className:"import-progress-modal",cardClassName:"import-progress-card",rootAttrs:"data-import-progress",hideClose:a,lockBackdrop:a,body:i,footer:l})}function Ua(e,t,a){return new Promise((s,n)=>{const r=new FileReader;r.onprogress=i=>{i.lengthComputable&&i.total>0?a(Math.min(100,Math.round(i.loaded/i.total*100))):a(null)},r.onload=()=>s(String(r.result??"")),r.onerror=()=>n(r.error??new Error("Failed to read file")),r.readAsText(t)})}async function Cs(e,t){var s;if(e.state.selectedId===null)return;const a=(s=t.files)==null?void 0:s[0];t.value="",a&&(e.state.calModalOpen=!0,await Ia(e,e.state.selectedId,a,{keepEditModalOpen:!0}))}async function Es(e,t){var m;const a=(m=t.files)==null?void 0:m[0];if(t.value="",!a)return;const s=e.root.querySelector('[data-form="create-cal"]'),n=s?new FormData(s):new FormData,r=n.get("holidays")==="on",i=n.get("readOnly")==="on";if(r){e.setFlash("error","Turn off “Holidays calendar” to import a .ics file into a new calendar."),e.state.createCalModalOpen=!0,e.render();return}if(i){e.setFlash("error","Turn off “Read-only” before importing — import cannot write to a read-only calendar."),e.state.createCalModalOpen=!0,e.render();return}let l=String(n.get("displayname")??"").trim();l||(l=a.name.replace(/\.ics$/i,"").trim()||"Imported calendar");const o=String(n.get("description")??""),c=String(n.get("color")??"").trim();e.state.busy=!0,e.clearFlash(),e.state.createCalModalOpen=!0,e.render();try{const p=await h.createCalendar({displayname:l,description:o,color:c,readOnly:!1});e.state.selectedId=p.calendar.id,e.state.createCalModalOpen=!1,await e.loadHome(),e.setFlash("success",`Created “${p.calendar.displayname}” — importing…`),await Ia(e,p.calendar.id,a,{keepEditModalOpen:!1,successPrefix:`Calendar “${p.calendar.displayname}” created. `})}catch(p){const u=p instanceof Error?p.message:"Create or import failed";e.state.createCalModalOpen=!0,e.setFlash("error",u),e.state.busy=!1,e.render()}}async function Ia(e,t,a,s={}){e.state.busy=!0,e.clearFlash(),ee(e),e.state.importProgress={kind:"calendar",fileName:a.name,fileSizeLabel:Ye(a.size),phase:"reading",readPercent:0,processPercent:null,processCurrent:0,processTotal:0,processImported:0,processUpdated:0,processSkipped:0,startedAt:Date.now(),elapsedSec:0,resultMessage:null,ok:null},Pa(e),e.render();try{const n=await Ua(e,a,l=>{if(!e.state.importProgress||e.state.importProgress.phase!=="reading")return;e.state.importProgress={...e.state.importProgress,readPercent:l};const o=e.root.querySelector(".import-progress-bar"),c=e.root.querySelector("[data-import-status-line]");o&&l!==null&&(o.classList.remove("is-indeterminate"),o.style.width=`${l}%`),c&&l!==null&&(c.textContent=`Reading file… ${l}%`)});Pe(e,"uploading",{readPercent:100}),Pe(e,"processing",{processPercent:0}),w.event("import.calendar.start",{file:a.name,bytes:a.size,calId:t});const r=await h.importCalendar(t,n,l=>{Aa(e,l)}),i=e.formatImportResult(r);e.state.selectedId===t&&await rt(e),ee(e),Pe(e,"done",{ok:!0,resultMessage:`${i} (from “${a.name}”)`}),e.setFlash("success",`${s.successPrefix||""}Import finished for “${a.name}”: ${i}.`)}catch(n){const r=n instanceof Error?n.message:"Import failed";ee(e),Pe(e,"error",{ok:!1,resultMessage:r}),e.setFlash("error",r)}finally{s.keepEditModalOpen&&(e.state.calModalOpen=!0),e.state.busy=!1,e.render()}}async function Ts(e,t){if(e.state.selectedId===null)return;const a=new FormData(t),s=String(a.get("username")??"").trim(),n=String(a.get("access")??"read");if(!s){e.setFlash("error","Select a user to share with"),e.render();return}e.state.calModalOpen=!0,e.state.busy=!0,e.clearFlash(),e.render();try{await h.share(e.state.selectedId,s,n),await Nt(e,e.state.selectedId),e.setFlash("success",`Shared with ${s}`)}catch(r){e.setFlash("error",r instanceof Error?r.message:"Share failed")}finally{e.state.busy=!1,e.render()}}async function Ps(e,t){if(!e.state.editingEvent||!e.state.editingEvent.canWrite)return;const a=new FormData(t),s=String(a.get("summary")??"").trim(),n=String(a.get("description")??"").trim(),r=String(a.get("location")??"").trim(),i=a.get("allDay")==="on",l=String(a.get("start")??"").trim(),o=String(a.get("end")??"").trim(),c=Number(a.get("instanceId"))||e.state.editingEvent.instanceId,m=Ie(a);if(!s){e.setFlash("error","Title is required"),e.render();return}if(!l){e.setFlash("error","Start is required"),e.render();return}let p,u;if(i)p=l.slice(0,10),u=o?o.slice(0,10):p;else if(/^\d{4}-\d{2}-\d{2}$/.test(l)){const $=Mt(l,o||null);p=new Date($.start).toISOString(),u=$.end?new Date($.end).toISOString():null}else p=new Date(l).toISOString(),u=o?new Date(o).toISOString():null;const b=e.state.editingEvent.instanceId,y=e.state.editingEvent.uri,f=e.state.creatingEvent;e.state.busy=!0,e.clearFlash(),e.state.eventModalOpen=!0,e.render(),w.event(f?"event.create":"event.update",{instanceId:c,uri:f?null:y,allDay:i,summary:s});try{const $={summary:s,description:n,location:r,allDay:i,start:p,end:u,instanceId:c,repeat:m},k=f?await h.createEvent(c,$):await h.updateEvent(b,y,$);(e.state.selectedId===null||k.event.instanceId!==e.state.selectedId)&&(e.state.selectedId=k.event.instanceId),await rt(e),e.state.eventModalOpen=!1,e.state.editingEvent=null,e.state.creatingEvent=!1,e.state.eventDtPicker=null,w.event(f?"event.created":"event.saved",{uri:k.event.uri,instanceId:k.event.instanceId}),e.setFlash("success",te("Event",k.event.summary||s,f?"created":"saved"))}catch($){w.warn("event.save failed",$ instanceof Error?$.message:$),e.setFlash("error",$ instanceof Error?$.message:"Save failed")}finally{e.state.busy=!1,e.render()}}async function As(e,t){if(e.state.selectedId===null)return;const a=new FormData(t),s=String(a.get("displayname")??"").trim(),n=String(a.get("description")??""),r=String(a.get("color")??"").trim();e.state.busy=!0,e.clearFlash(),e.render();try{const i=await h.updateCalendar(e.state.selectedId,{displayname:s,description:n,color:r});e.state.calModalOpen=!0,await e.loadHome(),e.state.selectedId=i.calendar.id,await Nt(e,e.state.selectedId),await rt(e),e.setFlash("success","Calendar updated")}catch(i){e.setFlash("error",i instanceof Error?i.message:"Update failed")}finally{e.state.busy=!1,e.render()}}async function Fs(e,t){const a=new FormData(t),s=String(a.get("displayname")??"").trim(),n=String(a.get("description")??""),r=String(a.get("color")??"").trim(),i=a.get("holidays")==="on",l=String(a.get("holidayCountry")??"").trim(),o=a.get("readOnly")==="on";if(e.state.createCalModalOpen=!0,i&&!l){e.setFlash("error","Select a country for the holidays calendar"),e.render();return}if(!i&&!s){e.setFlash("error","Display name is required"),e.render();return}e.state.busy=!0,e.clearFlash(),e.render();try{const c=await h.createCalendar({displayname:s,description:n,color:r,holidays:i,holidayCountry:i?l:void 0,readOnly:o});e.state.selectedId=c.calendar.id,e.state.selectedIds.includes(c.calendar.id)||(e.state.selectedIds=[...e.state.selectedIds,c.calendar.id]),e.state.createCalModalOpen=!1,await e.loadHome();let m=`Created “${c.calendar.displayname}”`;const p=c.holidayImport??c.calendar.holidayImport;p&&(m+=`. Holidays imported: ${e.formatImportResult(p)}.`),o&&(m+=" Calendar is read-only."),e.setFlash("success",m)}catch(c){e.state.createCalModalOpen=!0,e.setFlash("error",c instanceof Error?c.message:"Create failed")}finally{e.state.busy=!1,e.render()}}function Oa(e){const t=e.root.querySelector('[data-form="create-cal"]');if(!t)return;const a=t.querySelector('input[name="holidays"]'),s=t.querySelector("#holidays-country-wrap"),n=t.querySelector('input[name="displayname"]'),r=t.querySelector('input[name="readOnly"]');if(!a||!s)return;const i=a.checked;s.hidden=!i,n&&(n.required=!i,i&&!n.value.trim()?n.placeholder="Auto: Holidays (XX)":i||(n.placeholder="Work")),i&&r&&(r.checked=!0)}function Us(e){Oa(e)}function ea(e){const{state:t}=e,a=t.calendars.filter(f=>f.canShare),s=t.calendars.filter(f=>!f.canShare),n=t.calendars.find(f=>f.id===t.selectedId)??null,r=a.map(f=>{const $=t.selectedIds.includes(f.id),k=$?" is-selected":"",S=f.id===t.selectedId?" is-primary":"",v=f.color?`<span class="cal-swatch" style="background:${d(f.color)}"></span>`:'<span class="cal-swatch cal-swatch-empty"></span>',g=e.accessBadge(f.access)+(f.readOnly?'<span class="badge">read-only</span>':"")+(f.holidaysCountry?`<span class="badge badge-admin">holidays ${d(f.holidaysCountry)}</span>`:"");return`<div class="cal-row${k}${S}" data-action="select-cal" data-id="${f.id}" role="button" tabindex="0" title="Toggle on the month grid">
        ...
        <span class="cal-row-actions">
          <button type="button" class="btn btn-small" data-action="export-cal" data-id="${f.id}" ${t.busy?"disabled":""} title="Export as .ics">Export</button>
            ...
            </tr>`).join(""),c=n!=null&&n.color&&n.color.length>=7?n.color.slice(0,7):"#3B82F6",m=!!(n&&n.readOnly),p=t.calModalOpen&&n&&n.canShare?U({id:"cal-edit-modal",title:"Calendar details",titleId:"cal-modal-title",closeAction:"close-cal-modal",body:`
              ${e.renderFlashBanner()}
              <section>
  ...
  </svg>`,m=(f,$)=>`<button type="button" class="btn btn-small bulk-apply-btn" data-action="${f}"
      title="${d($)}" aria-label="${d($)}" ${e.state.busy||l===0?"disabled":""}>${c}</button>`,p=r?`<div class="bulk-bar" style="margin-top:0.75rem">
    ...
    <section class="stack items-edit-panel">
  ...
  </div>`}function Vs(e,t){if(!e.state.editingTask)return;const a=new FormData(t),s=String(a.get("due")??"").trim(),n=a.get("instanceId"),r=n!==null&&String(n)!==""?Number(n):e.state.editingTask.instanceId,i=String(a.get("parentUid")??"").trim();e.state.editingTask={...e.state.editingTask,instanceId:Number.isFinite(r)&&r>0?r:e.state.editingTask.instanceId,summary:String(a.get("summary")??e.state.editingTask.summary),description:String(a.get("description")??e.state.editingTask.description),status:String(a.get("status")??e.state.editingTask.status),due:s?new Date(s).toISOString():null,priority:Number(a.get("priority")??e.state.editingTask.priority??0),percent:Number(a.get("percent")??e.state.editingTask.percent??0),parentUid:i===""?null:i}}async function Hs(e,t){var r,i;const a=Na(e);if(a.length===0){e.setFlash("error","No writable tasks selected"),e.render();return}const s=a.map(l=>({instanceId:l.instanceId,uri:l.uri}));if(t==="bulk-task-delete"){e.state.busy=!0,e.clearFlash(),e.render();try{const l=await h.bulkTasks({op:"delete",items:s});e.state.checkedTaskKeys=[],e.state.selectedTaskKey&&a.some(o=>L(o.instanceId,o.uri)===e.state.selectedTaskKey)&&(e.state.selectedTaskKey=null,e.state.editingTask=null,e.state.creatingTask=!1),await Ze(e),l.failed>0?e.setFlash("error",`Deleted ${l.ok}, failed ${l.failed}${l.errors[0]?`: ${l.errors[0]}`:""}`):e.setFlash("success",`Deleted ${l.ok} task${l.ok===1?"":"s"}`)}catch(l){e.setFlash("error",l instanceof Error?l.message:"Bulk delete failed")}finally{e.state.busy=!1,e.render()}return}let n={};if(t==="bulk-task-status"){const l=e.root.querySelector("#bulk-task-status"),o=((r=l==null?void 0:l.value)==null?void 0:r.trim())??"";if(!o){e.setFlash("error","Choose a status to apply"),e.render();return}n={status:o}}else if(t==="bulk-task-due"){const l=e.state.bulkDueValue.trim();if(!l){e.setFlash("error","Choose a due date to apply"),e.render();return}const o=/^\d{4}-\d{2}-\d{2}$/.test(l)?new Date(l+"T00:00:00"):new Date((l.length===16,l));if(Number.isNaN(o.getTime())){e.setFlash("error","Invalid due date"),e.render();return}n={due:o.toISOString()}}else if(t==="bulk-task-clear-due")n={due:null};else if(t==="bulk-task-percent"){const l=e.root.querySelector("#bulk-task-percent"),o=((i=l==null?void 0:l.value)==null?void 0:i.trim())??"";if(o===""){e.setFlash("error","Enter a percent complete (0–100)"),e.render();return}const c=Number(o);if(!Number.isFinite(c)||c<0||c>100){e.setFlash("error","Percent must be between 0 and 100"),e.render();return}n={percent:Math.round(c)}}e.state.busy=!0,e.clearFlash(),e.render();try{const l=await h.bulkTasks({op:"update",items:s,fields:n});if(await Ze(e),e.state.editingTask&&!e.state.creatingTask){const c=L(e.state.editingTask.instanceId,e.state.editingTask.uri),m=e.state.tasks.find(p=>L(p.instanceId,p.uri)===c);m&&(e.state.editingTask={...m})}const o=t==="bulk-task-status"?"status":t==="bulk-task-due"||t==="bulk-task-clear-due"?"due date":"percent";l.failed>0?e.setFlash("error",`Updated ${o} on ${l.ok}, failed ${l.failed}${l.errors[0]?`: ${l.errors[0]}`:""}`):e.setFlash("success",`Updated ${o} on ${l.ok} task${l.ok===1?"":"s"}`)}catch(l){e.setFlash("error",l instanceof Error?l.message:"Bulk update failed")}finally{e.state.busy=!1,e.render()}}async function zs(e,t){const a=new FormData(t),s=String(a.get("summary")??"").trim(),n=String(a.get("description")??"").trim(),r=String(a.get("status")??"NEEDS-ACTION"),i=String(a.get("due")??"").trim(),l=i?new Date(i).toISOString():null,o=Number(a.get("priority")??0),c=Number(a.get("percent")??0),m=String(a.get("parentUid")??"").trim(),p=m===""?null:m;e.state.busy=!0,e.clearFlash(),e.render();try{if(e.state.creatingTask){const u=Number(a.get("instanceId"));if(!Number.isFinite(u)||u<=0)throw new Error("Select a calendar");const b=await h.createTask({instanceId:u,summary:s,description:n,status:r,due:l,priority:o,percent:c,parentUid:p});e.state.creatingTask=!1,e.state.selectedTaskKey=L(b.task.instanceId,b.task.uri),e.state.editingTask=b.task,e.setFlash("success",te(p?"Subtask":"Task",b.task.summary||s,"created"))}else if(e.state.editingTask){const u=await h.updateTask(e.state.editingTask.instanceId,e.state.editingTask.uri,{summary:s,description:n,status:r,due:l,priority:o,percent:c,parentUid:p});e.state.editingTask=u.task,e.state.selectedTaskKey=L(u.task.instanceId,u.task.uri),e.setFlash("success",te("Task",u.task.summary||s,"saved"))}await Ze(e)}catch(u){e.setFlash("error",u instanceof Error?u.message:"Save failed")}finally{e.state.busy=!1,e.render()}}async function Ks(e,t,a,s){var o;const{state:n,render:r,setFlash:i,clearFlash:l}=e;if(t==="sort-task"){const c=a.dataset.sort||"";if(!c)return!0;n.taskSort===c?n.taskOrder=n.taskOrder==="asc"?"desc":"asc":(n.taskSort=c,n.taskOrder=c==="due"||c==="summary"?"asc":"desc"),n.busy=!0,r();try{await e.loadTasks()}catch(m){i("error",m instanceof Error?m.message:"Sort failed")}finally{n.busy=!1,r()}return!0}if(t==="select-task"){if(s.target.closest("[data-stop-row], .task-check"))return!0;const c=Number(a.dataset.instance),m=a.dataset.uri??"";if(!Number.isFinite(c)||!m)return!0;const p=n.tasks.find(u=>u.instanceId===c&&u.uri===m)??null;return n.creatingTask=!1,n.selectedTaskKey=e.itemKey(c,m),n.editingTask=p?{...p}:null,l(),r(),!0}if(t==="task-check"){s.preventDefault(),s.stopPropagation();const c=Number(a.dataset.instance),m=a.dataset.uri??"";if(!Number.isFinite(c)||!m)return!0;const p=e.itemKey(c,m),u=n.tasks.find(b=>e.itemKey(b.instanceId,b.uri)===p);return!u||!u.canWrite||u.readOnly||(n.checkedTaskKeys.includes(p)?n.checkedTaskKeys=n.checkedTaskKeys.filter(b=>b!==p):n.checkedTaskKeys=[...n.checkedTaskKeys,p],r()),!0}if(t==="task-select-all"){s.preventDefault();const c=n.tasks.filter(p=>p.canWrite&&!p.readOnly);return c.length>0&&c.every(p=>n.checkedTaskKeys.includes(e.itemKey(p.instanceId,p.uri)))?n.checkedTaskKeys=[]:n.checkedTaskKeys=c.map(p=>e.itemKey(p.instanceId,p.uri)),r(),!0}if(t==="bulk-task-clear")return n.checkedTaskKeys=[],r(),!0;if(t==="bulk-task-status"||t==="bulk-task-due"||t==="bulk-task-clear-due"||t==="bulk-task-percent"||t==="bulk-task-delete"){if(t==="bulk-task-delete"){const c=n.checkedTaskKeys.length;return c===0?(i("error","No tasks selected"),r(),!0):(n.confirmDelete={scope:"bulk-task",title:c===1?"Delete task":`Delete ${c} tasks`,message:c===1?"Delete the selected task?":`Delete ${c} selected tasks?`,detail:"CalDAV clients will sync the removal. This cannot be undone.",count:c},r(),!0)}return e.runBulkTaskAction(t),!0}if(t==="new-task")return n.creatingTask=!0,n.selectedTaskKey=null,n.editingTask={uri:"",instanceId:((o=n.taskCalendars[0])==null?void 0:o.id)??0,calendarId:0,calendarName:"",calendarUri:"",uid:"",parentUid:null,summary:"",description:"",status:"NEEDS-ACTION",due:null,priority:0,percent:0,completed:null,lastmodified:0,readOnly:!1,canWrite:!0},l(),r(),!0;if(t==="new-subtask"){if(!n.editingTask||n.creatingTask||!n.editingTask.uid||!n.editingTask.canWrite)return!0;const c=n.editingTask;return n.creatingTask=!0,n.selectedTaskKey=null,n.editingTask={uri:"",instanceId:c.instanceId,calendarId:c.calendarId,calendarName:c.calendarName,calendarUri:c.calendarUri,uid:"",parentUid:c.uid,summary:"",description:"",status:"NEEDS-ACTION",due:null,priority:0,percent:0,completed:null,lastmodified:0,readOnly:!1,canWrite:!0},l(),r(),!0}if(t==="cancel-task")return n.creatingTask=!1,n.editingTask=null,n.selectedTaskKey=null,r(),!0;if(t==="delete-task"){if(!n.editingTask||n.creatingTask)return!0;const c=String(n.editingTask.summary||"this task").trim()||"this task";return n.confirmDelete={scope:"task",title:"Delete task",message:`Delete “${c}”?`,detail:"CalDAV clients will sync the removal. This cannot be undone."},r(),!0}return!1}async function Lt(e,t){const a=await h.contacts(t,e.state.contactSearch);e.state.contacts=a.contacts,e.state.selectedContactUri!==null&&!e.state.contacts.some(s=>s.uri===e.state.selectedContactUri)&&(e.state.selectedContactUri=null,e.state.creatingContact||(e.state.editingContact=null,e.state.photoPreview=null,e.state.photoBase64Pending=null,e.state.removePhotoPending=!1))}async function js(e,t){if(e.state.selectedAbId===null)return;const a=await h.getContact(e.state.selectedAbId,t);e.state.selectedContactUri=t,e.state.creatingContact=!1;const s=a.contact;e.state.editingContact={...s,emails:Array.isArray(s.emails)?s.emails:[],phones:Array.isArray(s.phones)?s.phones:[],custom:Array.isArray(s.custom)?s.custom:[],address:s.address??xa(),birthday:s.birthday??null},e.state.photoPreview=s.photoDataUri??(s.hasPhoto&&e.state.selectedAbId!==null?`${h.contactPhotoUrl(e.state.selectedAbId,t)}?t=${Date.now()}`:null),e.state.photoBase64Pending=null,e.state.removePhotoPending=!1,e.state.contactModalOpen=!0}function Ws(e){e.state.creatingContact=!0,e.state.selectedContactUri=null,e.state.contactModalOpen=!0,e.state.editingContact={uri:"",displayname:"",firstname:"",lastname:"",fullname:"",org:"",title:"",emails:[""],phones:[{type:"cell",value:""}],address:{street:"",city:"",region:"",postal:"",country:""},birthday:null,url:"",note:"",custom:[],hasPhoto:!1,photoDataUri:null},e.state.photoPreview=null,e.state.photoBase64Pending=null,e.state.removePhotoPending=!1}function xa(e){return{street:"",city:"",region:"",postal:"",country:""}}function Js(e,t){return new Promise((a,s)=>{const n=new FileReader;n.onload=()=>{const r=String(n.result??""),i=r.indexOf(",");a(i>=0?r.slice(i+1):r)},n.onerror=()=>s(new Error("Failed to read photo file")),n.readAsDataURL(t)})}async function Ys(e,t){var s;const a=(s=t.files)==null?void 0:s[0];if(t.value="",!!a){if(a.size>2.5*1024*1024){e.setFlash("error","Photo is too large (max ~2 MB)"),e.render();return}try{const n=await Js(e,a);e.state.photoBase64Pending=n,e.state.photoPreview=`data:${a.type||"image/jpeg"};base64,${n}`,e.state.removePhotoPending=!1,e.render()}catch(n){e.setFlash("error",n instanceof Error?n.message:"Failed to read photo"),e.render()}}}async function Gs(e,t){var n;if(e.state.selectedAbId===null)return;const a=(n=t.files)==null?void 0:n[0];if(t.value="",!a)return;const s=e.state.selectedAbId;e.state.abModalOpen=!0,e.state.busy=!0,e.clearFlash(),e.stopImportElapsedTimer(),e.state.importProgress={kind:"contacts",fileName:a.name,fileSizeLabel:Ye(a.size),phase:"reading",readPercent:0,processPercent:null,processCurrent:0,processTotal:0,processImported:0,processUpdated:0,processSkipped:0,startedAt:Date.now(),elapsedSec:0,resultMessage:null,ok:null},e.startImportElapsedTimer(),e.render();try{const r=await e.readFileTextWithProgress(a,o=>{if(!e.state.importProgress||e.state.importProgress.phase!=="reading")return;e.state.importProgress={...e.state.importProgress,readPercent:o};const c=e.root.querySelector(".import-progress-bar"),m=e.root.querySelector("[data-import-status-line]");c&&o!==null&&(c.classList.remove("is-indeterminate"),c.style.width=`${o}%`),m&&o!==null&&(m.textContent=`Reading file… ${o}%`)});e.setImportPhase("uploading",{readPercent:100}),e.setImportPhase("processing",{processPercent:0}),w.event("import.contacts.start",{file:a.name,bytes:a.size,abId:s});const i=await h.importAddressBook(s,r,o=>{e.applyServerImportProgress(o)}),l=e.formatImportResult(i);await e.loadHome(),e.state.selectedAbId===s&&await Lt(e,s),e.stopImportElapsedTimer(),e.setImportPhase("done",{ok:!0,resultMessage:`${l} (from “${a.name}”)`}),e.setFlash("success",`Import finished for “${a.name}”: ${l}.`)}catch(r){const i=r instanceof Error?r.message:"Import failed";e.stopImportElapsedTimer(),e.setImportPhase("error",{ok:!1,resultMessage:i}),e.setFlash("error",i)}finally{e.state.busy=!1,e.render()}}async function Qs(e,t){if(e.state.selectedAbId===null)return;const a=Is(e,t),s=mt(a);e.state.busy=!0,e.clearFlash(),e.state.contactModalOpen=!0,e.render();try{if(e.state.creatingContact){const n=await h.createContact(e.state.selectedAbId,a);e.state.creatingContact=!1,e.state.selectedContactUri=n.contact.uri,e.state.editingContact=null,e.state.contactModalOpen=!1,e.state.photoPreview=null,e.state.photoBase64Pending=null,e.state.removePhotoPending=!1,e.state.eventDtPicker=null,e.setFlash("success",te("Contact",mt(n.contact)||s,"created"))}else if(e.state.selectedContactUri){const n=await h.updateContact(e.state.selectedAbId,e.state.selectedContactUri,a);e.state.selectedContactUri=n.contact.uri,e.state.editingContact=null,e.state.contactModalOpen=!1,e.state.photoPreview=null,e.state.photoBase64Pending=null,e.state.removePhotoPending=!1,e.state.eventDtPicker=null,e.setFlash("success",te("Contact",mt(n.contact)||s,"saved"))}try{await e.loadHome()}catch(n){if(console.error(n),e.state.selectedAbId!==null)try{await Lt(e,e.state.selectedAbId)}catch{}}}catch(n){e.setFlash("error",n instanceof Error?n.message:"Save failed")}finally{e.state.busy=!1,e.render()}}async function Xs(e,t){const a=new FormData(t),s=String(a.get("displayname")??"").trim(),n=String(a.get("description")??"").trim();if(s){e.state.busy=!0,e.clearFlash(),e.render();try{const r=await h.createAddressBook({displayname:s,description:n});e.state.selectedAbId=r.addressbook.id,e.state.selectedContactUri=null,e.state.editingContact=null,e.state.creatingContact=!1,e.state.contactSearch="",await e.loadHome(),e.setFlash("success",`Address book “${r.addressbook.displayname}” created`)}catch(r){e.setFlash("error",r instanceof Error?r.message:"Create failed")}finally{e.state.busy=!1,e.render()}}}async function Zs(e,t){if(e.state.selectedAbId===null)return;const a=new FormData(t),s=String(a.get("displayname")??"").trim(),n=String(a.get("description")??"").trim();e.state.abModalOpen=!0,e.state.busy=!0,e.clearFlash(),e.render();try{await h.updateAddressBook(e.state.selectedAbId,{displayname:s,description:n}),await e.loadHome(),e.setFlash("success",te("Address book",s,"updated"))}catch(r){e.setFlash("error",r instanceof Error?r.message:"Update failed")}finally{e.state.busy=!1,e.render()}}function er(e){const{state:t}=e,a=t.addressBooks.map(k=>`<div class="cal-row${k.id===t.selectedAbId?" is-selected":""}" data-action="select-ab" data-id="${k.id}" role="button" tabindex="0">
      ...
      </div>`).join(""),m=l.map((k,S)=>`<div class="multi-row multi-row-phone" data-multi="phone" data-idx="${S}">
      ...
      </div>`).join(""),p=Array.isArray(r==null?void 0:r.custom)?r.custom:[],u=p.length===0?'<p class="muted small" style="margin:0 0 0.5rem">No custom fields yet. Labels and values can use any language (e.g. Супруг, 日本語).</p>':p.map((k,S)=>`<div class="multi-row multi-row-custom" data-multi="custom" data-idx="${S}">
            ...
            <div class="cal-modal-body">
              ...
              <form class="stack" data-form="contact">
                ...
                <fieldset class="fieldset fieldset-custom">
                  ...
                  ${u}
                  <button type="button" class="btn btn-ghost btn-small" data-action="add-custom" ${p.length>=30?"disabled":""}>+ Custom field</button>
                </fieldset>
                <label>Notes
    ...
    ${$}
    ${y}
    ${b}`}async function tr(e,t,a,s){var c,m;const{state:n,root:r,render:i,setFlash:l,clearFlash:o}=e;if(t==="select-ab"){const p=Number(a.dataset.id);if(!Number.isFinite(p))return!0;n.selectedAbId=p,n.abModalOpen=!1,n.selectedContactUri=null,n.editingContact=null,n.creatingContact=!1,n.contactModalOpen=!1,n.contactSearch="",n.contacts=[],n.photoPreview=null,n.photoBase64Pending=null,n.removePhotoPending=!1,o(),n.busy=!0,i();try{await e.loadContacts(p)}catch(u){l("error",u instanceof Error?u.message:"Failed to load contacts")}finally{n.busy=!1,i()}return!0}if(t==="edit-ab"){s.stopPropagation();const p=Number(a.dataset.id);if(!Number.isFinite(p)||!n.addressBooks.find(y=>y.id===p))return!0;const b=n.selectedAbId!==p;n.selectedAbId=p,n.abModalOpen=!0,n.contactModalOpen=!1,o(),b&&(n.selectedContactUri=null,n.editingContact=null,n.creatingContact=!1,n.contactSearch="",n.contacts=[],n.photoPreview=null,n.photoBase64Pending=null,n.removePhotoPending=!1),n.busy=!0,i();try{b&&await e.loadContacts(p)}catch(y){l("error",y instanceof Error?y.message:"Failed to open address book")}finally{n.busy=!1,i()}return!0}if(t==="close-ab-modal")return n.abModalOpen=!1,i(),!0;if(t==="select-contact"){const p=a.dataset.uri??"";if(!p)return!0;o();try{await e.openContact(p)}catch(u){l("error",u instanceof Error?u.message:"Failed to load contact")}return i(),!0}if(t==="new-contact")return n.selectedAbId===null||(e.startNewContact(),o(),i()),!0;if(t==="cancel-contact"||t==="close-contact-modal")return n.creatingContact=!1,n.contactModalOpen=!1,n.editingContact=null,n.selectedContactUri=null,n.photoPreview=null,n.photoBase64Pending=null,n.removePhotoPending=!1,n.eventDtPicker=null,o(),i(),!0;if(t==="add-email"||t==="add-phone"||t==="add-custom")return n.editingContact&&(Ae(e.contactsHost),Array.isArray(n.editingContact.emails)||(n.editingContact.emails=[""]),Array.isArray(n.editingContact.phones)||(n.editingContact.phones=[{type:"cell",value:""}]),Array.isArray(n.editingContact.custom)||(n.editingContact.custom=[]),t==="add-email"?n.editingContact.emails.length<10&&n.editingContact.emails.push(""):t==="add-phone"?n.editingContact.phones.length<10&&n.editingContact.phones.push({type:"other",value:""}):n.editingContact.custom.length<30&&n.editingContact.custom.push({label:"",value:""}),i()),!0;if(t==="remove-email"){if(!n.editingContact)return!0;Ae(e.contactsHost);const p=Number(a.dataset.idx);if(!Number.isFinite(p))return!0;const u=Array.isArray(n.editingContact.emails)?n.editingContact.emails:[""];return n.editingContact.emails=u.filter((b,y)=>y!==p),n.editingContact.emails.length===0&&(n.editingContact.emails=[""]),i(),!0}if(t==="remove-phone"){if(!n.editingContact)return!0;Ae(e.contactsHost);const p=Number(a.dataset.idx);if(!Number.isFinite(p))return!0;const u=Array.isArray(n.editingContact.phones)?n.editingContact.phones:[{type:"cell",value:""}];return n.editingContact.phones=u.filter((b,y)=>y!==p),n.editingContact.phones.length===0&&(n.editingContact.phones=[{type:"cell",value:""}]),i(),!0}if(t==="remove-custom"){if(!n.editingContact)return!0;Ae(e.contactsHost);const p=Number(a.dataset.idx);return Number.isFinite(p)&&(n.editingContact.custom=(Array.isArray(n.editingContact.custom)?n.editingContact.custom:[]).filter((u,b)=>b!==p),i()),!0}if(t==="remove-photo")return n.photoPreview=null,n.photoBase64Pending=null,n.removePhotoPending=!0,n.editingContact&&(n.editingContact.hasPhoto=!1),i(),!0;if(t==="delete-contact"){if(n.selectedAbId===null||!n.selectedContactUri)return!0;const p=String(((c=n.editingContact)==null?void 0:c.fullname)||((m=n.editingContact)==null?void 0:m.displayname)||"this contact").trim()||"this contact";return n.confirmDelete={scope:"contact",title:"Delete contact",message:`Delete “${p}”?`,detail:"CardDAV clients will sync the removal. This cannot be undone."},i(),!0}if(t==="delete-ab"){s.stopPropagation();const p=Number(a.dataset.id??n.selectedAbId);return!Number.isFinite(p)||!n.addressBooks.find(b=>b.id===p)||(n.deleteAbConfirmId=p,n.abModalOpen=!1,n.contactModalOpen=!1,o(),i()),!0}if(t==="cancel-delete-ab")return n.deleteAbConfirmId=null,i(),!0;if(t==="confirm-delete-ab"){const p=Number(a.dataset.id),u=r.querySelector("#delete-ab-confirm");if(!Number.isFinite(p)||!(u!=null&&u.checked))return!0;const b=n.addressBooks.find(f=>f.id===p);if(!b)return!0;const y=(b.cardCount??0)>0;n.busy=!0,o(),i();try{await h.deleteAddressBook(p,y),n.selectedAbId===p&&(n.selectedAbId=null,n.contacts=[],n.editingContact=null,n.selectedContactUri=null,n.creatingContact=!1),n.deleteAbConfirmId=null,n.abModalOpen=!1,n.contactModalOpen=!1,await e.loadHome(),n.selectedAbId===null&&n.addressBooks.length>0&&(n.selectedAbId=n.addressBooks[0].id,await e.loadContacts(n.selectedAbId)),l("success","Address book deleted")}catch(f){l("error",f instanceof Error?f.message:"Delete failed")}finally{n.busy=!1,i()}return!0}if(t==="export-ab"){s.stopPropagation();const p=a.dataset.id,u=p!==void 0&&p!==""?Number(p):n.selectedAbId;if(u===null||Number.isNaN(u))return!0;n.busy=!0,o(),i();try{const{blob:b,filename:y}=await h.exportAddressBook(u),f=await e.saveBlobAsFile(b,y);f==="cancelled"?l("info","Export cancelled"):f==="saved"?l("success",`Saved ${y}`):l("success",`Download started: ${y}`)}catch(b){l("error",b instanceof Error?b.message:"Export failed")}finally{n.busy=!1,i()}return!0}if(t==="export-contact"){if(n.selectedAbId===null||!n.selectedContactUri||n.creatingContact)return!0;n.contactModalOpen=!0,n.busy=!0,o(),i();try{const{blob:p,filename:u}=await h.exportContact(n.selectedAbId,n.selectedContactUri),b=await e.saveBlobAsFile(p,u);b==="cancelled"?l("info","Export cancelled"):b==="saved"?l("success",`Saved ${u}`):l("success",`Download started: ${u}`)}catch(p){l("error",p instanceof Error?p.message:"Export failed")}finally{n.busy=!1,i()}return!0}return!1}function _t(e){return e==="calendars"||e==="contacts"||e==="tasks"||e==="notes"||e==="files"||e==="admin"?e:null}function La(e){return e==="overview"||e==="users"||e==="settings"||e==="database"?e:null}function Rt(){const e=(typeof location<"u"?location.hash:"").replace(/^#/,"").split(/[?&]/)[0].replace(/^\/+/,"");if(!e)return{tab:null,adminPage:null,adminUsername:null};if(e==="admin"||e.startsWith("admin/")){const t=e.split("/").filter(Boolean),a=t[1]??"overview",s=La(a)??"overview";let n=null;if(s==="users"&&t[2])try{n=decodeURIComponent(t[2])}catch{n=t[2]}return{tab:"admin",adminPage:s,adminUsername:n}}return{tab:_t(e),adminPage:null,adminUsername:null}}function ar(){const e=Rt().tab;if(e)return e;try{const t=_t(sessionStorage.getItem(da));if(t)return t}catch{}return"calendars"}function nr(){const e=Rt().adminPage;if(e)return e;try{const t=La(sessionStorage.getItem(ca));if(t)return t}catch{}return"overview"}function sr(e,t=null){return e==="overview"?"#admin":e==="users"&&t?`#admin/users/${encodeURIComponent(t)}`:`#admin/${e}`}function ft(e,t="overview",a=null){try{sessionStorage.setItem(da,e),e==="admin"&&sessionStorage.setItem(ca,t)}catch{}if(typeof history>"u"||typeof location>"u")return;const s=e==="admin"?sr(t,a):`#${e}`;location.hash!==s&&history.replaceState(null,"",`${location.pathname}${location.search}${s}`)}function ta(e){return e==="readwrite"?'<span class="badge badge-admin">full access</span>':e==="read"?'<span class="badge">read-only</span>':e==="owner"?'<span class="badge badge-ok">owner</span>':`<span class="badge">${d(e)}</span>`}function bt(e){const t=[`${e.imported} new`,`${e.updated} updated`];return e.skipped>0&&t.push(`${e.skipped} skipped`),t.join(", ")}function rr(e){const{state:t,root:a}=e;if(!t.user){e.renderLogin();return}let s;switch(t.activeTab){case"calendars":s=ea(e);break;case"contacts":s=er(e);break;case"tasks":s=e.renderTasksTab();break;case"notes":s=e.renderNotesTab();break;case"files":s=e.renderFilesTab();break;case"admin":s=e.renderAdminSection();break;default:s=ea(e)}const n=t.activeTab==="calendars"?"my-calendars":t.activeTab==="contacts"?"my-contacts":t.activeTab==="tasks"?"tasks":t.activeTab==="notes"?"notes":t.activeTab==="files"?"files":"administration",r=t.activeTab==="admin"?`<div class="tabs" role="tablist" aria-label="Administration sections">
          ...
          <button type="button" class="info-btn tab-info" data-action="info"
            data-info="${n}"
            aria-label="About this tab" title="About this tab"><span aria-hidden="true">i</span></button>
        </div>`;a.innerHTML=e.shell(s,{tabs:r}),document.body.classList.toggle("cal-modal-open",t.calModalOpen||t.createCalModalOpen||t.deleteConfirmId!==null||t.deleteAbConfirmId!==null||t.eventModalOpen||t.contactModalOpen||t.abModalOpen||t.importProgress!==null||t.filesUploadProgress!==null||t.filesRenamePath!==null||t.filesDeletePaths!==null||t.filesTransfer!==null||t.filesMkdirOpen||t.filesUploadConflict!==null||t.confirmDelete!==null||t.adminUserCreateOpen||t.adminUserEditOpen||t.adminUserDeleteUsername!==null||t.adminResetModalOpen||t.adminDbConfirmOpen||t.adminCalModal!==null||t.adminAbModal!==null||t.adminResourceDelete!==null),document.body.classList.toggle("layout-contacts",t.activeTab==="contacts"),document.body.classList.toggle("layout-calendars",t.activeTab==="calendars"),document.body.classList.toggle("layout-tasks",t.activeTab==="tasks"||t.activeTab==="notes"),document.body.classList.toggle("layout-files",t.activeTab==="files"),document.body.classList.toggle("layout-admin",t.activeTab==="admin")}function ir(e){const{state:t,render:a}=e;e.unbindUserMenuOutside(),t.userMenuOpen&&e.bindUserMenuOutside(),Je(t),t.eventDtPicker&&pn(t,a),e.unbindFilesUploadMenuOutside(),t.filesUploadMenuOpen&&e.bindFilesUploadMenuOutside(),ha(e.filesHost),e.adminHost,e.bindHolidaysToggle(),e.bindImportInput(),e.bindContactPhotoInput()}async function lr(e,t,a,s){const{state:n,render:r,clearFlash:i,setFlash:l}=e;if(t==="confirm-delete-cancel")return Jt(n),r(),!0;if(t==="confirm-delete-ok"){const o=n.confirmDelete;if(!o)return r(),!0;const c=o.scope;if(Jt(n),c==="event"){if(!n.editingEvent||!n.editingEvent.canWrite||n.creatingEvent)return r(),!0;const m=n.editingEvent.instanceId,p=n.editingEvent.uri;n.busy=!0,i(),r();try{await h.deleteEvent(m,p),n.eventModalOpen=!1,n.editingEvent=null,await e.loadMonthEvents(),l("success","Event deleted")}catch(u){l("error",u instanceof Error?u.message:"Delete failed")}finally{n.busy=!1,r()}return!0}if(c==="task"){if(!n.editingTask||n.creatingTask)return r(),!0;n.busy=!0,i(),r();try{await h.deleteTask(n.editingTask.instanceId,n.editingTask.uri),n.selectedTaskKey=null,n.editingTask=null,await e.loadTasks(),l("success","Task deleted")}catch(m){l("error",m instanceof Error?m.message:"Delete failed")}finally{n.busy=!1,r()}return!0}if(c==="note"){if(!n.editingNote||n.creatingNote)return r(),!0;n.busy=!0,i(),r();try{await h.deleteNote(n.editingNote.instanceId,n.editingNote.uri),n.selectedNoteKey=null,n.editingNote=null,await e.loadNotes(),l("success","Note deleted")}catch(m){l("error",m instanceof Error?m.message:"Delete failed")}finally{n.busy=!1,r()}return!0}if(c==="contact"){if(n.selectedAbId===null||!n.selectedContactUri)return r(),!0;n.busy=!0,i(),n.contactModalOpen=!0,r();try{await h.deleteContact(n.selectedAbId,n.selectedContactUri),n.selectedContactUri=null,n.editingContact=null,n.creatingContact=!1,n.contactModalOpen=!1,n.eventDtPicker=null,n.photoPreview=null,await e.loadHome(),l("success","Contact deleted")}catch(m){l("error",m instanceof Error?m.message:"Delete failed")}finally{n.busy=!1,r()}return!0}if(c==="bulk-task")return await e.runBulkTaskAction("bulk-task-delete"),!0;if(c==="revoke-share"){const m=o.href??"";if(!m||n.selectedId===null)return r(),!0;n.calModalOpen=!0,n.busy=!0,i(),r();try{await h.revoke(n.selectedId,m),await e.loadShares(n.selectedId),l("success","Share revoked")}catch(p){l("error",p instanceof Error?p.message:"Revoke failed")}finally{n.busy=!1,r()}return!0}return r(),!0}if(t==="close-import-progress")return n.importProgress&&(n.importProgress.phase==="done"||n.importProgress.phase==="error")&&e.closeImportProgress(),!0;if(t==="logout"){n.busy=!0,w.event("logout");try{await h.logout()}catch{}return e.clearPortalSessionState(),i(),r(),!0}if(t==="info"){const o=a.dataset.info??"";return e.openInfoModal(o),!0}if(t==="info-close")return e.closeInfoModal(),!0;if(t==="flash-close")return i(),r(),!0;if(t==="user-menu-toggle")return s.stopPropagation(),n.userMenuOpen=!n.userMenuOpen,r(),!0;if(t==="user-menu-close")return n.userMenuOpen&&(n.userMenuOpen=!1,r()),!0;if(t==="tab"){const o=_t(a.dataset.tab);return o&&(o==="admin"&&(n.adminPage="overview"),await e.activateTab(o)),!0}return!1}async function _a(e,t){const a=t.target.closest("[data-action]");if(!a)return;const s=a.dataset.action;s&&(w.debug(`action:${s}`,{id:a.dataset.id,tab:a.dataset.tab,uri:a.dataset.uri}),!await lr(e,s,a,t)&&(s.startsWith("admin-")&&await rs(e.adminHost,s,a)||(s.startsWith("files-")||s==="close-files-upload-progress")&&await Mn(e.filesHost,s,a,t)||await Os(e,s,a,t)||await Ks(e,s,a,t)||await Ls(e,s,a)||await tr(e,s,a,t)))}const aa=new WeakMap;function or(e){if(aa.has(e.root)){w.debug("portalEvents: already bound for root");return}aa.set(e.root,!0),e.state.portalEventsBound=!0,e.state.escapeBound=!0;const{root:t}=e;t.addEventListener("click",a=>dr(e,a)),t.addEventListener("submit",a=>cr(e,a)),t.addEventListener("change",a=>ur(e,a)),t.addEventListener("input",a=>mr(e,a)),t.addEventListener("keydown",a=>pr(e,a)),document.addEventListener("keydown",a=>gr(e,a)),t.addEventListener("dragenter",a=>Re(e,"enter",a)),t.addEventListener("dragover",a=>Re(e,"over",a)),t.addEventListener("dragleave",a=>Re(e,"leave",a)),t.addEventListener("drop",a=>Re(e,"drop",a)),t.addEventListener("error",a=>br(e,a),!0),w.event("portalEvents.registered")}function dr(e,t){var n,r;const a=(r=(n=t.target)==null?void 0:n.closest)==null?void 0:r.call(n,"[data-action]");if(!a||!e.root.contains(a))return;const s=a.dataset.action??"";(s==="info"||s==="info-close")&&(t.preventDefault(),t.stopPropagation()),(s==="dt-set-month"||s==="dt-set-year")&&t.stopPropagation(),w.debug("portalEvents.click",{action:s}),_a(e,t)}function cr(e,t){var n,r;const a=(r=(n=t.target)==null?void 0:n.closest)==null?void 0:r.call(n,"form[data-form]");if(!a||!e.root.contains(a))return;const s=a.dataset.form??"";if(s)switch(t.preventDefault(),w.debug("portalEvents.submit",{form:s}),s){case"login":e.onLogin(a);return;case"share":e.onShare(a);return;case"edit-event":e.onSaveEvent(a);return;case"edit-cal":e.onEditCal(a);return;case"create-cal":e.onCreateCal(a);return;case"contact":e.onSaveContact(a);return;case"create-ab":e.onCreateAb(a);return;case"edit-ab":e.onEditAb(a);return;case"task":e.onSaveTask(a);return;case"note":e.onSaveNote(a);return;case"files-rename":In(e.filesHost,a);return;case"files-transfer":bn(e.filesHost,a);return;case"files-mkdir":On(e.filesHost,a);return;case"admin-user-create":zn(e.adminHost,a);return;case"admin-user-edit":Kn(e.adminHost,a);return;case"admin-cal":jn(e.adminHost,a);return;case"admin-ab":Wn(e.adminHost,a);return;case"admin-settings":Xn(e.adminHost,a);return;case"admin-database":Zn(e.adminHost,a);return;default:w.debug("portalEvents.submit.unknown",{form:s})}}function ur(e,t){const a=t.target;if(!a||!e.root.contains(a))return;const{state:s,root:n,render:r}=e,i=a.closest("[data-action]"),l=(i==null?void 0:i.dataset.action)??"";if(l==="dt-set-month"||l==="dt-set-year"){t.stopPropagation(),w.debug("portalEvents.change",{action:l}),_a(e,t);return}if(l==="admin-db-backend"&&a instanceof HTMLSelectElement){s.adminDbFormBackend=a.value==="pgsql"?"pgsql":"sqlite",r();return}if(l==="files-upload-pick-files"&&a instanceof HTMLInputElement){Qt(e.filesHost,a,!1);return}if(l==="files-upload-pick-folder"&&a instanceof HTMLInputElement){Qt(e.filesHost,a,!0);return}if(l==="import-cal"&&a instanceof HTMLInputElement){Cs(e.calendarsHost,a);return}if(l==="import-create-cal"&&a instanceof HTMLInputElement){Es(e.calendarsHost,a);return}if(l==="import-ab"&&a instanceof HTMLInputElement){e.calendarsHost.onImportContacts(a);return}if(l==="contact-photo"&&a instanceof HTMLInputElement){Ys(e.contactsHost,a);return}if(a instanceof HTMLInputElement&&a.id==="delete-cal-confirm"){const o=n.querySelector("#delete-cal-submit");o&&(o.disabled=!a.checked||s.busy);return}if(a instanceof HTMLInputElement&&a.id==="delete-ab-confirm"){const o=n.querySelector("#delete-ab-submit");o&&(o.disabled=!a.checked||s.busy);return}if(a instanceof HTMLSelectElement&&(a.name==="repeatFreq"||a.name==="repeatEndMode")){const o=a.closest('[data-form="edit-event"]');if(o&&s.editingEvent){const c=new FormData(o);s.editingEvent={...s.editingEvent,repeat:Ie(c),hasRrule:!!String(c.get("repeatFreq")??"").trim()},r()}return}if(a instanceof HTMLSelectElement&&a.name==="instanceId"){const o=a.closest('[data-form="task"]');if(o&&s.creatingTask&&s.editingTask){const m=Number(a.value);if(!Number.isFinite(m)||m<=0)return;e.syncEditingTaskFromForm(o);const p=s.editingTask.parentUid;s.editingTask={...s.editingTask,instanceId:m,parentUid:p&&s.tasks.some(u=>u.uid===p&&u.instanceId===m)?p:null},r();return}const c=a.closest('[data-form="note"]');if(c&&s.creatingNote&&s.editingNote){const m=Number(a.value);if(!Number.isFinite(m)||m<=0)return;e.syncEditingNoteFromForm(c),s.editingNote={...s.editingNote,instanceId:m},r();return}}if(a instanceof HTMLInputElement&&a.name==="holidays"&&a.closest('[data-form="create-cal"]')){Oa(e.calendarsHost);return}if(a instanceof HTMLInputElement&&a.name==="color"){const o=a.closest("form"),c=o==null?void 0:o.querySelector('input[name="color_picker"]');if(c){let m=a.value.trim();m&&!m.startsWith("#")&&(m=`#${m}`),/^#[0-9A-Fa-f]{6}/.test(m)&&(c.value=m.slice(0,7),a.value=m.toUpperCase())}return}}function mr(e,t){var o;const a=t.target;if(!a||!e.root.contains(a))return;const{state:s,root:n,render:r,setFlash:i}=e;if(a instanceof HTMLInputElement&&a.name==="color_picker"){const c=a.closest("form"),m=c==null?void 0:c.querySelector('input[name="color"]');m&&(m.value=a.value.toUpperCase());return}const l=((o=a.closest("[data-action]"))==null?void 0:o.dataset.action)??"";if(l==="contact-search"&&a instanceof HTMLInputElement){s.searchTimer&&clearTimeout(s.searchTimer);const c=a.value;s.searchTimer=setTimeout(()=>{s.contactSearch=c,(async()=>{try{s.selectedAbId!==null&&await e.loadContacts(s.selectedAbId),r()}catch(m){i("error",m instanceof Error?m.message:"Search failed"),r()}})()},250);return}if(l==="task-search"&&a instanceof HTMLInputElement){s.searchTimer&&clearTimeout(s.searchTimer);const c=a.value;s.searchTimer=setTimeout(()=>{s.taskSearch=c,(async()=>{try{await e.loadTasks(),r()}catch(m){i("error",m instanceof Error?m.message:"Search failed"),r()}})()},250);return}if(l==="note-search"&&a instanceof HTMLInputElement){s.searchTimer&&clearTimeout(s.searchTimer);const c=a.value;s.searchTimer=setTimeout(()=>{s.noteSearch=c,(async()=>{try{await e.loadNotes(),r()}catch(m){i("error",m instanceof Error?m.message:"Search failed"),r()}})()},250);return}if(l==="admin-db-confirm-input"&&a instanceof HTMLInputElement){s.adminDbConfirmText=a.value;const c=n.querySelector('[data-action="admin-db-confirm-save"]');c&&(c.disabled=s.busy||s.adminDbConfirmText.trim()!=="CONFIRM");return}if(l==="admin-reset-password"&&a instanceof HTMLInputElement){s.adminResetPassword=a.value;const c=n.querySelector('[data-action="admin-reset-confirm"]');c&&(c.disabled=s.busy||!s.adminResetConfirmChecked||s.adminResetPassword.trim()==="");return}}function pr(e,t){var s,n;if(t.key!=="Enter"&&t.key!==" ")return;const a=(n=(s=t.target)==null?void 0:s.closest)==null?void 0:n.call(s,"tr.contact-table-row[data-action], .cal-row[data-action], .month-cell[data-action]");!a||!e.root.contains(a)||t.target!==a&&t.target.closest("button, a, input, select, textarea")||(t.preventDefault(),w.debug("portalEvents.keydown.row",{action:a.dataset.action}),a.click())}function Re(e,t,a){var l,o,c;const{state:s,root:n}=e;if(s.activeTab!=="files"||s.busy||s.filesUploadProgress||!hn(a.dataTransfer))return;const r=(o=(l=a.target)==null?void 0:l.closest)==null?void 0:o.call(l,"[data-files-drop-target]");if(!r||!n.contains(r)){if(t==="leave"&&s.filesDropDepth>0){const m=a.relatedTarget;m&&m instanceof Node&&((c=n.querySelector("[data-files-drop-target]"))==null?void 0:c.contains(m))||(s.filesDropDepth=0,fr(e))}return}if(t==="enter"){a.preventDefault(),a.stopPropagation(),s.filesDropDepth+=1,qe(e,r,!0);return}if(t==="over"){a.preventDefault(),a.stopPropagation(),a.dataTransfer&&(a.dataTransfer.dropEffect="copy"),qe(e,r,!0);return}if(t==="leave"){a.preventDefault(),a.stopPropagation();const m=a.relatedTarget;if(m&&r.contains(m))return;s.filesDropDepth=Math.max(0,s.filesDropDepth-1),s.filesDropDepth===0&&qe(e,r,!1);return}a.preventDefault(),a.stopPropagation(),s.filesDropDepth=0,qe(e,r,!1);const i=a.dataTransfer;!i||s.busy||s.filesUploadProgress||(s.filesUploadMenuOpen=!1,e.unbindFilesUploadMenuOutside(),(async()=>{try{const m=await wn(i);if(m.length===0){e.setFlash("info","Nothing to upload from that drop"),e.render();return}await Ut(e.filesHost,m)}catch(m){e.setFlash("error",m instanceof Error?m.message:"Drop failed"),e.render()}})())}function qe(e,t,a){if(e.state.filesUploadDropActive===a){t.classList.toggle("is-dragover",a);return}e.state.filesUploadDropActive=a,t.classList.toggle("is-dragover",a)}function fr(e){e.state.filesUploadDropActive=!1,e.root.querySelectorAll("[data-files-drop-target].is-dragover").forEach(t=>{t.classList.remove("is-dragover")})}function br(e,t){const a=t.target;if(!(a instanceof HTMLImageElement)||!a.classList.contains("contact-avatar")||!a.dataset.avatarFallback||!a.isConnected)return;const s=a.dataset.avatarFallback||"?",n=document.createElement("span");n.className="contact-avatar contact-avatar-fallback",n.setAttribute("aria-hidden","true"),n.textContent=s,a.replaceWith(n)}function gr(e,t){if(t.key!=="Escape")return;const{state:a,render:s}=e;if(a.importProgress&&(a.importProgress.phase==="done"||a.importProgress.phase==="error")){e.closeImportProgress();return}if(!a.importProgress){if(a.filesUploadProgress&&(a.filesUploadProgress.phase==="done"||a.filesUploadProgress.phase==="error")){e.closeFilesUploadProgress();return}if(!a.filesUploadProgress){if(a.filesUploadMenuOpen){a.filesUploadMenuOpen=!1,e.unbindFilesUploadMenuOutside(),s();return}if(a.userMenuOpen){a.userMenuOpen=!1,e.unbindUserMenuOutside(),s();return}if(a.filesUploadConflict!==null){Ke(e.filesHost,"cancel");return}if(a.filesRenamePath!==null||a.filesDeletePaths!==null||a.filesTransfer!==null||a.filesMkdirOpen){a.filesRenamePath=null,a.filesDeletePaths=null,e.resetFilesTransferTree(),a.filesMkdirOpen=!1,s();return}if(a.confirmDelete){a.confirmDelete=null,s();return}if(e.closeInfoModal(),a.eventDtPicker){a.eventDtPicker=null,Je(a),s();return}if(a.eventModalOpen){a.eventModalOpen=!1,a.editingEvent=null,a.creatingEvent=!1,a.eventDtPicker=null,s();return}if(a.contactModalOpen){a.contactModalOpen=!1,a.editingContact=null,a.creatingContact=!1,a.photoPreview=null,a.photoBase64Pending=null,a.removePhotoPending=!1,s();return}if(a.abModalOpen){a.abModalOpen=!1,s();return}if(a.calModalOpen||a.createCalModalOpen||a.deleteConfirmId!==null||a.deleteAbConfirmId!==null){a.calModalOpen=!1,a.createCalModalOpen=!1,a.deleteConfirmId=null,a.deleteAbConfirmId=null,s();return}if(a.adminUserCreateOpen||a.adminUserEditOpen||a.adminUserDeleteUsername!==null){a.adminUserCreateOpen=!1,a.adminUserEditOpen=!1,a.adminUserDeleteUsername=null,s();return}if(a.adminResetModalOpen){a.adminResetModalOpen=!1,s();return}if(a.adminDbConfirmOpen){a.adminDbConfirmOpen=!1,a.adminDbConfirmText="",a.adminDbPendingBody=null,s();return}(a.adminCalModal!==null||a.adminAbModal!==null||a.adminResourceDelete!==null)&&(a.adminCalModal=null,a.adminAbModal=null,a.adminResourceDelete=null,s())}}}function gt(e){const{state:t}=e;t.activeTab==="admin"&&(!e.userIsAdmin()||!e.adminUiEnabled())&&(t.activeTab="calendars",t.adminPage="overview",e.persistTab(t.activeTab))}async function yr(e,t,a={}){return Ca(e.adminHost,t,a)}async function na(e,t,a={}){const{state:s,render:n,setFlash:r,clearFlash:i}=e;if(t==="admin"&&(!e.userIsAdmin()||!e.adminUiEnabled())&&(e.userIsAdmin()&&s.adminCapabilities&&!s.adminCapabilities.uiEnabled&&r("info","Portal Administration UI is disabled (portal_admin_ui_enabled)."),t="calendars"),t==="admin"){await e.activateAdminPage(s.adminPage||"overview",{...a,username:s.adminPage==="users"?s.adminSelectedUsername:null});return}s.activeTab=t,s.userMenuOpen=!1,e.persistTab(t),w.event("tab",{tab:t}),t!=="calendars"&&(s.calModalOpen=!1,s.deleteConfirmId=null),t!=="contacts"&&(s.deleteAbConfirmId=null),a.clearFlash!==!1&&i(),s.busy=!0,n();try{t==="contacts"&&s.selectedAbId!==null?await e.loadContacts(s.selectedAbId):t==="calendars"?await e.loadMonthEvents():t==="tasks"?await e.loadTasks():t==="notes"?await e.loadNotes():t==="files"&&await e.loadFiles()}catch(l){w.warn("tab load failed",l instanceof Error?l.message:l),r("error",l instanceof Error?l.message:"Failed to load")}finally{s.busy=!1,n()}}async function Se(e){const{state:t}=e;w.debug("loadHome");const[a,s,n]=await Promise.all([h.calendars(),h.directory().catch(()=>({users:[]})),h.addressbooks()]);if(t.calendars=a.calendars,t.directory=s.users,t.addressBooks=n.addressbooks,w.event("loadHome",{calendars:t.calendars.length,addressBooks:t.addressBooks.length,directory:t.directory.length}),t.holidayCountries.length===0)try{const r=await h.holidayCountries();t.holidayCountries=r.countries}catch{t.holidayCountries=[]}if(t.selectedIds=t.selectedIds.filter(r=>t.calendars.some(i=>i.id===r)),t.selectedId!==null&&!t.calendars.some(r=>r.id===t.selectedId)&&(t.selectedId=null,t.shares=[],t.calModalOpen=!1,t.deleteConfirmId=null),t.selectedIds.length===0){const r=e.pickDefaultCalendar();r?(t.selectedIds=[r.id],t.selectedId=r.id):t.calendars.length>0&&(t.selectedIds=[t.calendars[0].id],t.selectedId=t.calendars[0].id)}t.selectedId===null&&t.selectedIds.length>0&&(t.selectedId=t.selectedIds[0]),t.selectedId!==null&&t.calModalOpen?await e.loadShares(t.selectedId):t.selectedId!==null&&(t.shares=[]),t.activeTab==="calendars"&&await e.loadMonthEvents(),t.selectedAbId!==null&&!t.addressBooks.some(r=>r.id===t.selectedAbId)&&(t.selectedAbId=null,t.contacts=[],t.selectedContactUri=null,t.editingContact=null,t.creatingContact=!1),t.deleteAbConfirmId!==null&&!t.addressBooks.some(r=>r.id===t.deleteAbConfirmId)&&(t.deleteAbConfirmId=null),t.selectedAbId===null&&t.addressBooks.length>0&&(t.selectedAbId=t.addressBooks[0].id),t.selectedAbId!==null&&t.activeTab==="contacts"&&await e.loadContacts(t.selectedAbId),t.activeTab==="tasks"&&await e.loadTasks(),t.activeTab==="notes"&&await e.loadNotes(),t.activeTab==="files"&&await e.loadFiles()}function vr(e){const{state:t}=e;return It(t.portalUi.timeFormat)}function $r(e){const{state:t}=e;return Ot(t.portalUi.weekStart)}function kr(e){const{state:t}=e;return Ea(t.portalUi.weekStart)}function Ra(e,t,a){const{state:s}=e;return ds(t,a,s.portalUi.timeFormat)}function wr(e,t,a,s,n){var c,m;const{state:r}=e,i=ye(a),l=((c=r.eventDtPicker)==null?void 0:c.viewY)??Number(i.date.slice(0,4)),o=((m=r.eventDtPicker)==null?void 0:m.viewM)??Number(i.date.slice(5,7))-1;return ms({field:t,value:a,dateOnly:s,allowClear:n,viewY:l,viewM:o,weekStart:r.portalUi.weekStart,timeFormat:r.portalUi.timeFormat})}function yt(e){ps(e.root)}function De(e,t){var b;const{state:a}=e,{field:s,name:n,label:r,value:i,dateOnly:l=!1,required:o,disabled:c,allowClear:m=!0}=t,p=((b=a.eventDtPicker)==null?void 0:b.field)===s,u=Ra(e,i,l);return`<div class="dt-field${p?" is-open":""}" data-dt-id="${d(s)}">
    ...
    <button type="button" class="dt-trigger" data-action="dt-open" data-dt-field="${d(s)}"
      ...
      ${c?"disabled":""} aria-expanded="${p}">
      <span class="dt-trigger-text">${d(u)}</span>
      <span class="dt-trigger-icon" aria-hidden="true">▾</span>
    </button>
    ${p&&!c?wr(e,s,i,l,m):""}
  </div>`}function sa(e,t){var s,n,r,i,l,o,c,m;const{state:a}=e;return t==="start"?String(((s=a.editingEvent)==null?void 0:s.start)||""):t==="end"?String(((n=a.editingEvent)==null?void 0:n.end)||""):t==="until"?((i=(r=a.editingEvent)==null?void 0:r.repeat)==null?void 0:i.until)||ve((l=a.editingEvent)==null?void 0:l.start)||N(new Date):t==="due"?ke((o=a.editingTask)==null?void 0:o.due):t==="dtstart"?ke((c=a.editingNote)==null?void 0:c.dtstart):t==="bulk-due"?a.bulkDueValue:t==="birthday"?String(((m=a.editingContact)==null?void 0:m.birthday)||""):""}function ra(e,t,a){const{state:s}=e;if(t==="start"&&s.editingEvent){s.editingEvent={...s.editingEvent,start:a||""};return}if(t==="end"&&s.editingEvent){s.editingEvent={...s.editingEvent,end:a};return}if(t==="until"&&s.editingEvent){s.editingEvent={...s.editingEvent,repeat:{...s.editingEvent.repeat??e.defaultRepeat(),until:a,endMode:"until"}};return}if(t==="due"&&s.editingTask){if(a===null||a==="")s.editingTask={...s.editingTask,due:null};else if(/^\d{4}-\d{2}-\d{2}$/.test(a))s.editingTask={...s.editingTask,due:new Date(a+"T00:00:00").toISOString()};else{const n=new Date((a.length===16,a));s.editingTask={...s.editingTask,due:Number.isNaN(n.getTime())?a:n.toISOString()}}return}if(t==="dtstart"&&s.editingNote){if(a===null||a==="")s.editingNote={...s.editingNote,dtstart:null};else if(/^\d{4}-\d{2}-\d{2}$/.test(a))s.editingNote={...s.editingNote,dtstart:new Date(a+"T00:00:00").toISOString()};else{const n=new Date((a.length===16,a));s.editingNote={...s.editingNote,dtstart:Number.isNaN(n.getTime())?a:n.toISOString()}}return}if(t==="birthday"&&s.editingContact){s.editingContact={...s.editingContact,birthday:a&&/^\d{4}-\d{2}-\d{2}/.test(a)?a.slice(0,10):null};return}t==="bulk-due"&&(s.bulkDueValue=a||"")}function hr(e,t){const{root:a}=e,s=cn[t];if(!s)return;const n=a.querySelector("#info-modal"),r=a.querySelector("#info-modal-title"),i=a.querySelector("#info-modal-body");if(!n||!r||!i)return;r.textContent=s.title,i.innerHTML=s.paragraphs.map(o=>`<p>${d(o)}</p>`).join(""),n.hidden=!1,document.body.classList.add("info-modal-open");const l=n.querySelector(".info-modal-close");l==null||l.focus()}function Sr(e){const{root:t}=e,a=t.querySelector("#info-modal");a&&(a.hidden=!0,document.body.classList.remove("info-modal-open"))}async function Dr(e,t){const a=window;if(typeof a.showSaveFilePicker=="function")try{const i=await(await a.showSaveFilePicker({suggestedName:t})).createWritable();try{await i.write(e)}finally{await i.close()}return"saved"}catch(r){if(r instanceof DOMException&&r.name==="AbortError")return"cancelled"}const s=URL.createObjectURL(e),n=document.createElement("a");return n.href=s,n.download=t,n.rel="noopener",n.style.display="none",document.body.appendChild(n),n.click(),window.setTimeout(()=>{URL.revokeObjectURL(s),n.remove()},6e4),"started"}function Cr(e){const t=e.querySelector('input[name="color_picker"]'),a=e.querySelector('input[name="color"]');!t||!a||(t.addEventListener("input",()=>{a.value=t.value.toUpperCase()}),a.addEventListener("change",()=>{let s=a.value.trim();s&&!s.startsWith("#")&&(s=`#${s}`),/^#[0-9A-Fa-f]{6}/.test(s)&&(t.value=s.slice(0,7),a.value=s.toUpperCase())}))}function Er(e){const t=Xa({activeTab:ar(),adminPage:nr(),adminSelectedUsername:Rt().adminUsername??null});let a,s,n,r,i,l,o;function c(g,D){ua(t,g,D)}function m(){tn(t)}function p(){const g=jt(e);t.user?rr(o):Kt(e,t,(D,P)=>ct(t,D,P,{renderImportProgressModal:()=>_e(n),renderFilesUploadProgressModal:()=>Le(a)})),ir(o),Wt(e,g),requestAnimationFrame(()=>{var D;yt(o),(D=e.querySelector(".dt-time.is-selected"))==null||D.scrollIntoView({block:"center"})})}function u(){ee(n)}function b(){$e(a)}function y(){z(a)}function f(){kt(t)}function $(){Z(a)}function k(){an(t,{stopImportElapsedTimer:u,stopFilesUploadElapsedTimer:b,resetFilesTransferTree:y,unbindUserMenuOutside:f,unbindFilesUploadMenuOutside:$})}function S(g){nn(t,{message:g,clearSession:k,render:p})}function v(){return{state:t,render:p,handleSessionExpired:S,clearPortalSessionState:k,normalizeActiveTab:()=>gt(o),persistTab:ft,loadHome:()=>Se(o),loadAdminCapabilities:()=>St(s),loadAdminDashboard:()=>Ge(s),loadAdminUsers:()=>de(s),loadAdminUserDetail:g=>K(s,g),loadAdminUserResources:g=>ce(s,g),loadAdminSystemSettings:()=>Qe(s),loadAdminDatabaseSettings:()=>Xe(s),adminPageMeta:g=>ae(s,g),setFlash:c,clearFlash:m}}a={state:t,root:e,render:p,setFlash:c,clearFlash:m},s={state:t,root:e,render:p,setFlash:c,clearFlash:m,userIsAdmin:()=>oe(t),adminUiEnabled:()=>Ue(t),persistTab:ft,activateTab:(g,D)=>na(o,g,D),loadHome:()=>Se(o),normalizeActiveTab:()=>gt(o)},n={state:t,root:e,render:p,setFlash:c,clearFlash:m,localeWeekStart:()=>$r(o),localeDowLabels:()=>kr(o),formatDtDisplay:(g,D)=>Ra(o,g,D),timeFormatOpts:()=>vr(o),renderPortalDateTimeField:g=>De(o,g),getDtFieldCurrentValue:g=>sa(o,g),setDtFieldValue:(g,D)=>ra(o,g,D),positionDtPopovers:()=>yt(o),renderFlashBanner:()=>vt(t),accessBadge:ta,formatImportResult:bt,loadHome:()=>Se(o),onImportContacts:g=>Gs(l,g)},r={state:t,root:e,render:p,setFlash:c,clearFlash:m,renderPortalDateTimeField:g=>De(o,g)},i={state:t,root:e,render:p,setFlash:c,clearFlash:m,renderPortalDateTimeField:g=>De(o,g)},l={state:t,root:e,render:p,setFlash:c,clearFlash:m,renderPortalDateTimeField:g=>De(o,g),stopImportElapsedTimer:()=>ee(n),startImportElapsedTimer:()=>Pa(n),setImportPhase:(g,D)=>Pe(n,g,D),applyServerImportProgress:g=>Aa(n,g),readFileTextWithProgress:(g,D)=>Ua(n,g,D),formatImportResult:bt,loadHome:()=>Se(o)},o={state:t,root:e,render:p,setFlash:c,clearFlash:m,filesHost:a,adminHost:s,calendarsHost:n,notesHost:r,tasksHost:i,contactsHost:l,clearPortalSessionState:k,userIsAdmin:()=>oe(t),adminUiEnabled:()=>Ue(t),normalizeActiveTab:()=>gt(o),persistTab:ft,activateTab:(g,D)=>na(o,g,D),activateAdminPage:(g,D)=>yr(o,g,D),loadHome:()=>Se(o),handleSessionExpired:S,loadFiles:()=>X(a),loadShares:g=>Nt(n,g),loadMonthEvents:()=>rt(n),loadContacts:g=>Lt(l,g),loadTasks:()=>Ze(i),loadNotes:()=>Ma(r),loadAdminCapabilities:()=>St(s),loadAdminDashboard:()=>Ge(s),loadAdminUsers:()=>de(s),loadAdminUserDetail:g=>K(s,g),loadAdminUserResources:g=>ce(s,g),loadAdminSystemSettings:()=>Qe(s),loadAdminDatabaseSettings:()=>Xe(s),adminPageMeta:g=>ae(s,g),pickDefaultCalendar:()=>fs(n),toggleCalendarSelected:g=>gs(n,g),blankEventForDay:(g,D)=>hs(n,g,D),defaultRepeat:()=>xt(),itemKey:L,openContact:g=>js(l,g),startNewContact:()=>Ws(l),emptyAddress:()=>xa(),syncEditingEventFromForm:g=>Ss(n,g),syncEditingTaskFromForm:g=>Vs(i,g),syncEditingNoteFromForm:g=>Ns(r,g),runBulkTaskAction:g=>Hs(i,g),shell:(g,D)=>ct(t,g,D,{renderImportProgressModal:()=>_e(n),renderFilesUploadProgressModal:()=>Le(a)}),renderLogin:()=>Kt(e,t,(g,D)=>ct(t,g,D,{renderImportProgressModal:()=>_e(n),renderFilesUploadProgressModal:()=>Le(a)})),renderFlashBanner:()=>vt(t),renderMonthGrid:()=>$s(n),renderEventModal:()=>ws(n),renderImportProgressModal:()=>_e(n),renderFilesUploadProgressModal:()=>Le(a),renderTasksTab:()=>Bs(i),renderNotesTab:()=>Ms(r),renderFilesTab:()=>Un(a),renderAdminSection:()=>ns(s),adminSubnavButtons:()=>xn(s),renderPortalDateTimeField:g=>De(o,g),getDtFieldCurrentValue:g=>sa(o,g),setDtFieldValue:(g,D)=>ra(o,g,D),positionDtPopovers:()=>yt(o),accessBadge:ta,formatImportResult:bt,closeImportProgress:()=>Ds(n),closeFilesUploadProgress:()=>$a(a),resetFilesTransferTree:y,stopImportElapsedTimer:u,stopFilesUploadElapsedTimer:b,unbindUserMenuOutside:f,bindUserMenuOutside:()=>mn(t,p),unbindFilesUploadMenuOutside:$,bindFilesUploadMenuOutside:()=>Dn(a),onLogin:g=>on(g,v()),onShare:g=>Ts(n,g),onSaveEvent:g=>Ps(n,g),onEditCal:g=>As(n,g),onCreateCal:g=>Fs(n,g),onSaveContact:g=>Qs(l,g),onCreateAb:g=>Xs(l,g),onEditAb:g=>Zs(l,g),onSaveTask:g=>zs(i,g),onSaveNote:g=>xs(r,g),bindColorPair:Cr,bindImportInput:()=>void 0,bindHolidaysToggle:()=>Us(n),bindContactPhotoInput:()=>void 0,bindFilesDom:()=>ha(a),bindAdminDom:()=>void 0,saveBlobAsFile:Dr,openInfoModal:g=>hr(o,g),closeInfoModal:()=>Sr(o),captureScroll:()=>jt(e),restoreScroll:g=>Wt(e,g)},or(o),ln(v())}let ne="",E=null,A=!1,q=null,W=null,Q="sqlite",et=!1;async function it(e,t={}){const a={Accept:"application/json",...t.headers};t.body&&(a["Content-Type"]="application/json"),ne&&t.method&&t.method!=="GET"&&(a["X-CSRF-Token"]=ne);const s=await fetch(`/api/install${e}`,{credentials:"same-origin",...t,headers:a});let n;try{n=await s.json()}catch{throw new Error(`Request failed (${s.status})`)}if(!s.ok)throw new Error(n.error||`Request failed (${s.status})`);return n&&typeof n=="object"&&"data"in n&&n.data!==void 0?n.data:n}async function qt(){var e;E=await it("/status"),ne=E.csrfToken||ne,((e=E.defaults)==null?void 0:e.backend)==="pgsql"?Q="pgsql":Q="sqlite"}function Ce(e,t,a){return`<label class="check-row"><input type="checkbox" name="${d(e)}" ${t?"checked":""} ${A?"disabled":""} /> ${d(a)}</label>`}function Tr(){const e=E==null?void 0:E.permissions;return`<section class="card">
  ...
  </section>`}function Pr(){const e=E==null?void 0:E.defaults;return`<section class="card">
  ...
  </section>`}function Ar(){const e=E==null?void 0:E.defaults,t=(E==null?void 0:E.pdoDrivers)||[],a=t.includes("sqlite"),s=t.includes("pgsql");return`<section class="card">
    <h2>Database</h2>
  ...
  </section>`}function Fr(){return`<section class="card">
  ...
  </section>`}function Ir(){return`<section class="card card-danger-zone">
  ...
  </section>`}function B(){const e=document.getElementById("app");if(!e)return;const t=(E==null?void 0:E.step)||"permissions";let a="";E?t==="permissions"?a=Tr():t==="initialize"?a=Pr():t==="database"?a=Ar():t==="upgrade"?a=Fr():t==="done"?a=Ur():t==="locked"?a=Ir():a=`<section class="card"><p>Unknown step: ${d(t)}</p></section>`:a='<section class="card"><p class="muted">Loading installer…</p></section>',e.innerHTML=`
  ...
  `,Or()}function Or(){var t,a,s,n,r,i;const e=document.getElementById("app");e&&((t=e.querySelector('[data-action="reload"]'))==null||t.addEventListener("click",()=>{Mr()}),(a=e.querySelector('[data-action="backend-change"]'))==null||a.addEventListener("change",l=>{Q=l.target.value==="pgsql"?"pgsql":"sqlite",B()}),(s=e.querySelector('[data-action="upgrade-toggle"]'))==null||s.addEventListener("change",l=>{et=!!l.target.checked,B()}),(n=e.querySelector('[data-action="upgrade-run"]'))==null||n.addEventListener("click",()=>{Lr()}),(r=e.querySelector('[data-form="initialize"]'))==null||r.addEventListener("submit",l=>{l.preventDefault(),Nr(l.target)}),(i=e.querySelector('[data-form="database"]'))==null||i.addEventListener("submit",l=>{l.preventDefault(),xr(l.target)}))}async function Mr(){A=!0,q=null,B();try{await qt(),W=null}catch(e){q=e instanceof Error?e.message:"Failed to load installer status"}finally{A=!1,B()}}async function Nr(e){const t=new FormData(e),a=n=>{var r;return!!((r=e.querySelector(`input[name="${n}"]`))!=null&&r.checked)},s={timezone:String(t.get("timezone")??"").trim(),cal_enabled:a("cal_enabled"),card_enabled:a("card_enabled"),tasks_enabled:a("tasks_enabled"),notes_enabled:a("notes_enabled"),files_enabled:a("files_enabled"),dav_auth_type:String(t.get("dav_auth_type")??"Digest"),invite_from:String(t.get("invite_from")??"").trim(),session_max_age_minutes:Number(t.get("session_max_age_minutes")??15),admin_password:String(t.get("admin_password")??""),admin_password_confirm:String(t.get("admin_password_confirm")??"")};A=!0,q=null,W=null,B();try{E=await it("/initialize",{method:"POST",body:JSON.stringify(s)}),ne=E.csrfToken||ne,W="Server settings saved. Configure the database next.",w.event("install.initialize")}catch(n){q=n instanceof Error?n.message:"Initialize failed"}finally{A=!1,B()}}async function xr(e){const t=new FormData(e),a=String(t.get("backend")??Q),s={backend:a,admin_password:String(t.get("admin_password")??""),admin_password_confirm:String(t.get("admin_password_confirm")??"")};a==="sqlite"?s.sqlite_file=String(t.get("sqlite_file")??"").trim():(s.pgsql_host=String(t.get("pgsql_host")??"").trim(),s.pgsql_dbname=String(t.get("pgsql_dbname")??"").trim(),s.pgsql_username=String(t.get("pgsql_username")??"").trim(),s.pgsql_password=String(t.get("pgsql_password")??"")),A=!0,q=null,W=null,B();try{E=await it("/database",{method:"POST",body:JSON.stringify(s)}),ne=E.csrfToken||ne,W="Database configured. Installer is locked.",w.event("install.database"),E.completed||E.step}catch(n){q=n instanceof Error?n.message:"Database setup failed"}finally{A=!1,B()}}async function Lr(){if(et){A=!0,q=null,W=null,B();try{const e=await it("/upgrade",{method:"POST",body:JSON.stringify({confirm:!0})});W="Upgrade completed."+(e.messages&&e.messages.length?" "+e.messages.slice(0,3).join(" · "):""),w.event("install.upgrade"),await qt()}catch(e){q=e instanceof Error?e.message:"Upgrade failed"}finally{A=!1,B()}}}async function _r(e){document.title="AngaraDAV · Setup",document.body.classList.add("layout-install"),e.innerHTML='<section class="card"><p class="muted">Loading installer…</p></section>';try{await qt()}catch(t){q=t instanceof Error?t.message:"Failed to load installer"}B()}const Ct=document.getElementById("app");if(!Ct)throw new Error("#app missing");const ia=window.location.pathname.replace(/\/+$/,"")||"/";ia==="/portal/install"||ia.endsWith("/portal/install")?_r(Ct):Er(Ct);
