...
- **Upgrade keeps portal settings.** Confirming an upgrade no longer rewrites `configuration.yaml` in a way that drops **Portal log level**, **Time format**, or **Week starts on** (or other system keys the upgrade step does not edit). Those values stay as they were.
- **Config file name.** The live config file is `config/configuration.yaml` (template `config/configuration.yaml.dist`). A lab instance that still has `config/baikal.yaml` sees **Rename config file** in `/portal/install/`. That step renames the file and does not rewrite settings. The rename step is temporary and can be deleted after the lab has run it.
- **Lab name cleanup.** The template's sample PostgreSQL account is `angaradav` / `angaradav` (installer fields stay empty; a live database is not renamed). Docker no longer reads `BAIKAL_SKIP_CHOWN` or `BAIKAL_DAV_MAX_BODY_SIZE`. The portal session cookie is `ANGARAPORTAL`; session keys are `angara_portal_*` and `angara_install_*`. Existing browsers are signed out once. Digest realm `BaikalDAV`, `/var/www/baikal`, and the `Baikal\` namespace are unchanged.
