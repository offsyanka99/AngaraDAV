...
- **Upgrade keeps portal settings.** Confirming an upgrade no longer rewrites `baikal.yaml` in a way that drops **Portal log level**, **Time format**, or **Week starts on** (or other system keys the upgrade step does not edit). Those values stay as they were.
