...
class Database extends \Flake\Core\Controller {
    ...
    function render() {
        ...
        if ($this->oForm->persisted()) {
            $sMessage = "<p>AngaraDAV is now installed, and its database properly configured. <strong>For security reasons, this installation wizard is now disabled.</strong></p>";
            $sMessage . "<p>&nbsp;</p>";
            $sMessage .= "<p><a class='btn btn-success' href='" . PROJECT_URI . "admin/'>Start using AngaraDAV</a></p>";
            $sForm = "";
        } else {
            $sMessage = "";
        ...
        try {
            ...
            if (($aMissingTables = \Baikal\Core\Tools::isDBStructurallyComplete($oDb)) !== true) {
                ...
                if (count($aRequiredTables) !== count($aMissingTables)) {
                    ...
                    $sMessage .= "<p>You will find the SQL definition of AngaraDAV tables in this file: <strong>Core/Resources/Db/SQLite/db.sql</strong></p>";
            ...
            $oForm->declareError(
                $oMorpho->element("sqlite_file"),
                "AngaraDAV was not able to establish a connection to the configured SQLite database.<br />SQLite says: " . $e->getMessage() . (string) $e
            );
        }
    }
        ...
        try {
            ...
            if (($aMissingTables = \Baikal\Core\Tools::isDBStructurallyComplete($oDb)) !== true) {
                ...
                if (count($aRequiredTables) !== count($aMissingTables)) {
                    $sMessage = "<br /><p><strong>Database is not structurally complete.</strong></p>";
                    $sMessage .= "<p>Missing tables are: <strong>" . implode("</strong>, <strong>", $aMissingTables) . "</strong></p>";
                    $sMessage .= "<p>You will find the SQL definition of AngaraDAV tables in this file: <strong>Core/Resources/Db/PgSQL/db.sql</strong></p>";
                    $sMessage .= "<br /><p>Nothing has been saved. <strong>Please add these tables before continuing AngaraDAV initialization.</strong></p>";
            ...
            $oForm->declareError(
                $oMorpho->element("backend"),
                "AngaraDAV was not able to establish a connection to the configured PostgreSQL database.<br />PostgreSQL says: " . $e->getMessage()
            );
