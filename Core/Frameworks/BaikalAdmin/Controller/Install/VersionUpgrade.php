...
class VersionUpgrade extends \Flake\Core\Controller {
    ...
    function render() {
        ...
        if ($config['system']['configured_version'] === BAIKAL_VERSION) {
            ...
            $sMessage = "Upgrading AngaraDAV from version <strong>" . $sBaikalConfiguredVersion . "</strong> to version <strong>" . $sBaikalVersion . "</strong>";
        }

        $sHtml = <<<HTML
<header class="jumbotron subhead" id="overview">
    <h1><i class="{$sBigIcon}"></i>AngaraDAV upgrade wizard</h1>
...
HTML;
        ...
        if ($bSuccess === false) {
            $sHtml .= "<p>&nbsp;</p><p><span class='label label-important'>Error</span> AngaraDAV has not been upgraded. See the section 'Errors' for details.</p>";
        } else {
            $sHtml .= "<p>&nbsp;</p><p>AngaraDAV has been successfully upgraded. You may now <a class='btn btn-success' href='" . PROJECT_URI . "admin/'>Access the AngaraDAV admin</a></p>";
        }
