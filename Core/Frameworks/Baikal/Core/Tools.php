...
class Tools {
    ...
    static function assertEnvironmentIsOk() {
        ...
        if (!class_exists('XMLReader')) {
            exit('<strong>AngaraDAV Fatal Error</strong>: The <strong>XMLReader</strong> class is unavailable. The <strong>xml</strong> PHP extension is required by AngaraDAV.');
        }

        # Assert that the temp folder is writable
        if (!\is_writable(\sys_get_temp_dir())) {
            exit('<strong>AngaraDAV Fatal Error</strong>: The system temp directory is not writable.');
        }
    }
