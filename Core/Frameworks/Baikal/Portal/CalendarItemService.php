...
class CalendarItemService {
    ...
    private function notifyCalendarPush(string $username, int $instanceId, int $calendarId): void {
        try {
            $meta = $this->calendarMeta($username, $instanceId);
            $uri = (string) ($meta['uri'] ?? '');
            if ($uri === '') {
            ...
            ChangeNotifier::calendarContent($this->pdo, $username, $uri, $calendarId);
        } catch (\Throwable $e) {
            error_log('portal calendar-item push notify failed: ' . $e->getMessage());
        }
    }
}
